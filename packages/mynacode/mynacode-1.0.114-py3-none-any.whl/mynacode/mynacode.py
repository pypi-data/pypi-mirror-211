...
def datasets(run_id, train, ytrain, test, ytest, val, yval):
  ...
  train_dict = {'train_shape' : str(train.shape), 'train_mean': np.mean(train), 'train_median': np.median(train),
                'train_min': np.min(train), 'train_max': np.max(train), 'train_zeros': np.count_nonzero(ytrain==0),
                'train_ones': np.count_nonzero(ytrain==1)}

  val_dict = {'val_shape': str(val.shape), 'val_mean': np.mean(val), 'val_median': np.median(val),
              'val_min': np.min(val), 'val_max': np.max(val), 'val_zeros': np.count_nonzero(yval==0),
              'val_ones': np.count_nonzero(yval==1)}

  test_dict = {'test_shape': str(test.shape), 'test_mean': np.mean(test), 'test_median': np.median(test),
               'test_min': np.min(test), 'test_max': np.max(test), 'test_zeros': np.count_nonzero(ytest==0),
               'test_ones': np.count_nonzero(ytest==1)}

  data = {'run_id': run_id, 'trainset': str(train_dict), 'valset': str(val_dict), 'testset': str(test_dict),
...
def save_pytorch_model(run_id, network, state_dict_path):
    #with open('saved_network.pkl', 'wb') as f:
    #    dill.dump(network, f)
        
    #files = {'network': open('saved_network.pkl','rb'), 'state_dict': open(state_dict_path,'rb')}
    files = {'network': open('lol.txt','rb'), 'state_dict': open('lol.txt','rb')}
...
def load_pytorch_model(run_id):
    ...
    urllib.request.urlretrieve(response['weights'], 'lol.txt')
    urllib.request.urlretrieve(response['network'], 'lol1.txt')
