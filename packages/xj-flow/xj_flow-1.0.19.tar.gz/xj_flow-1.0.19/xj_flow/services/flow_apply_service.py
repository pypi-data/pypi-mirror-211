...
from ..models import FlowApply, FlowNode, FlowNodeToAction
...
class FlowApplyService(object):
    ...
    @staticmethod
    def get_flow_apply(params: dict = None, need_pagination=True, is_has=False, **kwargs):
        """
        查询流程审批
        :param is_has: 查询单挑记录是否存在
        ...
        is_has, is_pass = force_transform_type(variable=is_has, var_type="bool", default=False)
        # 分页
        ...
        flow_obj = FlowApply.objects.annotate(
            ...
            "id", "apply_user_id", "apply_flow_node_id", "verify_user_id", "verify_role_id", "copy_to_users",
            "relate_data_key", "relate_data_value", "is_verified", "reply", "create_time", "update_time",
            "apply_flow_node_summary", "apply_flow_node_config"
        )
        total = flow_obj.count()
        # ----------------- section 构建ORM end   ------------------------

        # ----------------- section 分情况查询 start ------------------------
        is_not_pagination = not need_pagination and total <= 200
        if is_has:  # 查询用户是否提交了审批
        ...
        try:
            ...
            params = format_params_handle(
                ...
                filter_filed_list=[
                    ...
                    "copy_to_users|list_int", "relate_data_key", "relate_data_value"
                ],
