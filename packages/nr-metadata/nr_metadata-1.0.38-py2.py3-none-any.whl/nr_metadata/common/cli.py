...
@click.group(name="nr-metadata.common")
def group():
    pass
...
def reindex(recreate, queue, *args, **kwargs):
    if recreate:
        index = CommonRecord.index._name
    ...
    id_generator = (
        x[0]
        for x in db.session.query(CommonRecord.model_cls.id).filter(
            CommonRecord.model_cls.is_deleted.is_(False)
        )
    )
    ...
    if queue:
        ...
        for rec_id in tqdm.tqdm(ids, "Indexing records"):
            record = CommonRecord.get_record(rec_id)
