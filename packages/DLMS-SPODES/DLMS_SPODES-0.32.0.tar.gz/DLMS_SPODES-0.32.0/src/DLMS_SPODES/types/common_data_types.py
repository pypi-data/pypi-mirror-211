...
class CommonDataType(ABC):
    ...
    def __init_subclass__(cls, **kwargs):
        ...
        if cls.MIN or cls.MAX:
            cls.NAME += F"({cls.MIN if cls.MIN else ''}..{cls.MAX if cls.MAX else ''})"
        if add_name := kwargs.get("name"):
            cls.NAME = add_name
        else:
            try:
                cls.NAME = config["DLMS"]["class_name"][cls.__name__]
...
class Digital(ABC):
    ...
    def validate(self):
        ...
        if self.VALUE and int(self) != self.VALUE:
            raise ValueError(F"The {self.NAME} must only {self.VALUE}, got {int(self)}")
        if self.MIN and self.MIN > int(self):
            raise ValueError(F"out of range, got {int(self)} expected more than {self.MIN}")
        if self.MAX and int(self) > self.MAX:
            raise ValueError(F'out of range,  got {int(self)} expected less than {self.MAX}')
