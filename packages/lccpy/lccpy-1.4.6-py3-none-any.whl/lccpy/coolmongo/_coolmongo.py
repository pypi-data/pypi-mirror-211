...
class dbORM():
    ...
    def getSheetNames(self):
        conn:MongoClient = self.connPool.get()
        sheets = conn[self.dbName].list_collection_names()
        self.connPool.put(conn)
        return sheets
    ...
    def __delitem__(self, key):
        ...
        if isinstance(key, str):
            ...
            for sheetName in key:
                db.drop_collection(sheetName)
        elif isinstance(key, slice):
            assert key.start is key.stop is key.step is None
            for sheetName in db.list_collection_names():
                db.drop_collection(sheetName)
        else:
            self.connPool.put(conn)
            raise TypeError(key)
...
class ORM():
    ...
    def __len__(self): return len(self.getDbNames())
