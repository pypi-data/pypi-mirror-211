...
class ToolPool():
    ...
    async def put(self, obj):
        await obj['cursor'].close()
        self.pool.append(obj)
    
    async def get(self, dbName=''):
        try:
            obj = self.pool.popleft()
            conn = obj['conn']
            await conn.ping(reconnect=True)
            await conn.commit()  # pymysql只有在commit时才会获取数据库的最新状态
            obj['cursor'] = await conn.cursor(DictCursor)
        except:
            obj = await self.mktool()
        if dbName and obj['dbName'] != dbName:
            await obj['cursor'].execute(f'use {dbName}')
            obj['dbName'] = dbName
        return obj

class acORM():
    async def asy_init(self, mkconn):
        async def _():
            conn = await mkconn()
            return dict(
                conn = conn,
        ...
        self.connPool = ToolPool()
        await self.connPool.asy_init(mktool=_, minlen=1, maxlen=1)
        # 当增删改查报错时, conn不再放回连接池, 以避免含有残留数据
    
    async def getDbList(self):
        obj = await self.connPool.get()
        await obj['cursor'].execute("show databases")  # 非本地操作, 需要连接到数据库
        r = await obj['cursor'].fetchall() or []
        await self.connPool.put(obj)
        return r
    async def getDbNames(self): return [x['Database'] for x in await self.getDbList()]
    async def len(self): return len(await self.getDbList())
...
async def ORM(mkconn):
    orm = acORM()
    await orm.asy_init(mkconn=mkconn)
    return orm
...
class dbORM():
    ...
    async def getSheetNames(self):
        obj = await self.connPool.get(self.dbName)
        sql = f'select table_name from information_schema.tables where table_schema = "{self.dbName}"'
        await obj['cursor'].execute(sql)
        r = [x['TABLE_NAME'] for x in await obj['cursor'].fetchall()]
        await self.connPool.put(obj)
        return r
...
class sheetORM():
        ...
        if isinstance(key, slice):
            ...
            if S in [None,1]:
                if (L in [None,1] and R in [None,-1]) or (L == -1 and R == 1):
                    rdata, cursor = await self.execute(f"delete from {self.sheetName}{self.where}")
                    return cursor
        # [1]且无排序
        ...
        try:
            ...
            if isinstance(pks, list):
                if pks:
                    ...
                    rdata, cursor = await self.execute(f"delete from {self.sheetName} where 1 = 2")
            else:
                rdata, cursor = await self.execute(f"delete from {self.sheetName}{mc[pk] == pks[pk]} limit 1")
        return cursor
    ...
    async def _updateBase(self, key, data:dict):
        ...
        if isinstance(key, slice):
            ...
            if S in [None,1]:
                if (L in [None,1] and R in [None,-1]) or (L == -1 and R == 1):
                    rdata, cursor = await self.execute(f"update {self.sheetName} set {data}{self.where}")
                    return cursor
        # [1]且无排序
        if key == 1 and not self._ParseOrder():
        ...
        try:
            ...
            if isinstance(pks, list):
                if pks:
                    ...
                    rdata, cursor = await self.execute(f"update {self.sheetName} set {data} where 1 = 2")
            else:
                rdata, cursor = await self.execute(f"update {self.sheetName} set {data}{mc[pk] == pks[pk]} limit 1")
        return cursor
    
    async def execute(self, sql:str, data=None, commit=True):
        mobj:dict = await self.connPool.get(self.dbName)
        conn = mobj['conn']
        cursor:DictCursor = mobj['cursor']
        if commit:
            try:
                await cursor.execute(sql, data)
                r = await cursor.fetchall() or []
                await conn.commit()
                await self.connPool.put(mobj)
                return r, cursor
            except:
                await conn.rollback()
                raise
    ...
    async def executemany(self, sql:str, data):
        mobj:dict = await self.connPool.get(self.dbName)
        conn = mobj['conn']
        cursor:DictCursor = mobj['cursor']
        try:
    ...
    async def len(self):
        rdata, cursor = await self.execute(f"select count(1) as tatal from {self.sheetName}{self.where}", commit=False)
