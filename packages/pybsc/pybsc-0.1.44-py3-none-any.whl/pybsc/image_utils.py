...
def _imread(image_path, color_type='bgr'):
    ...
    if color_type.lower() == 'gray':
        ...
        # PIL doesn't directly support BGRA,
        # but we can handle it in a workaround way.
        img = img.convert('RGBA')
        r, g, b, a = img.split()
        converted_img = PIL.Image.merge("RGBA", (b, g, r, a))
    elif color_type.lower() == 'bgr':
        # PIL doesn't directly support BGR,
        # but we can handle it in a workaround way.
        img = img.convert('RGB')
        r, g, b = img.split()
        converted_img = PIL.Image.merge("RGB", (b, g, r))
