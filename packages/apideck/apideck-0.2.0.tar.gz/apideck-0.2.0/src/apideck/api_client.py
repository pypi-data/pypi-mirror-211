...
from apideck import rest
from apideck.configuration import Configuration
from apideck.exceptions import ApiTypeError, ApiValueError, ApiException
from apideck.model_utils import (
    ModelNormal,
    ModelSimple,
    ModelComposed,
...
class ApiClient(object):
    ...
    def __call_api(
        ...
        def transform_tuples(tuple_list):
            # initialize an empty list
            new_list = []

            # loop through each tuple in the input list
            for tup in tuple_list:
                # if the tuple's first element is 'filter',
                # then process the dictionary value from the second element
                if tup[0] == 'filter':
                    for key, value in tup[1].items():
                        new_list.append(('filter[' + key + ']', value))
                # if the tuple's first element is 'sort',
                elif tup[0] == 'sort':
                    for key, value in tup[1].items():
        ...
        # transform the tuples and print the result
        result = transform_tuples(query_params)
