...
where_simple = [
    ...
    [
        Literal(SomeTable.field + " is null or field > 0"),
        None,
        None,
...
def test_limit(limit, offset, result):
    sql, _ = (
        Select(PgSqlDialect()).from_(TABLE_NAME, "*").limit(limit, offset).assemble()
    )
    assert sql == result
...
def test_page(page, page_rows, result):
    sql, _ = (
        Select(PgSqlDialect()).from_(TABLE_NAME, "*").page(page, page_rows).assemble()
    )
    assert sql == result
...
def test_for_update():
    ...
    sql, _ = (
        Select(PgSqlDialect()).from_(TABLE_NAME, "field").for_update(False).assemble()
    )
