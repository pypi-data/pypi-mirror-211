...
def pytest_addoption(parser):
    ...
    add_opt_ini("specs-regex", f"Regex for specification parsing. Default: '{DEFAULT_REQ_PATTERN}'")
    add_opt_ini("specs-junit-regex", f"Regex for JUnit name parsing. Default: '{JUNIT_PATTERN}'")
    add_opt_ini("specs-risk-assessment", "Input path for risk assessment.")
    add_opt_ini("specs-risk-priority-threshold", "Threshold for risk priority. Default: 'low'")
    add_opt_ini("specs-output", "Path for nydok json results file.")


def pytest_configure(config):
    """Pytest specific hook that is triggered after options are parsed."""
    if ra_file := config.getoption("specs-risk-assessment") or config.getini(
        "specs-risk-assessment"
    ):
...
def pytest_collection_finish(session):
    ...
    permissable_risk_priority = session.config.getoption(
        "specs-risk-priority-threshold"
    ) or session.config.getini("specs-risk-priority-threshold")
    if not permissable_risk_priority:
        permissable_risk_priority = "low"
...
def pytest_sessionfinish(session):
    ...
    def get_option_or_ini(setting):
        return session.config.getoption(setting) or session.config.getini(setting)

    # Optionally write the results to a file
    if output := get_option_or_ini("specs-output"):
