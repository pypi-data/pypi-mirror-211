...
def execute_extended_nlp_interface(self, anlp):
    ...
    expected_primals_lb = np.asarray(
        [-1, 2, -3, -np.inf, -5, -np.inf, -7, -np.inf, -9], dtype=np.float64
    )
    ...
    expected_constraints_ub = np.asarray(
        [1, 0, np.inf, 4, 5, 0, np.inf, 8, 9], dtype=np.float64
    )
    ...
    expected_ineq_lb = np.asarray(
        [-1, -3, -np.inf, -5, -7, -np.inf, -9], dtype=np.float64
    )
    ...
    expected_objective = sum(
        2.0**2 * (i + 1) * (j + 1) for i in range(9) for j in range(9)
    )
    self.assertEqual(expected_objective, anlp.evaluate_objective())
    anlp.set_primals(np.ones(9))
    ...
    expected_con = np.asarray(
        [45, 88, 3 * 45, 4 * 45, 5 * 45, 276, 7 * 45, 8 * 45, 9 * 45], dtype=np.float64
    )
    ...
    expected_con_eq = np.asarray(
        [2 * (88 + 2) - 2, 2 * (276 - 6) + 6], dtype=np.float64
    )
    self.assertTrue(np.array_equal(expected_con_eq, con_eq))
    anlp.set_primals(np.ones(9))
    ...
    expected_jac_eq = np.asarray(
        [[2, 4, 6, 8, 10, 12, 14, 16, 18], [6, 12, 18, 24, 30, 36, 42, 48, 54]],
        dtype=np.float64,
    )
    ...
    expected_jac_ineq = [
        [(i) * (j) for j in range(1, 10)] for i in [1, 3, 4, 5, 7, 8, 9]
    ]
...
class TestAmplNLP(unittest.TestCase):
    ...
    def test_names(self):
        ...
        self.assertEqual(
            len(expected_ineq_constraint_names), len(ineq_constraint_names)
        )
...
class TestPyomoNLP(unittest.TestCase):
    ...
    def test_indices_methods(self):
        ...
        self.assertTrue(
            expected_constraint_indices == nlp.get_constraint_indices([self.pm.c])
        )
        expected_constraint_indices = [0, 3, 8, 4]
        constraints = [self.pm.c[1], self.pm.c[4], self.pm.c[9], self.pm.c[5]]
        self.assertTrue(
            expected_constraint_indices == nlp.get_constraint_indices(constraints)
        )
        ...
        expected_jac = [[(i) * (j) for j in range(1, 10)] for i in range(1, 10)]
        expected_jac = np.asarray(expected_jac, dtype=np.float64)
        jac = nlp.extract_submatrix_jacobian(
            pyomo_variables=[self.pm.x], pyomo_constraints=[self.pm.c]
        )
        dense_jac = jac.todense()
        self.assertTrue(np.array_equal(dense_jac, expected_jac))
        ...
        jac = nlp.extract_submatrix_jacobian(
            pyomo_variables=variables, pyomo_constraints=constraints
        )
        dense_jac = jac.todense()
        self.assertTrue(np.array_equal(dense_jac, expected_jac))
    ...
    def test_invalid_bounds(self):
        ...
        with self.assertRaisesRegex(
            RuntimeError,
            "Some variables have lower bounds that "
...
class TestUtils(unittest.TestCase):
    ...
    def test_util_maps(self):
        anlp = AslNLP(self.filename)
        full_to_compressed_mask = build_compression_mask_for_finite_values(
            anlp.primals_lb()
        )
        ...
        self.assertTrue(
            np.array_equal(
                full_to_compressed_mask, build_bounds_mask(anlp.primals_lb())
            )
        )

        expected_compressed_primals_lb = np.asarray(
            [-1, 2, -3, -5, -7, -9], dtype=np.float64
        )
        ...
        self.assertTrue(
            np.array_equal(expected_compressed_primals_lb, compressed_primals_lb)
        )

        # test full_to_compressed
        compressed_primals_lb = full_to_compressed(
            anlp.primals_lb(), full_to_compressed_mask
        )
        self.assertTrue(
            np.array_equal(expected_compressed_primals_lb, compressed_primals_lb)
        )
        ...
        expected_full_primals_lb = np.asarray(
            [-1, 2, -3, -np.inf, -5, -np.inf, -7, -np.inf, -9], dtype=np.float64
        )
        full_primals_lb = compressed_to_full(
            compressed_primals_lb, full_to_compressed_mask, default=-np.inf
        )
        self.assertTrue(np.array_equal(expected_full_primals_lb, full_primals_lb))
        # test in place
        full_primals_lb.fill(0.0)
        ret = compressed_to_full(
            compressed_primals_lb,
            full_to_compressed_mask,
            out=full_primals_lb,
            default=-np.inf,
        )
        ...
        # test no default
        expected_full_primals_lb = np.asarray(
            [-1, 2, -3, np.nan, -5, np.nan, -7, np.nan, -9], dtype=np.float64
        )
        full_primals_lb = compressed_to_full(
            compressed_primals_lb, full_to_compressed_mask
        )
        ...
        expected_full_primals_lb = np.asarray(
            [-1, 2, -3, 0.0, -5, 0.0, -7, 0.0, -9], dtype=np.float64
        )
        full_primals_lb.fill(0.0)
        ret = compressed_to_full(
            compressed_primals_lb, full_to_compressed_mask, out=full_primals_lb
        )
        self.assertTrue(ret is full_primals_lb)
        self.assertTrue(np.array_equal(expected_full_primals_lb, full_primals_lb))
