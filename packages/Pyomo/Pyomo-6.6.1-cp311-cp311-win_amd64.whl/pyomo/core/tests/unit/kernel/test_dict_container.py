...
class _TestDictContainerBase(object):
    ...
    def test_init2(self):
        ...
        self._container_type((i, self._ctype_factory()) for i in index)
        self._container_type(((i, self._ctype_factory()) for i in index))
        with self.assertRaises(TypeError):
            self._container_type(*tuple((i, self._ctype_factory()) for i in index))
        c = self._container_type(a=self._ctype_factory(), b=self._ctype_factory())
    ...
    def test_ordered_init(self):
        ...
        cdict[3] = self._ctype_factory()
        self.assertEqual(list(cdict.keys()), [1, 'a', 2, None, -1, 'bc', 3])
    ...
    def test_len2(self):
        index = ['a', 1, None, (1,), (1, 2)]
        cdict = self._container_type((i, self._ctype_factory()) for i in index)
    ...
    def test_delitem(self):
        ...
        for cnt, i in enumerate(index, 1):
            self.assertTrue(i in c)
            cdata = c[i]
            self.assertEqual(id(cdata.parent), id(c))
            del c[i]
    ...
    def test_pickle(self):
        ...
        self.assertTrue(isinstance(pickled_cdict, self._container_type))
    ...
    def test_values(self):
        ...
        c = self._container_type(raw_constraint_dict)
        self.assertEqual(
            sorted(list(id(_v) for _v in raw_constraint_dict.values()), key=str),
            sorted(list(id(_v) for _v in c.values()), key=str),
        )
    ...
    def test_items(self):
        ...
        self.assertEqual(
            sorted(
                list((_i, id(_v)) for _i, _v in raw_constraint_dict.items()), key=str
            ),
            sorted(list((_i, id(_v)) for _i, _v in c.items()), key=str),
        )
    ...
    def test_name(self):
        ...
        for key, c in children.items():
            ...
            self.assertEqual(
                c.getname(fully_qualified=False, convert=str), "[%s]" % (str(key))
            )
            self.assertEqual(
                c.getname(fully_qualified=False, convert=repr), "[%s]" % (repr(key))
            )
            self.assertEqual(
                c.getname(fully_qualified=True, convert=str), "[%s]" % (str(key))
            )
            self.assertEqual(
                c.getname(fully_qualified=True, convert=repr), "[%s]" % (repr(key))
            )
        ...
        for key, c in children.items():
            ...
            self.assertEqual(
                c.getname(fully_qualified=True, convert=str), "cdict[%s]" % (str(key))
            )
            self.assertEqual(
                c.getname(fully_qualified=True, convert=repr), "cdict[%s]" % (repr(key))
            )
            self.assertEqual(c.name, names[c])
        for c in cdict.components():
            self.assertNotEqual(c.name, None)
        ...
        for key, c in children.items():
            ...
            self.assertEqual(
                c.getname(fully_qualified=True, convert=str),
                "model.cdict[%s]" % (str(key)),
            )
            self.assertEqual(
                c.getname(fully_qualified=True, convert=repr),
    ...
    def test_preorder_traversal_descend_check(self):
        ...
        descend.seen = []
        order = list(pmo.preorder_traversal(cdict, descend=descend))
...
class _TestActiveDictContainerBase(_TestDictContainerBase):
    ...
    def test_active(self):
        ...
        self.assertEqual(
            len(list(cdict.components())), len(list(cdict.components(active=True)))
        )
        ...
        self.assertNotEqual(
            len(list(cdict.components())), len(list(cdict.components(active=None)))
        )
    ...
    def test_preorder_traversal(self):
        cdict, traversal = super(
            _TestActiveDictContainerBase, self
        ...
        self.assertEqual(
            ...
            [
                c.name
                for c in pmo.preorder_traversal(cdict, active=True, descend=descend)
            ],
        )
        self.assertEqual(
            [id(cdict), id(cdict[0]), id(cdict[2])],
            [
                id(c)
                for c in pmo.preorder_traversal(cdict, active=True, descend=descend)
            ],
    ...
    def test_preorder_traversal_descend_check(self):
        ...
        if cdict.ctype._is_heterogeneous_container:
            ...
            self.assertEqual(
                [id(cdict), id(cdict[0]), id(cdict[2])], [id(c) for c in descend.seen]
            )
        else:
        ...
        self.assertEqual(
            [id(cdict), id(cdict[0]), id(cdict[1]), id(cdict[2])],
            [id(c) for c in order],
        )
        if cdict.ctype._is_heterogeneous_container:
            self.assertEqual(
