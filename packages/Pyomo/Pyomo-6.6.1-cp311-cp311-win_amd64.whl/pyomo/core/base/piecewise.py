...
def _characterize_function(name, tol, f_rule, model, points, *index):
    ...
    if not all(
        ...
        msg = (
            "**WARNING: Piecewise component '%s[%s]' has detected slopes of consecutive piecewise "
            "segments to be within "
            + str(tol)
            + " of one another. Refer to the Piecewise help "
...
class _PiecewiseData(_BlockData):
    ...
    def __call__(self, x):
        if self._constructed is False:
            raise ValueError(
                "Piecewise component %s has not been constructed yet" % self.name
            )
...
class _SimpleSinglePiecewise(object):
    ...
    def construct(self, pblock, x_var, y_var):
        ...
        if None in [x_pts, y_pts, bound_type]:
            raise RuntimeError(
                "_SimpleSinglePiecewise: construct() called duringinvalid state."
            )
        ...
        if not x_var.ub is None and x_var.ub > x_pts[1]:
            pblock.simplified_piecewise_domain_constraint_upper = Constraint(
                expr=x_var <= x_pts[-1]
            )
...
class _SimplifiedPiecewise(object):
    ...
    def construct(self, pblock, x_var, y_var):
        ...
        if None in [x_pts, y_pts, bound_type]:
            raise RuntimeError(
                "_SimplifiedPiecewise: construct() called during invalid state."
            )
        len_x_pts = len(x_pts)
...
class _SOS2Piecewise(object):
    ...
    def construct(self, pblock, x_var, y_var):
        ...
        if None in [x_pts, y_pts, bound_type]:
            raise RuntimeError(
                "_SOS2Piecewise: construct() called during invalid state."
            )
        len_x_pts = len(x_pts)
...
class _DCCPiecewise(object):
    ...
    def construct(self, pblock, x_var, y_var):
        ...
        if None in [x_pts, y_pts, bound_type]:
            raise RuntimeError(
                "_DCCPiecewise: construct() called during invalid state."
            )
        len_x_pts = len(x_pts)
        ...
        # create piecewise constraints
        pblock.DCC_constraint1 = Constraint(
            expr=x_var
            == sum(
        ...
        def con1_rule(model, i):
            if bound_type in [Bound.Upper, Bound.Equal]:
                ...
                return (
                    y_var
                    - y_pts[i - 1]
                    - ((y_pts[i] - y_pts[i - 1]) / (x_pts[i] - x_pts[i - 1]))
                    * (x_var - x_pts[i - 1])
                    <= rhs
                )
            elif bound_type == Bound.Lower:
