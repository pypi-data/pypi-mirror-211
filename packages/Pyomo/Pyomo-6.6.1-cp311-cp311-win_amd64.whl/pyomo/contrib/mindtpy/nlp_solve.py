...
def solve_subproblem(solve_data, config):
    ...
    if config.calculate_dual_at_solution:
        ...
        for c in fixed_nlp.MindtPy_utils.constraint_list:
            ...
            try:
                fixed_nlp.tmp_duals[c] = c_geq * max(0, c_geq * (rhs - value(c.body)))
            except (ValueError, OverflowError) as error:
                fixed_nlp.tmp_duals[c] = None
                evaluation_error = True
        if evaluation_error:
            for nlp_var, orig_val in zip(
                MindtPy.variable_list, solve_data.initial_var_values
            ):
    ...
    with SuppressInfeasibleWarning():
        with time_code(solve_data.timing, 'fixed subproblem'):
            results = nlpopt.solve(
                fixed_nlp, tee=config.nlp_solver_tee, load_solutions=False, **nlp_args
            )
...
def handle_nlp_subproblem_tc(fixed_nlp, result, solve_data, config, cb_opt=None):
    ...
    if result.solver.termination_condition in {
        ...
        config.logger.info('NLP subproblem failed to converge within the time limit.')
        ...
        handle_subproblem_other_termination(
            fixed_nlp, result.solver.termination_condition, solve_data, config, cb_opt
        )
...
def handle_subproblem_optimal(fixed_nlp, solve_data, config, cb_opt=None):
    ...
    if solve_data.primal_bound_improved:
        ...
        solve_data.best_solution_found_time = get_main_elapsed_time(solve_data.timing)
        if config.strategy == 'GOA':
            solve_data.num_no_good_cuts_added.update(
                {
                    solve_data.primal_bound: len(
                        solve_data.mip.MindtPy_utils.cuts.no_good_cuts
                    )
                }
            )
    # Add the linear cut
...
def handle_subproblem_other_termination(
    fixed_nlp, termination_condition, solve_data, config, cb_opt
):
...
def solve_feasibility_subproblem(solve_data, config):
    ...
    next(feas_subproblem.component_data_objects(Objective, active=True)).deactivate()
...
def handle_feasibility_subproblem_tc(
    subprob_terminate_cond, MindtPy, solve_data, config
):
