...
class CloudVerification:
    ...
    def verification_request(self, auth_data: Dict[str, Any]) -> Optional[Response]:
        verify_url = self.get_domain() + "/cli/verify"
        response = None
    ...
    def get_domain(self) -> str:
        ...
        if is_new_api():
            if self.context.server:
                ...
                if hasattr(SupportedServers, key):
                    domain = SupportedServers[key].value
                else:
                    domain = f"https://{self.context.server}.certora.com"
            else:
                domain = SupportedServers.PRODUCTION.value
        else:
            if Ctx.is_staging(self.context):
                domain = SupportedServers.STAGING.value
            else:
                domain = SupportedServers.PRODUCTION.value
        return domain
