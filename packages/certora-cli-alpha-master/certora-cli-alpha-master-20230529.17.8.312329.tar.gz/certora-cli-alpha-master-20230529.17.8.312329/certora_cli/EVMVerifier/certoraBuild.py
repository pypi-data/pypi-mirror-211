...
from Shared.certoraUtils import get_certora_config_dir, get_certora_build_file, get_certora_verify_file, \
    ...
    get_certora_verify_file_cvl1, find_filename_in, PRE_AUTOFINDER_BACKUP_DIR, POST_AUTOFINDER_BACKUP_DIR, find_in, \
    safe_copy_folder
from Shared.certoraUtils import remove_and_recreate_dir, as_posix, abs_posix_path
from Shared.certoraUtils import safe_create_dir, remove_file, normalize_double_paths, CERTORA_BUILD_DIRECTORY
...
from Shared.certoraUtils import read_json_file, abs_posix_path_obj, PACKAGE_FILE, REMAPPINGS_FILE, run_solc_cmd
...
class InputConfig:
    def __init__(self, context: CertoraContext) -> None:
        ...
        self.path = context.solc_allow_path if is_new_api() else context.path
        ...
        optimize_map_attr = context.solc_optimize_map if is_new_api() else context.optimize_map
...
class ContractInSDC:
    ...
    def as_dict(self) -> Dict[str, Any]:
        ...
        return {
            "name": self.name,
            "original_file": find_filename_in(get_certora_sources_dir(), self.original_file),
            "lang": self.lang,
...
class CertoraBuildGenerator:
    ...
    def get_contract_file_of_non_autofinder(self, build_arg_contract_file: str, reference: int) -> str:
        ...
        """
        file = abs_posix_path(self.get_contract_file_of(build_arg_contract_file, reference))
        if file in self.function_finder_file_remappings.keys():
            ret = Path(self.function_finder_file_remappings[file])
        else:
            ret = Path(file)
        abs_sources_dir = get_certora_sources_dir().resolve()
        ...
        in_sources = find_in(get_certora_sources_dir(), ret)
    ...
    def collect_type_descriptions_for_imported_contract(self, build_arg_contract_file: str, c_file: str) \
        ...
        for node_id in flattened_ast:
            ...
            if not NodeFilters.is_defined_in_a_contract_or_library(node) and \
               NodeFilters.is_user_defined_type_definition(node):
                        ...
                        (NodeFilters.is_defined_in_a_contract_or_library(node) and
                         (any([base_contract_name for _, base_contract_name in base_contract_files if
                          ...
                          self.is_library_def_node(c_file,
                                                   ...
                                                   build_arg_contract_file))):
                    source_type_descriptions.append(CT.UserDefinedType.from_def_node(
                        lambda ref: self.get_original_def_node(build_arg_contract_file, ref),
                        lambda ref: self.get_contract_file_of_non_autofinder(build_arg_contract_file, ref),
                        node
                    ))
    ...
    def collect_for_file(self,
        ...
        run_solc_cmd(collect_cmd, f"{sdc_name}.standard.json", self.config_path, wd=compile_wd,
        ...
        if get_certora_sources_dir().exists():
            ...
            for idx, orig_file in srclist.items():
                new_path = find_filename_in(get_certora_sources_dir(), orig_file)
                if new_path is None:
                    successful = False
                    break
    ...
    def get_contract_in_sdc(self,
        ...
        return ContractInSDC(contract_name,
                             # somehow make sure this is an absolute path which obeys the autofinder remappings (i.e. make
                             # sure this file doesn't start with autoFinder_)
                             source_code_file,
                             lang,
                             report_source_file,
                             address,
    ...
    def add_auto_finders(self, contract_file: str, sdc: SDC) -> \
            Optional[Tuple[Dict[str, InternalFunc], Dict[str, Dict[int, Instrumentation]]]]:
    ...
    def get_fresh_backupdir(backupdir: Path) -> Path:
        ...
        """
        folder_id = 0
        base = get_certora_sources_dir() / backupdir
        while True:
    ...
    def build(self, context: CertoraContext) -> None:
        ...
        for i, build_arg_contract_file in enumerate(sorted(self.input_config.files)):
            ...
            pre_backup_dir = self.get_fresh_backupdir(PRE_AUTOFINDER_BACKUP_DIR)
            ignore_patterns = shutil.ignore_patterns(f"{PRE_AUTOFINDER_BACKUP_DIR}.*",
                                                     f"{POST_AUTOFINDER_BACKUP_DIR}.*")
            build_logger.debug(f"Backing up current .certora_sources to {pre_backup_dir}")
            sources_dir = get_certora_sources_dir()
            safe_copy_folder(sources_dir, pre_backup_dir, ignore_patterns)
            ...
            if compiler_lang == CompilerLangSol():
                ...
                # of sources.
                post_backup_dir = self.get_fresh_backupdir(POST_AUTOFINDER_BACKUP_DIR)
                build_logger.debug(f"Backing up instrumented .certora_sources to {post_backup_dir}")
                safe_copy_folder(get_certora_sources_dir(), post_backup_dir, ignore_patterns)
                ...
                shutil.copytree(pre_backup_dir, get_certora_sources_dir(), dirs_exist_ok=True,
                ...
                if not success:
                    ...
                    for _, sdc in added_finders_to_sdc:
                        sdc.source_dir = str(post_backup_dir.relative_to(get_certora_sources_dir()))
            else:
    ...
    def build_source_tree(self, sources: Set[Path], context: CertoraContext, overwrite: bool = False) -> None:
        ...
        if hasattr(context, CONF_FILE_ATTR):
            ...
            conf_file_path = CERTORA_BUILD_DIRECTORY / conf_file_name
            target_path = get_certora_sources_dir() / conf_file_name
            build_logger.debug(f"Copying conf {conf_file_path} to {target_path}")
            try:
    ...
    def instrument_auto_finders(self, build_arg_contract_file: str, i: int, sdc_pre_finders: List[SDC]) -> \
            Tuple[List[Tuple[Dict[str, InternalFunc], SDC]], bool]:
        ...
        abs_build_arg_contract_file = abs_posix_path(build_arg_contract_file)
        ...
        try:
            ...
            for k, v in autofinder_remappings.items():
                self.function_finder_file_remappings[abs_posix_path(k)] = abs_posix_path(v)
            new_sdcs = self.collect_for_file(new_file, i, get_compiler_lang(build_arg_contract_file),
                                             get_certora_sources_dir(),
    ...
    def to_autofinder_file(self, contract_file: str) -> str:
        ...
        new_path = get_certora_sources_dir() / self.cwd_rel_in_sources / rel_directory / contract_filename
    ...
    def resolve_slot(self, primary_contract: str, slot_name: str) -> str:
        ...
        run_solc_cmd(asm_collect_cmd, f"{primary_contract}.asm", self.config_path, Path(os.getcwd()))
    ...
    def collect_sources(self, context: CertoraContext) -> Set[Path]:
        ...
        if REMAPPINGS_FILE.exists():
            sources.add(REMAPPINGS_FILE.absolute())
...
class SpecWithImports:
    ...
    def __get_eventual_path_to_spec(self, path: Path) -> str:
        return f"{get_certora_sources_dir()}/.{self.spec_idx}_{path.name}.spec"
...
class CertoraVerifyGenerator:
    def __init__(self, build_generator: CertoraBuildGenerator):
        ...
        if self.input_config.verify is not None:
            ...
            vq_spec = abs_posix_path(vq_spec)  # get full abs path, will need to update later to .certora_sources
        ...
        if self.input_config.verify is not None:
            ...
            def path_updater(path: str) -> str:
                if in_certora_sources:
                    res = find_filename_in(get_certora_sources_dir(), path)
    ...
    def copy_specs(self) -> None:
        ...
        build_logger.debug(f"copying spec file {spec.spec_file} to "
                           f"{abs_posix_path(spec.eventual_path_to_spec)}")
    ...
    def dump_cvl1(self) -> None:
        ...
        """
        build_logger.debug(f"writing {abs_posix_path(get_certora_verify_file_cvl1())}")
        with get_certora_verify_file_cvl1().open("w+") as output_file:
...
def build(context: CertoraContext, ignore_spec_syntax_check: bool = False) -> None:
    ...
    try:
        ...
        if mode_has_spec_file(context.mode) and not context.build_only and not ignore_spec_syntax_check:
            attr = context.disable_local_typechecking if is_new_api() else context.disableLocalTypeChecking
            if attr:
                ...
                if spec_check_exit_code != 0:
                    raise CertoraUserInputError("CVL specification syntax check failed")
