...
def check_mode_of_operation(context: CertoraContext) -> None:
    ...
    if len(context.files) > 0 and is_bytecode:
        raise Util.CertoraUserInputError("Cannot use --bytecode with other files")
...
def set_server(context: CertoraContext) -> None:
    """
    ...
    if Util.is_new_api():
        if context.server:
            key = context.server.upper()
            if hasattr(Util.SupportedServers, key):
                context.server = Util.SupportedServers[key]
            else:
                context.server = f"https://{context.server}.certora.com"
        else:
            context.server = Util.SupportedServers.PRODUCTION
    else:
        if Ctx.is_staging(context):
            context.server = Util.SupportedServers.STAGING.value
        else:
            context.server = Util.SupportedServers.PRODUCTION.value
