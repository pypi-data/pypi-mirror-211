...
from Shared.certoraUtils import get_certora_internal_dir, is_new_api, SupportedServers
...
def validate_version_and_branch(branch: Optional[str], commit_sha1: Optional[str]) -> None:
    ...
    """
    installed, package_name, version = get_package_and_version()
    if not installed:
        if not (branch or commit_sha1):
            raise CertoraUserInputError(
                ...
                "--cloud BRANCH or --staging BRANCH,"
    ...
    try:
        ...
        if "." in latest and "." in version:
            ...
            compare_cloud_versions_check(local_main, local_patch, local_sub, remote_main, remote_sub, remote_patch,
                                         package_name, recommended_upgrade_msg, must_upgrade_msg)

            if package_name not in [PRODUCTION_PACKAGE_NAME, BETA_PACKAGE_NAME]:
...
def compare_cloud_versions_check(local_main: int, local_patch: int, local_sub: int, remote_main: int, remote_sub: int,
                                 remote_patch: int,
                                 package_name: str, recommended_upgrade_msg: str, must_upgrade_msg: str) -> None:
    """
    ...
    elif package_name == BETA_PACKAGE_NAME:  # checks for certora-cli-beta
...
class CloudVerification:
    ...
    def __set_url(self, url_attr: str, index: str, user_id: int, anonymousKey: str,
        ...
        if self.runName == "":
            ...
            url = f"{self.context.server}/{index}/{user_id}/{self.runName}{resource_req}?anonymousKey={anonymousKey}"
    ...
    def prepare_auth_data(self, cl_args: str) -> Optional[Dict[str, Any]]:
        ...
        _, _, version = get_package_and_version()
        ...
        ext_args = self.context.prover_args if is_new_api() else self.context.settings
    ...
    def print_output_links(self) -> None:
        if Ctx.is_staging(self.context):
            print(f"Follow your job at {SupportedServers.STAGING.value}")
            print(f"If the job does not yet appear at the dashboard, follow it here {self.statusUrl}")
        else:
            print(f"Follow your job at {SupportedServers.PRODUCTION.value}")
    ...
    def cli_verify_and_report(self, cl_args: str, send_only: bool = False) -> bool:
        ...
        job_list = JobList()
        job_list.add_job(
            self.runName, self.reportUrl, self.msg, self.context.server, str(self.userId), self.anonymousKey)
    ...
    def verification_request(self, auth_data: Dict[str, Any]) -> Optional[Response]:
        verify_url = self.context.server + "/cli/verify"
        response = None
