...
class FolderDataset(Dataset):
    def __init__(self, path:str, gpus:str):
        ...
        self.files = [os.path.join(path, i) for i in os.listdir(path) if i.split('.')[-1].lower() in file_types]
...
def get_folder_loader(path:str, gpus:str, batch_size:int)-> torch.utils.data.DataLoader:
...
class FileLoader():
    ...
    def load_file(self, 
        ...
        assert file_path.split('.')[-1].lower() in list(self.file_types.keys()), f"filetype not supported: {file_path.split('.')[-1].lower()}"
...
class FileSaver():   
    ...
    def export_prediction_as_dicomseg(self, 
                                      mask:np.array, 
                                      outdir:str,  
                                      file_name:str
                                     ) -> None:
    ...
    def export_prediction_as_jpg(self, 
                                 mask:np.array, 
                                 outdir:str,  
                                 file_name:str
                                ) -> None:
