...
from platform_sdk.models.domain_lti import DomainLTIExtended, DomainLTI
...
class IDMapperClient:
    ...
    def get_domain_ltis_by_domain(self, domain_name) -> List[DomainLTIExtended]:
        ...
        domain_lti_list = self._get(url).json()
        return json_list_to_model_list(DomainLTIExtended, domain_lti_list)

    def get_domain_ltis_by_lti_identifier(self, lti_identifier) -> List[DomainLTIExtended]:
        """
        Send a GET request to the id mapper domain ltis route and receive a list of all domain LTI objects
        for a specific LTI
        """
    ...
    def post_domain(self, payload):
        """Send a POST request to the id mapper domain route and create a domain object and receive a200
        response = [
            {
                "name": str,
                "key": str,
