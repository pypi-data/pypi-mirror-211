...
def persist_lines(config, lines, table_cache=None, file_format_type: FileFormatTypes = None) -> None:
    ...
    event_loop = setup_flush_task(time_schedule, records_to_load, row_count, stream_to_sync, config,
                                  state, flushed_state, archive_load_files_data)
...
def flush_records(stream: str,
    ...
    if archive_load_files:
        ...
        archive_metadata = {
            ...
            'table': archive_table,
            'archived-by': 'macrometa_target_snowflake'
        }
...
def setup_flush_task(time_schedule, streams, row_count, stream_to_sync, config, state, flushed_state,
                     archive_load_files_data, filter_streams=None) -> AbstractEventLoop:
    event_loop = asyncio.new_event_loop()
    Thread(target=start_background_loop, args=(
        event_loop,), daemon=True).start()
    asyncio.run_coroutine_threadsafe(
        flush_task(time_schedule, streams, row_count, stream_to_sync,
                   config, state, flushed_state, archive_load_files_data, filter_streams),
        event_loop)
    return event_loop


async def flush_task(time_schedule, streams, row_count, stream_to_sync, config, state, flushed_state,
                     archive_load_files_data, filter_streams=None) -> None:
    while True:
        ...
        if (
            ...
            flushed_state = flush_streams(streams, row_count, stream_to_sync, config, state, flushed_state,
                                          archive_load_files_data, filter_streams)
            # emit latest state
            emit_state(copy.deepcopy(flushed_state))
            time_schedule['last_executed_time'] = datetime.now()
...
class SnowflakeTargetConnector(C8Connector):
    ...
    def package_name(self) -> str:
        ...
        return "macrometa-target-snowflake"

    def version(self) -> str:
        """Returns the version of the connector."""
        return pkg_resources.get_distribution('macrometa_target_snowflake').version
    ...
    def description(self) -> str:
        ...
        return "Send data into a Snowflake collection."
    ...
    def config(self) -> List[ConfigProperty]:
        ...
        return [
            ...
            ConfigProperty('primary_key_required', 'Primary Key Required', ConfigAttributeType.BOOLEAN, False, False,
                           ...
                           default_value='false'),
            ConfigProperty('batch_size_rows', 'Batch Size Rows', ConfigAttributeType.INT, False, True,
                           description='Number of rows per batch.',
                           default_value='100000'),
            ConfigProperty('batch_wait_limit_seconds', 'Batch Wait Limit (Seconds)', ConfigAttributeType.INT, False, True,
            ...
            ConfigProperty('hard_delete', 'Hard Delete', ConfigAttributeType.BOOLEAN, False, True,
