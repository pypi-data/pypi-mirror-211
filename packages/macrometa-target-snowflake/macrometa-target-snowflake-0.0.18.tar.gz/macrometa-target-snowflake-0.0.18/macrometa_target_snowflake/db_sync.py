...
def validate_config(config):
    ...
    config_target_schema = config.get('target_schema', None)
    if not config_target_schema:
        errors.append("'target_schema' (string) not defined in config.")

    # Check target table config
    config_target_table = config.get('target_table', None)
    if not config_target_table:
        errors.append("'target_table' (string) not defined in config.")
...
class DbSync:
    ...
    def __init__(self, connection_config, stream_schema_message=None, table_cache=None, file_format_type=None):
        ...
        if self.stream_schema_message is not None:
            ...
            #
            #   1: 'target_schema' key  : Target schema is the same for every incoming stream

            config_target_schema = self.connection_config.get(
                'target_schema', '').strip()

            stream_name = stream_schema_message['stream']

            if config_target_schema:
                self.schema_name = config_target_schema

            if not self.schema_name:
                raise Exception(
                    "Target schema name not defined in config. "
                    "'target_schema' (string) defines "
            ...
            #
            #   1: 'target_schema_select_permissions' key  : USAGE and SELECT privileges will be granted on every table to a given role
        ...
        return snowflake.connector.connect(
            ...
            session_parameters={
                ...
                'QUERY_TAG': create_query_tag(self.connection_config.get('query_tag'),
                                              ...
                                              table=self.table_name(self.connection_config.get('target_table'), False, True))
            }
        )
    ...
    # TODO: If stage needed at later stage, take name defined from user
    def get_stage_name(self, stream):
        ...
        table_name = self.table_name(self.connection_config.get(
            'target_table'), False, without_schema=True)
    ...
    def load_file(self, s3_key, count, size_bytes):
        ...
        self.logger.info("Loading %d rows into '%s'", count,
                         self.table_name(self.connection_config.get('target_table'), False))
        ...
        if len(self.stream_schema_message['key_properties']) > 0:
            try:
                ...
                self.logger.error(
                    ...
                    self.table_name(self.connection_config.get(
                        'target_table'), False), stream
                )
                raise ex
    ...
    def create_table_query(self, is_temporary=False):
        ...
        p_table_name = self.table_name(
            self.connection_config.get('target_table'), is_temporary)
    ...
    def update_columns(self):
        ...
        table_name = self.table_name(
            self.connection_config.get('target_table'), False, True)
        all_table_columns = []
        ...
        columns_to_replace = [
            ...
            #
            # Macrometa-target-snowflake maps every data-time JSON types to TIMESTAMP_NTZ but sometimes
    ...
    def drop_column(self, column_name, stream):
        ...
        drop_column = f"ALTER TABLE {self.table_name(self.connection_config.get('target_table'), False)} DROP COLUMN {column_name}"
    ...
    def add_column(self, column, stream):
        ...
        add_column = f"ALTER TABLE {self.table_name(self.connection_config.get('target_table'), False)} ADD COLUMN {column}"
    ...
    def _refresh_table_pks(self):
        ...
        new_pks = set(pk.upper()
                      for pk in self.stream_schema_message.get('key_properties', []))
        ...
        if not new_pks and current_pks:
            self.logger.info(
                'Table "%s" currently has PK constraint, but we need to drop it.', table_name)
            queries.append(f'alter table {table_name} drop primary key;')

        elif new_pks != current_pks:
            self.logger.info(
                'Changes detected in pk columns of table "%s", need to refresh PK.', table_name)
