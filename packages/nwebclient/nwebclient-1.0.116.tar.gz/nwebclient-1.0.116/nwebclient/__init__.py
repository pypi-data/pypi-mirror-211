...
class NWebDoc:
    ...
    def setMetaValue(self, ns, name, value):
        data = {'ns': ns, 'name':name, 'value':value}
        self.__client.req('w/api/doc/'+self.__data['document_id']+'/meta', data)
    def getMeta(self):
...
class NWebClient:
    ...
    def downloadImages(self, limit=1000, tag=None, size=None):
        ...
        for doc in docs:
            if size is None:
                ...
                self.reqToFile('image/'+str(doc.id())+'/thumbnail/'+str(doc.id())+'/t.jpg', str(doc.id())+ '.jpg')
    ...
    def imagesUrls(self, tag=None, limit=1000, size='cs', file=None):
        ...
        docs = self.docs(q)
        for doc in docs:
            url = self.__url + 'image/'+str(doc.id())+'/thumbnail/'+str(doc.id())+'/t.jpg'
