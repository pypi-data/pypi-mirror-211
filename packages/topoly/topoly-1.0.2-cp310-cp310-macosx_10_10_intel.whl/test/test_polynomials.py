...
def prepare_knot_polynomials_ideal():
    ...
    for algorithm in algorithms_knots.keys():
        ...
        for curve in knot_ideal.keys():
            ...
            polynomials[algorithm][curve] = algorithms_knots[algorithm](knot_ideal[curve], closure=Closure.CLOSED, chiral=True)
    log.info('Results:')
    ...
    for algorithm in algorithms_links.keys():
        ...
        for pdb in link_curves.keys():
            ...
            if expected == '2^2_1' and algorithm == 'Yamada':
                expected = '2^2_1' #'2^2_1|7^2_7' #było wcześniej #to wygląda bardzo na bardzo losowy if??? trzeba więcej komentarzy robić do tłumaczenia
            result = polynomials[algorithm][pdb].get(expected, 0)
            if result < 0.5:
