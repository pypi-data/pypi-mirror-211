...
class LocalDocQA:
    ...
    def init_cfg(self,
        ...
        if llm_model.startswith('moss'):
            ...
            self.llm = MOSS()
        elif llm_model.startswith('chatglm'):
    ...
    def get_knowledge_based_conent_test(self, query, vs_path, chunk_conent,
        ...
        related_docs_with_score = self.vector_store.similarity_search_with_score(query, k=vector_search_top_k)
        ...
        prompt = f"{query}. 你必须利用以下文档中包含的信息回答这个问题: /n/n```/n/n"
        prompt += "<br/>".join([f"({k}): " + doc.page_content for k, doc in enumerate(related_docs_with_score)])
        prompt += "/n/n```/n/n"
