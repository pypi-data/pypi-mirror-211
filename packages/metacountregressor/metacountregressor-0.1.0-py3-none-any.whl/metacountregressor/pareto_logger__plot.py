...
os.chdir(os.path.expanduser('Z:/fixed_the_gradients'))
os.chdir(os.path.expanduser('Z:/paper_fixed_3'))
#os.chdir(os.path.expanduser('Z:/this_should_actually'))
#os.chdir(os.path.expanduser('Z:/smaller_actually_230_300'))


#os.chdir(os.path.expanduser('Z:/phd_again_ali'))
...
for i in folders:
    ...
    try:
        ...
        if problem_number ==4:
            df = df[df['incumbent_bic'] >= 700]
        
        
        if problem_number ==8:
            df = df[df['incumbent_bic'] <= 18000]
                
        df =df[df['incumbent_MAE'] <= 100]
...
def seaborn_sa(data, ax):
    ...
    sns.heatmap(table, cmap='coolwarm', annot=False, fmt=".0f", linewidths=.5, ax=ax)
...
def seaborn_plot_de(data, ax):
   ...
   # df_pivot = data.pivot_table('percentage_of_crossover', 'population_size', 'best_solution', aggfunc='mean')
    ...
    # Create the heatmap using Seaborn
    sns.heatmap(df_pivot, cmap='coolwarm', annot=False, fmt='.0f', ax=ax)
...
for i in folders:
    try:
        ...
        if spread_new <spread_max:
            spread_max = spread_new
            store = file
        
        old = 0
        if old:
            if spread(x,y, true_x, true_y) < spread_max:
                spread_max = spread(x,y, true_x, true_y)
                store = file
    except:
        print('wh')   
...
if problem_number ==4:
    df = df[df['incumbent_bic'] >= 700]
df =df[df['incumbent_MAE'] <= -1+10*10**5]
#df = df[df['incumbent_pval_exceed'] == 0]
df = df.reset_index(drop =True)
