...
class Solution(dict):
    ...
    def add_objective(self, bic = None, MAE=None, loglik=None, num_parm = None, pval_exceed = None, aic = None):
        if bic is not None:
            if bic < 10000000.0:
                self['bic'] = bic
        if MAE is not None:
...
class Pareto(object):
    ...
    def __init__(self, obj_key1, obj_key_2, multiobjective = True) -> None:
    ...
    def weighted_obj(self, sln):
        val = sln[self.obj_key_1]/self.mean_ob1+sln[self.obj_key_2]/self.mean_ob2
        return val
    ...
    def check_dominance(self, obj1, obj2):
        ...
        if len(obj1) != 2:
            ...
            ob1 = {k:obj1[k] for k in obj1.keys() if k in desired_keys} #subdictionaries
    ...
    def update_means(self):
        f1 = mean(self.Pareto_F[x][self.obj_key_1] for x in range(len(self.Pareto_F)))
        f2 = mean(self.Pareto_F[x][self.obj_key_2] for x in range(len(self.Pareto_F)))
        self.mean_ob1 = f1
        self.mean_ob2 = f2
    ...
    def delta_calc(self, Proposed_Struct, Current_Struct):
        delta = (Proposed_Struct[self.obj_key_1] - Current_Struct[self.obj_key_1])/self.mean_ob1\
            +(Proposed_Struct[self.obj_key_2] - Current_Struct[self.obj_key_2])/self.mean_ob2
    ...
    def find_bestsol(self, Struct):
        ...
        weights_obj1 = [(Struct[x][self.obj_key_1])-min_obj1/(max_obj1-min_obj1) for x in range(len(Struct))]

        if self.multi_objective:
            max_obj2 = max(Struct[x][self.obj_key_2] for x in range(len(Struct)))
            min_obj2 = min(Struct[x][self.obj_key_2] for x in range(len(Struct)))
            weights_obj2 = [(Struct[x][self.obj_key_2])-min_obj2/(max_obj2-min_obj2) for x in range(len(Struct))]

            weights = [weights_obj1[x] + weights_obj2[x] for x in range(len(Struct))]
            
            weights_alt = [f_joined*(w_2*Struct[x][self.obj_key_1] + w_1*Struct[x][self.obj_key_2]) for x in range(len(Struct))]
