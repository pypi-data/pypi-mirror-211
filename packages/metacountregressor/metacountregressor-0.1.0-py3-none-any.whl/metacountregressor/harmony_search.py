...
DifferentialEvolutionMulti= namedtuple('DifferentialEvolutionMulti', ['elapsed_time', 'best_solutions', 'population_solutions'])
def _plot(x, y, z, xlabel=None, ylabel=None, zlabel=None, filename=None):
    from matplotlib import pyplot as plt

    # data_tuples = list(zip(x, y, z))
    # data_tupeples_df = pd.DataFrame(data_tuples, columns= ['iteration', 'incumbent', 'best'])
    fig, ax = plt.subplots()
...
def dict_mean(dict_list, ignore = ['layout', 'fixed_fit', 'rdm_fit', 'rdm_cor_fit', 'zi_fit', 'simple', 'pvalues']):
    mean_dict = {}
    if ignore is None:
...
def pareto_logger(pareto_set, iteration, complexity, folder = None):
    if folder is not None:
        name = folder + '/pareto_logger' + str(iteration) + 'complex'+ str(complexity) +".csv"
    
    else:   
        name = 'pareto_logger' + str(iteration) + 'complex'+ str(complexity) +".csv"
...
def logger(iteration, incumbent1, best1 = None, alt_method = True, name = None, multi = 0, local_best1 = None):
    ...
    if alt_method:
        ...
        iter = pd.DataFrame(data_tuples, columns=['iteration']) 
        if best is not None:
            if local_best is None:
                df = pd.concat([iter, incumbent, best], axis =1)
            else:
                df = pd.concat([iter, incumbent, best, local_best], axis =1)
        else:
            if local_best is None:
                df = pd.concat([iter, incumbent], axis =1)
            else:
                df = pd.concat([iter, incumbent, local_best], axis =1)
...
def different_evolution(objective_function, initital_slns=None, **kwargs):
    if not isinstance(objective_function, ObjectiveFunction):
        raise TypeError
    de = DifferentialEvolution(objective_function, **kwargs)
    results = de.differential_evolution_run(initial_slns=initital_slns)
    x = list()
    y = list()
    z = list()
...
def differential_evolution_non_mp(objective_function, initial_slns=None, **kwargs):
    ...
    if objective_function.is_multi:
        ...
        best, pare = de.differential_evolution_run(initial_slns=initial_slns)
        ...
        iterations , solutions, best_solutions, best_fitness, best_struct, average_best = de.differential_evolution_run(initial_slns=initial_slns)
...
"""
def differential_evolution(objective_function, num_processes, num_iterations, initial_slns=None, **kwargs):
    
           # Here, we use multiprocessing.Pool to do multiple differential evoluition runs simultaneously. Since DE is stochastic (unless random_seed is set),
         #   multiple runs can find different results. We run the specified number of iterations on the specified number of processes and return
          #  an instance of Diffential Evolution.
    ...
    start = datetime.now()
    pool_results = [pool.apply_async(worker, args=(objective_function, initial_slns, 'de', kwargs)) for i
                    in
    ...
    for result in pool_results:
        ...
        if objective_function.is_multi:
            population_t, population_p = result.get()
            
            #_plot(population_t['sol_num'], iter_fitness, fitness_collection, 'Iterations', 'Incumbent', 'Best', str(result_count))
        else:    
            iter, iter_fitness, fitness_collection, fitness, fitness_struct = result.get()  # multiprocessing.pool.AsyncResult is returned for each process, so we need to call get() to pull out the value
            data_tuples = list(zip(iter, iter_fitness, fitness_collection))
            df = pd.DataFrame(data_tuples, columns=['iteration', 'incumbent', 'best'])
            df.to_csv('result_frame' + str(result_count) + ".csv", index=False)
            if (objective_function.maximize() and fitness > best_fitness) or (
                    not objective_function.maximize() and fitness < best_fitness):
                best_solution = fitness_struct
                best_fitness = fitness
            iteration.append(iter)
            iteration_best.append(iter_fitness)
            fitness_best_for_average.append(fitness)
            fitness_best.append(fitness_collection)
            _plot(iter, iter_fitness, fitness_collection, 'Iterations', 'Incumbent', 'Best', str(result_count))
    
    if objective_function.is_multi:
        if not isinstance(population_p, list):
            raise Exception('not a list argh')
        if not isinstance(population_t, list):
            raise Exception('not a list pops')
        
        return DifferentialEvolutionMulti(elapsed_time=elapsed_time, best_solutions=population_t, population_solutions=population_p)
    else:    
        return DifferentialEvolutionResults(elapsed_time=elapsed_time, iteration=iteration,
                                        iter_solution=iteration_best, best_solutions=fitness_best,
                                        best_fitness=float(best_fitness),
                                        best_struct=best_solution, average_best=np.mean(fitness_best_for_average))
...
def simulated_annealing_non_mp(objective_function, initial_slns =None, **kwargs):
    ...
    return sa.run(initial_slns=initial_slns)
    
   
    
""""
def simulated_annealing(objective_function, num_processes, num_iterations, initial_slns=None, hyperparameters=None):
    
          #  Here, we use multiprocessing.Pool to do multiple differential evoluition runs simultaneously. Since DE is stochastic (unless random_seed is set),
         #   multiple runs can find different results. We run the specified number of iterations on the specified number of processes and return
        #    an instance of Simulated Annealing.
    ...
    start = datetime.now()
    pool_results = [pool.apply_async(worker, args=(objective_function, initial_slns, 'sa', hyperparameters)) for i
                    in
                    range(num_iterations)]
    pool.close()  # no more tasks will be submitted to the pool
    pool.join()  # wait for all tasks to finish before moving on
    ...
    for result in pool_results:
        ...
        if (objective_function.maximize() and fitness > best_fitness) or (
                not objective_function.maximize() and fitness[objective_function._obj_1] < best_fitness):
            best_solution = fitness_struct
            best_fitness = fitness[objective_function._obj_1]
        iteration.append(iter)
        iteration_best.append(iter_fitness)
        fitness_best_for_average.append(fitness[objective_function._obj_1])
        fitness_best.append(fitness_collection)
        _plot(iter, [a[objective_function._obj_1] for a in iter_fitness], [b[objective_function._obj_1] for b in fitness_collection], 'Iterations', 'Incumbent', 'Best', str(result_count))
    return SimulatedAnnealingResults(elapsed_time=elapsed_time, iteration=iteration,
                                        iter_solution=iteration_best, best_solutions=fitness_best,
                                        best_fitness=best_fitness,
                                        best_struct=best_solution, average_best=np.mean(fitness_best_for_average))
...
def harmony_search_non_mp(objective_function, initial_harmonies = None, hyperparameters = None):
    ...
    results = hs.run(initial_harmonies)
    end = datetime.now()
    elapsed_time = end - start
    if objective_function.is_multi:
...
"""""
def harmony_search(objective_function, num_processes, num_iterations, initial_harmonies=None, **kwargs):
    ...
    if not isinstance(objective_function, ObjectiveFunction):
        raise Exception('not part of the class')
    pool = Pool(num_processes)
   
    start = datetime.now()
    pool_results = [pool.apply_async(worker, args=(objective_function, initial_harmonies, 'hs', kwargs)) for i in
                    range(num_iterations)]
    pool.close()  # no more tasks will be submitted to the pool
    pool.join()  # wait for all tasks to finish before moving on
    end = datetime.now()
    ...
    for result in pool_results:
        ...
        if objective_function.is_multi:
            ...
            harmony, fitness, harmony_memory, harmony_history, iter, iter_fitness, fitness_collection = result.get()  # multiprocessing.pool.AsyncResult is returned for each process, so we need to call get() to pull out the value
            data_tuples = list(zip(iter, iter_fitness, fitness_collection))
            df = pd.DataFrame(data_tuples, columns=['iteration', 'incumbent', 'best'])
            df.to_csv('result_frame' + str(result_count) + ".csv", index=False)
            if (objective_function.maximize() and fitness > best_fitness) or (
    ...
    if objective_function.is_multi is False:        
        _plot(iter, iter_fitness, fitness_collection, 'Iterations', 'Incumbent', 'Best', str(result_count))
...
"""""
def worker(objective_function, initial_solutions=None, algorithm='hs', tuple_conditions = None, **kwargs):
    ...
    if algorithm == 'hs':
        try:
            if not terminating.is_set():
                ...
                hs = HarmonySearch(objective_function)
                result = hs.run(initial_solutions)
        ...
        try:
            ...
            if not terminating.is_set():
                sa = SimulatedAnnealing(objective_function, **kwargs)
                result = sa.run(initial_slns=initial_solutions)
        ...
        try:
            ...
            if not terminating.is_set():    
                ...
                return de.differential_evolution_run(initial_slns=initial_solutions)
        except KeyboardInterrupt:
            terminating.set()      
                # return de.run(initial_harmonies=initial_solutions)
...
class DifferentialEvolution(object):
    ...
    def hard_mutate_index_and_value(self):
        ...
        inject_this = self._obj_fun._discrete_values[random_index][random.randint(0, len(self._obj_fun._discrete_values[random_index]) - 1)]
    ...
    def mutation(self, struct_a, struct_b, struct_c) -> list:
        ...
        while same == True:
            for i in range(0, len(struct_a)):
                ...
                if old_code:
                    try:
                        difference = self._obj_fun.get_index(i, struct_b[i]) - self._obj_fun.get_index(i, struct_c[i])
                    except Exception as e:
                        print(e) 
                        print('i', i , 's_b, ', struct_b, 's_c', struct_c)  
                        
                    if (difference) >= 0:
                        # adjust down
                        new_a[i] = self._obj_fun.get_value(i, current_index - random.randint(0, min(self.F * difference,
                                                                                                    current_index)))
                    else:
                        # adjust up
                        ...
                        #print(min(-self.F * difference,self._obj_fun.get_num_discrete_values(i) - current_index - 1))
                        new_a[i] = self._obj_fun.get_value(i, current_index + random.randint(0, min(-self.F * difference,
                                                                                                    self._obj_fun.get_num_discrete_values(
                                                                                                        i) - current_index - 1)))
            endless += 1
            same = self.mixed_list_chescker(new_a, struct_a)
            if same:
    ...
    def does_it_appear(self, new):
        for d in self._population:
            if self.mixed_list_chescker(d['layout'], new):
                print('randm sln appears in population')
                return True
    ...
    def crossover(self, mutated, target, dims, cr):
        ...
        return trial
    def differential_evolution_run(self, initial_slns=None):
        ...
        self._population = self._initialize(initial_slns)
        if self._obj_fun.is_multi:
        ...
        while (i <= self.iter + 1) and (time_elapsed <= self._obj_fun.get_max_time()) and (iterations_without_improvement < self._obj_fun.get_termination_iter()):
            ...
            for j in range(self._pop_size):
                ...
                if self._obj_fun.is_multi:
                    trial = self.crossover(mutated, self._population[j]['layout'], len(self._population[j]['layout']), self.cr)
                else:
                    trial = self.crossover(mutated, self._population[j]['layout'], len(self._population[j]['layout']), self.cr)
                try:
                ...
                if self.pf.get_objective_is_multi():
                    ...
                    if self.pf.check_dominance([obj_trial[self.pf.obj_key_1], obj_trial[self.pf.obj_key_2]], [self._population[j][self.pf.obj_key_1], self._population[j][self.pf.obj_key_2]]):#if solution dominates existing #FIXME some error here true but not entering
                        ...
                        logger(self.it_process, obj_trial, self._population, True, self.instance_number +'/population_logger_strict_non_pareto.csv', 1 )
                        logger(self.it_process, obj_trial, self._pareto_population, True, self.instance_number +'/population_logger_pareto.csv', 1 )
                    else:
                    ...
                    logger(self.it_process, obj_trial, best_solution, name = self.get_instance_name(), local_best1 = it_best)   
    ...
    def _initialize(self, initial_slns=None):
        """
        ...
        initial_list = list()
        print('the pop size is', self._pop_size)
        #pool = mp.Pool(5)
        #time_before = time.time()
        #result_fitness = [pool.apply_async(self._obj_fun.get_fitness, args = (initial_slns[i], self.pf.get_objective_is_multi())) for i in range(self._pop_size)]
        #pool.close()
        #pool.join()
        #for i in result_fitness:
        #    fitness = i.get()
        #    if self.pf.get_objective_is_multi():
...
class SimulatedAnnealing(object):
    ...
    def __init__(self, objective_function, **kwargs):
        self._obj_fun = objective_function
        if not isinstance(objective_function, ObjectiveFunction):
            raise Exception('objective function not working properly')
    ...
    def run(self, initial_slns=None):
        """
        ...
        start_time = datetime.now()
        # fill randomly, but with initial_slns if provided
        initial_slns_struct = self._initialize(initial_slns)
        # Calculate the init   ial temperature, and seed this instance so it is always the same
        if self._obj_fun.use_random_seed():
            self._obj_fun.set_random_seed()
        self.temp_max = self.Calculate_Temp(initial_slns_struct, acceptance_prob=self.INTL_ACPT, multi = self.pf.get_objective_is_multi())
        if self.best_energy is None:
            raise Exception
        else:
            self.current_struct, self.current_enery = initial_slns_struct[0]
        ...
        while (iteration <= self.MAX_ITERATIONS) and (
                elapsed_time <= self._obj_fun.get_max_time()) and (iterations_without_improvement < self._obj_fun.get_termination_iter()):
            elapsed_time = (datetime.now() - start_time).total_seconds()
            for i in range(self.STEPS_PER_TEMP):
                # Get Neighbouring solution and Energy level
                #print('the current struct is ' ,self.current_struct)
                nbr_struct = self._get_neighbour(self.current_struct, round(self._crossover_perc*len(self.current_struct)))
                nbr_energy = self._obj_fun.get_fitness(nbr_struct, multi = self.pf.get_objective_is_multi())
               
                # Acceptance Criteria
                if random.uniform(0, 1) <= self.Energy_Acceptance_Criteria(nbr_energy, self.current_energy, current_temperature, self.pf.get_objective_is_multi()):
                    #print('Accepted New with fitness of', nbr_energy)
                    didchange = self.pf.did_it_change()
                    if didchange:
                        pareto_logger(self.pf.Pareto_F, iteration, self._obj_fun.complexity_level, self._obj_fun.instance_number)
                    self.current_energy = nbr_energy
                    ...
                    if not self.pf.get_objective_is_multi(): #if this is the global best (show) #TODO determine global best for multiobjective    
                     ...
                     # log data
                    iteration += 1
                    self.profiler.append([iteration, self.current_energy, self.best_energy])
                    
                try:
                    logger(iteration, nbr_energy, self.best_energy, name = self.get_instance_name(), local_best1 = self.current_energy)
                except Exception as e:
    ...
    def _get_neighbour(self, current, mutations=None):
        ...
        if current[-1] != 0: #sum of model plus dispersion paramater
            nParm = sum(prmVect)
        else:
            nParm = sum(prmVect) +1
        ...
        if mutations is None:
            ...
            for i in range(mutations):
                ...
                while allGood == 0:
                    ...
                    # new_nbr_i = self._obj_fun.get_value(rdm_i)
                                get_rdm_j = random.randint(0, self._obj_fun.get_num_discrete_values(rdm_i) - 1)
                                if (self._obj_fun.get_num_discrete_values(rdm_i) - 1) <1: #TODO: remove this is just a test
    ...
    def _initialize(self, initial_slns=None):
        ...
        for i in range(0, self._num_intl_slns):
            print('evaluating initial sln', i)
            try:
                fitness = self._obj_fun.get_fitness(initial_slns[i], multi = self.pf.get_objective_is_multi(), max_routine =2)
                if self.pf.get_objective_is_multi():
        ...
        if multi is False:
            for gene, fitness in slns:
                ...
                if fitness.get(self._obj_fun._obj_1) < feasibility:
                    ...
                    if self.best_energy and fitness.get(self._obj_fun._obj_1) <= self.best_energy.get(self._obj_fun._obj_1):
                        self.best_energy = fitness
            ...
            for gene, fitness in slns:
                ...
                fitness_list_2.append(fitness.get(self._obj_fun._obj_1))
    ...
    def Energy_Acceptance_Criteria(self, proposed, current, temperature, multi_objective = False):
        if not multi_objective:
            ...
            dominance = self.pf.check_dominance([proposed[self.pf.obj_key_1], proposed[self.pf.obj_key_2]], [current[self.pf.obj_key_1], current[self.pf.obj_key_2]]) #where proposed and current are list objectives
    ...
    def cooling_linear_m(self, step):
        return self.temp_max / (1 + step * self.alpha)
...
class HarmonySearch(object):
    ...
    def run(self, initial_harmonies=None):
        ...
        while (num_imp < self._obj_fun.get_max_imp()) and (
            ...
            iterations_without_improvement = self._update_harmony_memory(harmony, fitness, iterations_without_improvement, self.pf.get_objective_is_multi())
            num_imp += 1
            if iterations_without_improvement ==0: #if there is any kind of improvement updae the logs
                if self.pf.get_objective_is_multi():
                    logger(num_imp, fitness, self._harmony_memory,True, self.get_instance_name(), 1) #for consistency
                    ...
                    #update logs
                    try:
                        logger(num_imp, fitness, improvisation_best, name = self.get_instance_name(),local_best1 = generation_best)
                    except Exception as e:
                        print('log broken', e)    
            ...
            if num_imp % self._obj_fun.get_hms() == 0:
                ...
                if self.pf.get_objective_is_multi() is False:
                    ...
                    pareto_logger(self._pareto_harmony_memory, num_imp/self._obj_fun.get_hms(), self._obj_fun.complexity_level, self._obj_fun.instance_number)    
                generation += 1
                iterations_without_improvement += 1
        ...
        if self.pf.get_objective_is_multi():
            ...
            return self._harmony_memory[0]['layout'], self._harmony_memory[0], self._pareto_harmony_memory, self._harmony_history
        else:    
                        ...
                        if not isinstance(stuff, dict):   
                            for j, b in enumerate(stuff[i]['layout'][self._obj_fun._characteristics]):
                                if b:
                                    ...
                                    stuff[i]['layout'][j] = random.randint(1, self._obj_fun.get_num_discrete_values(j) - 1)
        ...
        if global_best_harmony:
            ...
            if self._obj_fun.is_variable(i):
                if self._obj_fun.is_discrete(i):
                    ...
                    old_code = 0
                    if old_code:
                        if random.random() < 0.5:  # adjust pitch down
                            harmony[i] = self._obj_fun.get_value(i, current_index -min(self._obj_fun.get_mpai(),
                                                                                                    current_index))
                            #print('up want',self._obj_fun.get_mpai(), 'down only',  current_index )
                            
                        else:  # adjust pitch up
                            #print('up want',self._obj_fun.get_mpai(), 'up only',  self._obj_fun.get_num_discrete_values(i) - current_index - 1) 
                            harmony[i] = self._obj_fun.get_value(i,
                                                                current_index + min(self._obj_fun.get_mpai(),
                                                                                                    self._obj_fun.get_num_discrete_values(
                                                                                                        i) - current_index - 1))
                else:
                    # continuous variable
    ...
    def _update_harmony_memory(self, considered_harmony, considered_fitness, iterations_improv = 0, IS_MULTI = True):
        ...
        if IS_MULTI:
            ...
            if considered_fitness['layout'] not in [a['layout'] for a in self._harmony_memory]: #TODO check just for integer values
            ...
            if considered_fitness['layout'] not in [a['layout'] for a in self._harmony_memory]:
                ...
                if (self._obj_fun.maximize() and considered_fitness[self.obj_1] < self._harmony_memory[-1][self.obj_1]) or (
                            not self._obj_fun.maximize() and considered_fitness[self.obj_1] > self._harmony_memory[-1][self.obj_1]):
