...
class OpenWithPandas(OpenWithBase):
    ...
    async def open_csv(self, filename: str, add_columns: dict, encoding) -> pandas.DataFrame:
        ...
        try:
            ...
            if self.parse_dates:
                self.parse_dates['infer_datetime_format'] = True
        except KeyError:
            pass
        try:
            # can we use pyarrow.
        ...
        if hasattr(self, 'bigfile'):
            ...
            try:
                ...
                result = detect_encoding(filename, encoding)
    ...
    async def run(self) -> Any:
        ...
        if self._filenames is None and not check_empty(self._data):
            if isinstance(self._data, list):
                for file in self._data:
                    ...
                    except (ValueError, pandas.errors.ParserError) as err:
                        raise ComponentError(
            ...
            for filename in self._filenames:
                ...
                if df is None or df.empty:
                    raise DataNotFound(f"Empty File {filename}")
                result.append(df)
        ...
        if len(result) == 1:
            ...
            if self.no_multi is True:  # get only one element
                ...
                try:
                    df = pandas.concat(
                        result, sort=False, axis=0, ignore_index=True
