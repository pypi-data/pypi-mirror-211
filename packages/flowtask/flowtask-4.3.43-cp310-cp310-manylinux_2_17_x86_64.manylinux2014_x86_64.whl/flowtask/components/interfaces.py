...
class DBInterface(ABC):
    ...
    def pg_connection(self, event_loop: asyncio.AbstractEventLoop = None) -> BaseDriver:
        pgargs: dict = {
            "server_settings": {
                "application_name": 'DataIntegration',
                "client_min_messages": "notice",
        ...
        if not event_loop:
            event_loop = asyncio.get_event_loop()
        return AsyncDB('pg', dsn=default_dsn, loop=event_loop, timeout=360000, **pgargs)
    ...
    async def get_driver(self, driver: str, conn: BaseDriver) -> BaseDriver:
        ...
        try:
            ...
            if error:
                raise Exception(f'DbClient: Error on Getting Datasource: {error!s}')
            if result:
                try:
