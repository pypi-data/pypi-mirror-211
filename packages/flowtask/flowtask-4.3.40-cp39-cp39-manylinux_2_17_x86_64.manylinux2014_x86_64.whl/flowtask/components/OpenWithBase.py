...
import numpy as np
from navconfig.conf import asyncpg_url, TASK_PATH
from asyncdb import AsyncDB
from asyncdb.exceptions import NoDataFound
from flowtask.exceptions import FileNotFound, ComponentError
...
supported_extensions = (
    '.xls', '.xlsx', '.xlsm', '.xlsb', '.xml', '.txt', '.csv', '.json', '.htm', '.html'
)

def detect_encoding(filename, encoding: str = 'utf-16le'):
...
class OpenWithBase(DtComponent):
    """
    OpenWithBase.
    ...
    def __init__(
        ...
        if 'clean_nat' in kwargs:
            self.clean_nat = kwargs['clean_nat']
            del kwargs['clean_nat']
        else:
            self.clean_nat = False
        ...
        super(OpenWithBase, self).__init__(
            loop=loop,
            job=job,
            stat=stat,
            **kwargs
    ...
    async def start(self, **kwargs):
        ...
        if self.filename:
            ...
            for fname in listing:
                logging.debug(f"Filename > {fname}")
                self._filenames.append(fname)
        elif self.previous and self.input:
        ...
        if not self._filenames:
            raise FileNotFound(
                "OpenWithPandas: File is empty or doesn't exists"
            )
        ...
        if self._filenames is None and self._data is not None:
            ...
            for file in self._filenames:
                ...
                if file.exists() and file.is_file():
                    ...
                    raise FileNotFound(
                        f"{__name__}: File doesn't Exists: {file}"
                    )
    ...
    async def colinfo(self):
        ...
        if hasattr(self, 'model'):
            raise NotImplementedError(
                "Using Models is not implemented yet."
            )
            ...
            if colinfo is not None:
                try:
                    ...
                    colinfo = {k: v for k, v in colinfo.items()
                               if k not in ignore}
                except KeyError:
                    pass
                # skipcols
                ...
                if replace:
                    if hasattr(self, 'pd_args') and isinstance(self.pd_args, dict) and 'skiprows' in self.pd_args:
                    ...
                    for column, dtype in coliter.items():
                        ...
                        if dtype in ('timestamp without time zone', 'timestamp with time zone', 'date'):
                            dates.append(column)
                        elif dtype in ('time', 'time with time zone', 'time without time zone'):
                            dates.append(column)
                        ...
                        elif dtype == 'integer' or dtype == 'smallint' or dtype == 'bigint':
                            dtypes[column] = 'Int64'
    ...
    async def run(self) -> Any:
        print(
            f"Opening File(s): {self._filenames!r}"
        )
            ...
            if hasattr(self, 'encoding'):
                ...
                with open(filename, 'rb') as f:
                    ...
                    if result_charset['confidence'] < 0.8:
                        ...
                        if result_charset['encoding'] == 'ASCII':
                            encoding = "ISO-8859–1"
                        else:
                            encoding = 'utf-8'
                    else:
