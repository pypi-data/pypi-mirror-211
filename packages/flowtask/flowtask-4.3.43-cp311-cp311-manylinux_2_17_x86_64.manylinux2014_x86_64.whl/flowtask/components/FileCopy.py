...
class FileCopy(FileBase):
    ...
    def start(self) -> bool:
        ...
        if hasattr(self, 'source'):
            # processing source
            if not 'directory' in self.source:
            ...
            if '*' in filename:
                ...
                path = self._source.joinpath(filename)
                if path.is_file():
                    logging.debug(f'Source Filename: {filename}')
        ...
        if hasattr(self, 'destination'):
            ...
            if not 'directory' in self.destination:
                raise ComponentError(
            ...
            if not self._destination.exists(): # create the destination directory
                self.debug(f'Creating new destination directory: {self._destination}')
                try:
                    self._destination.mkdir(parents=True, exist_ok=True)
                except OSError as ex:
                    raise FileError(
                        f"FileCopy: Error creating destination directory: {self._destination}"
                    ) from ex
            if 'filename' in self.destination:
                ...
                if self.destination['rename'] == True:
                    self._rename = True
                    self._destination = path
                    if path.is_file():
                        self.warning(
                            f'FileCopy: Warning, File {path} already exists, will be overwritten'
                        )
                else:
                    self._rename = False
                    if path.is_file():
