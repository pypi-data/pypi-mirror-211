...
class JWWithOpenAI4:
    ...
    def invoke(self,df1,df2):
        ...
        df = pd.read_json(stringmetric.invoke(df1,df2))
        print(df)
        ...
        table1_parts = np.array_split(table1_parts,JWWithOpenAI4.__get_split_size(len(table1_parts)))
        table2_parts = np.array_split(table2_parts, JWWithOpenAI4.__get_split_size(len(table2_parts)))
