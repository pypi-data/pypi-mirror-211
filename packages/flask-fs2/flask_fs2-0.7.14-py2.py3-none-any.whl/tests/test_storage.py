...
def test_url_from_config_without_scheme(app):
    ...
    with app.test_request_context(
        ...
        assert (
            storage.url("test.txt") == "http://somewhere.com/static/test.txt"
        )
    ...
    with app.test_request_context(
        ...
        assert (
            storage.url("test.txt") == "https://somewhere.com/static/test.txt"
        )
...
def test_metadata(app, mock_backend):
    ...
    url = url_for(
        "fs.get_file", filename="file.test", fs=storage.name, _external=True
    )
