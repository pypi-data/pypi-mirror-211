...
class MMO(Optimizer):
    ...
    def _check_params(self):
        ...
        if 'FWA' in self.methods:
            ...
            if self.constraints > 0:
                assert self.methods == 'Rank', f"FWA variant \'{self.methods['FWA']}\' does not support constraints! Use \'Rank\' method instead"
                ...
                if 'akb_model' in defined_params:                    
                    ...
                    if self.params['akb_model'] in ['FlyingStork', 'MessyTie', 'RightwardPeaks', 'OrigamiSnake']:   # w-list-based named akb_models                
                        if self.params['akb_model'] == 'FlyingStork':
                            ...
                            splinetype = 'linear'
                            if self.methods['PSO'] != 'TVAC':
                                self.log('Warning: akb_model \'OrigamiSnake\' was designed for TVAC PSO')
    ...
    def _init_method(self):
        ...
        if 'ABC' in self.methods:
            ...
            """ Initialize ABC """
            # Generate a swarm
            self.ABC_cS = np.array([Bee(self) for c in range(self.params['pop_size'])], dtype=Bee)
            ...
            # if all candidates are NaNs       
            if np.isnan([cP.f for cP in self.ABC_cS]).all():
                err_msg = 'ALL CANDIDATES FAILED TO EVALUATE.'
            
            if err_msg:
                return err_msg
            
            self.ABC_cB = np.array([cP.copy() for cP in self.ABC_cS])
    ...
    def _fitness_calc(self):
        ...
        for p, cP in enumerate(self.ABC_cS):
            temp_fitness = np.append(temp_fitness, self.ABC_cS[p].f)
    ...
    def _run(self):
        ...
        while True:
            ...
            if 'ABC' in self.methods:
                ...
                """probability update"""
                for p, cP in enumerate(self.ABC_cS):
                    # self.probability[p] = 0.1 + 0.9 * (self.ABC_cS[p].f/np.max(temp_fitness))
                    self.probability[p] = self.ABC_cS[p].f / np.sum(self._fitness_calc())
            ...
            if 'ABC' in self.methods:
                bests['ABC'] = np.min(self.ABC_cS).f
