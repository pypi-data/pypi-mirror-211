...
def build_pool_from_file(data_dir: str, plan_start_date: str, df_atb: pd.DataFrame):
    """
...
def process_shift(df_shift):
    ...
    for ind, row in df_shift.iterrows():
        ...
        shift_length = end_time - start_time if end_time > start_time else end_time + timedelta(hours=24) - start_time
        if shift_type not in shift_dict:
            shift_dict[shift_type] = {
                "c_list": [{"s_t": start_time_str, "e_t": end_time_str, "shift": shift_length}],
                "shift": shift_length,
            }
        else:
            shift_dict[shift_type]["shift"] += shift_length
            shift_dict[shift_type]["c_list"].append({"s_t": start_time_str, "e_t": end_time_str, "shift": shift_length})
    return shift_dict
    ...
    for index, row in df_job.iterrows():
        ...
        for op in process_ops.values():
            ...
            df_machine_op = df_product_process_machine[
                ...
                & (df_product_process_machine["process_id"] == int(op.process_id))
            ]
            ...
            op.process_id = int(op.process_id)
