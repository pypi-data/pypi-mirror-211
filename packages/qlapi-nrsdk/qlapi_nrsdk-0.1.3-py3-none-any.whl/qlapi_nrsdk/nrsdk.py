...
class NRSDK(object):
    ...
    def _start_stimulation(self, param, device=None):
        ...
        res = -1
        try:
            res = _api.QLNR_StartStimulationEx(self.login_handle, param_2, resp, len, device_code)
    ...
    def _stop_stimulation(self, device=None):
        ...
        device_code = c_char_p(device.encode('utf-8')) if device else c_char_p(None)
        try:
            res = _api.QLNR_StopStimulationEx(self.login_handle, resp, len, device_code)
    ...
    def _set_wifi(self, ssid, secret, device=None):
        self._assert_connected()
        print(f"ssid:{ssid}, secret:{secret}: device:{device}")

        p_resp = (c_char * 512)(0)
        len = 512
        res = -1
        p_ssid = c_char_p(device.encode('utf-8')) if device else c_char_p(None)
        p_secret = c_char_p(device.encode('utf-8')) if device else c_char_p(None)
        p_device = c_char_p(device.encode('utf-8')) if device else c_char_p(None)
        try:
            res = _api.QLNR_SetDeviceWifiConfigEx(self.login_handle, p_ssid, p_secret, p_resp, len, p_device)
        except Exception as e:
            print(e)
