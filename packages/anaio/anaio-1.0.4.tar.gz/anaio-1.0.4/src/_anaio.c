...
static PyObject *anaio_fzread(PyObject *self, PyObject *args) {
	...
	// Read ANA file
	if (debug == 1)
		printf("anaio_fzread(): Reading in ANA file\n");
...
static PyObject * anaio_fzwrite(PyObject *self, PyObject *args) {
	...
	switch (PyArray_TYPE((PyObject *) anadata)) {
		case (PyArray_INT8): 
			type = INT8; 
			if (debug == 1) 
				printf("anaio_fzwrite(): Found type PyArray_INT8\n");
			break;
		case (PyArray_INT16): 
			type = INT16; 
			if (debug == 1) 
				printf("anaio_fzwrite(): Found type PyArray_INT16\n");
			break;
		case (PyArray_FLOAT32): 
			type = FLOAT32; 
			if (debug == 1) 
				printf("anaio_fzwrite(): Found type PyArray_FLOAT32\n");
			break;
		case (PyArray_FLOAT64): 
			type = FLOAT64;
			if (debug == 1)
				printf("anaio_fzwrite(): Found type PyArray_FLOAT64\n");
			break;
