...
def swap(items: pd.DataFrame, assets: pd.DataFrame, *, partial_swaps_ok: bool = False,
    ...
    -------
    pandas.DataFrame
        A DataFrame with rows for each item swapped. Includes a "Swap Result"
    ...
    for index, row in items.iterrows():
        ...
        try:
            ...
            if len(swappable_assets) == 0:
                item_dict['Swap Result'] = ("This item does not depend on any assets, either as the item's parent or "
    ...
    for swap_group in swap_groups:
        ...
        for item_dependency_output, dependencies_with_relevant_assets in item_list:
            ...
            try:
                ...
                if status.errors == 'raise':
                    raise SPyRuntimeError(_common.format_exception(e))
                else:
                    swap_result['Swap Result'] = _common.format_exception(e)
