...
from tests.utils import (
    SOURCE, fixtures_path, fake_download, fake_grouped_cycles, fake_grouped_impacts, start_year, end_year,
...
@patch('hestia_earth.aggregation.cycle.quality_score.get_emissions_system_boundary', return_value=[])
...
def test_aggregate_cycle(*args):
    ...
    from hestia_earth.aggregation.cycle.quality_score import calculate_score
...
def test_aggregate_cycle_relative(*args):
    from hestia_earth.aggregation.cycle.utils import (
        AGGREGATION_KEYS, _update_cycle, _format_terms_results, _format_for_grouping
    )
    from hestia_earth.aggregation.cycle.quality_score import calculate_score

    with open(f"{fixtures_path}/cycle/relative-unit.jsonld", encoding='utf-8') as f:
        cycles = json.load(f)
    with open(f"{fixtures_path}/cycle/terms/relative-unit-aggregated.jsonld", encoding='utf-8') as f:
        expected = json.load(f)

    cycles = _group_by_product({
        "@type": "Term",
        "name": "Meat, chicken (liveweight)",
        "termType": "animalProduct",
        "@id": "meatChickenLiveweight",
        "units": "kg liveweight"
    }, _format_for_grouping(cycles), AGGREGATION_KEYS)
    results = aggregate(AGGREGATION_KEYS, cycles)
    results = list(map(_format_terms_results, results))
    results = list(map(_update_cycle(None, start_year, end_year, SOURCE), results))
    results = list(map(calculate_score, results))
...
@patch('hestia_earth.aggregation.impact_assessment.utils.download_hestia', return_value={})
@patch('hestia_earth.aggregation.utils.queries.download_hestia', side_effect=fake_download)
