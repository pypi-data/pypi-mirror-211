...
def check_latest_version(curversion):
    try:
        ...
        if latest_version > cur_version_num*1000+cur_version_frac+10:
            return True
...
def convert_coco_dict_to_df(coco_dict: dict, input_dir: str):
    ...
    assert 'filename' in df.columns and 'col_x' in df.columns and 'height' in df.columns, f"Failed to find columns in coco label dataframe {str(coco_dict)[:250]}"
