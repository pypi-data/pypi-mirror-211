...
def run(input_dir='',             
    ...
    fd_model = False
    if model_path.lower().startswith('dinov2') or model_path.lower() in ['efficientnet', 'resnet50', 'clip','clip336']:
    ...
    if bounding_box in ['face', 'yolov5s', 'rotated', 'xywh_bbox']:
        ...
        if bounding_box in ['face', 'yolov5s','rotated', 'xywh_bbox']:
            ...
            assert len(out_df), f"Read a dataframe with no columns from {local_file}"
