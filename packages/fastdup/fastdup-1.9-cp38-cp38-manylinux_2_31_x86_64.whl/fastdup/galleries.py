...
if __name__ == "__main__":
    ...
    if False:
        ...
        fd.run(overwrite=True, model_path='dinov2s', verbose=False,num_threads=1)
        fd.run(overwrite=True, verbose=True, num_threads=1)
    elif False:
        import fastdup
        os.chdir("../unittests")
        ...
        ret = fd.run(annotations=coco_annotations.head(1000), overwrite=True, num_images=1000, cc_threshold=0.97, threshold=0.97, print_summary=False, verbose=0, num_threads=6)
        assert ret == 0
        ret = fd.vis.component_gallery(draw_bbox=True, load_crops=False)
        assert ret == 0
        ...
        fd.vis.component_gallery(metric='size', slice='diff')
        fd.vis.component_gallery(metric='size', slice='diff')
        fd.vis.outliers_gallery(draw_bbox=True)
        fd.vis.outliers_gallery(draw_bbox=True)
    elif False:
        import fastdup
        os.chdir("../unittests")
        ...
        #
        fd = fastdup.create(work_dir=work_dir, input_dir=input_dir)
        fd.run(threshold=0.8, overwrite=True, annotations=df_annot, verbose=True)
        print(fd.invalid_instances())
        print(fd.annotations(valid_only=False))
    elif False:
