...
from fastdup.utils import convert_coco_dict_to_df
import pathlib
import re
...
class FastdupController:
    ...
    def connected_components_grouped(self, sort_by: str = 'comp_size', ascending: bool = True, metric = None, load_crops: bool = None, kwargs = {}) -> pd.DataFrame:
        """
        ...
        files = external_df.groupby('component_id')['filename'].apply(list)
        distance = external_df.groupby('component_id')['min_distance'].apply(np.min)
        crop_files = None
        if 'crop_filename' in external_df.columns:
            crop_files = external_df.groupby('component_id')['crop_filename'].apply(list)
        labels = None
        if 'label' in external_df.columns:
            labels = external_df.groupby('component_id')['label'].apply(list)
        if metric is not None:
            metrics = external_df.groupby('component_id')[metric].apply(np.mean)

        external_df = pd.DataFrame({'files': files.values, 'component_id': files.index.values,
    ...
    def run(self, input_dir: Union[str, Path] = None, annotations: Union[pd.DataFrame,list] = None, subset: list = None,
        ...
        if annotations is not None:
            if isinstance(annotations, list):
                ...
                assert os.path.exists(annotations), f"Failed to find annotations file {annotations}"
                import json
    ...
    def _add_annot_and_split(self, df: pd.DataFrame, data: bool, merge_on: List[str],
        ...
        if self._bbox in ["face","yolov5s"]:
            if self._bbox == "face":
                ...
                assert len(df_annot2), f"Failed to merge {df_annot.head()} {df_crops.head()} " + self.get_status_string()
                df_annot = df_annot2
    ...
    def _expand_annot_df(self):
        ...
        if self._run_mode == FD.MODE_CROP or self._run_mode == FD.MODE_ROTATED_BBOX or self._dtype == FD.BBOX:
            ...
            df_annot = pd.merge(self._df_annot, df_crops_annot[["crop_filename", "filename", "index"]], on=FD.ANNOT_FD_ID
                                                                          , how='left')
