...
def get_js_file(site_package_dir, logo_code):
    static_dir = os.path.normpath(os.path.join(site_package_dir, "share", "jupyter", "labextensions", "jupyterlab_templates", "static"))
    src_files = []
    ...
    if src_files:
        src_file_path = os.path.join(site_package_dir, "share", "jupyter", "labextensions", "jupyterlab_templates", "static", src_files[0])
...
def jupyterlab():
    ...
    if len(out) == 1:
        ...
        get_js_file(os.path.abspath(os.path.join(str_out, os.pardir, '..', '..')),logo_code)
