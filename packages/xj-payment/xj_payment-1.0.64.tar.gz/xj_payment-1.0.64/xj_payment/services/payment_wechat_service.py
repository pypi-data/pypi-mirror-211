...
class PaymentWechatService:
    ...
    def payment_applets_pay_v3(params):
        ...
        try:
            ...
            code, message = wxpay.pay(
                ...
                # amount={'total': 1},
                payer={'sub_openid': params['openid']}
            )
    ...
    def payment_web(params):
        ...
        try:
            ...
            if code in range(200, 300):
                ...
                noncestr = random_string(32)
                package = 'prepay_id=' + prepay_id
                paysign = wxpay.sign([subscription_app_id, timestamp, noncestr, package])
                signtype = 'RSA'
            else:
                return {"error": str(result)}
        except Exception as e:
    ...
    def wechat_jsapi(params, payment_type=None):
        ...
        if err:
            return None, {"error": "60005", "msg": "未授权登录"}
        if not sso['sso_unicode']:
            return None, {"error": "60005", "msg": "未授权登录"}
        try:
            wxpay = my_ali_pay_v3()
            code, message = wxpay.pay(
                ...
                notify_url=notify_url_v3,
                amount={'total': int(params['total_fee'])},
                payer={'sub_openid': sso['sso_unicode']}
            )
            result = json.loads(message)
            if code in range(200, 300):
                prepay_id = result.get('prepay_id')
    ...
    # 微信支付 现金红包
    @staticmethod
    def payment_red_envelopes(params):
        redpack = applets_red_envelopes()
        # pay = my_ali_pay()
        # redpack = pay.redpack.send(
        #     user_id="oj8KR5AR0sV20j96VBcnWulf7bbs",
        #     total_amount=1,
        #     send_name=merchant_name,
        #     act_name="余额提现",
        #     wishing="恭喜发财",
        #     remark="余额提现红包，请查收",
        # )
    ...
    def callback(headers, body):
        ...
        if result and result.get('event_type') == 'TRANSACTION.SUCCESS':
            ...
            PaymentWechatService.payment_logic_processing_v2(param)
    ...
    @staticmethod
    @transaction.atomic
    def payment_logic_processing_v2(param):
        ...
        try:
            ...
            if 'transaction_id' in param:
                transact_no = param['transaction_id']  # 微信支付订单号
            data = {
