...
class Conan():
    ...
    def restore(self) -> subprocess.CompletedProcess:
        ...
        return subprocess.run(
            f"conan install . --output-folder {self.build_directory} --build=missing --profile {self.profile}".split(),
            check=True,
    ...
    def build(self) -> subprocess.CompletedProcess:
        ...
        """
        return subprocess.run(
            f"conan build {pathlib.Path.cwd()} --output-folder {self.build_directory} --profile {self.profile}".split(),
            check=True
        )
