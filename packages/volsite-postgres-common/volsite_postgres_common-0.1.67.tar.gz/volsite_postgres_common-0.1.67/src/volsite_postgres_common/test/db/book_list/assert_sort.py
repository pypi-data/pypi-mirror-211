...
def assert_sorted_books_by_title(resp, direction: str, compared_end: int):
    books = resp[CA.Data][CA.Book]
    title_ids = list()
    for b in books:
        title_ids.append(b[CA.TitleId])
...
def assert_sorted_books(resp, val_name: str, read_fn, direction=CA.Ascending, sort_type: SortType=SortType.String):  # , ext_total_count=16):
    asc = False
    if direction == CA.Ascending:
        asc = True
    books = resp[CA.Data][CA.Book]
