...
#-------------------------------------------------------------------------------------------
    def calcODF(self,nbr=0,bw=0.2):
        ...
        return kde
#--------------------------------------------------------------------------------------------
    def plotODF(self,nbr=10000,bw=0.2,projz=1,plot_cm=True,plotOT=True,angle=np.array([30.,60.]),cline=10,ax=None,xoz_plane=False,**kwargs):
        '''
        ...
        sp=ax.tricontourf(xx, yy, np.exp(weights)/np.mean(np.exp(weights)), cline, **kwargs)
...
#-------------------------------------------------------------------------------------------
    def plotly_ODF(self,nbr=10000,bw=0.2,plot_cm=True,plotOT=True,cf=True,cline=10,xoz_plane=False,res=400,add_dot=None):
            '''
            Plot the Orienation Density Function
            ...
            if plot_cm:
                ...
                # plot contourf
                fig=odfplot.ODF_background(w0,contourf=cf,cline=cline,fig=fig,xoz_plane=xoz_plane)
