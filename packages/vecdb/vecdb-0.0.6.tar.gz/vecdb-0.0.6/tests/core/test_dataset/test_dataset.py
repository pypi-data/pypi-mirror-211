...
class TestDataset2:
    ...
    def test_update(self, full_dataset: Dataset):
        old_documents = full_dataset.search(20)["documents"]
        for document in old_documents:
            document["sample_1_value"] += 1
        res = full_dataset.update(old_documents)
...
class TestFilters:
    def test_equals(self, static_dataset: Dataset):
        ...
        res = static_dataset.search(page_size=1, filters=filters)
    ...
    def test_less_than(self, static_dataset: Dataset):
        filters = static_dataset["numeric_field"] < 5
        res = static_dataset.search(page_size=20, filters=filters)
        assert res["count"] == 5
    ...
    def test_ids(self, static_dataset: Dataset):
        res = static_dataset.search(page_size=20)
        documents = res["documents"]
...
class TestDatasetMedia:
    def test_upload_medias(self, empty_dataset: Dataset):
        urls = empty_dataset.insert_local_medias(["railogo.webp", "railogo.webp", "railogo.webp"])
        assert len(urls) == 3
...
class TestChunkFilters:
    def test_chunk_filters(self, mixed_dataset: Dataset):
        ...
        filtered = mixed_dataset.search(100, filters=filters)
