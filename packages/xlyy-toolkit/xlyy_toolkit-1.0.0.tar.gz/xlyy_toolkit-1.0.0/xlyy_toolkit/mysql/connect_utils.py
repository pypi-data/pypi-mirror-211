...
class basic_db:
    ...
    def connect_database(self,db_name:str) -> None:
        try:
            if not self.is_database_exists(db_name):
                ...
                xmprint(f"[检测到库不存在,已创建库[{db_name}]!")
            self.connection.select_db(db_name)
        except pymysql.err.OperationalError as e:
            xmprint(f"连接库{db_name}失败: ", e)
            raise
    ...
    def execute_sql_language(self, sql_language:str) -> tuple[tuple[any,...],...]:  #返回每行
        if sql_language is None:
            xmprint('sql_language = null')
            return ()
        sv = JavaStr(sql_language)
        ...
        try:
            ...
            xmprint("执行sql语句发生数据库错误: "+ lg)
        except pymysql.err.ProgrammingError as e:
            xmprint("你还没有选择数据库: " + e.__str__() )
            raise
        finally:
            if cursor is not None:
                cursor.close()
