...
class HDSoCPackager(PackagerLight):
    ...
    Since windows can appear which originate from disabled channels,
    these undesirable packets are discarded.
    ...
    def __init__(
        ...
        # Location in data to check if the packet is from a disabled channel
        min, max = self._data_check_range = (10, 12)
        self._data_check_ref = b"\x00" * (max - min)
    ...
    def _is_data_package(self, package: bytearray) -> bool:
        ...
        Will return `False` if the input is a data package, but the
        data is all zeros.
        ...
        """
        min, max = self._data_check_range
        return (
            len(package) == self._package_size
            and package[min:max] != self._data_check_ref
        )
