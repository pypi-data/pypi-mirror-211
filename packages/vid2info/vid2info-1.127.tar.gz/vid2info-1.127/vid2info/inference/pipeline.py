...
from vid2info.inference.configs.constants import BASE_MODEL, BASE_MODEL_CONFIG, QUALITY_ACCEPTABILITY_CONFIG
...
class Pipeline:
    def __init__(self, pipeline_config_yaml: str, detector_model_path: str = MODEL_PT_PATH, video_frame_rate: int | None = DEFAULT_VIDEO_FRAME_RATE,
        ...
        # Set the quality checker
        quality_checker_yaml = os.path.normpath(os.path.join(os.path.dirname(pipeline_config_yaml), config[QUALITY_ACCEPTABILITY_CONFIG]))
        self.quality_checker = QualityChecker(config_yaml=quality_checker_yaml)
