...
class ElementFiniteStateMachine:
    ...
    def __str__(self, default_tabs = 2):
        ...
        for change in self.history:
            ...
            text += f"{default_tabs} At: {time_as_str(timestamp, time_format=TIME_FORMAT)} - From : {prev_state} " \
                    f"to {new_state}.\n"
        return text
    ...
    def merge(self, other_finite_state_machine: 'ElementFiniteStateMachine'):
        ...
        """
        assert all(transition_key in other_finite_state_machine.transitions for transition_key in self.transitions), \
            f"Both finite state machines have different transitions. They are not safe to merge. " \
            f"Got: {self.transitions.keys()} and {other_finite_state_machine.transitions.keys()}."
