...
class VideoReader:
    def __init__(self, video_path: str, simulated_frame_rate: None | int = None, as_rgb: bool = False,
                 ...
                 simulate_webcam: bool = False, run_in_background: bool = True):
        """
        ...
        if simulate_webcam and run_in_background:
            logger.warning(f"Simulating webcam doesn't work properly with reading in background (because of the reading all frames nature)."
                           f"Setting run_in_background to False.")
        ...
        # Set an offset for virtually reading the video as if it was recorded at a different frame rate
        self.frames_offset = int(self.fps // simulated_frame_rate) if simulated_frame_rate is not None else 1
        self.as_rgb = as_rgb

        if h is not None or w is not None:
            # If only one dimension is None, calculate the other
            if sum(1 for x in (h, w) if x is None) == 1:
                h, w = self.calculate_frame_size_keeping_aspect_ratio(h=h, w=w)
            # If the expected aspect ratio is the same as the original, w
    ...
    def __get_webcam_simulated_frame(self):
        ...
        # If the current video second is greater than the current second, we need to wait
