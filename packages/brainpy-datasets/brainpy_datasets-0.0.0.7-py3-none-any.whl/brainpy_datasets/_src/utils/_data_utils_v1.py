...
def is_generator_or_sequence(x):
  """Check if `x` is a generator type."""
  builtin_iterators = (str, list, tuple, dict, set, frozenset)
  if isinstance(x, (bm.ndarray, jnp.ndarray, np.ndarray) + builtin_iterators):
    return False
  return (
...
def get_file(
  ...
  cache_dir `~/.brainpy`, placed in the cache_subdir `datasets`,
  and given the filename `fname`. The final location of a file
  `example.txt` would therefore be `~/.brainpy/datasets/example.txt`.
  ...
  Args:
      ...
      cache_dir: Location to store cached files, when None it
          defaults to the default directory `~/.brainpy/`.
  ...
  if cache_dir is None:
    cache_dir = os.path.join(os.path.expanduser("~"), ".brainpy")
  if md5_hash is not None and file_hash is None:
    file_hash = md5_hash
    hash_algorithm = "md5"
  datadir_base = os.path.expanduser(cache_dir)
  if not os.access(datadir_base, os.W_OK):
    datadir_base = os.path.join("/tmp", ".brainpy")
...
def init_pool_generator(gens, random_seed=None, id_queue=None):
  ...
  worker_proc.name = f"brainpy_worker_{worker_proc.name}"
    ...
    try:
      ...
      if "generator already executing" in str(e):
        raise RuntimeError(
          ...
          "brainpy requires a thread-safe generator when "
