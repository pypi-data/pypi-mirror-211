...
IPv4_Exp_ABS = "^{}$".format(IPv4_Exp)
NETIPv4_Exp = "^{}/\d+$".format(IPv4_Exp)

IPv6_Exp = r"([\dabcdef]{1,4}:){1,7}:[\dabcdef]{0,4}"
IPv6_Exp_ABS = "^{}$".format(IPv6_Exp)
NETIPv6_Exp = "^{}/\d+$".format(IPv6_Exp)
...
# Pause With Prompt
def Pause(prompt="Press ENTER to continue",pause=0,no_msg=False):
	"""
	...
	if pause > 0 or prompt.isdigit():
		if prompt.isdigit():
			pause = int(prompt)

		if not no_msg: Msg("Pausing for {} seconds".format(pause))
		time.sleep(pause)
	else:
		reply = input(prompt)
...
def Ask(prompt):
	...
	return Pause(prompt,pause=0,no_msg=False)
