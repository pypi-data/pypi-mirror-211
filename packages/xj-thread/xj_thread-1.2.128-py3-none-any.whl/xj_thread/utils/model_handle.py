...
def filter_dict_field(result_list, filter_filed_list=None, remove_filed_list=None, alias_dict=None):
    # 转换的数据类型不符合，直接返回出去
    if filter_filed_list is None and remove_filed_list is None and alias_dict is None:
        return result_list
    result = []
    for item in result_list:
        row_res = format_params_handle(item, filter_filed_list=filter_filed_list, remove_filed_list=remove_filed_list, alias_dict=alias_dict)
        if row_res:
...
def request_params_wrapper(func):
    ...
    def wrapper(instance, arg_request=None, *args, request=None, **kwargs):
        ...
        if content_type == "application/json":
            ...
            data = getattr(request, 'data', {})
        # 闭包抛出
        response = func(instance, *args, request=request, request_params={k: v for k, v in data.items()}, **kwargs, )
        return response
