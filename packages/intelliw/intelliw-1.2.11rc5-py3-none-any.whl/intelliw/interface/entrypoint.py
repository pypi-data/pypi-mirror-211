...
def __parse_args():
    ...
    train = parser.add_argument_group(
        '[train]', description='intelliw train -p project_path -r train_set_ratio')
...
def __package(task: str, alg_path: str, output_path: str):
    ...
    with zipfile.ZipFile(output, 'w', zipfile.ZIP_DEFLATED) as zipf:
        for root, _, files in os.walk(alg_path):
            if any(root.endswith(x) for x in ('__pycache__', '.git', '.idea')):
                continue
...
def __dataset(dataset_id=None, dataset_ratio=[], local_csv=None, local_corpus=None):
    """
    config dataset
    """
    if not dataset_ratio:
        dataset_ratio = [0.7, 0.3, 0]
    config.set_dataset_config(
