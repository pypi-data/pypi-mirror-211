...
class Response:
    ...
    def _try_json(self):
        try:
            return _py_json.loads(self.body)
        except Exception:
            return None
...
def stream_download(url, output_path, method="get", params=None, body=None, json=None, headers=None,
    ...
    with requests.request(method=method, url=url, stream=True, verify=False, data=body, params=params, json=json, timeout=DEFAULT_TIMEOUT) as r, open(output_path, 'wb') as file:
...
def post_json(url: str, headers: dict = None, params: dict = None, json: object = None,
    ...
    return sign_and_request(url, 'POST', headers, params, json=json,
                            timeout=timeout, auth_type=auth_type)
...
def sign_yht(refresh=False):
    ...
    if not config.is_server_mode():
        if config.TEMPORARY_USER_COOKIE:
            ...
            raise SignError("为配置用户Token", "本地调试数据集，请先在Cookie中获取用户Token, 并配置到环境变量 TEMPORARY_USER_COOKIE ")
...
def __do_request(url: str,
    ...
    for i in range(1, 5):
        try:
            ...
            # 验签失败重试
            if "<title>登录</title>" in resp.text:
