...
from intelliw.datasets.datasets import DataSets, AlgorithmsType, MultipleDataSets
...
class Pipeline:
    def __init__(self, reporter: Tuple[str, Recorder] = None, report_interval=-1):
        ...
        self.recorder = reporter if isinstance(reporter, Recorder) else Recorder(
            reporter, self.report_interval <= 0)
        self.instance = None
        self.custom_router = []
        self.alg_describe = ''

    def report_callback_infer(self):
        """
        推理上报
        """
    ...
    def _initialize_algorithm(self, algorithm_yaml_file):
        cfg = load_config(algorithm_yaml_file)
    ...
    def _initialize_model(self, model_yaml_file, parameters, option):
        model_cfg = load_config(model_yaml_file)
        ...
        # 从环境变量中加载PARAMETERS
        env_model_ps = os.environ.get('PARAMETERS')
        if env_model_ps:
        ...
        target_metadata_cfg = metadata.get("target", [])
        ...
        # 用户环境变量，取自metadata.userEnv
        user_env = metadata.get("userEnv", [])
        for ue in user_env:
            key = str(ue.get("key", ""))
            if key:
                os.environ[key] = str(ue.get("value", ""))

        # linkserver
        linkserver.config = self.model_cfg.get('linkserver', None)

        return parameters, option

    def _initialize(self, alg_file, model_file=None):
        try:
            # algorithm.yaml
    ...
    def _importalg_phase(self, path):
        ...
        if not os.path.isfile(os.path.join(path, Const.algorithm_yaml)):
            self._raise_exception(message.err_import_alg_missing_algorithyaml)
        cfg = load_config(os.path.join(path, Const.algorithm_yaml))
    ...
    def _load_mode(self, path, model_path):
    ...
    def importalg(self, path, need_initialize=True):
        ...
        try:
            ...
            self._raise_exception(message.err_import_algorithm, e)
        self.recorder.report(message.ok_import_algorithm)

    def importmodel(self, path, need_initialize=True, load_model=True):
        """
        导入模型
        :param path: 算法包所在路径
        ...
        if not os.path.isfile(model_yaml_path):
            ...
            self._raise_exception(message.err_import_model_missing_modelyaml)
        ...
        try:
            ...
            if need_initialize:
                self.instance = self._initialize(
                    algorithm_yaml_path, model_yaml_path)
                ...
                if load_model:
                    try:
                        ...
                        self._load_mode(path, model_path)
                    except Exception as e:
    ...
    def _get_train_kwargs(self, reader, column_meta):
        ...
        # 时间序列参数
        if gl.model_type == AlgorithmsType.TIME_SERIES:
            timeseries_process = gl.pop("timeseries_process")
            if timeseries_process is not None:
                kwargs["timeseries_process_cfg"] = timeseries_process.get(
                    Const.pre_train)

        return kwargs

    def train(self, datasets: Tuple[DataSets, MultipleDataSets]):
        """
        训练入口
        """
        if datasets is None:
            self._raise_exception(message.err_empty_train_data)
        ...
        try:
            ...
            self._raise_exception(message.err_function_process, e)
        ...
        try:
            ...
            if "kwargs" not in varnames:
                ...
                if len(varnames) == 1:
                    ...
                    self._raise_exception(message.err_missing_train_parameter)
    ...
    @linkserver()
    def _infer_process(self, data, request, func='infer', need_feature=True):
        if need_feature:  # 前处理
            data = self._transform_helper(Const.pre_predict, data, False)
            data = self._feature_helper(Const.pre_predict, data)

        # 推理
        self.instance.request = request
        infer_func = getattr(self.instance, func)
        if infer_func.__code__.co_argcount > 1:  # 获取函数参数数量
            result = infer_func(data)
        else:
            result = infer_func()

        if need_feature:  # 后处理
            result = self._transform_helper(
                Const.post_predict, result, False)

        return result

    def infer(self, data, request, func='infer', need_feature=True):
        """
        推理入口
        """
        ...
        try:
            ...
            result = self._infer_process(data, request, func, need_feature)

            # 上报
            self.recorder.record_infer_status(
                '', True, starttime, int(time.time() * 1000)
            )
            return result, None
        except Exception as e:
            ...
            self.recorder.record_infer_status(
                '', False, starttime, int(time.time() * 1000), '{}'.format(e)
            )
    ...
    def transform_call_chains(self, funcs, src):
        ...
        try:
            ...
            self._raise_exception(msg, e)

    def _not_select_columns(self):
        """
        如果有特征工程前处理，就需要忽略select_columns，要不会导致列下标错（时间序列有自己的特征工程）
        """
        if gl.feature_process and \
           gl.feature_process.get(Const.pre_train) and \
           self.model_type != AlgorithmsType.TIME_SERIES:
            return True
        return False
    ...
    def _raise_exception(self, msg: message.CommonResponse,
