...
from discord import AutocompleteContext, Option, SlashCommandOptionType
...
class Drifters(commands.Cog):
    ...
    async def clear(
        ...
        try:
            ...
            logger.error(e)
            return await ctx.respond(f"User not Found")
    ...
    async def list_system_jumps_wormholes(
        ...
        return await ctx.respond(f"Not Yet Implemented")
    ...
    async def list_region_wormholes(
        ...
        for wormhole in Wormhole.objects.filter(region=everegion, archived=False):
            embed.add_field(
                name=wormhole.complex, value=f"{wormhole.mass}, {wormhole.formatted_lifetime}, Updated:<t:{wormhole.updated_at.timestamp():.0}:R>", inline=False
            )
