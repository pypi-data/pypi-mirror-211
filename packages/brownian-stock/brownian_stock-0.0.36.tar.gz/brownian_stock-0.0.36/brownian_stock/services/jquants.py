...
def donwload_daily_stock(login_token: str, date: datetime.date) -> pl.DataFrame:
    try:
...
def donwload_daily_statements(login_token: str, date: datetime.date) -> Optional[pl.DataFrame]:
    try:
        date_str = date.strftime("%Y-%m-%d")
        headers = {"Authorization": "Bearer {}".format(login_token)}
...
if __name__ == "__main__":
    ...
    login_token = fetch_login_token(refresh_token)
    df = download_stock(login_token, code)
    print(df.head())
