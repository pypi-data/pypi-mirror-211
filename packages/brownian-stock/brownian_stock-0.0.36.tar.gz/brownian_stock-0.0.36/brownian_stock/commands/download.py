...
class DailyStockCrawler:
    def __init__(
        ...
        self.crawl_start = self.crawl_end - relativedelta(years=10)
    ...
    def crawl(self, limit: Optional[int] = None, interval: int = 1) -> None:
        ...
        for d in date_ls:
            ...
            try:
                # ダウンロードして保存
                df = jquants.donwload_daily_stock(self.auth.login_token, d)
...
class DailyStatementsCrawler:
    ...
    def crawl(self, limit: Optional[int] = None, interval: int = 1) -> None:
        ...
        for d in date_ls:
            ...
            try:
                # ダウンロードして保存
                df = jquants.donwload_daily_statements(self.auth.login_token, d)
                filename = d.strftime("%Y-%m-%d.csv")
                if df is not None:
