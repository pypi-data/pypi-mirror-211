...
class Listener:
    ...
    def close(self):
        ...
        if self.final_status == "FAIL":
            self._send_status_log_files(self.path_log, self.path_report, self.path_xml,"FAILURE", "Ha ocurrido un error en el Robot")
        elif self.errors:
            self._send_status_log_files(self.path_log, self.path_report, self.path_xml, "WARNING",
                                        "El robot ha finalizado la ejecucion con advertencias")
        else:
            self._send_status_log_files(self.path_log, self.path_report, self.path_xml, "SUCCESS", "Robot ejecutado con exito")
