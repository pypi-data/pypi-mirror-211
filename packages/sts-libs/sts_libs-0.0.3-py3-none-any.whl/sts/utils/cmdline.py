...
def run(
    ...
    Returns:
        A CompletedProcess object. See: https://docs.python.org/3.11/library/subprocess.html#subprocess.CompletedProcess
        CompletedProcess.stderr is always None, as we're combining both streams into stdout.
    """
    ...
    if use_popen:
        ...
        try:
            ...
            return CompletedProcess(
                args=cmd,
                returncode=retcode if retcode else 1,
                stdout=output_str.rstrip() if output_str else None,
            )
        except subprocess.TimeoutExpired:
            if verbose:
                print("WARN: Command timed out")
            return CompletedProcess(args=cmd, returncode=124, stdout=output_str)
    ...
    try:
        ...
        if not capture_output and verbose and output.stdout:
            print(output.stdout)
        return output

    except subprocess.TimeoutExpired as e:
        return CompletedProcess(args=e.args, returncode=124, stdout=e.stdout if e.stdout else None)
...
def run_ret_out(
    ...
    if return_output:
        return completed_command.returncode, completed_command.stdout
