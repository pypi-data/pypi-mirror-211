...
def journalctl_timestamp() -> str:
    ...
    Usage example:
        ...
        failure_logs = run(f"journalctl -S {logging_start}, capture_output=True).stdout"
    """
...
def kernel_check():
    ...
    tainted = run("cat /proc/sys/kernel/tainted", capture_output=True).stdout
    ...
    prev_taint = run(f"cat {previous_tainted_file}", capture_output=True).stdout
    ...
    output = run("cat /proc/modules | grep -e '(.*)' |  cut -d' ' -f1", capture_output=True).stdout
...
def messages_dump_check():
    ...
    last_run = run(f"cat {previous_time_file}", capture_output=True).stdout
...
def dmesg_check():
    ...
    for msg in error_mgs:
        output = run(f"dmesg | grep -i '{msg}'", capture_output=True).stdout
        if output:
