...
def scsi_driver_of_host_id(host_id):
    ...
    output = run(f"cat {scsi_drv_sysfs}", capture_output=True, verbose=False).stdout
    scsi_driver = output
    if scsi_driver == "(null)" or scsi_driver == "":
        ...
        if Path(driver_sysfs_file).is_file():
            output = run(f"cat {driver_sysfs_file}", capture_output=True, verbose=False).stdout
            return output

        model_sysfs_file = f"/sys/class/scsi_host/host{host_id}/model_name"
        if Path(model_sysfs_file).is_file():
            output = run(f"cat {model_sysfs_file}", capture_output=True, verbose=False).stdout
        ...
        if Path(symbolic_sysfs_file).is_file():
            output = run(f"cat {symbolic_sysfs_file}", capture_output=True, verbose=False).stdout
    ...
    if scsi_driver == "fcoe":
        ...
        output = run(f"cat {drv_version_path}", capture_output=True, verbose=False).stdout
...
def disk_sys_check(scsi_disk):
    ...
    state = run(cmd, capture_output=True, verbose=False).stdout
    if not state:
...
def timeout_of_disk(scsi_disk):
    ...
    cmd = f"cat {sys_path} 2>/dev/null"
    timeout = run(cmd, capture_output=True, verbose=False).stdout
    if not timeout:
