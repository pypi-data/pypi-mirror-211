...
def mpath_conf_remove(path, value):
    ...
    if matched_path:
        out = run(f'augtool -s rm "{matched_path}"', capture_output=True, verbose=False).stdout
...
def mpath_conf_match(path, value):
    ...
    out = run(f'augtool match "{full_path}" "{value}"', capture_output=True, verbose=False).stdout
...
def mpath_conf_set(path, value):
    ...
    if not matched_path:
        if not linux.is_installed("augeas"):
            sts_print("FAIL: Could not install augeas")
            return False
        out = run(f'augtool -s set "{full_path}" "{value}"', capture_output=True, verbose=False).stdout
        if "Saved 1 file(s)" not in out:
