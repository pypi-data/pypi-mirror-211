...
def get_driver_info(driver: str):
    ...
    for info in infos:
        ...
        if not Path(info_path).is_file():
            continue
        output = run(f"cat {info_path}", capture_output=True, verbose=False).stdout
    ...
    if sys_driver_parameter.is_dir():
        ...
        for param in param_files:
            output = run(f"cat {sys_driver_parameter}/{param}", capture_output=True, verbose=False).stdout
