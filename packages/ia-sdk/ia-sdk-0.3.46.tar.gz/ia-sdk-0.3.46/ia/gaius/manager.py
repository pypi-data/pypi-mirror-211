...
from ia.scripts.spawn_agent import stopit, start_agent
from ia.scripts.spawn_tf import spawn_tf, kill_tf, connect_networks_tf
...
class AgentInfo:
    ...
    def get_docker_networks(self):
        ...
        gapi_container = [container for container in gapi_container if container.name == f'gaius-api-{self.user_id}-{self.agent_id}']
    ...
    def connect_jia(self):
        ...
        jias = [container for container in jias if any(['registry.digitalocean.com/intelligent-artifacts/jia' in tag for tag in container.image.tags])]
...
class TFInfo:
    ...
    def spawn(self) -> bool:
        ...
        if spawn_tf(container_name=self.container_name, api_key=self.api_key, port=self.port, docker_image=self.docker_image, env=self.environment):
            self.save_config_file()

        return True
    
    def kill(self):
        kill_tf(self.container_name)
    ...
    def save_config_file(self):
        ...
        print(f'saved to {self.thinkflux_dir}/{self.tf_name}')
    ...
    def connect_agents(self, agents: List[AgentInfo]):
        ...
        print(f'{nets=}')
        if not connect_networks_tf(networks=list(nets), container_name=self.container_name):
            return False
...
class AgentManager:
    def __init__(self, genome_dir: str = None, agents_dir: str = None, thinkflux_dir: str = None):
        ...
        self.thinkflux_dir = f'{self.data_dir}/thinkflux'
        if thinkflux_dir:
            self.thinkflux_dir = thinkflux_dir
        ...
        print(f'   genome dir: {self.genome_dir}')
        print(f'thinkflux dir: {self.thinkflux_dir}')
        ...
        if agents_to_connect:
            for agent_name in agents_to_connect:
                if agent_name not in self.current_agents:
                    raise Exception(f'Agent {agent_name} not found in current agents')
    ...
    def start_agent(self, genome_file=None, genome_name=None, user_id: str = 'jia', agent_id: str = '1', agent_name=None, connect_jia=True, api_key='ABCD-1234', single_container=False):
    ...
    def update_current_agents(self):
        ...
        self.current_agents = [p for p in os.listdir(self.agents_dir) if not os.path.isdir(p)]
        self.current_tfs = [p for p in os.listdir(self.thinkflux_dir) if not os.path.isdir(p)]

        # get full paths of agent files
        self.current_agents = [os.path.join(
            self.agents_dir, agent) for agent in self.current_agents]

        self.current_tfs = [os.path.join(
            self.thinkflux_dir, tf) for tf in self.current_tfs]
