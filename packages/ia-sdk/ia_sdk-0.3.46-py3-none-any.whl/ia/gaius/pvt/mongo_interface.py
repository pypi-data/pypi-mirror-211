    ...
    def __init__(self,
        ...
        try:
            ...
            if DR == 1 and DF == 1:
                ...
                self.test_sequences = dataset_records[num_train_sequences:(num_train_sequences + num_test_sequences)]
...
class MongoData:
    ...
    def __init__(self,
        ...
        ds_count = mongo_db[dataset_collection_name].count_documents(dataset_obj)
        if ds_count != 1:
    ...
    def upload_dataset(cls, mongo_db, dataset_details: dict, filepath: str):
        ...
        if 'user_id' not in dataset_details:
            raise Exception(f'user_id field missing from dataset_details ({dataset_details=})')
        if 'dataset_id' not in dataset_details:
            raise Exception(f'dataset_id field missing from dataset_details ({dataset_details=})')
        ...
        try:
            with zipfile.ZipFile(filepath, 'r') as zipObject:
                dataset_obj['size'] = sum([zinfo.file_size for zinfo in zipObject.filelist])
                ...
                try:
                    insert_result = mongo_db[data_files_collection_name].insert_many(records)
                except Exception as e:
                    print(f'failed to insert records: {str(e)}')
                    try:
                        mongo_db[data_files_collection_name].delete_many({'dataset_id': dataset_obj['dataset_id']})
                    except Exception as e2:
                        print(f'failed to remediate data files collection: {str(e2)}')
                    finally:
    ...
    def delete_dataset(cls, mongo_db, dataset_details: dict):
        ...
        try:
            if mongo_db[dataset_collection_name].count_documents({**dataset_obj}) != 1:
                raise Exception(f'found {mongo_db[dataset_collection_name].count_documents({**dataset_obj})} datasets in mongo, expecting 1')
            dataset = mongo_db[dataset_collection_name].find_one({**dataset_obj})
        except Exception as e:
        ...
        try:
            mongo_db[data_files_collection_name].delete_many({'_id': {'$in': dataset['dataset']['files']}})
        except Exception as e:
            print(e)
            raise e
...
class MongoResults:
    ...
    def deleteResults(self):
        ...
        if delete_result is not None:
            try:
                self.log_collection.delete_many({'_id': {'$in': delete_result['training_logs']}})
                self.log_collection.delete_many({'_id': {'$in': delete_result['testing_logs']}})
        ...
        if self.result_obj['testing_logs']:
            self.log_collection.delete_many({'_id': {'$in': self.result_obj['testing_logs']}})

        if self.result_obj['training_logs']:
            self.log_collection.delete_many({'_id': {'$in': self.result_obj['training_logs']}})
    ...
    def retrieveResults(self):
        ...
        training_logs = [item for item in self.log_collection.find({'_id': {'$in': training_ids}})]
        testing_logs = [item for item in self.log_collection.find({'_id': {'$in': testing_ids}})]
