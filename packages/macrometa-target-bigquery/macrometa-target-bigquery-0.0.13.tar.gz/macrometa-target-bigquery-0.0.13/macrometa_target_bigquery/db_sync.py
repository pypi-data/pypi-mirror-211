...
from macrometa_target_bigquery import flattening
from macrometa_target_bigquery.stream_ref_helper import StreamRefHelper
from macrometa_target_bigquery import sql_utils

logger = singer.get_logger('macrometa_target_bigquery')
...
class DbSync:
    def __init__(self, connection_config, stream_schema_message=None):
        ...
        if self.stream_schema_message is not None:
            ...
            config_target_schema = self.connection_config.get('target_schema', '').strip()
            ...
            if not self.schema_name:
                raise Exception("Target schema name not defined in config for {} stream.".format(stream_name))
            ...
            self.grantees = self.connection_config.get('default_target_schema_select_permission')
    ...
    def load_avro(self, f, count):
        ...
        target_table_ref = self.ref_helper.get_table_ref(self.connection_config['target_table'], is_temporary=False)
        target_table = self.client.get_table(target_table_ref)
        logger.info("Loading {} rows into '{}'".format(count, target_table_ref.table_id))

        temp_table_ref = self.ref_helper.get_table_ref(self.connection_config['target_table'], is_temporary=True)
        ...
        results = self.query([query, drop_temp_query]).result()
        logger.info('LOADED {} rows'.format(results[0].num_dml_affected_rows))
    ...
    def create_table(self, is_temporary=False):
        ...
        table_ref = self.ref_helper.get_table_ref(self.connection_config['target_table'], is_temporary)
