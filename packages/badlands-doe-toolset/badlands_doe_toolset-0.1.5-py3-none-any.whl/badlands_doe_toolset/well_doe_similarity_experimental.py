...
def well_attr_plot(model_extract,Exp,Well,obslog,well_log_file,expattrib,exp_srm=5,log_smth=5,roll_win=10):    
    ...
    if obslog in loglist:
        obs_log=log_df[obslog]
        obs_depth=log_df['Depth']
        obs_log_smooth=obs_log.rolling(log_smth).mean()
        #obs_log_smooth = gaussian_filter1d(obs_log, log_smth)
    else:
...
#    dtt=DThick_df.rolling(roll_win,center=True,win_type='gaussian').sum(std=5)
    ...
    if expattrib in model_extract['/'+Exp+'/'+Well+'/'].keys():
        ...
        exp_log=exp_log.ravel()
        #exp_log_smooth = gaussian_filter1d(exp_log, log_smth)
...
#    ax1b.legend(loc="upper right")
    ...
    textstr = 'avg: '+str(round(np.nanmean(DThick),1))+' m/100kyr' 
...
def exp_attr_plot(model_extract,Exp,obslog,well_loc,explog,exp_srm=5,log_smth=10,roll_win=10):
    if type(model_extract)==str: # this is read like this to keep consistency
        model_extract=h5py.File(model_extract,'r')
    wells=ppu.Welldata()
    ...
    for i in range(0,len(wells.well_name)):
        ...
        well_attr_plot(model_extract,Exp,Well,obslog,well_log_file,explog,exp_srm,log_smth,roll_win)
     
def well_attr_plot_DoEgen(model_extract,obslog,well_loc,explog,exp_srm=5,log_smth=10,roll_win=10):
#    model_file_loc=model_extract # hdf5 arrays aren't picklable as the hdf5 attributes are read on demand, send the file loc as a string to the child functions. 
    #if type(model_extract)==str: # this is read like this to keep consistency 
    model_extract=h5py.File(model_extract,'r')
    explist=list(model_extract.keys())
