...
PASSWORD_VERBIAGE = (
    f"If you prefer a prompt (with hidden input) enter {settings.SHOW_PROMPT_VALUE}"
)
...
def etl_select_option(required: bool = False, default: str = None) -> List:
    return _etl_option(
...
def _etl_option(help: str, required: bool = False, default: str = None) -> List:
    return [click.option("--name", help=help, required=required, default=default)]
...
S3_OPTIONS = [
    click.option(
        "--bucket", help="S3 Bucket where query logs are contained.", required=True
    ),
...
BASE_DATABRICKS_OPTIONS = [
    click.option(
        "--databricks-workspace-url", help="Databricks workspace URL.", required=True
    ),
    click.option(
        "--databricks-token",
...
DATABRICKS_OPTIONS = [
    *BASE_DATABRICKS_OPTIONS,
    click.option(
        "--databricks-workspace-id", help="Databricks workspace ID.", required=True
    ),
    click.option(
        "--databricks-cluster-id", help="Databricks cluster ID.", required=True
    ),
]
...
DATABRICKS_SQL_WAREHOUSE_OPTIONS = [
    *BASE_DATABRICKS_OPTIONS,
    click.option(
        "--databricks-warehouse-id", help="Databricks warehouse ID.", required=True
    ),
]
...
DATABRICKS_DATA_COLLECTOR_OPTIONS = [
    ...
    click.option(
        ...
        help="Databricks secret key.",
        default=DEFAULT_SECRET_NAME,
        show_default=True,
    ),
    click.option(
        "--databricks-secret-scope",
        help="Databricks secret scope.",
...
EVENT_TYPE_OPTIONS = [
    click.option(
        ...
        type=click.Choice(
            CloudResourceService.MCD_EVENT_TYPE_FRIENDLY_TO_COLLECTOR_OUTPUT_MAP.keys(),
            case_sensitive=True,
        ),
    ),
]
...
@integrations.command(
    help=f"Setup a Hive metastore integration (MySQL). {METADATA_VERBIAGE}."
)
@click.pass_obj
@click.option(
    "--port", help="HTTP port.", default=3306, type=click.INT, show_default=True
)
@click.option(
    "--use-ssl",
...
@click.option("--host", help="Hostname.", required=True)
@click.option(
    "--port", help="HTTP port.", default=8889, type=click.INT, show_default=True
)
...
@click.option(
    "--port", help="HTTP port.", default=10000, type=click.INT, show_default=True
)
...
def add_presto_logs(ctx, role, **kwargs):  # DEPRECATED
    ...
    """
    PrestoOnboardingService(config=ctx["config"]).onboard_presto_s3(
        assumable_role=role, **kwargs
    )
...
def add_hive_logs(ctx, role, **kwargs):  # DEPRECATED
    ...
    """
    HiveOnboardingService(config=ctx["config"]).onboard_hive_s3(
        assumable_role=role, **kwargs
    )
...
@click.pass_obj
@click.option(
    "--region", help=f"Glue catalog region. {REGION_VERBIAGE}.", required=False
)
...
@integrations.command(
    help="Setup an Athena integration. For query logs and health queries."
)
@click.pass_obj
@click.option(
    "--catalog",
...
@click.option(
    "--region", help=f"Athena cluster region. {REGION_VERBIAGE}.", required=False
)
@add_common_options(warehouse_select_option())
@add_common_options(role_options())
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
...
@integrations.command(
    help="Setup a Databricks Delta integration. For metadata queries on delta tables when using an external metastore in databricks."
)
@click.pass_obj
@add_common_options(DATABRICKS_OPTIONS)
@add_common_options(warehouse_select_option())
...
@integrations.command(
    help=f"Setup a Databricks SQL Warehouse integration. {SQL_VERBIAGE}"
)
@click.pass_obj
...
def add_databricks_sql_warehouse(ctx, name, **kwargs):
    ...
    DatabricksOnboardingService(
        config=ctx["config"], mc_client=create_mc_client(ctx)
    ).onboard_databricks_sql_warehouse(warehouseName=name, **kwargs)


@integrations.command(help=f"Setup a Redshift integration. {WAREHOUSE_VERBIAGE}.")
@click.pass_obj
@click.option(
    "--port", help="HTTP port.", default=5439, type=click.INT, show_default=True
)
@add_common_options(warehouse_create_option())
@add_common_options(DATABASE_OPTIONS)
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
...
@click.option(
    ...
    help=f"User's private key passphrase if using key pair auth. This argument is only needed when the private key is encrypted. {PASSWORD_VERBIAGE}.",
    required=False,
    cls=AdvancedOptions,
    prompt_if_requested=True,
    mutually_exclusive_options=["password"],
...
def add_snowflake(ctx, name, **kwargs):
    ...
    """
    WarehouseOnboardingService(config=ctx["config"]).onboard_snowflake(
...
@click.pass_obj
@click.option(
    "--key-file", help="JSON Key file.", type=click.Path(exists=True), required=True
)
@add_common_options(warehouse_create_option())
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
@click_config_file.configuration_option(settings.OPTION_FILE_FLAG)
...
@integrations.command(
    help=f"Setup an Airflow integration to receive events from Airflow callbacks."
)
@click.pass_obj
@click.option("--host", help="Hostname.", required=True)
...
@click.option(
    ...
    help="Type of the integration.",
    type=click.Choice(
        [HIVE_S3_CONNECTION_TYPE, PRESTO_S3_CONNECTION_TYPE], case_sensitive=True
    ),
    required=True,
)
@add_common_options(warehouse_select_option())
...
@click.option(
    ...
    cls=AdvancedOptions,
    mutually_exclusive_options=["password", "username"],
...
@click.option(
    ...
    cls=AdvancedOptions,
    mutually_exclusive_options=["token_name", "token_value"],
...
@click.option(
    "--name", help="Friendly name for the Tableau integration.", required=False
)
...
@click.option(
    "--name", help="Friendly name for the Looker integration.", required=False
)
@add_common_options(BI_OPTIONS)
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
...
def refresh_bi_to_warehouse_connections(
    ctx, bi_container_id, warehouse_source_details, **kwargs
):
    BiService(
        config=ctx["config"],
...
@click.pass_obj
@click.option(
    "--tenant-id", help="The tenant ID from the Azure Active Directory.", required=True
)
@click.option(
    "--auth-mode",
...
@click.option(
    "--name", help="Friendly name for the Power BI integration.", required=False
)
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
...
@click.option(
    "--webhook-hmac-secret",
    "webhook_hmac_secret",
    help="The secret token `hmac_secret` provided by dbt after a webhook is successfully created in dbt.",
    required=False,
)
@add_common_options(warehouse_select_option())
@add_common_options(ONBOARDING_CONFIGURATION_OPTIONS)
...
def add_fivetran(ctx, name, **kwargs):
    ...
    """
    FivetranOnboardingService(config=ctx["config"]).onboard_fivetran(
        etl_name=name, **kwargs
    )
...
def add_sql_server(ctx, name, **kwargs):
    ...
    """
    TransactionalOnboardingService(config=ctx["config"]).onboard_transactional_db(
        warehouseName=name, dbType=SQL_SERVER_DB_TYPE, **kwargs
    )
...
def add_mysql(ctx, name, **kwargs):
    ...
    """
    TransactionalOnboardingService(config=ctx["config"]).onboard_transactional_db(
        warehouseName=name, dbType=MYSQL_DB_TYPE, **kwargs
    )
...
def create_role(ctx, file, aws_profile):
    ...
    CloudResourceService(
        config=ctx["config"], aws_profile_override=aws_profile
    ).create_role(path_to_policy_doc=file)


@integrations.command(
    help="Update credentials for a connection. Only replaces/inserts the keys in changes by default.",
    name="update",
)
@click.pass_obj
...
def create_event_topic(ctx, **kwargs):
    CloudResourceService(config=ctx["config"], aws_wrapper=None).create_event_topic(
        **kwargs
    )
...
@integrations.command(
    help="Create an integration key. The resulting key id and secret will be printed to the console."
)
@click.pass_obj
...
@click.pass_obj
@click.option(
    "--bi-connection-id", required=True, help="UUID of the existing BI Connection"
)
