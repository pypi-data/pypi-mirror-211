...
from lumaconf.period_parser import get_period_expr
...
from lumaconf.config_tree import (ConfigInclude, ConfigList, ConfigQuotedString,
                                 ConfigSubstitution, ConfigTree,
                                 ConfigUnquotedString, ConfigValues, NoneValue)
from lumaconf.exceptions import (ConfigException, ConfigMissingException,
...
class ConfigFactory(object):
    ...
    def parse_file(cls, filename, encoding='utf-8', required=True, resolve=True, unresolved_value=DEFAULT_SUBSTITUTION):
        ...
        If it is set to lumaconf.STR_SUBSTITUTION then it will replace the value by its substitution expression (e.g., ${x})
                    ...
                    if has_char:
                        try:
                            ...
                            raise ConfigSubstitutionException(
                                    ...
                                    curr_token + ' in expression: ' + expr) from ex
                    else:
                        out_expr += curr_token
                if i < len(expr) - 1:
                    out_expr += c
