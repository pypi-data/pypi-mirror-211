...
class convert_utf16le_to_utf8(object):
    ...
    def __init__(self, filename='', input_directory='', output_directory='', inplace=False):
        ...
        if filename:
            ...
            self.files = sorted([f for f in os.listdir(self.input_directory) if 'avgwnd' in f.lower() or 'rawwnd' in f.lower()])
    ...
    def single_file(self):
        ...
        if self.inplace:
            ...
            out_file = (self.filename.split(".")[0] + "_utf8." + self.filename.split('.')[-1]).replace(".zip", ".csv")

        if self.output_directory:
            out_file = os.path.join(self.output_directory, os.path.basename(out_file))

        try:
            infile = pd.read_csv(self.filename, encoding='UTF_16_LE', parse_dates=True, index_col=[0])
