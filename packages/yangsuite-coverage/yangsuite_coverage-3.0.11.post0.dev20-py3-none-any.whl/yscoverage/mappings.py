...
class MibYangWriter:
    ...
    def get_mapping_data(cls, file_ref, translate_oids):
        ...
        if path:
            ...
            for dfrow in mapped_df.iterrows():
                row = dfrow[1].to_dict()
                oid = row.get('OID')
                numeric_oid = row.get('Numeric OID', translate_oids([oid], to_numeric=True)[-1])
    ...
    @classmethod
    def find_mappings(cls, oids, filepath, filename, translate_oids):
        def process_rows(df_rows, translate_oids):
            ...
            for df_row in df_rows:
                if len(df_row) not in [3, 4]:
                    ...
                    row = [
                        ...
                        translate_oids([df_row[0]], to_numeric=True)[-1],
                        # Readable OID, XPath, and YANG module
                        *df_row,
                        filename
                    ]
                elif len(df_row) == 4:
        ...
        if not mapped_df.empty:
            ...
            for oid in oids:
                ...
                with futures.ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
                    ...
                    future_objs = [
                        executor.submit(process_rows, part, translate_oids)
