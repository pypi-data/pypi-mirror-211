        ...
        if (mypobj is None and device is not None) \
           or (
            ...
            mypobj.map_filepath = os.path.join(
                get_path('mibyang_mappings_dir', user=user or self.user),
                map_filename
            )
        return mypobj
                ...
                for key, value in oids.items():
                    if self.OID_KEY not in key:
                        ...
                        if oid_has_keys:
                            ...
                            alpha_tokens.append(self.OID_KEY)
        ...
        for xpath in formated_xpaths:
            ...
            try:
                if self.OID_KEY in oid:
                    ...
                    # If no key, then do normal snmpget to get OID value
                    oid_result, oid_str = self.get_oids_to_values_from_device(
                        'snmpget',
                        options=[oid]
                    )
