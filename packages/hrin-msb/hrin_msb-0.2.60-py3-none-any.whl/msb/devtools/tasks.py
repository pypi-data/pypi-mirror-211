...
from .django import (DjangoFixtures)
from .funcs import (
...
def _drop_database_task(**kwargs):
	"""[ -a db=<db_name> ] : Drops all tables from the databse"""
	...
	if not (db := kwargs.get("db", None)):
		return logging.warning(msg=f"Please specify a database name")

	drop_database_tables(db_name=db)


def _load_database_fixtures_task(env=None, **kwargs):
...
def _fresh_db_setup_task(**kwargs):
	...
	_drop_database_task(**{"db": kwargs.get('db', NameConst.DEFAULT_DATABASE_NAME), **kwargs})
...
def _migrate_task(**kwargs):
	""": Migrate all apps."""
	for app in kwargs.get('apps', []):
		run_management_command("migrate", app,"--no-input")
...
class MsbSetupTask:
	__task_details: dict = {
		...
		"drop_db": _drop_database_task,
		"fresh_db_setup": _fresh_db_setup_task
	}
	_kwargs: dict = {}
	...
	def usage_text(self):
		...
		return f"python setup.py -t task_name -a arg1=value1  arg2=value2 if any.\n{''.join(_task_list)}\n\n\n"
