...
@pytest.mark.skip(reason="fixture link is returning 504 error.")
@unit_test
...
class TestDICOMSource:
    ...
    def test_get_data_3d_only(
        ...
        assert isinstance(result, pd.DataFrame)
        assert len(result.columns) == 19  # 14 cols + 5 pixel arrays
        assert "Pixel Data 3" in result.columns
        assert len(result) == 5
    ...
    def test_get_data_2d_3d_mix(self, dicom_2d_3dsource: DICOMSource) -> None:
        ...
        assert data.shape == (10, 19)  # type:ignore[union-attr]
