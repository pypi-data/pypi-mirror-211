...
from bitfount.federated.exceptions import PodSchemaMismatchError
    ...
    def on_run_end(
        ...
        if context == TaskContext.WORKER:
            ...
            if isinstance(results, list):
                ...
                assert table is not None  # nosec[assert-used]
...
class _Worker:
    ...
    def _load_data_for_worker(
        ...
        if self._pod_db and self.run_on_new_data_only:
            ...
            self.load_new_records_only_for_task(
                cast(Connection, project_db_con), pod_db_table=target_table
            )
        return table

    def get_pod_db_table_name(self, base_table_name: str) -> str:
    ...
    def load_new_records_only_for_task(
        self, project_db_con: Connection, pod_db_table: str
    ) -> None:
        ...
        task_data = pd.read_sql(
            f'SELECT "datapoint_hash" FROM "{self._task_hash}"', project_db_con  # nosec
        )
        ...
        data = pd.read_sql(f'SELECT * FROM "{pod_db_table}"', pod_db_con)  # nosec
        pod_db_con.close()
