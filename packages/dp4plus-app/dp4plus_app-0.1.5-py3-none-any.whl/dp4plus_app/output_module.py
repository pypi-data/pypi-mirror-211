...
def gen_out_dp4(mode, inputs, parameters,  outputs, custom_mode=None): 
    ...
    with pd.ExcelWriter(output_file) as writer: 
        ...
        if 'Custom' in mode: 
            custom_mode.to_excel(writer,sheet_name='results', 
                                 index=True, startrow=18)
            parameters.to_excel(writer,sheet_name='results', 
                                index=True,float_format="%.3f", startrow=24)
