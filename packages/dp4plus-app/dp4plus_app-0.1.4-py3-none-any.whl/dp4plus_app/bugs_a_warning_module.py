...
def miss_data(data_df, isom ):
    ...
    if data_df.shape[1]/3 > 1 and data_df.shape[1]/3 != len(isom):
        print ('ACA7')
...
def xlsx_trap(inputs, sheet):
    ...
    '''
    if 'xl' in inputs['xlsx'] [-4:]: 
        eng = 'openpyxl'
    if 'od' in inputs['xlsx'] [-4:]: 
        eng = 'odf'
    
    data = pd.read_excel(inputs['xlsx'], sheet_name=sheet , engine= eng)
    miss_column(data)
    try: 
        miss_data(data , inputs['isom'])
    except : 
        miss_data(data , sheet)
