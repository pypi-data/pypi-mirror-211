...
from kiwi.core.bio_quantity import FlowRate


@class_mock_enable
class Pump(SignalPeriphery):
    def __init__(self, flow_rate: FlowRate, control_periphery: ControlPeriphery, port: int, name="", usage=PeripheryUsage.BASE, mock=False,
                 mock_obj=None):
    ...
    def start(self):
        ...
        print("pump port {} start, id {}".format(self.port, self.id))

    def shutdown(self):
        self.attach_to.unset_signal(self.id)
        print("pump port {} shutdown, id {}".format(self.port, self.id))
