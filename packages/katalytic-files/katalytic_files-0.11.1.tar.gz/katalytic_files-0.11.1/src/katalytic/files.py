...
from katalytic.data import as_list_of_dicts, as_list_of_lists, sort_dict_by_keys, is_none_of, is_any_of, _UNDEFINED
...
def copy_dir(src, dest, *, dir_exists='merge', file_exists='replace', make_dirs=True):
    ...
    Args:
        ...
        file_exists (str, optional):
            Specifies the behavior if a file with the same name already exists in the destination directory. Defaults to 'replace'.
...
def delete_dir(path, *, missing_ok=True, non_empty_dir=True):
    """
    Delete a directory.
...
__load_funcs = _find_grouped_functions('load::*')
__save_funcs = _find_grouped_functions('save::*')
...
def load(path, default=_UNDEFINED, **kwargs):
    ...
    """
    for ext, f in __load_funcs.items():
...
def save(data, path, *, exists='replace', make_dirs=True, **kwargs):
    ...
    """
    for ext, f in __save_funcs.items():
        if path.lower().endswith(f'.{ext}'):
