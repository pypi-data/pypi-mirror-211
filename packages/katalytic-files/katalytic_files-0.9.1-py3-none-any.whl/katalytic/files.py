...
from katalytic.data import as_list_of_dicts, as_list_of_lists, sort_dict_by_keys, is_none_of, is_any_of
...
def _setup_load():
    ...
    Returns:
        function:
            The load function that dynamically selects the appropriate load function based on the file extension.
...
def _setup_save():
    """
    Set up a save function based on the available grouped save functions.

    Returns:
        function:
            The save function that dynamically selects the appropriate save function based on the file extension.
...
def copy_dir(src, dest, *, dir_exists='error', file_exists='error', make_dirs=True):
    ...
    Args:
        ...
        dir_exists (str, optional):
            Specifies the behavior if the destination directory already exists. Defaults to 'error'.
        ...
        file_exists (str, optional):
            Specifies the behavior if a file with the same name already exists in the destination directory. Defaults to 'error'.
...
def copy_file(src, dest, *, exists='error', make_dirs=True):
    ...
    Args:
        ...
        exists (str, optional):
            Specifies the behavior if the destination file already exists. Defaults to 'error'.

            - 'error': Raise an error.
            - 'replace': Replace the existing file.
            - 'skip': Skip copying the file.
        make_dirs (bool or str, optional):
            Specifies whether to create the destination directory if it doesn't exist. Defaults to 'error'.

            - True: Create the directory if it doesn't exist.
            - False: Raise an error if the destination directory doesn't exist.
            - 'error': Raise an error if the destination directory doesn't exist.
    ...
    elif is_none_of(make_dirs, ('error', False, True)):
...
def delete_dir(path, *, missing_ok=True, non_empty_dir='error'):
    ...
    Args:
        ...
        non_empty_dir (str or bool, optional):
            Specifies the behavior if the directory is non-empty.  Defaults to 'error'.

            - 'error': Raise an error if the directory is non-empty.
            - False: Delete the directory even if it is non-empty.
            - True: Delete the directory and its contents, recursively.
...
def is_dir_empty(path, *, missing='error'):
    ...
    Args:
        ...
        missing (bool or str, optional):
            ...
            - True: Return True if the directory is empty.
            - False: Return False if the directory is not empty.
