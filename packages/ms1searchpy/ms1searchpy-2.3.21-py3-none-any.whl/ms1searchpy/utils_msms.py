...
def prot_peptides(prot_seq, enzyme, mc, minlen, maxlen, is_decoy, dont_use_seen_peptides=False):
    ...
    if enzyme == 'yions':
        peptides = [prot_seq[i:] for i in range(1, len(prot_seq))]
        peptides.extend(['b'+prot_seq[:i] for i in range(0, len(prot_seq)-1)])
    else:
...
def get_prot_pept_map(args, rt_prot=0):
    seen_target.clear()
    seen_decoy.clear()
    ...
    for desc, prot in prot_gen(args):
        dbinfo = desc.split(' ')[0]
        if rt_prot:
            rt_val = float(desc.split(' ')[1].split('_')[1].strip())
        else:
            rt_val = 0
        ...
        for pep in prot_peptides(prot, enzyme, mc, minlen, maxlen, desc.startswith(prefix), dont_use_seen_peptides=True):
            ...
            pept_RT.setdefault(pep, []).append(rt_val)
