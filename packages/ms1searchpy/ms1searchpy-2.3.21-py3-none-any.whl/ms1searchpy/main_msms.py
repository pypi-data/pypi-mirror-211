...
from . import utils_msms as utils
...
def peptide_processor(peptide, **kwargs):
    ...
    if seqm[0] == 'b':
        seqm = seqm[1:]
        bmass -= 18.010564683699954
    results = []
    m = cmass.fast_mass(seqm, aa_mass=kwargs['aa_mass']) + kwargs['aa_mass'].get('Nterm', 0) + kwargs['aa_mass'].get('Cterm', 0) + bmass
    acc_l = kwargs['acc_l']
    acc_r = kwargs['acc_r']
    acc_rt = kwargs['rtol']
    pep_rt_all = kwargs['pept_RT'][peptide]
    dm_l = acc_l * m / 1.0e6
    if acc_r == acc_l:
        dm_r = dm_l
    else:
    ...
    for i in range(start, end):
        for idx, pep_rt in enumerate(pep_rt_all):
            rt_diff = (pep_rt - rts[i])
            if abs(rt_diff) <= acc_rt: 
                massdiff = (m - nmasses[i]) / m * 1e6
                mods = 0
                results.append((peptide, massdiff, rt_diff, i, idx))
    return results


def prepare_peptide_processor(fname, args, pept_RT):
    global nmasses
    global rts
    global charges
    global ids
    ...
    return {'aa_mass': aa_mass, 'acc_l': acc_l, 'acc_r': acc_r, 'args': args, 'rtol': rtol, 'pept_RT': pept_RT}, df_features
...
def process_peptides(args):
    ...
    prefix = args['prefix']
    protsN, pept_prot, pept_RT = utils.get_prot_pept_map(args, rt_prot=1)
    # protsN, pept_prot, protsNc = utils.get_prot_pept_map(args)

    ms1results = []
    peps = utils.peptide_gen(args)
    kwargs, df_features = prepare_peptide_processor(fname_orig, args, pept_RT)
    ...
    resdict['mc'] = np.array([0 for z in resdict['seqs']])
    resdict['plen'] = np.array([len(z) for z in resdict['seqs']])
    ...
    if len(p1):
        ...
        #                             full_output=True)
        filtered_prots = list(prots_spc.items())
        ...
        # df1.to_csv('~/test_ms1ms2/table1.tsv')
        ...
        pept_prot_origin = deepcopy(pept_prot)

        pept_prot = defaultdict(list)
        for seq, rt_id in zip(resdict['seqs'], resdict['rt_idx']):
            pept_prot[seq].append(pept_prot_origin[seq][rt_id])
        ...
        # true_rt = []
        # RT_pred = []
        # for pep, iorig in zip(resdict2['seqs'], resdict2['iorig']):
        #     rt_exp = rts[iorig]
        #     for rt_val in pept_RT[pep]:
        #         true_seqs.append(pep)
        #         true_rt.append(rt_exp)
        #         RT_pred.append(rt_val)
        # true_seqs = np.array(true_seqs)
        ...
        # df1.to_csv('~/test_ms1ms2/table2.tsv')

        # idx = np.abs(RT_pred - true_rt) <= 0.3
        # true_seqs = true_seqs[idx]
        # true_rt = true_rt[idx]
        # RT_pred = RT_pred[idx]
        ...
        e_ind = []
        for seq, rt_id in zip(resdict2['seqs'], resdict2['rt_idx']):
            if pept_prot_origin[seq][rt_id] in true_prots:
                e_ind.append(True)
            else:
                e_ind.append(False)
        ...
        ttt = np.array([Scans[iorig] for iorig in resdict2['iorig']])

        added = set()
        added2 = set()
        e_ind = []
        for seq, rt_id, abs_rd, iorig in sorted(zip(resdict2['seqs'], resdict2['rt_idx'], ttt, resdict2['iorig']), key=lambda x: -x[2]):
            if seq+str(rt_id) not in added and iorig not in added2:
                added.add(seq+str(rt_id))
                added2.add(iorig)
                e_ind.append(True)
            else:
                e_ind.append(False)
        ...
        logger.info('First-stage peptides used for RT prediction: %d', len(resdict2['rd']))


        df1 = pd.DataFrame()
        df1['rd'] = resdict2['rd']
        df1['md'] = resdict2['md']
        df1['seqs'] = resdict2['seqs']

        df1.to_csv('~/test_ms1ms2/table2.tsv')


        rt_diff_tmp = resdict2['rd']
        RT_left = -min(rt_diff_tmp)
        RT_right = max(rt_diff_tmp)

        start_width = (scoreatpercentile(rt_diff_tmp, 95) - scoreatpercentile(rt_diff_tmp, 5)) / 20
    ...
    decoys_map_local = set()
    for pep_seq, prots_list in pept_prot.items():
        if all(z.startswith(prefix) for z in prots_list):
            decoys_map_local.add(pep_seq)
    df1['decoy'] = df1['seqs'].apply(lambda x: x in decoys_map_local)
    ...
    local_decoy_1 = set()
    for pep_seq in set(df1['seqs']):
        if not any(z in target_prots_25_fdr for z in pept_prot[pep_seq]):
            local_decoy_1.add(pep_seq)
    df1['decoy'] = df1['seqs'].apply(lambda x: x in local_decoy_1)
    ...
    # df1.to_csv('~/test_ms1ms2/table3.tsv')

    df1['qpreds'] = pd.cut(df1['preds'], 50, labels=range(50))
    ...
    df1un['qpreds'] = pd.cut(df1un['preds'], 10, labels=range(10))
    ...
    tmp_list = []
    for seq, rt_id in zip(resdict['seqs'], resdict['rt_idx']):
        pept_prot[seq+str(rt_id)].append(pept_prot_origin[seq][rt_id])
        tmp_list.append(seq+str(rt_id))

    resdict['seqs'] = np.array(tmp_list)
