...
class DBRollbackExtension(Extension):  # Todo: maybe rename
...
def serialize_date(value: date | datetime):
...
def create_graphql(schema_path, bindables: list[SchemaBindable], debug: bool):
    ...
    schema = make_executable_schema(
        [type_defs, type_defs_common], bindables, model_types, date_scalar, datetime_scalar, json_scalar,
        fallback_resolvers, directives={
    ...
    return GraphQL(
        ...
        http_handler=GraphQLHTTPHandler(
            middleware=[check_permission_middleware],  # Todo: cancel everything if there's one error
