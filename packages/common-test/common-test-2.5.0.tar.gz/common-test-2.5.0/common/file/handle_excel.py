...
def check_excel_data(testdata:dict, _filter, _checkData):
    if _checkData:
        ...
        if DataProcess.isNotNull(get_system_key(Constant.TEST_SRTCYCLE_ID)):
            caseRuninfo = ServicePlatForm.getCaseByCycle(casename, Constant.STATUS_PRE, caseNo)
            caserunid = caseRuninfo['caserunid']
            if DataProcess.isNotNull(caserunid) and caserunid != '00000':
                ServicePlatForm.updateByRunid(caserunid, Constant.STATUS_AUTOTEST, "")
                logger.info(f'用例信息:{str(caseRuninfo)} 添加到执行队列')
                return True
            else:
                logger.info(f'用例信息:{str(caseRuninfo)} 已经被添加到执行队列')
                return False
