...
def genOBRbook(self,auto_overwrite=False) -> pd.DataFrame:
    ...
    if not isinstance(self.OBRbook,pd.DataFrame):
        ...
        save_df  = True
        save_obj = True
    elif not isinstance(self.OBRbook,pd.DataFrame) and len(self.OBRfiles) == 0:
        print('OBR book found but files not registered in object')
        print(' Registering ...')
        save_df  = False
        save_obj = True
    elif isinstance(self.OBRbook,pd.DataFrame) and len(self.OBRfiles) != 0:
        ...
        raise Exception('Something was not working properly with genOBRbook first checking')
