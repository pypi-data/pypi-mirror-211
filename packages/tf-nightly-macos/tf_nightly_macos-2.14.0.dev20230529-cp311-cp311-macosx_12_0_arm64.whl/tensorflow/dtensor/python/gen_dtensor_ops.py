...
def copy_to_mesh(input, layout, name=None):
  r"""TODO: add doc.
  ...
  if tld.is_eager:
    try:
      _result = pywrap_tfe.TFE_Py_FastPathExecute(
        _ctx, "CopyToMesh", name, input, "layout", layout)
      return _result
    except _core._NotOkStatusException as e:
      _ops.raise_from_not_ok_status(e, name)
    except _core._FallbackException:
    ...
    try:
      ...
      _result = _dispatch.dispatch(
            copy_to_mesh, (), dict(input=input, layout=layout, name=name)
          )
      if _result is not _dispatch.OpDispatcher.NOT_SUPPORTED:
        return _result
      raise
  ...
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "CopyToMesh", input=input, layout=layout, name=name)
  except (TypeError, ValueError):
    _result = _dispatch.dispatch(
          copy_to_mesh, (), dict(input=input, layout=layout, name=name)
        )
...
def copy_to_mesh_grad(input, forward_input, reference_layout="", name=None):
  r"""TODO: add doc.
  ...
  if tld.is_eager:
    ...
    try:
      _result = _dispatcher_for_copy_to_mesh_grad(
          (input, forward_input, reference_layout, name,), None)
      if _result is not NotImplemented:
        return _result
      return copy_to_mesh_grad_eager_fallback(
          input, forward_input, reference_layout=reference_layout, name=name,
          ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
    except (TypeError, ValueError):
  ...
  if reference_layout is None:
    reference_layout = ""
  reference_layout = _execute.make_str(reference_layout, "reference_layout")
  try:
    _, _, _op, _outputs = _op_def_library._apply_op_helper(
        "CopyToMeshGrad", input=input, forward_input=forward_input,
                          reference_layout=reference_layout, name=name)
  except (TypeError, ValueError):
    _result = _dispatch.dispatch(
          copy_to_mesh_grad, (), dict(input=input,
                                      forward_input=forward_input,
  ...
  _result = _outputs[:]
  if _execute.must_record_gradient():
    _attrs = ("reference_layout", _op.get_attr("reference_layout"), "T",
...
def copy_to_mesh_grad_eager_fallback(input, forward_input, reference_layout, name, ctx):
  ...
  _attrs = ("reference_layout", reference_layout, "T", _attr_T)
...
def relayout_like(input, layout_input, name=None):
  r"""TODO: add doc.

  Args:
    input: A `Tensor`.
    layout_input: A `Tensor`. Must have the same type as `input`.
    name: A name for the operation (optional).
...
def relayout_like_eager_fallback(input, layout_input, name, ctx):
  _attr_T, _inputs_T = _execute.args_to_matching_eager([input, layout_input], ctx, [])
  (input, layout_input) = _inputs_T
