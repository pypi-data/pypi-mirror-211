...
class FATask:
    ...
    def __init__(self,
        ...
        :param identity: task identity string indicating from which MS Excel, worksheet and line this task was created
        """
        self._version = 2
        _d = (0, 0, 0)
        ...
        #         "translations":                       {
        #                 f"{language.lower()}_{language.upper()}": title
        #                 },
        #         "description_translations":           {
        #                 f"{language.lower()}_{language.upper()}": details
        #                 },
        ...
        self._task = {
            ...
            "task_title_id": "",
            "translation": title,
            "description_translation": details,
            "element": [],
            "date_start": start.strftime("%Y-%m-%d"),
            "hour_start": start.strftime("%H"),
            "minute_start": start.strftime("%M"),
    ...
    def getsha512(self) -> bytes:
        """
        Get SHA512 hash of FA task as bytes
        """
        return sha512(dumps({"_version": self._version, **self._task}, ensure_ascii=False).encode("utf-8")).digest()

    def getversion(self) -> int:
        """
        Get version of FATask object structure
        """
        return self._version

    def getidentity(self) -> str:
        """
        Get the FA task identity string

        :return: string with task identity as specified at task creation
        """
