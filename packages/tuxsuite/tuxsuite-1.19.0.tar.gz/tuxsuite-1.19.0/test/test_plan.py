...
def test_submit(config, plan_config, mocker):
    ...
    def post(url, headers, data):
        ...
        if url == "https://tuxapi.tuxsuite.com/v1/groups/tuxgrp/projects/tuxprj/tests":
            ...
            assert data[11]["waiting_for"] == "my-build-08"
            assert data[12]["waiting_for"] == "my-build-08"
            assert data[13]["waiting_for"] == "my-build-08"
            assert data[14]["waiting_for"] == "my-build-09"
            assert data[15]["waiting_for"] == "my-build-09"
            assert data[16]["waiting_for"] == "my-build-09"
            assert data[17]["waiting_for"] == "my-build-09"
            assert data[18]["waiting_for"] == "my-build-09"
            assert data[19]["waiting_for"] == "my-build-10"
            assert data[20]["waiting_for"] == "my-build-10"
            assert data[21]["waiting_for"] == "my-build-10"
            assert data[22]["waiting_for"] == "my-build-10"
            assert data[23]["waiting_for"] == "my-build-10"
    ...
    with tuxapi returned object along with uid and downlaod url
    """
