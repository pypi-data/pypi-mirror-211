...
class TestBitbake:
    ...
    def test_handle_manifest(
        ...
        result = tuxsuite.build.handle_manifest(sample_manifest_file)
        ...
        with pytest.raises(SystemExit):
            tuxsuite.build.handle_manifest(file_path)
        output, error = capsys.readouterr()
        assert (
            "Error: Is provided manifest/kas override file is a valid xml/yaml file ?"
            in error
        )

        # if kas override is passed but extension is not .yaml/.yml
        with pytest.raises(SystemExit):
            tuxsuite.build.handle_manifest(sample_manifest_file, True)
        output, error = capsys.readouterr()
        assert "Error: Uknown kas override file extension" in error

        # if manifest is passed but extension is not .xml
        unknown_manifest = tmp_path / "manifest.yml"
        unknown_manifest.write_text("<xml></xml>")
        with pytest.raises(SystemExit):
            tuxsuite.build.handle_manifest(str(unknown_manifest))
        output, error = capsys.readouterr()
        assert "Error: Unknown manifest file extension" in error

        # if invalid manifest is passed
        invalid_manifest = tmp_path / "invalid_manifest.xml"
        invalid_manifest.write_text("<xml><//xml>")
        with pytest.raises(SystemExit):
            tuxsuite.build.handle_manifest(str(invalid_manifest))
        output, error = capsys.readouterr()
        assert "Error: Error parsing manifest file" in error
        ...
        with pytest.raises(SystemExit):
            tuxsuite.build.handle_manifest(str(invalid_override_file), True)
        output, error = capsys.readouterr()
        assert "Error: Error parsing kas override file: " in error

    def test_handle_manifest_url(self):
        url = "https://test.example.com/url"
        result = tuxsuite.build.handle_manifest(url)
        assert isinstance(result, type(()))
        assert isinstance(type(result[0]), type(str))
        assert result[0] == url
...
class TestBuild:
    ...
    def test_requires_mandatory_attributes(self, build_attrs, attr, value):
        ...
        if attr == "target_arch":
            assert "target-arch" in str(assertion)
        else:
