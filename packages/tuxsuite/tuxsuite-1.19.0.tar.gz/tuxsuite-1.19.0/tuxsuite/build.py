...
def handle_manifest(manifest_path, kas_override=False):
    ...
    if urlparse(manifest_path).scheme in ["http", "https"]:
        ...
        if os.path.isfile(manifest_path) and ext in [".xml", ".yml", ".yaml"]:
            if kas_override and ext not in [".yml", ".yaml"]:
                error("Uknown kas override file extension")
            elif not kas_override and ext != ".xml":
                error("Unknown manifest file extension")
        else:
        ...
        with open(manifest_path, "r") as f:
            try:
                ...
                error(f"Error parsing manifest file: {e}")
            except yaml.YAMLError as e:
                error(f"Error parsing kas override file: {e}")
    ...
    def generate_build_request(self, plan=None):
        ...
        if plan is None:
            ...
            if self.build_definition.pinned_manifest:
                (build_data["pinned_manifest"], manifest) = handle_manifest(
                    self.build_definition.pinned_manifest
                )
                manifests[build_data["pinned_manifest"]] = manifest
            # kas override
            if self.build_definition.kas_override:
                (build_data["kas_override"], override_content) = handle_manifest(
                    self.build_definition.kas_override, True
                )
                manifests[build_data["kas_override"]] = override_content
        else:
            # we are seperately handling manifest for plan
