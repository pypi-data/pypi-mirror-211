...
from infra_operator.operators.mod import dict_matcher, list_to_dict_by, remove_inner_fields, tags_to_list, to_args
...
def create_load_balancer(kind, content, info):
    ...
    listeners = content.pop("Listeners") if "Listeners" in content else []
    args = with_tags(content, args, info)
    res = client.create_load_balancer(**args)
    alb = res["LoadBalancers"][0]
    attributes_update(
        content, {"LoadBalancerArn": alb["LoadBalancerArn"]}, info)
    create_listeners(alb["LoadBalancerArn"], listeners, info)
...
def attributes_update(content, current, info):
    ...
    yaml_attributes = tags_to_list(content.get("Attributes", {}))
...
def tags_update(content, current, info):
    ...
    wanted_tags = with_tags(content, {}, info).get(tag_key, [])
