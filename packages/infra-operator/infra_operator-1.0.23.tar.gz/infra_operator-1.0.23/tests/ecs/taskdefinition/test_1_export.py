...
def test_taskdefinition_export():
    config = Config.parse_commandline_args([
        "-w",
        "n",
        "-t",
        "aws/mainsite/dev/ap-northeast-1/ecs/bin-dev-infra-operator-tests-cluster/comb-1/taskDefinition.yaml",
        "export"
    ])
    result = cli.main(config)
    expected = yaml_load(
        "aws/mainsite/dev/ap-northeast-1/ecs/bin-dev-infra-operator-tests-cluster/comb-1/taskDefinition.exported.yaml")
