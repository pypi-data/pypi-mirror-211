...
def test_sg_export():
    ...
    expected = yaml_load(
        "aws/mainsite/dev/ap-northeast-1/ecs/bin-dev-infra-operator-tests-cluster/comb-1/alb.sg.exported.yaml")
    ...
    kind, info, expected = preprocess_file(
        "aws/mainsite/dev/ap-northeast-1/ecs/bin-dev-infra-operator-tests-cluster/comb-1/service.sg.exported.yaml", {}, enable_ref_file=True, wait_for_ref=False)
    assert result == expected
