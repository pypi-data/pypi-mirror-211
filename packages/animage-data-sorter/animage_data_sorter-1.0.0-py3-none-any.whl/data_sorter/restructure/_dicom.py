...
def get_csa_ascii_list(dataset: pydicom.Dataset) -> list:
    try:
        ...
        csa_ascii = MrPhoenixProtocol[MrPhoenixProtocol.find(csa_ascii_begin):MrPhoenixProtocol.find(csa_ascii_end)].split('\n')
        return csa_ascii
    except:
        return []
...
class SeriesModel(List[InstanceModel]):
    ...
    def append(self, instance: InstanceModel):
        if instance.SeriesInstanceUID != self.SeriesInstanceUID:
            logger.warning(f'The UID({instance.SeriesInstanceUID}) of the instance is different from that of the series({self.SeriesInstanceUID})')
            return
        # 已记录的文件排除
    ...
    def inference_data_type(self, manufacturer: MANUFACTURER, sequence_name:str=None, series_description: str=None)->(ASL_Type,MRI_Type):
        '''
            ...
            for r in range(self.Repetition):
                ...
                for n in range(self.SliceNumber):
                    ...
                    if self[no].InstanceNumber != first_number + d * n:
                        logger.info(f'未验证通过序列：{self.SeriesDescription}；层数：{self.SliceNumber}，重复次数：{self.Repetition}，'
