...
class MakeDataset(BaseAlgorithm):
    ...
    def make_dataset(
        ...
        if os.path.exists(path):
            shutil.rmtree(path)
        feature_split = cls.__split_dataset(data_name, num_users)
        train_dataset, test_dataset = cls.__make_data(data_name, normalize)
    ...
    @classmethod
    def __split_dataset(
        cls, data_name: str, num_users: int
    ) -> list[np.ndarray]:
        '''
        Placeholder
    ...
    @classmethod
    def __make_data(
        cls, data_name: str, normalize: int
