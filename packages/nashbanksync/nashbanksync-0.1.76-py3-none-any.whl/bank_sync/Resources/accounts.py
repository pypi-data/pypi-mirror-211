        ...
        try:
            ...
            if super().get_operation() == super().BALANCE:
                ...
                if super().get_bank_id() == super().COOP:
                    ...
                    data["total_transactions"] = response_data.get("total_transactions", "0")
                    data["num_pages"] = response_data.get("num_pages", "0")
                        ...
                        transaction.update(
                            {
                                ...
                                "debit_limit": f'{float(transactions[i].get("DebitLimit", "0"))}',
                ...
                # If bank_id is UBA
                    ...
                    for i in range(len(transactions)):
                        ...
                        if transactions[i].get("reference", None):
                            transaction['nash_reference']=transactions[i].get("reference")

                        transaction.update(
                            {
                                "currency_code": transactions[i].get("currency_code",""), 
                                "reference": transactions[i].get("reference",None),                               
                                "date": transactions[i].get("alert",{}).get("date", ""),
                                "description": f'{transactions[i].get("alert",{}).get("status", "")} {transactions[i].get("alert",{}).get("narrative", "")} {transactions[i].get("alert",{}).get("customer_name", "")}',
                                "transaction_id": transactions[i].get("alert",{}).get("trans_id",transactions[i].get("transaction_id","")),
                                "value_date": transactions[i].get("alert",{}).get("date", ""),
                                "posted_date": transactions[i].get("alert",{}).get("date", ""),
                                "description": f'{transactions[i].get("alert",{}).get("status", "")}',
                                "amount": f'{round(float(transactions[i].get("alert",{}).get("trans_ammount", "-1.00"),2))}',
                                "type": f'{transactions[i].get("alert",{}).get("type", "")}',
                            }
                        )
