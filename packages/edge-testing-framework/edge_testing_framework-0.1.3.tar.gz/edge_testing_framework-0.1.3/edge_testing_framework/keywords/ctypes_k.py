...
from ctypes import *
import time
from injson_check.injson_check import check
...
def execute_call_c(data):
    ...
    logger.info('execute c++ func: {}, kwargs: {}'.format(func, str(kwargs)))
...
def reset_except_response(expected_, response_, kwargs_):
    """
    ...
    for e_k_, e_v in expected_['json'].items():
        if type(e_v) in (str, int, float, bool):  # value支持的类型
            ...
            if isinstance(e_k_v_v, bytes):
                real_kwargs_['{}_{}'.format(e_k_k, e_k_v)] = e_k_v_v.decode('utf-8')  # 默认bytes转str， check不对bytes类型支持
            else:
            ...
            for e_v_index, e_v_dict in enumerate(e_v):
                for e_v_d_k, e_v_d_v in e_v_dict.items():
                    ...
                    if isinstance(e_k_v_v, bytes):
                        real_kwargs_batch['{}_{}'.format(e_v_d_k_k, e_v_d_k_v)] = e_k_v_v.decode('utf-8')  # 默认bytes转str， check不对bytes类型支持
                    else:
