...
class TradingClient:
    ...
    market_maker_key : str
        The market maker key, used for additional authentication (default is None).
    user : object
    ...
    def __init__(self, 
        ...
        self.market_maker_key = market_maker_key
        self.user = None
    ...
    def _headers(self):
        ...
        if self.market_maker_key:
            headers["market-maker-key"] = self.market_maker_key
        return headers
...
# __________________Helpers____________________
    ...
    def _request(self, method, url, params=None, data=None, obj_class=None):
        ...
        if response.status_code not in [200, 204]:
            error = response.json().get("error") or {"message": "Unknown error", "code": None}
            raise Exception(
        ...
        if method == requests.delete:
            return {"status": response.status_code, "message": "Delete operation completed."}
    ...
    def _post_helper(self, url, body=None, params=None, obj_class=None):
        return self._request(requests.post, url, params=params, data=body, obj_class=obj_class)
        ...
        for k, v in kwargs.items():
            if isinstance(v, list):
                if all(isinstance(item, str) for item in v):
                    params[k] = ','.join(v)  # If all items in list are strings, join them with commas
                else:
                    params[k] = self._list_to_string(v)  # Otherwise, use the _list_to_string function
...
# __________________Accounts____________________
    ...
    def get_account(self) -> Account:
        ...
        self.user = Account(**account_data) if account_data else None
        return self.user
...
# _________________________________________________
    ...
    def get_markets(self, 
                    ...
                    strike_price: Optional[Union[float, List[float]]]=None) -> List[Instrument]:
        """
        ...
        return [Instrument(**instrument) for instrument in data]

    def get_orderbook(self, 
                    instrument_id: Optional[Union[int, List[int]]]=None, 
                    instrument_name: Optional[Union[str, List[str]]]=None, 
                    depth: Optional[int]=None) -> Orderbook:
        """
        ...
        if (instrument_id is None and instrument_name is None) or (instrument_id is not None and instrument_name is not None):
            raise ValueError("Either instrument_id or instrument_name must be provided, but not both.")
        ...
        data = self._get_helper(url, params=params)
        bids = [BidAsk(**bid) for bid in data.get('bids', [])]
        asks = [BidAsk(**ask) for ask in data.get('asks', [])]
    ...
    def cancel_order(self, order_id: str) -> Dict[str, Any]:
        ...
        return self._delete_helper(url)
    ...
    def cancel_all_orders(self, 
                        ...
                        strike_price: Optional[Union[float, List[float]]] = None) -> Dict[str, Any]:
        """
    ...
    def get_trades(self,
        ...
        params = self._build_params(instrument_id=instrument_id,
                                    instrument_name=instrument_name,
                                    instrument_type=instrument_type,
                                    strike_price=strike_price,
                                    expiration_timestamp=expiration_timestamp,
    ...
    def close_position(self, instrument_id: Optional[int] = None, instrument_name: Optional[str] = None):
        """
        ...
        if not instrument_id and not instrument_name:
            raise ValueError("Either instrument_id or instrument_name must be provided.")
        elif instrument_id and instrument_name:
            raise ValueError("Only one of instrument_id or instrument_name should be provided.")

        url = f"{self.base_url}/positions"
        body = {"instrument_id": instrument_id} if instrument_id else {"instrument_name": instrument_name}
