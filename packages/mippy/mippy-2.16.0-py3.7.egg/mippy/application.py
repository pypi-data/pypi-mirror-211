...
class MIPPYMain(Frame):
    ...
    def __init__(self, master):
        ...
        if not self.frozen:
            if len(sys.argv)>1:
                if sys.argv[1]=='multiprocess_off':
        ...
        # Add progress number to use when loading a folder
        self.master.statusmsg = StringVar()
        self.master.statusmsg.set('')
        self.master.statuslabel = Label(self.master,textvariable=self.master.statusmsg)
        ...
        self.master.progressbar.grid(row=3,column=0,rowspan=1,columnspan=1,sticky='nsew')
        self.master.statuslabel.grid(row=3,column=1,rowspan=1,columnspan=1,sticky='w')
...
#            self.master.destroy()
    ...
    def status(self,message):
        self.master.statusmsg.set(message)
        self.master.statuslabel.update()
    ...
    def filter_dicom_files(self):
        ...
        if self.multiprocess and not (('win' in sys.platform and not 'darwin' in sys.platform)
            ...
            self.tag_list = multithread(f,self.path_list,progressbar=self.progress,status=self.status)
        ...
        self.status("Sorting DICOM files")
    ...
    def build_dicom_tree(self):
        ...
        uid_list = []
        to_remove = []
        self.status("Building DICOM tree")
        progress_update = 0
        i = 0
        # for i in range(len(self.sorted_list)):
        for scan in self.sorted_list:
            i+=1
            current_progress=math.floor(float(i)/float(len(self.sorted_list))*100.)
            if current_progress>progress_update:
                progress_update=current_progress
                self.progress(float(progress_update))
        ...
        self.status(f"{len(self.sorted_list)} DICOM instances loaded")
                        ...
                        with ZipFile(this_eggpath, 'r') as modulezip:
                            ...
                            with modulezip.open('EGG-INFO/PKG-INFO', 'r') as pkg_info_file:
                                ...
                                try:
                                    ...
                                    cprint("ERROR: Unable to connect to repository for version check")
                                            ...
                                            with modulezip.open(cfg_file,'r') as file_object:
                                                ...
                                                for mod in self.module_list:
                                                    if (module_info['dirname']==mod['dirname'] and
                                                        ...
                                                        cprint("ERROR: MODULE CLASH: {} {}".format(module_info['dirname'],module_info['version']))
                                                        self.module_info = []
                                                        return
...
#                imp.reload(active_module)
            ...
            raise
            cprint("ERROR: No module selected")
        return
