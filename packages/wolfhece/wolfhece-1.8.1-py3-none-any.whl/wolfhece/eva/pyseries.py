...
RETURN_PERIOD_ESTIMATE = np.asarray([1.005, 1.0101, 1.0204, 1.111, 1.25, 1.4286, 2, 3, 5, 10, 15, 20, 25, 30, 40, 50, 60, 70, 80, 90, 100, 125, 150, 200, 300, 400, 500, 600, 750, 1000])
...
dists_2params = {'Gumbel':[stats.gumbel_r,'b',-1],       #special case of GEV with c=0
                ...
                'Gibrat':[stats.gilbrat,'orange',0],    #special case of lognorm with s=1
...
class EVA_Serie:
    ...
    def save_max_events(self, filename, years_bounds:list=None, seasons=None):
        """
        Enregistrement des crues maximales dans un fichier Excel
    ...
    def _fit_multistarts(self, data, fdist, nb=0, method='MLE'):
        ...
        if res is None:
            ...
            if dist is stats.genextreme :
                ...
                res = stats.gilbrat.fit(data,method=method)
                res = dist.fit(data,0.,loc=res[0],scale=res[1],method=method)
            else:
    ...
    def _evaluate_ci(self, data, fdist, nboot=500, method='MLE'):
        ...
        for curp in p_cum:
            if dist.numargs>0:
                ...
                ic['funcs'] = [dist(loc=curres[1],scale=curres[2]) for curres in all_params]
    ...
    def get_q_from_T(self,
        ...
        q = {curT : {'mean':lawfit.ppf(1-1/curT)} for curT in return_periods}
    ...
    def plot_annual(self, years=None, hydrol=True, fig:Figure=None, ax:Axes=None, show=False, withdatemax=False) -> tuple([Figure,Axes]):
        """
    ...
    def plot_serie(self, fig:Figure=None, ax:Axes=None, show=False) -> tuple([Figure,Axes]):
    ...
    def plot_maxevents(self, seasons=None, before=1, after=2, adim=True, fig:Figure=None, ax:Axes=None, show=False, alpha=.2) -> tuple([Figure,Axes]):
        """
    ...
    def plot_comp_maxevents(self, seasons=None, before=1, after=2, fig:Figure=None, ax:Axes=None, show=False, alpha=.2) -> tuple([Figure,Axes]):
        """
    ...
    def plot_summary(self, seasons=None, fig:Figure=None, ax:Axes=None, show=False) -> tuple([Figure,Axes]):
            ...
            if 'fit' in self.maxima[curseason].keys():
                ...
                for curmethod in curdict.keys():
                    if curmethod=='MLE':
                        ...
                        aic = [curdictm[curlaw]['aic'] for curlaw in curdictm.keys()]
                        aicc = [curdictm[curlaw]['aicc'] for curlaw in curdictm.keys()]
                        bic = [curdictm[curlaw]['bic'] for curlaw in curdictm.keys()]
                        rss = [curdictm[curlaw]['rss'] for curlaw in curdictm.keys()]

                        labels = curdictm.keys()

                        curax.plot(aic, 'b*', label='AIC')
                        curax.plot(aicc, 'r*', label='AICc')
                        curax.plot(bic, 'gd', label='BIC')
                        curax2.plot(rss, 'co', label='RSS')

                        curax.set_xticks(np.arange(len(curdictm.keys())))
                        if idx == nb_seasons:
                            curax.set_xticklabels(labels, rotation='vertical')
...
class EVA_Series:
    ...
    def find_maxima(self, excluded_years=[], hydrol=True, method:typing.Literal['BM', 'POT']='BM', threshold: float=0., r: typing.Union[pd.Timedelta, typing.Any] = "24H", verbose = True):
        """
    ...
    def bake_data(self, durations=[1], excluded_years=[], hydrol=True, method:typing.Literal['BM', 'POT']='BM', threshold: float=0., r: typing.Union[pd.Timedelta, typing.Any] = "24H", verbose=True):
        self.create_all_durations(durations, verbose)
        self.find_maxima(excluded_years, hydrol,method, threshold, r, verbose)

    def plot_series(self, durations=None, oneyear = True, show=False):
        """
    ...
    def evaluate_ci(self, durations=[1], laws=['GEV'], nboot=100, show=False) -> dict():
        """
        ...
        for curdur in durations:
            ...
            locserie.fit(laws=laws, ic=True, nboot=nboot)
    ...
    def evaluate_ic(self, durations=[1], laws=['GEV'], nboot=100, show=False) -> dict():
        """ alias evaluate_ci """
        return self.evaluate_ci(durations, laws, nboot, show)
    ...
    def get_one_MFSH(self, rising_time=6, return_period=50, deltat=1) -> Hydro_HSMF:

        durees = list(self.all_series.keys())
        Qp = [d.ppf(1-1/return_period) for d in self.bests]
    ...
    def plot_selected_max(self, seasons=None, durations=None,
        ...
        if split_seasons and len(seasons)>1:
            ...
            ax.set_xticklabels( [curyear for curyear in np.arange(locserie._get_dates(locserie.years_bounds[0],self.hydrological_year)[0].year
                                                                         ,locserie._get_dates(locserie.years_bounds[1]+1,self.hydrological_year, 'winter')[1].year)] + ['w->s']*len(ticks_season), rotation='vertical')
