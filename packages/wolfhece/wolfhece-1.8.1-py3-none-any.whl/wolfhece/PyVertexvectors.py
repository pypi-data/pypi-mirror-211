...
from .textpillow import PRIORITY_FONTSIZE, PRIORITY_WIDTH
from .PyTranslate import _
from .CpGrid import CpGrid
from .PyVertex import wolfvertex,getIfromRGB,getRGBfromI
from .PyParams import Wolf_Param
from .laz_viewer import myviewer
...
class vector:
    ...
    def __init__(self, lines:list=[], is2D=True, name='NoName', parentzone=None) -> None:
        """
    ...
    def add_vertices_from_array(self, xyz:np.ndarray):
        """
        Ajout de vertices depuis une matrice numpy -- shape = (nb_vert,3)
        """
        if xyz.shape[1]==3:
            for cur in xyz:
    ...
    def get_s3d(self):
        """
        calcul et retour des positions curvuilignes 3D
        """
...
class zone:
    ...
    def __init__(self,lines=[],name='NoName',parent=None,is2D=True) -> None:
...
class Zones(wx.Frame, Element_To_Draw):
    ...
    def Oncapture(self, event:wx.MouseEvent):
        ...
        if self.wx_exists:
            ...
            self.mapviewer.action='capture vertices'
    ...
    def OncaptureandDynapar(self, event:wx.MouseEvent):
        ...
        if self.wx_exists:
            ...
            self.mapviewer.action='dynamic parallel'
            firstvert=wolfvertex(0.,0.)
            self.active_vector.add_vertex(firstvert)
            self.mapviewer.mimicme()
    ...
    def Onmodify(self, event:wx.MouseEvent):
        ...
        if self.wx_exists:
            ...
            self.mapviewer.action='modify vertices'
            self.mapviewer.mimicme()
    ...
    def Oninsert(self, event:wx.MouseEvent):
        ...
        if self.wx_exists:
            ...
            self.mapviewer.action='insert vertices'
            self.mapviewer.mimicme()
    ...
    def OnClickfindactivate_vector(self, event:wx.MouseEvent):
        ...
        if self.wx_exists:
            ...
            if ret==wx.YES:
                self.mapviewer.action = 'select active vector inside'
            else:
                self.mapviewer.action = 'select active vector all'
    ...
    def OnClickfindactivate_vector2(self, event:wx.MouseEvent):
        ...
        if self.wx_exists:
            ...
            if ret==wx.YES:
                self.mapviewer.action = 'select active vector2 inside'
            else:
                self.mapviewer.action = 'select active vector2 all'
