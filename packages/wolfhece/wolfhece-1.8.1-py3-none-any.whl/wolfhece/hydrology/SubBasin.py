...
class SubBasin:
    ...
    # @unit $[\si{m}^3/\si{s}}]$
    outFlow:dict                    # [m^3/s] Hydro of the hydrological subbasin. Combined with the potentiel upstream hydros. Consider timeDelay so that time is at 0
        ...
        if(self.model==cst.tom_UH):
            ...
            if(self.dateEnd!=currDate):
                if(self.dateEnd!=currDate+datetime.timedelta(seconds=diffTimeInSeconds)):
                    ...
                    logging.warning("Date read = ", currDate)
                    logging.warning("Date expected = ", self.dateEnd)
                    logging.warning("The time series will be adapted to fit the expected dimensions")
    ...
    def get_outFlowRaw_noDelay(self, unit='m3/s'):
        ...
        if(index==0):
            ...
            logging.error("index = ", index)
            logging.error("len(myOutFlow) = ", len(myOutFlow))
            logging.error("self.timeDelay = ", self.timeDelay)
            return
    ...
    def compute_hydro(self):
        ...
        if(self.model==cst.tom_UH or self.model==cst.measures or self.model==cst.tom_GR4):
            ...
            if(index==0):
                ...
                if(self.model==cst.tom_GR4):
                    tmpHydro[index:] = self.myHydro[:-index,0]*self.surfaceDrained/3.6
                else:
                    tmpHydro[index:] = self.myHydro[:-index]*self.surfaceDrained/3.6
            else:
                logging.error("ERROR: the simulation time is not long enough for this subbasin to be taken into account")
                logging.error("Error informations : ")
                logging.error("Function name : compute_hydro()")
                logging.error("Name = ", self.name)
                logging.error("ID = ", self.iD)
                logging.error("index = ", index)
                logging.error("len(myOutFlow) = ", len(self.myHydro))
                logging.error("self.timeDelay = ", self.timeDelay)
                logging.error("All inlets timeDelay :")

                for element in self.intletsObj:
                    curObj = self.intletsObj[element]
                    logging.error(curObj.timeDelay)
                logging.error("=================")
                # sys.exit()
            ...
            if(index==0):
                ...
                tmpOutFlow[index:] = np.sum(self.myHydro[:-index],1)*self.surfaceDrained/3.6
            else:
                logging.error("ERROR: the simulation time is not long enough for this subbasin to be taken into account")
                logging.error("Error informations : ")
                logging.error("Function name : compute_hydro()")
    ...
    def get_outFlow(self, typeOutFlow="Net", unit='m3/s'):
