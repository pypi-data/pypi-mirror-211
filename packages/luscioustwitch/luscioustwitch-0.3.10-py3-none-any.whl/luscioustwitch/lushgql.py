...
class TwitchGQL_API:
  ...
  def __fetch_all_chat(self, video_id):
    ...
    while True:
      ...
      try:
        if r.json()[0]['data']['video']['comments'] == None:
          continue
      ...
      for commentdata in commentlist:
        ...
        commentOffset = commentdata['node']['contentOffsetSeconds']
        if commentOffset > contentOffsetSeconds:
          contentOffsetSeconds = commentOffset
      ...
      if has_next_page:
        content[0]['variables']["contentOffsetSeconds"] = int(contentOffsetSeconds)
      else:
        return comments
