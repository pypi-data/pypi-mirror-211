...
The following functions are available:
    ...
    * :func:`ks_test`
"""
#pylint: disable=too-many-arguments, too-many-locals, invalid-name, c-extension-no-member
#pylint:disable=too-many-lines, line-too-long, invalid-name, too-many-locals
...
def chi_squared_independence(data, key, observed_data=None, correction=False):
    """
...
def _ttest_base(data, mu=None, test_type=None, paired=None, var_equal=None,
                conf_level=None):
    conn = data.connection_context
    unique_id = str(uuid.uuid1()).replace('-', '_').upper()
...
def ttest_1samp(data, col=None, mu=0, test_type='two_sides', conf_level=0.95):
    """
...
def ttest_ind(data, col1=None, col2=None, mu=0, test_type='two_sides',
...
def ttest_paired(data, col1=None, col2=None, mu=0, test_type='two_sides',
                 conf_level=0.95):
    """
...
def wilcoxon(data, col=None, mu=None, test_type=None, correction=None):
    '''
    ...
    col : str or a list of str, optional
        Name of the data column that needs to be tested.
...
def median_test_1samp(data, col=None, mu=None, test_type=None,
...
def grubbs_test(data, key, col=None, method=None, alpha=None):
    ...
    conn = data.connection_context
    require_pal_usable(conn)
    method_map = {'two_sides':1, 'one_side_min':2, 'one_side_max':3, 'repeat_two_sides':4}
...
def entropy(data, col=None,
    ...
    conn = data.connection_context
    require_pal_usable(conn)
    distinct_value_count_detail = arg('distinct_value_count_detail', distinct_value_count_detail, bool)
    thread_ratio = arg('thread_ratio', thread_ratio, float)
    data_ = data
    if col is not None:
        if isinstance(col, str):
...
def factor_analysis(data, key, factor_num, col=None,
    ...
    DataFrame
     ...
     Variance explanation, structured as follows:
        ...
        - ROT_VAR_PROP: Variance proportion to the total variance explained with rotation.
     ...
     Rotation, structured as follows:
         ...
         - ROTATION:  rotation
         - ROTATION\_ + i (i sequences from 1 to number of columns in OBSERVED_VARS (in input table) : Rotation matrix.

     DataFrame 8:
     Factor correlation, structured as follows:

         - FACTOR_ID:  Factor id.
         - FACTOR\_ + i (i sequences from 1 to number of columns in OBSERVED_VARS (in input table): Factor correlation matrix.
     ...
     Scores, structured as follows:
         ...
         - FACTOR_ID:  Factor id.
         - FACTOR\_ + i (i sequences from 1 to number of columns in OBSERVED_VARS(in input table)): scores.
...
def ks_test(data, distribution_name=None, distribution_parameter=None, test_type=None):
    '''
    ...
    distribution_parameter : dict, optional
    ...
    if distribution_name is None:
        if len(data.columns) < 2:
            raise ValueError("Distribution is not provided. Using two-sample test failed due to the column number is less than 2.")
        data = data.select([data.columns[0], data.columns[1]])
    else:
        data = data.select(data.columns[0])
    test_type_map = {'two-sided': 0, 'less': 1, 'greater': 2}
    distribution_name_map = {"beta" : "beta",
                             "cauchy" : "cauchy",
                             ...
                             "uniform" : "uniform",
                             "weibull" : "weibull"}
    distribution_name = arg('distribution_name', distribution_name, distribution_name_map)
    distribution_parameter = arg('distribution_parameter', distribution_parameter, dict)
    test_type = arg('test_type', test_type, test_type_map)

    unique_id = str(uuid.uuid1()).replace('-', '_').upper()

    res_tbl = "#DISTRIBUTION_FIT_RESULT_TBL_{}".format(unique_id)

    param_rows = [('DISTRIBUTION_NAME', None, None, distribution_name),
                  ('TEST_TYPE', test_type, None, None)]
    if distribution_parameter is not None:
        param_rows.extend([(kkey.upper(), None, vval, None) for kkey, vval in distribution_parameter.items()])

    if check_pal_function_exist(conn, '%KS%', like=True):
        try:
            call_pal_auto_with_hint(conn,
                                    None,
                                    "PAL_KS_TEST",
        ...
        msg = 'The version of your SAP HANA does not support KS Test!'
        logger.error(msg)
        raise ValueError(msg)
    return conn.table(res_tbl)
