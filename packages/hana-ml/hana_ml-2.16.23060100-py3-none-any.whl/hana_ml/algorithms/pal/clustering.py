...
def SlightSilhouette(data,
    ...
    SAP HNAN PAL provides a light version of silhouette called slight silhouette. SlightSihouette
    is an wrapper for this light version silhouette method.
...
class AffinityPropagation(PALBase):
    ...
    def fit(self, data, key=None, features=None):
        ...
        with conn.connection.cursor() as cur:
            execute_logged(cur,
                           'CREATE LOCAL TEMPORARY COLUMN TABLE {} ("ID" INTEGER, "SEED_ID" INTEGER)'.format(seed_tbl),
                           conn.sql_tracer,
                           conn)
...
class KMeansOutlier(PALBase):
    ...
    def __init__(self,
        ...
        self.distance_level = self._arg('distance_level', distance_level,
                                        dict(manhattan=1, euclidean=2, minkowski=3))
