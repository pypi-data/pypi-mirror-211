...
from hana_ml.ml_base import execute_logged, ListOfStrings
    ...
    def predict(self, data):
        ...
        Prediction output: hana_ml DataFrame
            ...
            **'extra_applyout_settings'** = **{'APL/ApplyExtraMode': 'BestProbabilityAndDecision'}**.
            ...
            Parameters in the `SAP HANA APL Developer Guide <https://help.sap.com/viewer/p/apl>`_.
...
class GradientBoostingClassifier(_GradientBoostingClassifierBase):
    ...
    def fit(self, data,
        ...
        super(GradientBoostingClassifier, self).fit(data, key, features, label, weight, build_report)
    ...
    def score(self, data):
        ...
        -------
            Float or pandas DataFrame
                ...
                for each segment.
        """
        version = self._apl_version.loc[self._apl_version['name'] == 'APL.Version.ServicePack'].iloc[0]['value']
        if int(version) >= 2303:
            self._score(data)
