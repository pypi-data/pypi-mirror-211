    ...
    **{'APL/ApplyExtraMode': 'First Forecast with Stable Components and Residues and Error Bars'}**.
    ...
    def fit(self, data, key=None, features=None, build_report=False): #pylint: disable=too-many-branches
        ...
        try:
            ...
            super(AutoTimeSeries, self)._fit(
                data=data_sort,
    ...
    def predict(self, data, apply_horizon=None, apply_last_time_point=None, build_report=False):
        ...
        **{'APL/ApplyExtraMode': 'First Forecast with Stable Components and Residues and Error Bars'}**.
        ...
        try:
            ...
            self._last_apply_out = self._rewrite_applyout_df(applyout_df=applyout_df)
        except dbapi.Error as db_er:
        ...
        if self.train_data_ is not None:
            ...
            if segment_column_name is None and self.last_training_time_point is None:
                ...
                self.last_training_time_point = self.train_data_.dropna(subset=[self.target]) \
                    .tail().collect()[self.time_column_name].iloc[0].strftime("%Y-%m-%d %H:%M:%S")
        else:
    ...
    def _call_apl_forecast(self, data, key, features, target, horizon):
        ...
        try:
            ...
            applyout_table_arg = applyout_table if self._sql_generation_mode != SqlGenerationMode.SQLBLOCK_DU \
    ...
    def build_report(self, segment_name=None):
        """
        Build model report.
        ...
        if self._last_apply_out is not None and \
           self._last_apply_out.columns[offset:] == expected_columns:
            detail_level = _ModelReportDetailLevel.FULL
            forecast_df = self._last_apply_out.collect()
        else:
            if self._last_apply_out is not None:
        ...
        if detail_level != _ModelReportDetailLevel.NO_FORECAST:
            ...
            forecast_df['ACTUAL'].fillna(value=np.nan, inplace=True)
            forecast_df['ACTUAL'] = forecast_df['ACTUAL'].astype(float)
        ...
        components_df['Type'].replace('Cycles', 'Cycle', inplace=True)
        components_df['Type'].replace('Influencers', 'Influencer', inplace=True)
