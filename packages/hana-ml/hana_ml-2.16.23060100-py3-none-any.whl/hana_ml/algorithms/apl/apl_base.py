...
class APLBase(MLBase, ModelSavingServices): #pylint: disable=too-many-instance-attributes
    ...
    def _init_versions_and_options(self):
        ...
        if not (self._sql_generation_mode_forced and self._sql_generation_mode):
            ...
            if first_digit >= 4:
                self._sql_generation_mode = SqlGenerationMode.SQLBLOCK_DU  # Sql block + DU
            else:
    ...
    def set_scale_out(self,
        ...
        The execution of APL functions can then be routed to a specific compution node.
    ...
    def _create_func_header_table(self):
        ...
        max_tasks = getattr(self, 'max_tasks', None)
        if max_tasks is not None:
            data.append(('MaxTasks', str(max_tasks)))
    ...
    def _get_train_config_data(self):
        ...
        config_data.append(
            ('APL/IndicatorDataset', 'Validation', None)
        )
        return config_data
    ...
    def _predict(self, data, apply_config_data_df=None):
        ...
        try:
            ...
            # --- Call procedure
            if self._sql_generation_mode == SqlGenerationMode.SQLBLOCK_DU:
                procedure_name = "APL_APPLY_MODEL"
            else:
                procedure_name = "APL_APPLY_MODEL__OVERLOAD_4_3"
    ...
    def get_model_info(self):
        ...
        self._create_artifact_table(func_header_table)
        # --- OPERATION_CONFIG
        config_table = self._create_train_config_table()
        self._create_artifact_table(config_table)
        # === OUTPUT ===============
        # --- SUMMARY
        summary_table = APLArtifactTable(
    ...
    def _generate_debrief_tables(self):
        ...
        try:
            ...
            # DEBRIEF_METRIC
            debrief_metric_table = APLArtifactTable(
                name='#DEBRIEF_METRIC_{}'.format(self.id),
                type_name=APLArtifactTable.DEBRIEF_METRIC,
                apl_version=self._apl_version)

            # DEBRIEF_PROPERTY
            debrief_property_table = APLArtifactTable(
            ...
            # SUMMARY
            summary_table = APLArtifactTable(
                name='#DEBRIEF_SUMMARY_{}'.format(self.id),
                type_name=APLArtifactTable.SUMMARY,
                apl_version=self._apl_version)
    ...
    def get_debrief_report(self, report_name):
        """
        ...
        query = 'select * from "SAP_PA_APL"."sap.pa.apl.debrief.report::' \
                + report_name + '"(' + self.debrief_property_table_.name + ', ' \
                + self.debrief_metric_table_.name + ');'
    ...
    def _score(self, data):
        ...
        test_config_data_tuples = []

        segment_column_name = getattr(self, 'segment_column_name', None)
        if segment_column_name is not None:
            test_config_data_tuples.append(('APL/SegmentColumnName', segment_column_name, None))
        ...
        try:
            ...
            try:
                ...
                self._create_view(view_name=dataset_table, data=data)
            except dbapi.Error:
                dataset_table = '#TEST_DATASET_{}'.format(self.id)
                self._materialize_w_type_conv(name=dataset_table, data=data)

            test_config_table = APLArtifactTable(
                name='#TEST_CONFIG_{}'.format(self.id),
                type_name=APLArtifactTable.OPERATION_CONFIG_EXTENDED,
                apl_version=self._apl_version,
                data=test_config_data_tuples)

            func_header_table = self._create_func_header_table()

            unique_id = str(uuid.uuid4()).replace('-', '_').upper()
            model_table = APLArtifactTable(
                name='#MODEL_TEST_BIN_{}'.format(unique_id),
                type_name=APLArtifactTable.MODEL_BIN_OID,
                apl_version=self._apl_version)
            ...
            self._create_artifact_table(func_header_table)
            self._create_artifact_table(test_config_table)
            self._create_artifact_table(model_table)
