...
from hana_ml.dataframe import DataFrame, quotename, ConnectionContext, create_dataframe_from_pandas
...
class ModelStorage(object): #pylint: disable=useless-object-inheritance
    ...
    **delete** models.
    ...
    def export_model(self, name, version, directory=None):
        """
    ...
    def load_model_from_files(self, path, model_schema=None, use_temporary_table=True, force=False):
        ...
        model_ = []
        for m_name, m_path in files.items():
            model_.append(
    ...
    def import_model(self, path, model_schema=None, force=False, table_structure=None):
        ...
        for m_name, m_path in files.items():
            create_dataframe_from_pandas(self.connection_context,
                                         ...
                                         force=force)
        artifacts['artifacts']['model_tables'] = list(files.keys())
    ...
    def save_model_to_files(self, model, directory, save_report=False, storage_type='default'):
        ...
        """
        js_str, model_list = model._encode(self.schema)
        directory = os.path.join(directory, model.name + '_' + str(model.version))
        os.makedirs(os.path.join(directory, 'models'), exist_ok = True)
        pickled_name = 'info.zip'
        lib = 'PAL'  # lib
        ...
        if isinstance(model_list, (list, tuple)):
            ...
            pf.to_csv(os.path.join(os.path.join(directory, 'models'), model_list + '.zip'), index=False, encoding='utf-8', compression='zip')
    ...
    @classmethod
    def load_mlflow_model(cls, connection_context, model_uri):
        """
    ...
    def set_schedule(self,
        ...
        """
        metadata = self._get_model_metadata(name, version)
        config = self.schedule_config_template
