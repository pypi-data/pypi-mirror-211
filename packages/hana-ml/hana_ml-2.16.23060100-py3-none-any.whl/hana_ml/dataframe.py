...
class ConnectionContext(object):
    ...
    def copy(self):
        ...
        if self.userkey is None:
            conn_config = self.connection.__str__().replace('<dbapi.Connection Connection object : ', '').replace('>', '').split(',')
            url = conn_config[0]
            port = conn_config[1]
            user = conn_config[2]
    ...
    def _get_connection_id_by_tablename(self, tablename):
        schema = self.get_current_schema()
        sql = "SELECT TOP 1 CONNECTION_ID FROM M_TEMPORARY_TABLES WHERE SCHEMA_NAME='{}' AND TABLE_NAME='{}'".format(schema, tablename)
        return self.sql(sql).collect()["CONNECTION_ID"].iat[0]

    def get_connection_id(self):
        """
        Returns the connection id.
        """
        uuid_str = '#connect{}'.format(uuid.uuid1()).replace('-', '_').upper()
        self.create_table(uuid_str, table_structure={"connection_id": "INT"})
        connection_id = self._get_connection_id_by_tablename(uuid_str)
        self.drop_table(uuid_str)
    ...
    def upsert_streams_data(self, table_name, key, data, schema=None):
        ...
        for col in in_blob_dict.keys():
            in_blob_dict[col].write(data=data[col])
        for col in in_blob_dict.keys():
            in_blob_dict[col].close()
        self.connection.commit()
        cursor.close()
...
restricted by the keys of ``to_replace`` and ``value``.
    ...
    def corr(self, first_col, second_col):
        ...
        corr_select = "SELECT CORR({}, {}) FROM ({})".format(first_col, second_col, self.select_statement)
    ...
    def rearrange(self, key=None, features=None, label=None, type_ts=False):
        ...
        if features is None:
            features = cols
        return self[id_col + [label] + features] if type_ts else self[id_col + features + [label]]
...
def import_csv_from(connection_context,
    ...
    threads : int, optional
        ...
        Defaults to 1 and then maximun allowd value is 256.
