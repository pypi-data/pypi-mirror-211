...
class ConfusionMatrixReportBuilder(object):
    ...
    def build(self):
        ...
        self.__generated_js = EchartsUtil.generate_load_heatmap_js(chart_id, self.__table1_title, classname_list, self.__table1_columns, z_max, x_y_z_list)
...
class VariableImportanceReportBuilder(object):
    ...
    def set_variable_importance_table(self, target_df):
        ...
        if target_df is not None:
            if isinstance(target_df, dataframe.DataFrame):
                target_df = target_df.sort_values(by=[self.__variable_importance_table_columns[1]])
                target_df = target_df.collect()
            if target_df.empty is False:
                self.__variable_importance_table = target_df
    ...
    def build_items(self):
        ...
        option['legend'] = legend
        option['series'] = series
        item1 = ChartItem(self.__chart1_title, option)
        ...
        item2 = ChartItem(self.__chart2_title, option)
...
class _UnifiedClassificationReportBuilder(object):
    def __init__(self, param_columns=None, display_columns=None):
        ...
        self.__report_builder: ReportBuilder = None
    ...
    def _render_report(self):
        ...
        if self.framework_version == 'v2':
            ...
            if self.__shapley_explainer_of_score_phase is not None:
                ...
                pages.append(page)
            self.__report_builder = ReportBuilder(title='Unified Classification Model Report')
            self.__report_builder.addPages(pages)
            self.__report_builder.build()
        else:
            self.__all_html = ''
            self.__all_js = ''
    ...
    def report(self):
        ...
        if self.framework_version == 'v2':
            try:
                return self.__report_builder.getIframeHTMLText()
    ...
    def generate_html_report(self, filename):
        ...
        if self.framework_version == 'v2':
            try:
                self.__report_builder.generate_html('{}_unified_classification_model'.format(filename))
            except BaseException:
                raise Exception(self.__build_error_msg)
        else:
    ...
    def generate_notebook_iframe_report(self):
        ...
        if self.framework_version == 'v2':
            try:
                self.__report_builder.generate_notebook_iframe()
            except BaseException:
                raise Exception(self.__build_error_msg)
        else:
            if self.__iframe_report_html is None:
            ...
            if self.__iframe_report_html is None:
                raise Exception(self.__build_error_msg)
            print('\033[31m{}'.format('In order to review the unified classification model report better, you need to adjust the size of the left area or hide the left area temporarily!'))
...
class _UnifiedRegressionReportBuilder(object):
        ...
        if self.framework_version == 'v2':
            ...
            if self.__shapley_explainer_of_score_phase is not None:
                ...
                if self.__display_force_plot_of_score_phase is True:
                    page.addItem(self.__shapley_explainer_of_score_phase.get_force_plot_item())
                pages.append(page)
            self.__report_builder = ReportBuilder(title='Unified Regression Model Report')
            self.__report_builder.addPages(pages)
            self.__report_builder.build()
        else:
            self.__all_html = ''
    ...
    def generate_html_report(self, filename):
        ...
        if self.framework_version == 'v2':
            try:
                self.__report_builder.generate_html('{}_unified_regression_model'.format(filename))
            except BaseException:
                raise Exception(self.__build_error_msg)
        else:
            if self.__iframe_report_html is None:
    ...
    def generate_notebook_iframe_report(self):
        ...
        if self.framework_version == 'v2':
            ...
            if self.__iframe_report_html is None:
                raise Exception(self.__build_error_msg)
            print('\033[31m{}'.format('In order to review the unified regression model report better, you need to adjust the size of the left area or hide the left area temporarily!'))
            display(HTML(self.__iframe_report_html))
