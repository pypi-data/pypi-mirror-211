...
def _checkt_plotly():
    ...
    for pkg in list(installed_packages):
        if 'plotly' in pkg.key:
            if version_compare(plotly.__version__, "4.14.3"):
                return True
    logger.warn("plotly (>=4.16.3) has not been installed.")
    return False
...
def quarter_plot(data,
    ...
    for quarter in ["Q1", "Q2", "Q3", "Q4"]:
        for year in years:
            ...
            ax.plot(xx_filter_collect.iloc[:, 0], xx_filter_collect.iloc[:, 1], color="C0")
...
def timeseries_box_plot(data,
    ...
    data_.save(temp_tab_name)
    data_ = data_.connection_context.table(temp_tab_name)
    ax, _ = eda_plot.box_plot(data=data_, key=key, column=col,
                              groupby=data_.columns[2], ax=ax, legend=False, vert=True)
    data_.connection_context.drop_table(temp_tab_name)
    return ax
...
def plot_acf(data,
             ...
             title=None):
    """
    Plot autocorrelation function (PACF).
    ...
    Examples
    --------
    >>> plot_acf(df, key='ID', col='ts', method = 'fft', thread_ratio=0.4, calculate_confint=True, max_lag=40)
    ...
    if ax is None:
        ax = plt.axes()
    ax.stem(fetch_xy["LAG"].to_numpy(), fetch_xy["CF"].to_numpy(), use_line_collection=True)
...
def plot_pacf(data,
    ...
    Examples
    --------
    >>> plot_pacf(df, key='ID', col='ts', method = 'fft', thread_ratio=0.4, max_lag=20, calculate_confint=True)
    ...
    if ax is None:
        ax = plt.axes()
    ax.stem(fetch_xy["LAG"].to_numpy(), fetch_xy["PACF"].to_numpy(), use_line_collection=True)
...
def plot_time_series_outlier(data,
                             ...
                             ax=None):
    """
    Perform OutlierDetectionTS and plot time series with the highlighted outliers.
    ...
    odts = OutlierDetectionTS(window_size=window_size,
                              ...
                              threshold=threshold)
    result = odts.fit_predict(data=data, key=key, endog=col)
    ...
    ax.plot(result[res_col[0]], result[res_col[1]], color='blue')
    ax.scatter(outliers[res_col[0]], outliers[res_col[1]], color='red')
...
def plot_change_points(data,
    ...
    >>> plot_change_points(data=df, cp_object=bcpd)
    ...
    if isinstance(cp_object, BCPD):
        ...
        if display_trend is True:
            ax.plot(components.collect()[key], components.collect()["TREND"], color='orange', label="Trend Component")

        data_ = data.select([key, col]).sort_values(key).collect()

        ax.plot(data_.iloc[:, 0], data_.iloc[:, 1], color='blue', label="Original Time Series")
        if title is None:
            title = "Change Points"
        ax.set_title(title)
        ax.set_xlabel(key)
...
def plot_moving_average(data, col, rolling_window, key=None, ax=None, compare=True):
    ...
    if ax is None:
        ax = plt.axes()
    if compare:
        ax.plot(data_.iloc[:, 0], data_.iloc[:, 1], label=data_.columns[1])
    ax.plot(data_.iloc[:, 0], data_.iloc[:, 2], label=data_.columns[2])
    ax.legend()
...
def plot_seasonal_decompose(data,
    ...
    for num, color in zip(range(0, 3), ['b', 'r', 'g']):
        ax = axes[num]
        ax.plot(res.select(res.columns[0]).collect()[res.columns[0]].to_numpy(),
                res.select(res.columns[num + 1]).collect()[res.columns[num + 1]].to_numpy(),
...
def kdeplot(data, key, features=None, kde=kernel_density.KDE(), points=1000, enable_plotly=False, **kwargs):
    """
    ...
    if len(columns) == 1:
        ...
        if enable_plotly and _checkt_plotly():
            fig = go.Figure()
...
def hist(data, columns, bins=None, debrief=False, x_axis_fontsize=10,
         ...
         rounding_precision=3, replacena=0, enable_plotly=False, **kwargs):
    """
...
class EDAVisualizer(Visualizer):
    ...
    def __init__(self, ax=None, size=None, cmap=None, enable_plotly=False, fig=None, no_fig=False):
        super(EDAVisualizer, self).__init__(ax=ax, size=size, cmap=cmap, enable_plotly=enable_plotly, fig=fig, no_fig=no_fig)
    ...
    def distribution_plot(self, data, column, bins, title=None, x_axis_fontsize=10, #pylint: disable= too-many-locals, too-many-arguments
        ...
        if self.enable_plotly:
            return eda_plotly.distribution_plot(data, column, bins, title=title, x_axis_label=x_axis_label, y_axis_label=y_axis_label,
    ...
    def pie_plot(self, data, column, explode=0.03, title=None, legend=True,
        ...
        """
        if self.enable_plotly:
            return eda_plotly.pie_plot(data, column, title=title, title_fontproperties=title_fontproperties, fig=self.fig,
                                       subplot_pos=subplot_pos, **kwargs)
        else:
    ...
    def bar_plot(self, data, column, aggregation, title=None, label_fontsize=12, title_fontproperties=None, orientation=None, **kwargs): #pylint: disable=too-many-branches, too-many-statements
        ...
        >>> ax1 = f.add_subplot(111)
        >>> eda = EDAVisualizer(ax1)
        >>> ax, bar_data = eda.bar_plot(data=data, column='COLUMN', aggregation={'COLUMN':'count'})

        Returns : bar plot (count) of 'COLUMN'

        >>> ax1 = f.add_subplot(111)
        >>> eda = EDAVisualizer(ax1)
        >>> ax, bar_data = eda.bar_plot(data=data, column='COLUMN', aggregation={'OTHER_COLUMN':'avg'})

        Returns : bar plot (avg) of 'COLUMN' against 'OTHER_COLUMN'
        """
        if self.enable_plotly:
            return eda_plotly.bar_plot(data, column, aggregation, title=title, orientation=orientation,
                                       title_fontproperties=title_fontproperties, **kwargs)
        else:
    ...
    def box_plot(self,
        ...
        ax : Axes
            The axes for the plot.
        sta_table : pandas.DataFrame or list of pandas.DataFrame
            The data used in the plot.
        ...
        .. image:: image/box_plot2.png
           :align: center
           :scale: 80 %

        """
        if self.enable_plotly:
            return eda_plotly.box_plot(data,
                                       column,
                                       outliers=outliers,
                                       title=title,
                                       groupby=groupby,
