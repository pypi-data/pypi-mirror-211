    ...
    def __init__(self, feature_value_df: dataframe.DataFrame, reason_code_df: dataframe.DataFrame):
        ...
        for cell in reason_code_list:
            ...
            for feature_name in self.feature_name_list:
                ...
                if res is None:
                    ...
                    self.existed_feature_values_dict[feature_name].append(self.feature_values_dict[feature_name][k])
        ...
        for sum_order in np.argsort(existed_feature_sums):
            ...
            try:
                ...
                min_feature_value = 0
                max_feature_value = 0
            self.existed_feature_details.append((feature_name, existed_feature_sums[sum_order], min_feature_value, max_feature_value))
...
class BeeswarmPlot(object):
    def __init__(self, feature_value_and_effect: FeatureValueAndEffect):
        ...
        for existed_f_detail in feature_value_and_effect.existed_feature_details:
            option_data = []
            k = k + 1
            current_f_name, current_sum_f_effect, current_min_f_value, current_max_f_value = existed_f_detail
            ...
            if len(current_values) == 0 and len(current_effects) == 0:
                should_colored = False
            temp_index_array = np.arange(len(current_effects))
            np.random.shuffle(temp_index_array)
            new_current_effects = []
            for i in temp_index_array:
                new_current_effects.append(current_effects[i])
            if len(new_current_effects) == 0:
                new_current_effects = [0]
            min_effect = np.min(new_current_effects)
            max_effect = np.max(new_current_effects)
            quant = np.round(100 * (new_current_effects - min_effect) / (max_effect - min_effect + 1e-8))
            inds = np.argsort(quant + np.random.randn(len(new_current_effects)) * 1e-6)
            layer = 0
            last_bin = -1
            base_y = np.zeros(len(new_current_effects))
            for ind in inds:
                ...
                layer += 1
                last_bin = quant[ind]
            base_y *= 0.9 * (0.4 / np.max(base_y + 1))
            ...
            if should_colored:
                visualMaps.append({
                    'min': current_min_f_value,
                    'max': current_max_f_value,
                    'dimension': 2,
                    'precision': 2,
...
class DependencePlot(object):
    def __init__(self, feature_value_and_effect: FeatureValueAndEffect):
        ...
        for existed_f_detail in feature_value_and_effect.existed_feature_details:
            ...
            self.items.append(ChartItem('Dependence Plot for {}'.format(current_f_name), current_option))
...
class ForcePlot(object):
    ...
    def generate_notebook_iframe(self, iframe_height):
        ...
        HTMLFrameUtils.display(build_frame_html(self.frame_id, self.frame_src_str, iframe_height))
...
class ShapleyExplainer(object):
    ...
    def summary_plot(self, iframe_height=600):
        ...
        if self.__summary_plotter is None:
            self.__summary_plotter = ReportBuilder(title='Shap Summary Report')
