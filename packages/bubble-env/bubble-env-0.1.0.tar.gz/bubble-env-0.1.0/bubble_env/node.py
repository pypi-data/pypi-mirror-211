...
from bubble_env.base.host import Host
from bubble_env.base.process import Process
from bubble_env.utils import join_path, tar_files
...
class Node(Process):
    ...
    def __init__(self,
        ...
        super().__init__(host, port=p2p_port, name=f'b{p2p_port}', base_dir=base_dir)
        # 本地文件信息
        self.loc_bubble = bubble
        ...
        self.bubble = join_path(self.deploy_path, 'bubble')
        ...
        self.log_file = join_path(self.log_dir, 'bubble.log')
    ...
    def gql(self, scheme: Literal['ws', 'http'] = 'http'):
        ...
        if match:
            return f"{self.rpc(scheme)}/graphql/bubble"
    ...
    def install(self,
        ...
        # 可指定参数
        bubble = bubble or self.loc_bubble
    ...
    def init(self, force=False):
        ...
        """
        if force:
            self.host.ssh(f'rm -rf {join_path(self.data_dir, "bubble")}', sudo=True, warn=False, strip=False)

        result = self.host.ssh(f'{self.bubble} --datadir {self.data_dir} init {self.genesis_file}',
                               warn=False,
                               strip=False
                               )
    ...
    def upload_bubble(self, bubble_file):
        """ 使用缓存上传bubble
        """
        self.host.fast_put(bubble_file, self.bubble)
        self.host.ssh(f'chmod u+x {self.bubble}')
        logger.debug(f'Node {self} upload bubble success!')
    ...
    def supervisor_config(self):
        ...
        command = (f'command={self.bubble} --identity {self.name} '
