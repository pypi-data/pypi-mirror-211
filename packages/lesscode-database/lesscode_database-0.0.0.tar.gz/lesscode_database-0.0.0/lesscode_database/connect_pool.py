...
class Pool:
    ...
    async def create_es_pool(conn_info):
        ...
        hosts = [
            f"{conn_info.params.get('protocol', 'http')}://{conn_info.user}:{conn_info.password}@{host}:{conn_info.port}"
...
def get_pool(conn_info: ConnectionInfo):
    if conn_info.dialect == "es":
        if conn_info.async_enable:
            return Pool.create_es_pool(conn_info)
        else:
        ...
        if conn_info.async_enable:
            return Pool.create_mysql_pool(conn_info)
        else:
