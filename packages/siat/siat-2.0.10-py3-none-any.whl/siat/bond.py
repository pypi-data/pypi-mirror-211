...
def exchange_bond_price(symbol,fromdate,todate,power=0):
    ...
    try:
        ...
        print("  Currently support specific treasury/financial/convetible bonds traded in SSE only")
        return None
    ...
    lang=check_language()
    if lang=='Chinese':
        titletxt=texttranslate('沪深债券收盘价历史行情：')+get_exchange_bond_name_china(symbol)
        footnote="\n"+texttranslate("数据来源：新浪财经，")+today 
        close_txt='收盘价'
        ylabel_txt='价格'
    else:
        titletxt='Exchange Bond Price Trend: '+get_exchange_bond_name_china(symbol)
