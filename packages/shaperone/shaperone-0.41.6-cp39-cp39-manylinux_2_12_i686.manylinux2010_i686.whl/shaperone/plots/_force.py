...
def force(base_value, shap_values=None, features=None, feature_names=None, out_names=None, link="identity",
    ...
    contribution_threshold : float
        ...
        Only features that the magnitude of their shaperone value is larger than min_perc * (sum of all abs shaperone values)
        will be displayed.
    """
    ...
    if (isinstance(base_value, np.ndarray) or type(base_value) == list):
        if not isinstance(shap_values, list) or len(shap_values) != len(base_value):
            raise Exception("In v0.20 force_plot now requires the base value as the first parameter! " \
                            "Try shaperone.force_plot(explainer.expected_value, shap_values) or " \
                            "for multi-output models try " \
                            "shaperone.force_plot(explainer.expected_value[0], shap_values[0]).")
    ...
    if shap_values.shape[0] == 1:
        ...
        if shap_values.shape[0] > 3000:
            warnings.warn("shaperone.plots.force is slow for many thousands of rows, try subsampling your data.")
...
def initjs():
    assert have_ipython, "IPython must be installed to use initjs()! Run `pip install ipython` and then restart shaperone."
...
def save_html(out_file, plot, full_html=True):
    ...
    plot : BaseVisualizer
        Visualizer returned by shaperone.force_plot()
    ...
    assert isinstance(plot, BaseVisualizer), "save_html requires a Visualizer returned by shaperone.force_plot()."
    internal_open = False
...
class SimpleListVisualizer(BaseVisualizer):
    ...
    def html(self):
        # assert have_ipython, "IPython must be installed to use this visualizer! Run `pip install ipython` and then restart shaperone."
        return """
