...
def compile_cuda_module(host_args):
    ...
    nvcc_command = "shaperone/cext/_cext_gpu.cu -lib -o {} -Xcompiler {} -I{} " \
                   "--std c++14 " \
...
def run_setup(with_binary, test_xgboost, test_lightgbm, test_catboost, test_spark, test_pyod,
    ...
    if with_binary:
        ...
        ext_modules.append(
            Extension('shaperone._cext', sources=['shaperone/cext/_cext.cc'],
    ...
    if with_cuda:
        try:
            ...
            ext_modules.append(
                Extension('shaperone._cext_gpu', sources=['shaperone/cext/_cext_gpu.cc'],
                          extra_compile_args=compile_args,
                          library_dirs=[lib_dir, cudart_path],
                          libraries=[lib, 'cudart'],
                          depends=['shaperone/cext/_cext_gpu.cu', 'shaperone/cext/gpu_treeshap.h','setup.py'])
            )
    ...
    setup(
        ...
        packages=[
            ...
            'shaperone.plots', 'shaperone.plots.colors', 'shaperone.benchmark', 'shaperone.maskers', 'shaperone.utils',
            'shaperone.actions', 'shaperone.models'
        ],
        package_data={'shaperone': ['plots/resources/*', 'cext/tree_shap.h']},
