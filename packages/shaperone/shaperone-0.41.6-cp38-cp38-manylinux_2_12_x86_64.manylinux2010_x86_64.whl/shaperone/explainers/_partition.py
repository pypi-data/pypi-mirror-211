...
class Partition(Explainer):
    ...
    def __init__(self, model, masker, *, output_names=None, link=links.identity, linearize_link=True,
        ...
        masker : function or numpy.array or pandas.DataFrame or tokenizer
            ...
            functions are available in shaperone such as shaperone.maksers.Image for images and shaperone.maskers.Text
            for text.
        ...
        if not safe_isinstance(self.model, "shaperone.models.Model"):
        ...
        if getattr(self.masker, "clustering", None) is None:
            raise ValueError("The passed masker must have a .clustering attribute defined! Try shaperone.maskers.Partition(data) for example.")
    ...
    def __str__(self):
        return "shaperone.explainers.Partition()"
