...
def text(shap_values, num_starting_labels=0, grouping_threshold=0.01, separator='', xmin=None, xmax=None, cmax=None, display=True):
    ...
    shap_values : [numpy.array]
        List of arrays of SHAP values. Each array has the shaperone values for a string(# input_tokens x output_tokens).
    ...
    xmin : float
        Minimum shaperone value bound.

    xmax : float
        Maximum shaperone value bound.

    cmax : float
        Maximum absolute shaperone value for sample. Used for scaling colors for input tokens.
...
def heatmap(shap_values):
    ...
    # generate dictionary containing precomputed background colors and shaperone values which are addressable by html token ids
    colors_dict = {}
    shap_values_dict = {}
