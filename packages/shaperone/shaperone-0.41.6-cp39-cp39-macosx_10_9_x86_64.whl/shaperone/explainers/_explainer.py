...
class Explainer(Serializable):
    ...
    def __init__(self, model, masker=None, link=links.identity, algorithm="auto", output_names=None, feature_names=None, linearize_link=True,
        ...
        masker : function, numpy.array, pandas.DataFrame, tokenizer, None, or a list of these for each model input
            ...
            functions are available in shaperone such as shaperone.ImageMasker for images and shaperone.TokenMasker
            ...
            shaperone.TabularMasker(data, hclustering="correlation") will enforce a hierarchial clustering
        ...
        link : function
            ...
            default it is shaperone.links.identity, but shaperone.links.logit can be useful so that expectations are
        ...
        # wrap the incoming masker object as a shaperone.Masker object
        ...
        elif safe_isinstance(self.model, "shaperone.models.TeacherForcing") and safe_isinstance(self.masker, ["shaperone.maskers.Text", "shaperone.maskers.Image"]):
            self.masker = maskers.OutputComposite(self.masker, self.model.text_generate)
        elif safe_isinstance(self.model, "shaperone.models.TopKLM") and safe_isinstance(self.masker, "shaperone.maskers.Text"):
    ...
    def save(self, out_file, model_saver=".save", masker_saver=".save"):
        ...
        """
        super().save(out_file)
        with Serializer(out_file, "shaperone.Explainer", version=0) as s:
    ...
    def load(cls, in_file, model_loader=Model.load, masker_loader=Masker.load, instantiate=True):
        ...
        with Deserializer(in_file, "shaperone.Explainer", min_version=0, max_version=0) as s:
