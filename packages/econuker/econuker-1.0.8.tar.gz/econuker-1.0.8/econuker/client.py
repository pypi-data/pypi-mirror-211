...
class Client:
    ...
    def verify(self, auth_token) -> Token:
        """
        Verifies an authentication token.
    ...
    class ServersResult:
        ...
        def __init__(self, raw_data):
            self._raw: dict = raw_data
            self.count: int = raw_data.get("count", 0)
            self.server_ids: list = raw_data.get("servers", [])
    ...
    def fetch_server(self, id:str) -> ServerResult:
        """
    ...
    def fetch_items(self, hidden:bool=True) -> ItemsResult:
        ...
        if not hidden:
            if not self.auth_level == 'admin':
                raise Forbidden("You tried to see hidden items without a admin level token.")
    ...
    def fetch_item(self, id:int|str=None, name:str=None) -> ItemResult:
        """
