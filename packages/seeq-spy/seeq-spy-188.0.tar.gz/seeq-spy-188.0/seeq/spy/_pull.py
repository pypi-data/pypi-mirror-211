...
def _process_query_row(session: Session, at_least_one_signal, calculation, shape, capsule_properties, grid, header,
    ...
    if item_type == 'Asset':
        ...
        item_id, item_type, calculation_to_use = _request_swap_items(item_id, item_type, calculation_to_use)
    ...
    if shape == 'samples':
        ...
        for capsule in capsules_output['capsules']:  # type: dict
            pd_capsule_start = _common.convert_to_timestamp(
                capsule['start'] if capsule.get('start', None) is not None else 0, tz)
            pd_capsule_end = _common.convert_to_timestamp(
                capsule['end'] if capsule.get('end', None) is not None else 7258118400000000000, tz)
