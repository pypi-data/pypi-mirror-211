...
def create_higher_candle(df: pd.DataFrame, candle: str) -> pd.DataFrame:
    ...
    df_copy = df.copy()
    df_copy["open_time"] = df_copy["open_time"] // 1000
    df_copy["open_time_date"] = pd.to_datetime(df_copy["open_time"], unit="s")
    df_copy.set_index("open_time_date", inplace=True)
    ...
    resampled_data["open_time"] = resampled_data["open_time"] * 1000
            ...
            if window - max_indices[-1] <= last_extrema_space:
                ...
                if (lower_mid_tol <= local_min_val <= upper_mid_tol) and (
                    ...
                    if lower_second_tol <= local_max_val_2 <= upper_second_tol:
                        ...
                        df[f"dtop_index_maxima_1{extension}"].iloc[i] = max_indices[-1]
                        df[f"dtop_index_minima_2{extension}"].iloc[i] = min_indices[-1]
                        df[f"dtop_index_maxima_3{extension}"].iloc[i] = max_indices[-2]
            ...
            if window - min_indices[-1] < last_extrema_space:
                ...
                if (lower_mid_tol <= local_max_val <= upper_mid_tol) and (
                    ...
                    if lower_second_tol <= local_min_val_2 <= upper_second_tol:
                        ...
                        df[f"dbot_index_minima_1{extension}"].iloc[i] = min_indices[-1]
                        df[f"dbot_index_maxima_2{extension}"].iloc[i] = max_indices[-1]
                        df[f"dbot_index_minima_3{extension}"].iloc[i] = min_indices[-2]
