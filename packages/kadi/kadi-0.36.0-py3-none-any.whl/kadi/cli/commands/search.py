...
def ls():
    ...
    for model in _get_searchable_models():
        ...
        try:
            current_index = list(es.indices.get_alias(tablename).keys())[0]
        except:
            current_index = None

        for index in es.indices.get(f"{tablename}*").keys():
    ...
    for model in _get_searchable_models(tablename):
        ...
        for obj in model_query.order_by("created_at"):
            if not add_to_index(obj, index=new_index):
                echo_danger(f"Error indexing {tablename} with ID '{obj.id}'.")
