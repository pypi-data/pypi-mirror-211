...
class RecordRDFGraph(RDFGraph):
    ...
    def add_record_metadata(self, record):
        ...
        if record_data["type"] is not None:
            record_type = quote(record_data["type"], safe="")
            self.add((record_ref, SDO.additionalType, record_ns[record_type]))
...
class RecordROCrate(ROCrate):
    ...
    def __init__(self, record_or_records, root_dir, export_filter=None, user=False):
        ...
        for record in record_or_records:
            args = [record, self.export_filter, self.user]
            record_export_data[record.id] = {
                "json": get_record_json_data(*args),
                "rdf": get_record_rdf_data(*args),
            }
    ...
    def _add_author_entity(self, user_data):
        ...
        if "email" in user_data["identity"]:
            author_entity["email"] = user_data["identity"]["email"]
    ...
    def add_record_metadata(self, record, record_export_data):
        ...
        # Collect the metadata of the record JSON and RDF export.
        for export_type in ["json", "rdf"]:
        ...
        for file_data in record_data["files"]:
            filename = quote(file_data["name"], safe="")
    ...
    def add_record_data(self, record, record_export_data):
        ...
        # Add the record JSON and RDF export files.
        for export_type in ["json", "rdf"]:
            export_meta = const.EXPORT_TYPES["record"][export_type]
...
def get_record_export_data(record, export_type, export_filter=None, user=None):
    ...
    :param export_type: The export type, one of ``"json"``, ``"pdf"``, ``"qr"``,
        ``"rdf"`` or ``"ro-crate"``.
    :param export_filter: (optional) A dictionary specifying various filters to adjust
        ...
        combination with the ``"json"``, ``"pdf"``, ``"qr"`` and ``"ro-crate"`` export
        types.
