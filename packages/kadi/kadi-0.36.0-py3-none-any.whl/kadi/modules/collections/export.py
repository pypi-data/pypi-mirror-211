...
def get_rdf_data(collection, export_filter, user):
    ...
    record_graph = RecordRDFGraph(records, export_filter=export_filter, user=user)
    rdf_data = (c_graph + record_graph).serialize(format="turtle")
...
def get_export_data(collection, export_type, export_filter=None, user=None):
    ...
    :param export_type: The export type, one of ``"json"``, ``"qr"``, ``"rdf"`` or
        ``"ro-crate"``.
    :param export_filter: (optional) A dictionary specifying various filters to adjust
        ...
        combination with the ``"json"``, ``"rdf"`` and ``"ro-crate"`` export types.
