...
/** Mixin to add common functionality to graph visualization Vue components. */
export default {
  data() {
    return {
  ...
  watch: {
    isRendered() {
      this.resizeView(this.$refs.container, this.$refs.svgContainer);
    },
    filter() {
      this.filterNodes();
    },
  ...
  methods: {
    ...
    downloadGraph() {
      ...
      hyperlinkElement.href = svgData;
      hyperlinkElement.download = filename;
    ...
    resizeView(containerElement, svgContainerElement) {
      ...
      this.width = containerElement.getBoundingClientRect().width - 2;
      ...
      if (kadi.utils.isFullscreen()) {
        ...
        svgContainerElement.style.height = `${this.height}px`;
        containerElement.style.borderRadius = '0.25rem';
      }
    ...
    isNodeVisible(node) {
      return this.nodesContainer
        .select(`#${this.prefixes.node}-${node.id}-${this.suffix}`)
        .style('visibility') === this.visibilities.visible;
    },

    filterNodes() {
      // First, filter all nodes depending on the identifier filter, excluded types and excluded collections.
      this.nodesContainer.selectAll(`.${this.prefixes.node}-${this.suffix}`).each((d, i, nodes) => {
        ...
        if (d.identifier_full.includes(this.filter.trim())) {
          node.style('visibility', this.visibilities.visible);

          if (d._type === this.types.collection) {
            if (this.excludedCollections.includes(d.id)) {
              node.style('visibility', this.visibilities.hidden);
            }
          } else {
      ...
      // Next, filter all links depending on the node visibilities.
      this.linksContainer.selectAll(`.${this.prefixes.link}-${this.suffix}`).each((d, i, nodes) => {
        const link = d3.select(nodes[i]);

        if (this.isNodeVisible(d.source) && this.isNodeVisible(d.target)) {
          link.style('visibility', this.visibilities.visible);
        } else {
          link.style('visibility', this.visibilities.hidden);
        }
      });
    },

    createContainers(svgContainerElement) {
      this.zoom = d3.zoom().on('zoom', (e) => this.graphContainer.attr('transform', e.transform));
      this.svg = d3.select(svgContainerElement)
        .append('svg')
        .call(this.zoom)
    ...
    drawNodes(highlightCollections = false, recordsCallback = null, childrenCallback = null) {
      ...
      const nodesGroup = this.nodesContainer
        .selectAll(`.${this.prefixes.node}-${this.suffix}`)
      ...
      const hoverCollection = (e, d, hover) => {
        const visibility = hover ? this.visibilities.visible : this.visibilities.hidden;
      ...
      collectionGroup
        ...
        .style('fill', (d) => this.getCollectionColor(collectionColors, d.id))
        .style('stroke', (d) => this.getCollectionColor(collectionColors, d.id, true))
        .style('stroke-width', (d) => (startNode && d.id === startNode.id ? 5 : 3))
      ...
      const controlsGroup = collectionGroup
        ...
        .style('visibility', this.visibilities.hidden);
      ...
      const recordsButton = controlsGroup
        ...
        .on('click', (e, d) => {
          ...
          if (use.style('opacity') === String(buttonDisabledOpacity)) {
            use.style('opacity', 1);
          } else {
            use.style('opacity', buttonDisabledOpacity);
          }
        });
      ...
      recordsButton
        ...
        .attr('r', 11)
        .attr('cx', -23)
        .style('fill', (d) => this.getCollectionColor(collectionColors, d.id, true));
      ...
      recordsButton
        ...
        .style('opacity', buttonDisabledOpacity);
        ...
        this.linksContainer
          ...
          .each((dLink, i, nodes) => {
            if (dLink.source._type === this.types.record && (dLink.source.id === d.id || dLink.target.id === d.id)) {
              ...
              path.style('stroke', stroke);
              path.style('marker-end', `url(#${marker}-${this.suffix})`);
            }
          });
      };
      ...
      if (highlightCollections) {
        recordGroup
          ...
          .attr('r', 15)
          .style('cursor', 'pointer')
          .style('fill', (d) => this.getTypeColor(typeColors, d.type_full));
      }
      ...
      nodesGroup
        ...
        .on('mouseover', (e) => {
          d3.select(e.target)
            .style('fill', this.colors.textFocus)
            .style('font-size', '115%');
        })
        .on('mouseout', (e) => {
          d3.select(e.target)
    ...
    drawLinks() {
      ...
      const linksGroup = this.linksContainer
        .selectAll(`.${this.prefixes.link}-${this.suffix}`)
      ...
      linksGroup
        ...
        .style('font-family', this.fontFamily)
        .style('font-size', linkNameFontSize)
        .style('text-anchor', 'middle')
        .on('mouseover', (e) => {
          d3.select(e.target)
            .style('fill', this.colors.textFocus)
    ...
    drawLegend() {
      ...
      legendGroup
        ...
        .style('fill', (d) => this.getTypeColor(typeColors, d))
        .style('stroke', (d) => this.getTypeColor(typeColors, d, true))
        ...
        .style('opacity', typeExcludedOpacity);
      ...
      legendGroup
        ...
        .style('fill', (d) => this.getTypeColor(typeColors, d, true))
        .style('cursor', 'default');
    },
  },
};
