...
class OAuth2ServerClient(
    ...
    def scope(self):
        ...
        return self.client_metadata.get("scope", None)
    ...
    def update_client_metadata(self, **kwargs):
        ...
        for key, value in kwargs.items():
            if key not in self._client_metadata or self._client_metadata[key] != value:
                ...
                # Make sure to alert SQLAlchemy of the change, as it won't be persisted
                # otherwise, but only if something actually changed.
...
class OAuth2ServerAuthCode(SimpleReprMixin, OAuth2AuthorizationCodeMixin, db.Model):
    ...
    def is_expired(self):
        ...
        """
        expiration_time = (
            self.auth_time + current_app.config["OAUTH_AUTH_CODE_EXPIRES_IN"]
        )
