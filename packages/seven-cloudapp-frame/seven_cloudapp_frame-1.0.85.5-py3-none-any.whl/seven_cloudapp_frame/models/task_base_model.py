...
@LastEditTime: 2023-05-19 09:23:40
...
class TaskBaseModel(FrameBaseModel):
    ...
    def get_task_asset_type(self, task_asset_type_json, task_type):
        ...
        :param task_asset_type:任务资产类型配置  key:0自定义1次数2积分3价格档位4店铺会员积分9999其他（混合搭配）
    ...
    def get_client_task_list_v2(self, app_id, act_id, module_id, user_id, task_types="", daily_repeat_browse=False):
        ...
        for task_info_dict in task_info_list:
            ...
            result = {}
            result["sort_index"] = task_info_dict["sort_index"]
            result["id"] = task_info_dict["id"] # 任务id
            result["task_type"] = task_info_dict["task_type"] # 任务类型
            result["task_type_name"] = SevenHelper.get_enum_key(TaskType, task_info_dict["task_type"]) # 任务类型名称
            result["title"] = task_info_dict["task_name"] # 任务名称
            result["desc"] = '' # 描述
            result["current"] = 0 #当前轮计数值
            result["total"] = 0 #当前轮领取阀值（如果任务没有（1/2）这样的填0）消费任务表示领取奖励需要消费的金额
            result["data"] = [] #如果有额外数据放这里，如果商品数据，或者子任务数据
            result["complete_count"] = 1 #可以完成的次数
            result["current_complete_count"] = 0 #当前完成的次数
            result["reward_value"] = 0 # 奖励值
            result["completed_ids"] = [] # 完成的ids，比如：已收藏的商品ids或者已完成的子任务ids
            result["get_ways"] = 0 # 领取方式(0直接领取 1需要用户主动领取)
            ...
            #每周签到(status:0当天未签到1当天已签到2已完成)
                ...
                if task_count.complete_count >= max_sign_day:
                    ...
                    result["status"] = 1 if task_count.modify_day == now_day else 0
                result["complete_count"] = max_sign_day #可以完成的次数
                result["current_complete_count"] = task_count.complete_count #当前完成的次数
