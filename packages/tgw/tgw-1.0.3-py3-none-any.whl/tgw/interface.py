...
def QueryKline(req_kline_cfg, query_spi = None, return_df_format = True):
    ...
    + 参数2 query_spi: 用户层回调实现。如果为None，表示同步使用模式。默认为同步模式。如果选择异步模式，需要实现回调函数【OnResponse(result, err : str) : result为数据结果，如果为None，可以查看status】
    + 参数3 return_df_format: 返回结果的格式。默认为pandas中dataframe格式，如果为False，为json格式
    + 返回值: 一个元组(result,err)。result表示为查询结果集。err为错误信息。如果结果集为None，可以通过err查看相关错误信息。异步模式下，result为True或者False
    异常描述: 无
    ...
    result = None
    if not g_api_model:
        err = 'no api model. please set api model first'
    else:
        # 没有回调，说明是同步
        if not query_spi:
            ...
            if error_code != tgw.ErrorCode.kSuccess:
                result = None
                err = tgw.GetErrorMsg(error_code)
            else:
                wait_event.wait()
...
def QueryOrderQueue(req_order_queue_cfg, query_spi = None, return_df_format = True):
    ...
    if not g_api_model:
        ...
        err = "kInternetMode have no 'QueryOrderQueue' permission"
    # 没有回调，说明是同步
    else:
        if not query_spi:
            wait_event = threading.Event()
...
def QueryTickExecution(req_tick_exec_cfg, query_spi = None, return_df_format = True):
    ...
    if not g_api_model:
        ...
        err = "kInternetMode have no 'QueryTickExecution' permission"
    # 没有回调，说明是同步
    else:
        if not query_spi:
            wait_event = threading.Event()
...
def QueryTickOrder(req_tick_order_cfg, query_spi = None, return_df_format = True):
    ...
    if not g_api_model:
        ...
        err = "kInternetMode have no 'QueryTickOrder' permission"
    else :
        # 没有回调，说明是同步
        if not query_spi:
            wait_event = threading.Event()
...
def QueryCodeTable(query_spi = None, return_df_format = True):
    """
    功能描述: 查询代码表
    + 参数1 query_spi: 用户层回调实现。如果为None，表示同步使用模式。默认为同步模式。如果选择异步模式，需要实现回调函数【OnResponse(result, err : str) : result为数据结果，如果为None，可以查看status】
...
def QueryETFInfo(req_etf_info_cfg, query_spi = None, return_df_format = True):
    ...
    + 参数2 query_spi: 用户层回调实现。如果为None，表示同步使用模式。默认为同步模式。如果选择异步模式，需要实现回调函数【OnResponse(result, err : str) : result为数据结果，如果为None，可以查看status】
    + 参数3 return_df_format: 返回结果的格式。默认为pandas中dataframe格式，如果为False，为json格式
    + 返回值: 一个元组(result,err)。result表示为查询结果集。err为错误信息。如果结果集为None，可以通过err查看相关错误信息。异步模式下，result为True或者False。对于dataframe格式，result结果为[(df1,df1'),(df2,df2'),...]，df1为etf信息，df1'为对应成分股信息；对于json格式，result结果为[(json1, [json1', json1'']),... ]，json1为etf信息，json1',json2'' 为对应成分股信息
    异常描述: 无
...
def QueryFactor(req_factor_cfg, query_spi = None):
    ...
    + 参数2 query_spi: 用户层回调实现。如果为None，表示同步使用模式。默认为同步模式。如果选择异步模式，需要实现回调函数【OnResponse(result, err : str) : result为数据结果，如果为None，可以查看status】
    + 返回值: 一个元组(result,err)。result表示为查询结果集。err为错误信息。如果结果集为None，可以通过err查看相关错误信息。result为json数组。异步模式下，result为True或者False
    异常描述: 无
    ...
    if not g_api_model:
        ...
        if not query_spi:
            ...
            error_code = tgw.IGMDApi_QueryFactor(query_factor_spi, req_factor_cfg)
            if error_code != tgw.ErrorCode.kSuccess:
                result = None
                err = tgw.GetErrorMsg(error_code)
            else:
...
def ReplayKline(replay_cfg, replay_spi, return_df_format = True):
    ...
    + 参数2 replay_spi: 应用层回调函数【OnResponse(task_id, result, err : str) : result为数据结果，如果为None，可以查看err】。
