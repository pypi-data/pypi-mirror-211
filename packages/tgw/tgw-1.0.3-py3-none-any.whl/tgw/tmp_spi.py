...
class TmpQuerySnapshotSpi(tgw.IGMDSnapshotSpi):
    ...
    def OnStatus(self, status):
        if not self._spi is None:
            err = 'error_code:{0},error_msg:{1}'.format(status.error_code, status.error_msg)
            self._spi(None, err) 
    ...
    def OnResponse(self, result, status):
        ...
        if not status is None:
            if  isinstance(status, str):
                ...
                self._err = 'error_code:{0},error_msg:{1}'.format(status.error_code, status.error_msg)
...
class TmpReplaySpi(tgw.IGMDHistorySpi):
    ...
    def OnRspTaskStatus(self, task_id, task_status):
        if not self._spi is None:
            self._err = 'error_code:{0},error_msg:{1}'.format(task_status.error_code, task_status.error_msg)
