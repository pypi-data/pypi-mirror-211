...
from engine.libs.utils import (RemoveAssignFromAST, _get_external_cfg_base_path,
...
class Config:
    ...
    def _substitute_predefined_vars(filename: str, temp_config_name: str):
        ...
        config_file = url_file_open(filename)
    ...
    def _file2dict(
        ...
        with tempfile.TemporaryDirectory() as temp_config_dir:
            ...
            for k in cfg_dict:
                if k == 'train_dataloader':
                    if 'dataset' in cfg_dict[k]:
                        if 'pipeline' in cfg_dict[k]['dataset']:
                            cfg_dict[k]['dataset'] = "TRAIN_CASE"
                        elif 'dataset' in cfg_dict[k]['dataset']:
                            cfg_dict[k]['dataset']['dataset'] = "TRAIN_CASE"
                if k == 'val_dataloader':
                    if 'dataset' in cfg_dict[k]:
                        if 'pipeline' in cfg_dict[k]['dataset']:
                            cfg_dict[k]['dataset'] = "VAL_CASE"
                        elif 'dataset' in cfg_dict[k]['dataset']:
                            cfg_dict[k]['dataset']['dataset'] = "VAL_CASE"
                if k == 'test_dataloader':
                    if 'dataset' in cfg_dict[k]:
                        if 'pipeline' in cfg_dict[k]['dataset']:
                            cfg_dict[k]['dataset'] = "TEST_CASE"
                        elif 'dataset' in cfg_dict[k]['dataset']:
                            cfg_dict[k]['dataset']['dataset'] = "TEST_CASE"

                if k == 'train_pipeline' or k == 'test_pipeline' or k == 'val_pipeline':
                    if cfg_dict[k][0]['type'] == 'LoadImageFromFile':
                        cfg_dict[k][0]['type'] = 'LoadBandsFromFile'
                        cfg_dict[k][0]['to_float32'] = True
                    else:
                        cfg_dict[k].insert(0, {'type': 'LoadBandsFromFile',
                                               'to_Float32': True})
        ...
        cfg_text = filename + '\n'
        cfg_text += url_file_open(filename)
...
def transform_dataloaders(cfg_string):
    cfg_string = cfg_string.replace("'TRAIN_CASE'", """dict(
        type=dataset_type,
        data_path=data_path,
        raster_dir_path=raster_dir_path,
        vector_dir_path=vector_dir_path,
        class_title=class_title,
        split="train",
        pipeline=train_pipeline)""")

    cfg_string = cfg_string.replace("'VAL_CASE'", """dict(
            type=dataset_type,
            data_path=data_path,
            raster_dir_path=raster_dir_path,
            vector_dir_path=vector_dir_path,
    ...
    cfg_string = cfg_string.replace("'TEST_CASE'", """dict(
            type=dataset_type,
            data_path=data_path,
            raster_dir_path=raster_dir_path,
            vector_dir_path=vector_dir_path,
...
def get_pyconfig_string(url):
    url = url.replace('github.com', 'raw.githubusercontent.com').replace('blob/', '')
    config = Config.fromfile(filename=url)
    cfg_string = transform_dataloaders(config.pretty_text)
