...
class TokenizerWrapper:
    def __init__(
        ...
        **kwargs,
        ...
        self.mask_token_map = {
            self.template_mask_token: self.tokenizer.mask_token
            if hasattr(self.tokenizer, "mask_token")
            else ""
        }
        self.eos_token_map = {
            self.template_eos_token: self.tokenizer.eos_token
            if hasattr(self.tokenizer, "eos_token")
            else ""
        }
        self.bos_token_map = {
            self.template_bos_token: self.tokenizer.bos_token
            if hasattr(self.tokenizer, "bos_token")
            else ""
        }
        self.sep_token_map = {
            self.template_sep_token: self.tokenizer.sep_token
            if hasattr(self.tokenizer, "sep_token")
            else ""
        }
        self.cls_token_map = {
            self.template_cls_token: self.tokenizer.cls_token
            if hasattr(self.tokenizer, "cls_token")
            else ""
        }
        self.pad_token_map = {
            self.template_pad_token: self.tokenizer.pad_token
    ...
    @staticmethod
    def balanced_truncate(input_dict: Dict, num_tokens_to_truncate: int = 0) -> Dict:
        """truncate the inputs with balance, number of cut tokens is proportional to the part's length."""
        shortenable_lens = [
            len(parts) if parts[0] == 1 else 0
        ...
        for key in input_dict:
            ...
            for num_tokens_to_truncate_part, part in zip(
                num_tokens_to_truncate_each_part, parts
            ):
                truncated_example[key].append(
                    part[: len(part) - num_tokens_to_truncate_part]
                )
        return truncated_example

    @staticmethod
    def truncate_from_tail(input_dict: Dict, num_tokens_to_truncate: int = 0) -> Dict:
        ...
        for key in input_dict:
            ...
            for i, part in enumerate(parts[::-1]):
                if len(part) == 0:  # to prevent some part are empty after tokenization
                    continue
                if (
                    shortenable_ids[-1 - i][0] == 0
                ):  # ==0 means the part is not shortenable
                    continue
    ...
    @staticmethod
    def truncate_from_head(input_dict: Dict, num_tokens_to_truncate: int = 0) -> Dict:
    ...
    def padding(
        input_dict: Dict,
        max_len: int,
        pad_id_for_inputs: int = 0,
        pad_id_for_others: int = 0,
    ) -> None:
        for key, value in input_dict.items():
            if len(input_dict[key]) > max_len:
                print(len(input_dict[key]), max_len)
                raise ValueError(
        ...
        for key in encoder_inputs:
            if key == "input_ids":
                ...
                special_tokens_mask = np.array(
                    self.tokenizer.get_special_tokens_mask(encoder_inputs[key])
                )
    ...
    def truncate(self, encoder_inputs, num_special_times):
        # print('encoder_inputs',encoder_inputs['input_ids'])
        total_tokens = sum([len(part) for part in encoder_inputs["input_ids"]])
        num_specials = self.num_special_tokens_to_add
        num_tokens_to_truncate = total_tokens - self.max_seq_length + num_specials * num_special_times
        self.total_passed_sentences += 1
        if num_tokens_to_truncate > 0:
            self.num_truncated_sentences += 1
            encoder_inputs = self.truncate_fct(
                input_dict=encoder_inputs, num_tokens_to_truncate=num_tokens_to_truncate
            )
        return encoder_inputs
