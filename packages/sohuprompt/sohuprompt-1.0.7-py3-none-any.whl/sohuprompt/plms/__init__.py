...
def load_plm_from_config(config: CfgNode):
    ...
    if "glm" in plm_config.model_name:
        model = model_class.model.from_pretrained(
            plm_config.model_path, config=model_config
        ).half()
    else:
        model = model_class.model.from_pretrained(plm_config.model_path)
    tokenizer = model_class.tokenizer.from_pretrained(plm_config.model_path)
    wrapper = model_class.wrapper
    model, tokenizer = add_special_tokens(
        model, tokenizer, specials_to_add=config.plm.specials_to_add
    )
    return model, tokenizer, model_config, wrapper
...
def add_special_tokens(
    ...
    additional_special_token = {"additional_special_tokens": []}
    for token in specials_to_add:
        if "pad" in token.lower():
            if tokenizer.pad_token is None:
                ...
                logger.info(
                    "pad token is None, set to id {}".format(tokenizer.pad_token_id)
                )
        additional_special_token["additional_special_tokens"].append(token)
    tokenizer.add_special_tokens(additional_special_token)
    return model, tokenizer
