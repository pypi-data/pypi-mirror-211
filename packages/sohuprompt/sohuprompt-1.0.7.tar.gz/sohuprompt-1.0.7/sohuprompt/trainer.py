...
class BaseRunner(object):
    ...
    def __init__(
        ...
        infer_dataloader: Optional[PromptDataLoader] = None,
    ):
        self.model = model
        self.config = config
        ...
        self.writer = SummaryWriter(
            os.path.join(self.config.logging.path, "tensorboard")
        )
        ...
        self.use_chat_glm = False
        if isinstance(self.model.plm, ChatGLMForConditionalGeneration):
            self.use_chat_glm = True
        if (self.config.train.fp16 or self.config.train.bf16) and not self.config.train.deepspeed:
            # deepspeed  manage own half precision
            if self.config.environment.num_gpus != 0:
                self.use_cuda_amp = True
                self.amp_dtype = (torch.float16 if self.config.train.fp16 else torch.bfloat16)
        ...
        if self.config.train.label_smoothing_factor != 0:
            self.label_smoother = LabelSmoother(
                epsilon=self.config.train.label_smoothing_factor
            )
        else:
    ...
    def set_stop_criterion(self):
        ...
        if (
            hasattr(self.config.train, "num_training_steps")
            and self.config.train.num_training_steps is not None
        ):
            if self.config.train.num_epochs is not None:
                logger.warning(
                    "num_training_steps set explicitly, num_epochs is not in use."
                )
            ...
            if self.config.train.num_epochs is None:
                raise RuntimeError(
                    "At least num_training_steps & num_epochs should be specified."
                )
            self.num_training_steps = (
                self.steps_per_epoch * self.config.train.num_epochs
            )
    ...
    def wrap_model(self):
        self.model = model_to_device(self.model, self.config)
    ...
    def configure_optimizers(self):
        ...
        if (
            hasattr(template_config, "optimize")
            and template_config.optimize is not None
        ):  # TODO should add optimize config in each yaml
                ...
                optimizer_grouped_parameters = [
                    {
                        "params": [
                            p
                            for name, p in self.inner_model.template.named_parameters()
                            if "raw_embedding" not in name
                        ]
                    }
                ]
                if template_config.optimize.name.lower() == "adamw":
                    template_optimizer = AdamW(
                        optimizer_grouped_parameters,
                    ...
                    if (
                        hasattr(template_config.optimize, "scheduler")
                        and template_config.optimize.scheduler is not None
                    ):
        ...
        if hasattr(self.inner_model, "verbalizer") and self.inner_model.verbalizer:
            ...
            if (
                hasattr(verbalizer_config, "optimize")
                and verbalizer_config.optimize is not None
            ):
                if not hasattr(self.inner_model.verbalizer, "optimize"):
                    # using default gradient descent optimizer.
                    verbalizer_optimizer = AdamW(
                        self.inner_model.verbalizer.parameters(),
                        lr=verbalizer_config.optimize.lr,
                    )
                    optimizers.append(verbalizer_optimizer)
                    if (
                        hasattr(verbalizer_config.optimize, "scheduler")
                        and verbalizer_config.optimize.scheduler is not None
                    ):
    ...
    def checkpoint_path(self, ckpt: str) -> str:
        return os.path.join(
            os.path.join(self.config.logging.path, "checkpoints"), f"{ckpt}.ckpt"
        )
    ...
    def load_checkpoint(self, ckpt: str, load_state=True) -> bool:
        ...
        try:
            state_dict = torch.load(
                self.checkpoint_path(ckpt), pickle_module=dill, map_location="cpu"
            )
        ...
        logger.info(
            f"Load Checkpoint finished, the current validation metric: {state_dict['validation_metric']}"
        )
        return True

    def save_checkpoint(
        self, ckpt: str, save_state=True, extra: dict = {}, copy: str = None
    ):
        if self.clean:
            return
        ...
        if save_state:
            state_dict["optimizer"] = [
                opt.state_dict() if isinstance(opt, torch.optim.Optimizer) else None
        ...
        if copy:
            logger.info(
                f"Copying checkpoint {self.checkpoint_path(ckpt)} to {self.checkpoint_path(copy)}..."
            )
    ...
    def inference_epoch(self, split: str, infer_dataloader=None):
        if split == "infer" and not infer_dataloader:
            raise Exception("inference must have own infer_dataloader")
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(tqdm(data_loader, desc=split)):
                if self.config.environment.num_gpus != 0 and not self.config.onnx.infer:
                    batch = batch.to(
                        "cuda:{}".format(self.config.environment.local_rank)
                    ).to_dict()
                else:
                    batch = batch.to_dict()
                output = self.inference_step(batch, batch_idx, split)
        ...
        if split != "infer":
            ...
            return metrics.popitem(last=False)[
                1
            ]  # TODO the first metric is the most important one
        else:
            return outputs
            ...
            for batch_idx, batch in enumerate(self.train_dataloader):
                ...
                with self.compute_loss_context_manager():
                    loss = self.training_step(
                        batch, batch_idx, self.label_smoother, self.use_chat_glm
                    )
                ...
                if (batch_idx + 1) % self.config.train.gradient_accumulation_steps == 0:
                    ...
                    if self.config.train.max_grad_norm > 0 and not self.deepspeed:
                        if self.do_grad_scaling:
    ...
    def fit(self, ckpt: Optional[str] = None):
        ...
        for self.cur_epoch in range(self.cur_epoch, self.num_epochs):
            ...
            if (
                ...
                or ((score - self.best_score) >= 0)
                == self.config.checkpoint.higher_better
            ):
                copy = "best"
    ...
    def infer(self, infer_dataloader, ckpt: Optional[str] = None) -> dict:
        return self.inference_epoch("infer", infer_dataloader)
...
class ClassificationRunner(BaseRunner):
    ...
    Args:
        ...
        valid_dataloader (:obj:`PromptDataloader`, optionla): The dataloader to bachify and process the val data.
        test_dataloader (:obj:`PromptDataloader`, optional): The dataloader to bachify and process the test data.
        infer_dataloader (:obj:`PromptDataloader`, optional): The dataloader to bachify for infer process.
        config (:obj:`CfgNode`): A configuration object.
        loss_function (:obj:`Callable`, optional): The loss function in the training process.
    """
    ...
    def __init__(
        ...
        self.loss_function = (
            loss_function if loss_function else self.configure_loss_function()
        )
    ...
    def inference_step(self, batch, batch_idx, split):
        if split != "infer":
        ...
        if split != "infer":
            ...
            return pred.cpu().tolist(), prob.cpu().tolist()
    ...
    def training_step(self, batch, batch_idx, label_smoother, is_chat_glm):
        logits = self.model(batch)
        # print('batch',batch,'logits',logits,'label',batch['label'])
    ...
    def prompt_initialize(self):
        ...
        ) and not hasattr(self.inner_model.template, "optimize_to_initialize"):
            return None
...
class GenerationRunner(BaseRunner):
    ...
    def training_step(self, batch, batch_idx, label_smoother, is_chat_glm):
        loss = self.model(batch, label_smoother, is_chat_glm)
        return loss
