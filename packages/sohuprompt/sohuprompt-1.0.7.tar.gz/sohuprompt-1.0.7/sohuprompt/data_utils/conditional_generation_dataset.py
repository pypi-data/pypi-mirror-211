    ...
    def get_examples(self, data_dir: str, split: str) -> List[InputExample]:
        ...
        with open(path) as f:
            ...
            for line in f.readlines()[1:]:
                items = line.strip('\n').split('\t')
                _, title, content, comment, _, like_count = items
                if len(comment)>100 or title == "":
                    continue
                examples.append(InputExample(guid=str(count), text_a=title, text_b=content, tgt_text=comment))
                count += 1
        return examples
