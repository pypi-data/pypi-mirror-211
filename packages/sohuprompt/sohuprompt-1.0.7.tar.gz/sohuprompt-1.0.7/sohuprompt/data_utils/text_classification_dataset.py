    ...
    def get_examples(self, data_dir, split):
        ...
        if split != "test":
            ...
            for row in values:
                ...
                if label == 2:
                    label = 0 
                example = InputExample(text_a=row[3], text_b=row[4], label=int(label))
                examples.append(example)
        return examples
                         
class RWAProcessor(DataProcessor):
    def __init__(self):
        super().__init__()
        self.labels = [0,1]
    ...
    def get_examples(self, data_dir, split):
        ...
        if split != "test":
            ...
            with open(path, encoding='utf8') as f:
                reader = csv.reader(f, delimiter=',', quoting=csv.QUOTE_NONE)
                for row in reader:
                    text_a, text_b, text_c, label_id = row[0]+'。'+row[1], row[2] ,row[3], int(row[4])
                    example = InputExample(text_a=text_a, text_b=text_b, text_c=text_c, label=label_id)
                    examples.append(example)
        else:
            path = os.path.join(data_dir, "{}.xlsx".format(split))
            df = pd.read_excel(path,engine='openpyxl')
...
PROCESSORS = {
    ...
    "reply": ReProcessor,
    "reply_with_article": RWAProcessor,
}
