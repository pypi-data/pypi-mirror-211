...
class PromptDataLoader(object):
    ...
    def __init__(self,
        ...
        assert hasattr(self.template, 'wrap_one_example'), "Your prompt has no function variable named wrap_one_example"
        ...
        self.dataloader = DataLoader(self.tensor_dataset, batch_size=self.batch_size, sampler=sampler, collate_fn=InputFeatures.collate_fct, drop_last=drop_last, )
...
class PromptForGeneration(nn.Module, GenerationMixin):
    ...
    def _forward(self, batch: Union[Dict, InputFeatures], label_smoother, is_chat_glm) -> torch.Tensor:
        r"""
        ...
        if is_chat_glm:
            model_inputs = batch
            model_inputs.update(self.prompt_model.plm.prepare_inputs_for_generation(input_ids=batch['input_ids']))
        else:
            model_inputs = batch
        outputs = self.prompt_model(model_inputs)
    ...
    def prepare_inputs_for_generation(self, input_ids: Optional[torch.Tensor] = None, **model_kwargs):
        ...
        """
        if self.generate_ith_token == 0 and 'encoder_outputs' not in model_kwargs and not isinstance(self.plm, ChatGLMForConditionalGeneration):  # generating the first token in decoder only setting.
        ...
        elif isinstance(self.plm, ChatGLMForConditionalGeneration):
            model_kwargs['input_ids'] = input_ids
            model_inputs = model_kwargs
            model_inputs.pop('attention_mask')
            # input_ids生成position_ids和attention_mask
            model_inputs.update(self.plm.prepare_inputs_for_generation(**model_inputs))
            if model_inputs['past_key_values'] is None:
                # input_ids生成input_embs
                model_inputs.update(self.prompt_model.prepare_model_inputs(model_inputs))
            model_inputs = {key: model_inputs[key] for key in model_inputs if key in self.prompt_model.forward_keys}
    ...
    def _update_model_kwargs_for_generation(self, outputs, model_kwargs: Dict[str, Any], is_encoder_decoder: bool = False) -> Dict[str, Any]:
        ...
        """
        if isinstance(self.prompt_model.plm, ChatGLMForConditionalGeneration):
            model_kwargs = self.prompt_model.plm._update_model_kwargs_for_generation(outputs=outputs, model_kwargs=model_kwargs, is_encoder_decoder=is_encoder_decoder)
        else:
