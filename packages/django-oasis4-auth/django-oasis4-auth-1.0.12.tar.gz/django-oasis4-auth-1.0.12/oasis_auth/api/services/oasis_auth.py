...
from zibanu.django.auth.lib.signals import change_password
from zibanu.django.auth.lib.signals import request_password
            ...
            if {"email"} <= request.data.keys():
                ...
                if user is not None and user.email == request.data.get("email"):
                    ...
                    if send_code.load_templates():
                        ...
                        user.save()
                        status_return = status.HTTP_200_OK
                        request_password.send(sender=self.__class__, user=user, action="retrieve_password",
                                               request=request)
                    else:
