...
<marlin xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xsi:noNamespaceSchemaLocation="http://ilcsoft.desy.de/marlin/marlin.xsd">
 ...
 <!--Automated analysis-->
  ...
  def test_getnewldlibs_cornercase(self):
    # TODO: Understand method: Currently this method ignores every library
    # path except the last one in the list and just ignores if
    # getSoftwareFolder fails
    ...
    if 'LD_LIBRARY_PATH' in os.environ:
      ...
      with patch.dict('os.environ', {'LD_LIBRARY_PATH': ''}), \
          ...
          patch("%s.removeLibc" % MODULE_NAME, new=Mock(return_value=True)) as mock_removelibc:
        result = PrepareOptionFiles.getNewLDLibs(None, None, None)
    self.assertEqual("%s:%s" % ('bFolder/LDLibs', reference), result)
  ...
  def test_getnewldlibs_nochange(self):
    ...
    with patch("%s.getSoftwareFolder" % MODULE_NAME, new=Mock(side_effect=[S_ERROR(), S_OK(''), S_OK('')])), \
      ...
      self.assertEqual(reference, PrepareOptionFiles.getNewLDLibs(None, None,
                                                                   None))
  ...
  def test_getnewldlibs_noldlibpath(self):
    with patch("%s.getSoftwareFolder" % MODULE_NAME, new=Mock(side_effect=[S_ERROR(), S_OK('basetest'), S_ERROR()])), \
      ...
      self.assertEqual('basetest/lib', PrepareOptionFiles.getNewLDLibs(None,
                                                                        None,
                                                                        None))

  def test_getnewpath_cornercase(self):
    # TODO: Understand method: Currently this method ignores every path except
    # the last one in the list and just ignores if getSoftwareFolder fails
    ...
    with patch("%s.getSoftwareFolder" % MODULE_NAME, new=Mock(side_effect=[S_ERROR(), S_OK('bFolder')])), \
         ...
         patch("%s.os.path.exists" % MODULE_NAME, new=Mock(side_effect=[True, False, False, True])), \
         patch("%s.removeLibc" % MODULE_NAME, new=Mock(return_value=True)):
      self.assertEqual("%s:%s" % ('bFolder/bin', reference),
  ...
  def test_getnewpath_nochange(self):
    ...
    with patch("%s.getSoftwareFolder" % MODULE_NAME, new=Mock(side_effect=[S_ERROR(), S_OK(''), S_OK('')])), \
         ...
         patch("%s.removeLibc" % MODULE_NAME, new=Mock(return_value=True)):
      self.assertEqual(reference, PrepareOptionFiles.getNewPATH(None, None, None))
  ...
  def test_getnewpath_nokey(self):
    with patch("%s.getSoftwareFolder" % MODULE_NAME, new=Mock(side_effect=[S_ERROR(), S_OK("testfolder"), S_ERROR()])), \
        ...
        patch("%s.removeLibc" % MODULE_NAME, new=Mock(return_value=True)), \
        patch("%s.os.environ" % MODULE_NAME, {}):
      self.assertEqual("testfolder/bin",
                         PrepareOptionFiles.getNewPATH(None, None, None))
  ...
  def test_prepareWhizFileTemplate(self):
    ...
    # Template strings are the keys of the parameter dictionary concatenated
    # with themselves, e.g. SEEDSEED for the entry 'SEED' : 135431
    ...
    with patch("%s.ElementTree.parse" % MODULE_NAME, new=parseModified):
      ...
      for child in params:
        if child.attrib['name'] == 'GearXMLFile' and 'value' in child.attrib:
          self.assertEqual(child.attrib['value'], 'inputGEAR')
  ...
  @patch("%s.open" % MODULE_NAME, mock_open(), create=True)
  @patch("ILCDIRAC.Core.Utilities.MarlinXML.getOverlayFiles",
         new=Mock(return_value=['overlayfile1', 'verycomplicated_other_file.txt']))
  def test_prepareXMLFile_getoverlayUsed(self):
    ...
    with patch("%s.ElementTree.parse" % MODULE_NAME, new=parseModified):
      ...
      mytree = TestPrepareOptionsFile.currenttree
      procs = mytree.findall('processor')
      procs = [x for x in procs if x.attrib['name'].lower().count(
          'overlaytiming')]  # Only get the one with overlaytiming
      for proc in procs:
  ...
  def test_prepareXMLFile_slciotypeerror(self):
    ...
    with patch("%s.ElementTree.parse" % MODULE_NAME, new=parseModified):
      ...
      assertDiracFailsWith(result, 'inputslcio is neither string nor list!',
                            self)
      assertDiracFailsWith(result, ["actual type is <type 'int'>",  # python2
                                    "actual type is <class 'int'>",  # python3
                                    ],
                           self)
  ...
  def test_prepareLCSIM(self):
    ...
    with patch("%s.ElementTree.parse" % MODULE_NAME, new=parseModified):
      ...
      for d in drivers:
        if 'type' in d.attrib:
          self.assertTrue(
              not d.attrib['type'] == 'org.lcsim.recon.tracking.seedtracker.steeringwrappers.SeedTrackerWrapper' or checkstrat(
                  self, d))  # note: short circuit operator
  ...
  def test_prepareTomato_stdcase(self):
    ...
    with patch("%s.ElementTree.parse" % MODULE_NAME, new=parseModified):
      ...
      next(it)
      assertEqualsXml(next(it), ET.Comment("input file list changed"), self)
      assertEqualsXml(next(it), ET.Comment("input file list changed"), self)
  ...
  def test_prepareTomato_noinputfile(self):
    ...
    file_contents = [[]]
    # Means 2 files will be opened, nothing is written to first file, and
    # 'firstlineentry' and 'line100' are written (in different calls and
    # exactly these strings) to the second file. If more/less is written this
    # fails!
    expected = [[TestPrepareOptionsFilePatch.teststr]]
    handles = FileUtil.getMultipleReadHandles(file_contents)
    with patch("%s.ElementTree.parse" % MODULE_NAME, new=parseModified), \
