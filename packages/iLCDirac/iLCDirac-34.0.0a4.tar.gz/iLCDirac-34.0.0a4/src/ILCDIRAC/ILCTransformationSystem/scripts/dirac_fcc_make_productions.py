    ...
    if parameter.prodConfigFilename is not None:
      ...
      if self._flags.gen:
        if len(self.numberOfTasks) != len(self.energies) or \
           (self.generatorSteeringFile and (len(self.generatorSteeringFile) != len(self.energies))):
          raise AttributeError("Lengths of numberOfTasks, generatorSteeringFile, and Energies do not match")
...
""" % (pDict)
  ...
  def createAllTransformations(self):
    ...
    for energy, process, prodID, eventsPerJob, eventsPerBaseFile, genFile, nbTasks, taskName in \
        zip_longest(self.energies, self.processes, self.prodIDs, self.eventsPerJobs, self.eventsInSplitFiles,
                     self.generatorSteeringFile, self.numberOfTasks, self.taskNames, fillvalue=None):
      taskDict = self.createTaskDict(prodID, process, energy, eventsPerJob, genFile,
