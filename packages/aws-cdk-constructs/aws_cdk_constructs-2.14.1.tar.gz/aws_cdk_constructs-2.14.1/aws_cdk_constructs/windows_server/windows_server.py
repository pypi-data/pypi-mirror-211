...
class WindowsServer(Construct):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.aws_account = environments_parameters["accounts"][environment.lower()]
        self.hostname = hostname
        self.id = id

        is_production = self.environment == "production" or self.environment == "sharedservices"
        ...
        self.stickiness_cookie_duration_in_hours = stickiness_cookie_duration_in_hours
        self.load_balancer_idle_timeout_in_seconds =load_balancer_idle_timeout_in_seconds
        ...
        self.vpc = _ec2.Vpc.from_vpc_attributes(self,
                                        "VPC",
        ...
        self.machine_image = _ec2.GenericWindowsImage({"eu-west-1": self.ec2_ami_id})
            ...
            _iam.Policy(
                ...
                statements=[
                    ...
                    _iam.PolicyStatement(
                        actions=["s3:List*"],
                        resources=[
                            "arn:aws:s3:::" + self.aws_account["s3_config_bucket"],
                            "arn:aws:s3:::"
        ...
        self.ec2_security_group.add_ingress_rule(
            ...
            connection=_ec2.Port.tcp(3389), # RDP
        ...
        # EC2 Instance
        # ==============================================================================
        self.instance = _ec2.Instance(self, f"ec2_instance_{app_name}_{main_component_name}",
                                vpc=self.vpc,
        ...
        fao_mandatory_security_groups = [
            {"id":self.aws_account["bastion_host_windows_admin_security_group"], "name": "bastion_windows_team"},
            {"id":self.aws_account["scan_target_security_group"], "name": "it_sec_scan"},
            {"id":self.aws_account["domain_control_access_security_group"], "name": "domain_control_access"},
        ]

        for sec_group in fao_mandatory_security_groups:
            self.instance.add_security_group(
                _ec2.SecurityGroup.from_security_group_id(self, sec_group["name"]+"sg", sec_group["id"],
                mutable=False
            ))
        ...
        if is_not_production:
            if tag_scheduler_uptime_skip:
                cdk.Tags.of(self.instance).add("SchedulerSkip", tag_scheduler_uptime_skip)

            if tag_scheduler_uptime:
                cdk.Tags.of(self.instance).add("SchedulerUptime", tag_scheduler_uptime)

            if tag_scheduler_uptime_days:
                cdk.Tags.of(self.instance).add("SchedulerUptimeDays", tag_scheduler_uptime_days)
        ...
        if create_load_balancer:
            ...
            self.listener.add_targets(f"target{app_name}_{main_component_name}",
                                port=int(ec2_traffic_port),
                                protocol=_alb.ApplicationProtocol.HTTP,
                                targets=[_alb_targets.InstanceIdTarget(
