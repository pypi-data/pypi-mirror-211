...
def gen_sp3_content(sp3_df):
    ...
    sp3_content = (
        "P"
        + sp3_df.index.get_level_values(1).values.astype(object)
        + " "
...
def diff_sp3_rac(sp3_a, sp3_b, hlm_mode=None, use_cubic_spline=True):
    ...
    xyz_cols_a = _np.argwhere(
        sp3_a_eci.columns.isin([("EST", "X"), ("EST", "Y"), ("EST", "Z")])
    ).ravel()  # get indices of EST X Y Z cols
    ...
    df_rac = _pd.DataFrame(
        nd_rac.reshape(-1, 3), index=sp3_a.index, columns=[["EST_RAC"] * 3, ["Radial", "Along-track", "Cross-track"]]
    )
