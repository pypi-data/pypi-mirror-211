...
def xyz2llh_larson(xyz_array, ellipsoid=_gn_const.WGS84, tolerance=1e-10, deg=False):
    ...
    for __ in range(10):  # 10 iterations cap as per Larson
        ...
        phi[error_mask] = _np.arctan((z_arr[error_mask] / _r[error_mask]) / (1 - ellipsoid.ecc1sq * _n / (_n + hei)))
...
def xyz2llh_heik(xyz_array: _np.ndarray, ellipsoid=_gn_const.WGS84, deg=False, make_lon_positive=True):
    """
    This is an exact transformation from ECEF to LLH and is pretty fast
    Output
      phi: latitude rad (geodetic)
      lam: longitude rad
      hei: height meters
    References:
    1. Heikkinen, M. (1982)
    ...
    if make_lon_positive:
        llh_array[llh_array[:, 1] < 0, 1] += 2 * _np.pi
...
def xyz2llh_zhu(xyz_array, ellipsoid=_gn_const.WGS84, deg=False, make_lon_positive=True):
    """
    Another exact transformation from ECEF to LLH
    Output
      phi: latitude rad (geodetic)
      lam: longitude rad
      hei: height meters
...
def xyzdiff_origin2enu(xyzdiff, x0y0z0):
    ...
    :returns: Nx3 numpy array containing east, north and up components of the displacement.
    """
    lat_lon_height = xyz2llh_heik(x0y0z0)
    return xyzdiff2enu(xyzdiff, lat_lon_height[:, 0], lat_lon_height[:, 1])
