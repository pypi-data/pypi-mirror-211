...
def _compare_clk(
    ...
    _logging.info(f"using {norm_type} clk normalization")
    if norm_type == "sv":
        norm_type = _gn_io.clk.select_norm_svs_per_gnss(clk_a_unst=clk_a_unst, clk_b_unst=clk_b_unst)
        _logging.info(f"selecting {norm_type} clk normalization as 'sv' norm_type provided")
...
def sisre(
    ...
    *BeiDou different coeffs for MEO/GEO not implemented yet
    ...
    if (clk_a is not None) & (clk_b is not None):  # check if clk data is present
        clk_diff = _compare_clk(
            clk_a, clk_b, norm_type=norm_type, ext_dt=rac_unstack.index, ext_svs=rac_unstack.columns.levels[1]
        )  # units are meters
        if clean:
            if cutoff is not None:
...
def diffclk(clk_a_path, clk_b_path, tol, log_lvl, norm_type="both"):
    ...
    status = 0
    diff_clk = _compare_clk(clk_a=clk_a, clk_b=clk_b, norm_type=norm_type)
...
def rac_df_to_rms_df(rac_df):
    ...
    rms_df = rac_df.join(rac_df.attrs["diff_eci"]).pow(2).groupby("PRN").mean().pow(0.5)
    rms_df["EST", "1D_MEAN"] = rms_df.EST.mean(axis=1)
    rms_df["EST_RAC", "3D_RMS"] = rms_df.EST_RAC.pow(2).sum(axis=1).pow(0.5)
