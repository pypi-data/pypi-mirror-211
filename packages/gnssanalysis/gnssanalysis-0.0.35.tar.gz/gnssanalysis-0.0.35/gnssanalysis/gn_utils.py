...
@_click.option(
    "-n",
    "--norm-type",
    type=_click.Choice(["daily", "epoch", "both", "sv"], case_sensitive=False),
    default=None,
    help="norm-type can be 'daily', 'epoch', 'both' (epoch + daily) or 'sv' (selects an PRN for each constellation to remove the bias). If None - the step is skipped",
)
@_click.option(
    "-s",
    "--norm-sv",
    ...
    multiple=True,
    default=(),
    help="norm-sv overrides norm_type with a PRN to use (multiple inputs are allowed through '-s G01 -s E01'. Skipped by default",
)
def clk(ctx, norm_type, norm_sv):
    ...
    status = diffclk(
        ...
        tol=ctx.parent.params["atol"],
        log_lvl=ctx.parent.params["log_lvl"],
        norm_type=norm_type if norm_sv is () else norm_sv,
    )
    diffutil_verify_status(status=status, passthrough=ctx.parent.params["passthrough"])
...
@_click.option(
    ...
    nargs=2,
    type=str,
    help="paths to the compared sp3 files, can be compressed with LZW (.Z) or gzip (.gz). Takes exactly two arguments",
)
@_click.option(
    "-h",
    "--hlm_mode",
...
def orbq(input, hlm_mode):
    ...
    if hlm_mode is not None:
        logger.log(msg=f"Helmert coeffs computed in {hlm_mode}: {rac.attrs['hlm'][0][0].reshape(-1)}", level=60)
    # log dataframes but first convert km to m and round
    logger.log(msg=rms_df.mul(1000).round(5).to_csv(sep="\t", index=True, header=True), level=60)
    logger.log(msg="=======================================================================", level=60)
    logger.log(msg=rms_df.attrs["summary"].mul(1000).round(5).to_csv(sep="\t", index=True, header=True), level=60)
