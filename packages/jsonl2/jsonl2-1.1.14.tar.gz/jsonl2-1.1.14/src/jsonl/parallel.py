"""Parallel versions of jsonl read and write"""


def split(n: int, parts: int) -> list[tuple[int, int]]:
    """split the interval from [0, 1, 2, ..., n-2, n-1]
     of size `n` in `parts` parts of almost equal size.
    ...
    if parts < 1 or n < 1:
        raise ValueError(
            f"split(n,parts) expects n>=1 and parts>=1. Values provided n={n}, parts={parts}"
        )
