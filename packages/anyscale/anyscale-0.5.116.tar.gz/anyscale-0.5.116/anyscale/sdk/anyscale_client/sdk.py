...
from anyscale.api_utils.job_util import (
    _is_job_on_v2_cloud,
    _is_production_job_on_v2_cloud,
)
from anyscale.api_utils.exceptions.log_retrieval_errors import (
    ExpectedLogRetrievalError,
    LogRetrievalTimeoutError,
    UnsupportedLogRetrievalMethodError,
)
from anyscale.api_utils.logs_util import _remove_ansi_escape_sequences
...
from anyscale.sdk.anyscale_client.models.apply_production_service_v2_model import (
...
from anyscale.sdk.anyscale_client.models.jobs_logs import JobsLogs
from anyscale.sdk.anyscale_client.models.jobslogs_response import JobslogsResponse
...
from anyscale.utils.deprecation_util import deprecated
...
def _upload_and_rewrite_working_dir_ray_serve_config(
    ...
    if isinstance(ray_serve_config, dict):
...
class AnyscaleSDK(DefaultApi):  # type: ignore
    ...
    def fetch_job_logs(self, job_id: str) -> str:
        """
        Retrieves logs for a job run (eg. 'job_123').

        This will try in following order until it finds a retrieval method that returns the logs:
        1. Ray Dashboard API on the cluster that the job run is on
           NOTE: The caller of this function must be able to reach the Ray dashboard host.
        2. (only for anyscale v1 clouds) Anyscale backend / cache layer which stores logs for recently finished job runs
        3. The storage bucket for the Anyscale cloud the job run (and its cluster) is on

        Args:
            job_id (str): ID of a job run

        Returns:
            str: All logs for the job run
                (so far, if the job run is still running)
        """
        get_job_logs_fns: List[Callable[..., str]] = [
            lambda: asyncio.run(_get_logs_from_active_job_run(self, job_id)),
            lambda: self._fetch_job_logs_legacy(job_id),
            lambda: asyncio.run(
                _get_job_logs_from_storage_bucket(self, job_run_id=job_id)
            ),
        ]
        logs = ""
        # Return logs from the first retrieval method that has non-empty logs
        for fn in get_job_logs_fns:
    ...
    def _fetch_job_logs_legacy(self, job_id: str) -> str:
        """
        ...
        Raises
            UnsupportedLogRetrievalMethodError if on an Anyscale v2 cloud
            Exception if we fail to fetch logs
        """
        if _is_job_on_v2_cloud(self, job_id):
            raise UnsupportedLogRetrievalMethodError(
                "Logs are not stored Anyscale-side on clouds set up with Anyscale v2."
            )
        else:
            warnings.warn(
                "Fetching logs stored in Anyscale backend is deprecated. "
                "Please consider moving to a cloud set up with Anyscale v2.",
                DeprecationWarning,
            )
        ...
        while not logs.ready:
            if time() > timeout:
                raise LogRetrievalTimeoutError(f"Failed to get logs from Job {job_id}")

            sleep(15)
            logs = self.get_job_logs(job_id).result

        # Logs will be empty if on anyscale v1 and job logs are no longer in cache
        return _remove_ansi_escape_sequences(logs.logs)

    @deprecated(new_function_name="fetch_production_job_logs")
    def get_production_job_logs(self, job_id: str) -> JobslogsResponse:
        """
        DEPRECATED: Please use `fetch_production_job_logs` to retrieve
        the logs string instead.
        ...
        Returns
            Response containing the log output for the job
        ...
        return JobslogsResponse(
            result=JobsLogs(
                logs=self.fetch_production_job_logs(job_id),
                ready=True,
            )
        )

    def _get_production_job_logs_legacy(self, job_id: str) -> JobslogsResponse:
    ...
    def fetch_production_job_logs(self, job_id: str) -> str:
        ...
        This will try in following order until it finds a retrieval method that returns the logs:
        1. Ray Dashboard API on the cluster that the job is running on
           NOTE: The caller of this function must be able to reach the Ray dashboard host.
        2. (only for anyscale v1 clouds) Anyscale backend / cache layer which stores logs for recently finished jobs
        3. The storage bucket for the Anyscale cloud the job (and its cluster) is running on
        ...
        """
        get_job_logs_fns: List[Callable[..., str]] = [
            lambda: asyncio.run(_get_logs_from_running_production_job(self, job_id)),
            lambda: self._fetch_production_job_logs_legacy(job_id),
            lambda: asyncio.run(_get_job_logs_from_storage_bucket(self, job_id=job_id)),
        ]
        logs = ""
        # Return logs from the first retrieval method that has non-empty logs
        for fn in get_job_logs_fns:
    ...
    def _fetch_production_job_logs_legacy(self, job_id: str) -> str:
        """
        Retrieves logs for latest run of a Production Job.
        ...
        Raises
            ...
            Exception if we fail to fetch logs
        """
        if _is_production_job_on_v2_cloud(self, job_id):
            raise UnsupportedLogRetrievalMethodError(
                "Logs are not stored Anyscale-side on clouds set up with Anyscale v2."
            )
        else:
        ...
        logs = self._get_production_job_logs_legacy(job_id).result
