...
from anyscale.util import get_endpoint, is_anyscale_workspace, poll
...
class ServiceController(BaseController):
    ...
    def deploy(
        ...
        if is_anyscale_workspace() and is_entrypoint_cmd:
            # If is_entrypoint_cmd is true, service_config_file is expected to be "python"
            # Therefore, we need to combine args to create the final entrypoint
    ...
    def deploy_from_config(self, service_config: ServiceConfig):
        ...
        service_config.project_id = infer_project_id(
            self.anyscale_api_client,
            self.api_client,
            self.log,
            project_id=service_config.project_id,
            cluster_compute_id=service_config.compute_config_id,
            cluster_compute=service_config.compute_config,
            cloud=service_config.cloud,
        )

        if service_config.healthcheck_url:
            # Override for services v1 on v1 stack
            # Will be deprecated once all customers migrate to v2 stack
            new_runtime_env = override_runtime_env_config(
            ...
            service_config.runtime_env = new_runtime_env
        else:
            if not service_config.ray_serve_config:
                service_config.ray_serve_config = {}

            new_runtime_env = override_runtime_env_config(
                runtime_env=service_config.ray_serve_config.get("runtime_env"),
                anyscale_api_client=self.anyscale_api_client,
                api_client=self.api_client,
                workload_type=Workload.SERVICES,
                compute_config_id=service_config.compute_config_id,
            ...
            service_config.ray_serve_config["runtime_env"] = new_runtime_env
        ...
        self.log.info(
            f"Service {service_id} has been deployed. Current state of service: {service.state.current_state}."
        )
        self.log.info(
    ...
    def _populate_service_config(self, config_dict: Dict[str, Any]) -> ServiceConfig:
        """
        If the service is deployed from the workspace, we populate values from the workspace
    ...
    def rollout(
        ...
        if canary_percent is not None and len(service.versions) > 1:
    ...
    def list(
        ...
        *,
        ...
        try:
            ...
            if "Services v2 is not enabled for your organization." in str(e):
                # An exception will be thrown if the "services_v2_enable_all_features" flag is not enabled
                # Therefore, we default to showing only Service v1.
                # TODO: shawnp. Remove the exception logic once we enable service v2 flag for all customers
                self.job_controller.list(
                    name=name,
                    job_id=service_id,
                    project_id=project_id,
                    include_all_users=not created_by_me,
                ...
                self.log.info(
                    "We are unable to get a list of Services."
                    "This is unexpected and please contact Anyscale support for more info."
                )
        ...
        *,
        ...
        if service_id:
            if _is_v2_service_id(service_id):
                ...
                service_v1_result: DecoratedProductionJob = self.api_client.get_job_api_v2_decorated_ha_jobs_production_job_id_get(
                    service_id
                ).result
                services.append(
                    [
                        service_v1_result.name,
                        service_v1_result.id,
                        service_v1_result.state.current_state,
                        service_v1_result.creator.email,
                    ]
                )
