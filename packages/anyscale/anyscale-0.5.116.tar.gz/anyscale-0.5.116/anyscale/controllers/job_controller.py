...
class JobController(BaseController):
    ...
    def _resolve_job_object(
        ...
        if job_id is None:
            ...
            jobs_resp: DecoratedProductionJob = self.api_client.get_job_api_v2_decorated_ha_jobs_production_job_id_get(
    ...
    def _wait_for_a_job_run(self, job: DecoratedProductionJob) -> str:
        """Given a job, wait until there is a job run generated for it."""
        last_job_run_id: str = job.last_job_run_id
        with self.log.spinner("Waiting for job run...") as spinner:
            while last_job_run_id is None:
                job_object = self._get_job(job.id)
        ...
        with handle_and_log_lines_in_click_exception(log_callback=self.log.error):
            if should_follow:
                ...
                with wrap_job_run_log_not_retrievable_on_active_cluster_error(
                    ...
                    # run_until_complete() is used here b/c the KeyboardInterrupt exception was
                    # not propogated all the way up (ie. it was being handled by internals of asyncio.run())
    ...
    def _print_logs_for_inactive_job_run(self, job_run_id: str) -> None:
        # Retrieves logs for a job run in terminal state
        logs = ""
    ...
    def logs(
        ...
        * If the job is not running, it will retrieve logs from Anyscale's backend cache.
        ...
        last_job_run_id = self._wait_for_a_job_run(job)

        self.log.open_block(
            last_job_run_id, f"Job Run Id: {last_job_run_id}", auto_close=True,
        )
        get_logs_fns: List[Callable[[], None]] = []
        if job.state.current_state == HaJobStates.RUNNING:
            get_logs_fns.append(
                lambda: self._print_logs_for_active_job_run(
                    last_job_run_id, should_follow
                )
            )
        get_logs_fns.extend(
            [
                lambda: self._print_logs_legacy(job, should_follow),
                lambda: self._print_logs_for_inactive_job_run(last_job_run_id),
            ]
        )
