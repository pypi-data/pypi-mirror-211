...
def test_print_logs_for_active_job_run(mock_auth_api_client) -> None:
    ...
    mock_job_run_not_found_exception = ClientResponseError(
        request_info=Mock(), history=Mock(), status=404
    )
    mock_websocket.receive = CoroutineMock(side_effect=mock_job_run_not_found_exception)
    with patch.object(
        ClientSession, ClientSession.ws_connect.__name__, return_value=mock_websocket
    ), pytest.raises(
        LogNotFoundOnActiveClusterError,
        match='Job run "job_123" is not a part of the current active session',
    ):
        job_controller._print_logs_for_active_job_run(job_id, should_follow=True)
...
def test_print_logs_legacy(mock_auth_api_client, capsys, mode) -> None:
    job_controller = JobController(log=FakeLogger())
    ...
    def do_loki_logs() -> None:
        job_controller._print_logs_legacy(
...
def test_print_logs_legacy_errors_when_job_on_v2_cloud(mock_auth_api_client) -> None:
    job_controller = JobController()
    mock_job = Mock(id="prodjob_123")
    with patch.object(
        job_controller_module,
        _is_production_job_on_v2_cloud.__name__,
        return_value=True,
    ) as mock_is_production_job_on_v2_cloud, pytest.raises(
        UnsupportedLogRetrievalMethodError, match="Logs are not stored Anyscale-side"
    ):
        job_controller._print_logs_legacy(mock_job)
    mock_is_production_job_on_v2_cloud.assert_called_once_with(
        job_controller.anyscale_api_client, mock_job.id
    )


def test_print_logs_legacy_errors_when_not_logged_at_all(mock_auth_api_client) -> None:
    job_controller = JobController()
    mock_job = Mock(
        state=Mock(current_state=HaJobStates.TERMINATED, error=None),
        last_job_run_id="job_123",
    )
    fake_log_provider_cls = fake_log_provider("empty")
    with patch.object(
        job_controller_module,
        _is_production_job_on_v2_cloud.__name__,
        return_value=False,
    ), pytest.raises(ExpectedLogRetrievalError):
        job_controller._print_logs_legacy(
            mock_job, log_provider_cls=fake_log_provider_cls
        )
...
def test_logs(mock_auth_api_client) -> None:
    ...
    job_run_id = "job_123"
    mock_job = Mock(id=prod_job_id, state=Mock(current_state=HaJobStates.TERMINATED))
    job_controller._resolve_job_object = Mock(return_value=mock_job)  # type: ignore
    job_controller._wait_for_a_job_run = Mock(return_value=job_run_id)  # type: ignore
    job_controller._print_logs_for_active_job_run = mock_stream_logs = Mock()  # type: ignore
    job_controller._print_logs_legacy = mock_loki_logs = Mock()  # type: ignore
    job_controller._print_logs_for_inactive_job_run = mock_download_logs = Mock()  # type: ignore

    def reset_mocks() -> None:
        for mock in [mock_stream_logs, mock_loki_logs, mock_download_logs]:
            mock.reset_mock()
    ...
    # Test Loki logs and does not follow by default if job is not in RUNNING state
    job_controller.logs(job_id=prod_job_id)
    do_assertions(not_called=[mock_stream_logs, mock_download_logs])
    mock_loki_logs.assert_called_once_with(mock_job, False)

    # Test download logs (ie. falls back) if Loki has no logs (raises `ExpectedLogRetrievalError`)
    reset_mocks()
    job_controller._print_logs_legacy.side_effect = ExpectedLogRetrievalError
    job_controller.logs(job_id=prod_job_id)
    do_assertions(
        called=[mock_loki_logs, mock_download_logs], not_called=[mock_stream_logs]
    )
    ...
    job_controller.logs(job_id=prod_job_id)
    do_assertions(
        called=[mock_stream_logs], not_called=[mock_loki_logs, mock_download_logs]
    )
...
def test_wait_for_job_running(mock_auth_api_client) -> None:
    ...
    job_controller._get_job = Mock(side_effect=get_latest_job_run)  # type: ignore
...
def test_override_runtime_env_config_and_infer_project_id(mock_auth_api_client):
    ...
    with patch.multiple(
        "anyscale.controllers.job_controller", validate_successful_build=Mock(),
    ):
        job_config = JobConfig.parse_obj(config_dict)
