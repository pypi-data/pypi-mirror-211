...
async def test_get_logs_from_active_job_run_throws_exception_ray_dashboard_api():
    ...
    # Test job run not found on active Ray cluster
    response_error = ClientResponseError(
        status=HTTPStatus.NOT_FOUND, request_info=Mock(), history=Mock()
    )
    with patch.object(
        job_logs_util,
        _download_log_from_ray_json_response.__name__,
        CoroutineMock(side_effect=response_error),
    ), pytest.raises(
        LogNotFoundOnActiveClusterError,
        match='Job run "job_123" is not a part of the current active session of the cluster.',
    ):
        await _get_logs_from_active_job_run(mock_base_api, job_run_id)

    # Test unexpected 4xx error
    response_error.status = HTTPStatus.BAD_REQUEST
    with patch.object(
        job_logs_util,
        _download_log_from_ray_json_response.__name__,
        CoroutineMock(side_effect=response_error),
