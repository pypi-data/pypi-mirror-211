...
def find_table(tables: List[PageElement], contains: List[str], should_not_contains: List[str]):
    for table in tables:
        if all([c in remove_strs(table.text, [" ", "\xa0"]) for c in contains]) and \
                all([c not in remove_strs(table.text, [" ", "\xa0"]) for c in should_not_contains]):
            return table
    return None
