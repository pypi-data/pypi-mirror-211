...
class Action(ShellSTM):
    ...
    async def wait(self, *actions, timeout=900, sleep=0.15):
    ...
    async def create_file(
        ...
        **ctx,
        ...
        if mode:
            attributes or await load_attr()
            try:
                flag = int(mode) != int(attributes.get('mode'))
            except:
                flag = False
