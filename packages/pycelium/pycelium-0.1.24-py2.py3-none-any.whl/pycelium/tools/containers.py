...
def retrieve(
    ...
    **ctx,
    ...
    if _force_ or not os.path.exists(path):
        assert (
            builder is not None
        ), "you must set a builder when data is missing"
        ...
        if _save_:
            ...
            if _csv_review:
                if isinstance(response, (pd.DataFrame, pd.Series)):
                    ...
                    yaml.dump(
                        response, open(path, "w"), default_flow_style=False
                    )
    else:
...
DF_REGEXP = (
    r"(?P<format>\.(?P<ext>(csv|pickle|xls|xlsx|ods)))(\.(?P<cmp>xz))?$"
)
...
def load_dataframes(kind, folder="", top=".", relative=False):
    ...
    for path, d in fileiter(
        os.path.join(top, folder), regexp=regexp, info="d"
    ):
...
class Item:
    ...
    def __getstate__(self):
        state = dict(
            [(k, getattr(self, k, None)) for k in self.__getkeys__()]
        )
    ...
    def __str__(self):
        ...
        fields = ", ".join(
            [f"{key}: {getattr(self, key, None)}" for key in keys]
        )
    ...
    def asdict(self, string=True):
        ...
        if string:
            return dict(
                [(key, f"{getattr(self, key, None)}") for key in keys]
            )
...
BASIC_TYPES_EXT = tuple(
    list(BASIC_TYPES)
    + list(CONTAINERS_TYPES)
    + list(DATETIME_TYPES)
    + [Item]
)
ITERATOR_TYPES = tuple(
    [type({}.keys()), type({}.values()), type({}.items())]
    + list(CONTAINERS_TYPES)
)
...
def walk(
    ...
    # func = container.items
    elif hasattr(container, "__slots__"):
...
def dive(
    container, key, stop_before=0, default_container=dict, create=False
):
...
def rebuild(
    ...
    for key, item in walk_iterator:
        ...
        assert (
            result is not None
        ), "root element must be found before any other"
...
def diffdict(current, last):
    ...
    new_items = dict(
        [(k, current[k]) for k in current_keys.difference(last_keys)]
    )
    deleted_items = dict(
        [(k, last[k]) for k in last_keys.difference(current_keys)]
    )
...
def divepush(container, key, item, merge=True, default_container=dict):
    ...
    for key in key:
        if container is None:
            return parent
        parent, container = container, container.setdefault(
...
async def amerge(base, new, mode="add", inplace=False, **kw):
    ...
    # )
    result = rebuild(
        walk_iterator=stream, converters=converters, result={}, **kw
    )
    if inplace:
        base.clear()
        base.update(result)
...
def rediff(new, old, mode="both", **kw):
    """Diff between 2 containers using regexp in new container"""
...
def sort_iterable(iterable, key=0):
    ...
    while iterable:
        l = min(
            [
                len(k[key]) if hasattr(k[key], "__len__") else 0
                for k in iterable
            ]
        )
        subset = list()
