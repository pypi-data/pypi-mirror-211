...
class LatchPersistence(DataPersistence):
    ...
    def download_directory(self, remote_path: str, local_path: str) -> bool:
        ...
        if internal_execution_id is not None:
            auth_headers = {"Authorization": f"Latch-Execution-Token {internal_execution_id}"}
            r = _session.post(
            ...
            res_data = r.json()
            if r.status_code != 200:
                raise FlyteUserException(f"failed to get presigned urls for `{remote_path}`")
            ...
            try:
                # Note that above, all children of the remote directory are
                # directly placed in the destination directory, so that a remote
                ...
                #
                # /tmp/flyte-zdwdkh_k/820637e4d06dd8f3abcea74f669ab2c6
                # ├── a.txt
                # ├── b.txt
                # └── c.txt
                #
                ...
                #     └── c.txt
                #
                # We want that same exact behavior here, so we enforce that
                # the source directory has a slash at the end. This leverages
                # the fact that `latch cp` mimics GNU `cp`, which has the
                # desired behavior if the source directory argument ends with a
                # slash. This is shown below:
                #
                # $ cp -r source without_slash
                # $ cp -r source/ with_slash
                # $ tree .
                # .
                # ├── source
                # │   ├── a.txt
                ...
                cp(remote_path, local_path, progress=Progress.none, verbose=False)
    ...
    def download(self, remote_path: str, local_path: str) -> bool:
        ...
        if internal_execution_id is not None:
            ...
            res_data = r.json()
            if r.status_code != 200:
                raise FlyteUserException(f"failed to get presigned url for `{remote_path}`")
            ...
            try:
                ...
                raise FlyteUserException(f"failed to get presigned url for `{remote_path}`") from e
    ...
    def _upload(file_path: str, to_path: str, chunk_size: int, endpoint: str):
        ...
        if internal_execution_id is not None:
            ...
            res_data = r.json()
            if r.status_code != 200:
                raise FlyteUserException(f"failed to get presigned upload urls for `{to_path}`: {res_data['error']}")
            ...
            try:
                ...
                cp(file_path, to_path, progress=Progress.none, verbose=False)
            except Exception as e:
                raise FlyteUserException(f"failed to get presigned upload urls for `{to_path}`") from e
