...
''' Funções para tratamento de dataset depois de importados do Mongo '''
...
def tratar_genero(genero):
    if (genero.lower() == 'male') | (genero.lower() == 'masculino'):
        genero = 'M'
    elif (genero.lower() == 'female') | (genero.lower() == 'feminino'):
        genero = 'F'
    else:
        genero = '?'
    return genero
...
def tratar_especialidade_encaminhamento(especialidade_enc):
    # dataset[coluna_especialidade] = dataset[coluna_especialidade].title()
    if ('emerg' in especialidade_enc.lower()) | ('urg' in especialidade_enc.lower()) | ('interna' in especialidade_enc.lower()):
        especialidade = 'Emergência'
    elif ('interna' in especialidade_enc.lower()) | ('socor' in especialidade_enc.lower()) | ('emr' in especialidade_enc.lower()):
        especialidade = 'Emergência'
    elif ('encaminhamento_medico_ao_ps' in especialidade_enc.lower()) | ('encaminhamento_ao_ps' in especialidade_enc.lower()):
        especialidade = 'Emergência'
...
def tratar_respostas_gordura_fast_food(resposta):
    if ('nao' in unidecode(resposta.lower())):
        resposta = 'Nao Come'
    elif unidecode(resposta.lower()) == 'menos de 1 vez/mes':
        resposta = 'Menos de uma vez por mes'
    elif 'menos de 1 vez por mes' in unidecode(resposta.lower()):
        resposta = 'Menos de uma vez por mes'
    elif 'menos que mensalmente' in unidecode(resposta.lower()):
        resposta = 'Menos de uma vez por mes'
    elif unidecode(resposta.lower()) == 'menos de 1 vez por mes':
        resposta = 'Menos de uma vez por mes'
    elif unidecode(resposta.lower()) == 'menos de 1x/mes':
        resposta = 'Menos de uma vez por mes'
    elif 'diaria' in unidecode(resposta.lower()):
        resposta = 'Diariamente'
    elif 'semana' in unidecode(resposta.lower()):
        resposta = 'Semanalmente'
    elif 'mensal' in unidecode(resposta.lower()):
        resposta = 'Mensalmente'
    return resposta
...
def adicionar_faixa_etaria(dataset, col_idade):
    ...
    dataset.loc[dataset[col_idade].between(18, 35), 'FAIXA_ETARIA'] = '18 a 34'
    ...
    dataset['FAIXA_ETARIA'].fillna('?', inplace=True)

    dataset.loc[dataset[col_idade] < 18, 'FAIXA_ETARIA_ORDEM'] = int(1)
    dataset.loc[dataset[col_idade].between(18, 35), 'FAIXA_ETARIA_ORDEM'] = int(2)
    dataset.loc[dataset[col_idade].between(35, 49), 'FAIXA_ETARIA_ORDEM'] = int(3)
    dataset.loc[dataset[col_idade].between(50, 64), 'FAIXA_ETARIA_ORDEM'] = int(4)
    dataset.loc[dataset[col_idade] >= 65, 'FAIXA_ETARIA_ORDEM'] = int(5)
    dataset['FAIXA_ETARIA_ORDEM'].fillna(int(6), inplace=True)
...
def adicionar_faixa_etaria_atendimento(dataset, col_idade, nome_coluna_faixa_etaria, nome_coluna_faixa_etaria_ordem):
    ...
    dataset.loc[dataset[col_idade].between(18, 35), nome_coluna_faixa_etaria] = '18 a 34'
    ...
    dataset[nome_coluna_faixa_etaria].fillna('?', inplace=True)

    dataset.loc[dataset[col_idade] < 18, nome_coluna_faixa_etaria_ordem] = int(1)
    dataset.loc[dataset[col_idade].between(18, 35), nome_coluna_faixa_etaria_ordem] = int(2)
    dataset.loc[dataset[col_idade].between(35, 49), nome_coluna_faixa_etaria_ordem] = int(3)
    dataset.loc[dataset[col_idade].between(50, 64), nome_coluna_faixa_etaria_ordem] = int(4)
    dataset.loc[dataset[col_idade] >= 65, nome_coluna_faixa_etaria_ordem] = int(5)
    dataset[nome_coluna_faixa_etaria_ordem].fillna(int(6), inplace=True)

    return dataset


def adicionar_ordem_faixa_etaria(dataset, col_idade):
    dataset.loc[dataset[col_idade] < 18, 'FAIXA_ETARIA_ORDEM'] = int(1)
    dataset.loc[dataset[col_idade].between(18, 35), 'FAIXA_ETARIA_ORDEM'] = int(2)
    dataset.loc[dataset[col_idade].between(35, 49), 'FAIXA_ETARIA_ORDEM'] = int(3)
    dataset.loc[dataset[col_idade].between(50, 64), 'FAIXA_ETARIA_ORDEM'] = int(4)
