...
# # Handler - 1
file = logging.FileHandler('logs.txt', 'a', 'utf-8')
...
# proxyhost = 'http://web-proxy.in.hpicorp.net:8080'
...
# retries = Retry(total=10,backoff_factor=1,status_forcelist=responseCodeForceList,raise_on_status=False)
retries = doRetry
...
# import pythonproxy
def sendRequest(method, requestUrl, cookies=None, headers=None, data=None, json=None, timeout=None, proxyhost=None, validResponseCodes=False, allow_redirects=True, verify=False):
    ...
    try:
        if method=='get' or method=='GET':
            response=s.get(requestUrl, cookies=cookies, headers=headers, data=data, json=json, timeout=timeout, allow_redirects=allow_redirects, verify=False)
        if method=='post' or method=='POST':
            response=s.post(requestUrl, cookies=cookies, headers=headers, data=data, json=json, timeout=timeout, allow_redirects=allow_redirects, verify=False)
        if method=='patch' or method=='PATCH':
            response=s.patch(requestUrl, cookies=cookies, headers=headers, data=data, json=json, timeout=timeout, allow_redirects=allow_redirects, verify=False)
        if method=='delete' or method=='DELETE':
            response=s.delete(requestUrl, cookies=cookies, headers=headers, data=data, json=json, timeout=timeout, allow_redirects=allow_redirects, verify=False)
        return response
    # except requests.exceptions.ConnectionError as e:
    except (MalformedRequest, InternalError, StatusUnknown, ConnectionError, ConnectionResetError, http.client.RemoteDisconnected, RemoteDisconnected, ProtocolError, HTTPException) as e:
...
# path = r'C:\Users\NanwaniS\OneDrive - HP Inc\HP\TechPulse\NFR 1048195\package-files-from-github-search'
