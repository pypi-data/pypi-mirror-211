...
PYBIND11_MODULE(pyeCam, m) {
    py::class_<DeviceManager>(m, "DeviceManager")
        .def(py::init<>())
        .def("getDeviceCountWrapper", &DeviceManager::getDeviceCountWrapper, "Get the device count")
        .def("getDeviceInfoWrapper", &DeviceManager::getDeviceInfoWrapper, py::arg("deviceIndex"), "Get device information");
}
