 ...
 class Interface
  {
   ...
   /* static std::unique_ptr<e_CAM512_HID> buildHID()
	...
	py::tuple Cam::getFormatWrapper() 
	{
		...
		// Create a tuple to return the result, width, and height
		py::tuple pyFormat;
		if (result) {
			// Convert the width and height to Python integers using reinterpret_borrow
			py::object pyWidth = py::reinterpret_borrow<py::int_>(py::cast(gWidth));
			py::object pyHeight = py::reinterpret_borrow<py::int_>(py::cast(gHeight));

			// Create a tuple with the result, width, and height
			pyFormat = py::make_tuple(result, pyWidth, pyHeight);
		} else 
		{
			pyFormat = py::make_tuple(result);
		}
		return pyFormat;
	}
	...
	py::tuple Cam::getCameraPropertiesWrapper(uint32_t Index)
	{
		...
		else
		{
			...
			resultTuple = py::make_tuple(py::none());
		}
	...
	py::tuple Cam::getframeWrapper() 
	{
		...
		// Create a tuple to return the current frame and result
		py::tuple pyCurrFrame;
		if (result == Result::Ok) 
		{
			// Convert the width, height, and total_size to Python integers using reinterpret_borrow
			py::object pyWidth = py::reinterpret_borrow<py::int_>(py::cast(currFrame.width));
			py::object pyHeight = py::reinterpret_borrow<py::int_>(py::cast(currFrame.height));
			py::object pyTotalSize = py::reinterpret_borrow<py::int_>(py::cast(currFrame.total_size));

			// Create a tuple with the frame data and result
			pyCurrFrame = py::make_tuple(pyWidth, pyHeight, py::str(currFrame.pixel_format), pyTotalSize, result);
		} else {
			pyCurrFrame = py::make_tuple(result);
		}
