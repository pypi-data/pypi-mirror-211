...
def browser_type_launch_args(pytestconfig: Any) -> Dict:
    ...
    if DataProcess.isNotNull(get_system_key("browser-channel")):
        browser_channel_option = get_system_key("browser-channel")
    if browser_channel_option:
        launch_options["channel"] = browser_channel_option
    slowmo_option = pytestconfig.getoption("--slowmo")
    if DataProcess.isNotNull(get_system_key("slowmo")):
        slowmo_option = get_system_key("slowmo")
    if slowmo_option:
...
def browser_context_args(
    ...
    if DataProcess.isNotNull(get_system_key("video")):
        video_option = get_system_key("video")
...
def context(
    ...
    if DataProcess.isNotNull(get_system_key("screenshot")):
        screenshot_option = get_system_key("screenshot")
