...
from .errors import AssignException
...
class RPCContractBase(KoiledModel):
    max_assign_retry: int = 3
    max_stream_retry: int = 3
    ...
    async def astream_retry(
        ...
        try:
            async for i in self.astream(args, kwargs, parent, yield_timeout):
                yield i
        except AssignException as e:
            logger.warning(f"Stream failed with {e}")
            if retry < self.max_stream_retry:
                await asyncio.sleep(retry_delay_ms or self.retry_delay_ms)
                async for i in self.astream_retry(
                    args, kwargs, parent, yield_timeout, retry=retry + 1
                ):
                    yield i
            else:
                raise e
    ...
    async def aassign_retry(
        ...
        try:
            i = await self.aassign(args, kwargs, parent, assign_timeout)
            return i
        except AssignException as e:
            logger.warning(f"Assign failed with {e}")
            if retry < self.max_assign_retry:
                await asyncio.sleep(retry_delay_ms or self.retry_delay_ms)
                return await self.aassign_retry(
                    args, kwargs, parent, assign_timeout, retry=retry + 1
                )
            else:
                raise e
...
class arkiuse(RPCContractBase):
    ...
    async def aassign(
        ...
        ), "Reservation is not active at the moment"
