...
def test_version_mismatch(request):
    ...
    if settings.SITE.kernel.editing_front_end.app_label == 'react':
        vm_data = dict(version_mismatch=True)
        try:
            if float(request.GET.get(
                    constants.URL_PARAM_MAIN_JS_TIMESTAMP)
            ) != os.stat(Path(settings.STATIC_ROOT) / 'react' / 'main.js'):
                vm_data.update(js_version_mismatch=True)
        except Exception:
            pass
        return vm_data
    ...
    def get(self, request, app_label=None, actor=None, pk=None):
        ...
        if pk and pk != '-99999' and pk != '-99998':
            ...
            try:
                ...
                raise http.Http404(f"Objects {sr} does not exists on {rpt}")
            # print(
