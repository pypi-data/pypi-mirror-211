...
class DiscoveryResponse(IResponseGetterMixin):
    ...
    def from_dpa(dpa: bytes) -> DiscoveryResponse:
        ...
        if rcode == ResponseCodes.OK:
            ...
            result = {'discNr': dpa[8]}
        return DiscoveryResponse(hwpid=hwpid, rcode=rcode, dpa_value=dpa_value, pdata=pdata, result=result)
    ...
    def from_json(json: dict) -> DiscoveryResponse:
        ...
        return DiscoveryResponse(msgid=msgid, hwpid=hwpid, dpa_value=dpa_value, rcode=rcode, pdata=pdata, result=result)
