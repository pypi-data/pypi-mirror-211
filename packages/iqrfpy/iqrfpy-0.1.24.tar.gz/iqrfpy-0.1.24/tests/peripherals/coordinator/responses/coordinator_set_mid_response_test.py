...
data_ok: dict = {
    'mtype': CoordinatorMessages.SET_MID,
...
class SetMIDResponseTestCase(unittest.TestCase):
    ...
    @parameterized.expand([
        ...
        ['from_json', data_ok, SetMidResponse.from_json(generate_json_response(data_ok)), True],
        ['from_json', data_ok_1, SetMidResponse.from_json(generate_json_response(data_ok_1)), True],
        ['from_dpa_error', data_error, SetMidResponse.from_dpa(data_error['dpa']), False],
        ['from_json_error', data_error, SetMidResponse.from_json(generate_json_response(data_error)), True]
    ])
