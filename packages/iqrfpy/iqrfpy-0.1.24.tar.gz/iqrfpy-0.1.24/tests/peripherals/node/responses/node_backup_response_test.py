...
class BackupResponseTestCase(unittest.TestCase):
    ...
    def test_factory_methods_ok(self, _, response_data, response, json):
        ...
        with self.subTest():
            self.assertEqual(response.get_pnum(), EmbedPeripherals.NODE)
        with self.subTest():
            self.assertEqual(response.get_pcmd(), NodeResponseCommands.BACKUP)
        with self.subTest():
        ...
        if json:
            with self.subTest():
                self.assertEqual(response.get_mtype(), NodeMessages.BACKUP)
            with self.subTest():
    ...
    def test_get_network_data(self, _, backup_data, response: BackupResponse):
        self.assertEqual(response.get_backup_data(), backup_data)
