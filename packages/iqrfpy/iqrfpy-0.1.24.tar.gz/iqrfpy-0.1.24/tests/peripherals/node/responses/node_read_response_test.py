...
from iqrfpy.peripherals.node.responses.read import ReadResponse, NodeReadData
...
class ReadResponseTestCase(unittest.TestCase):
    ...
    def test_factory_methods_ok(self, _, response_data, response, json):
        ...
        with self.subTest():
            self.assertEqual(response.get_pcmd(), NodeResponseCommands.READ)
        with self.subTest():
        ...
        if json:
            with self.subTest():
                self.assertEqual(response.get_mtype(), NodeMessages.READ)
            with self.subTest():
    ...
    def test_get_os_read_data(self, _, data: NodeReadData, response: ReadResponse):
        self.assertEqual(response.get_node_data(), data)
