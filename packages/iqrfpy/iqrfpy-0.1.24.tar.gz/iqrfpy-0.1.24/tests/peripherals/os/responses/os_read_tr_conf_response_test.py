...
from iqrfpy.peripherals.os.responses.read_tr_conf import ReadTrConfResponse, OsReadTrConfData
...
class ReadTrConfResponseTestCase(unittest.TestCase):
    ...
    @parameterized.expand([
        ['from_dpa', data_ok, ReadTrConfResponse.from_dpa(data_ok['dpa']), False],
        [
            'from_json',
            data_ok,
        ...
        ['from_dpa_error', data_error, ReadTrConfResponse.from_dpa(data_error['dpa']), False],
        [
            'from_json_error',
            data_error,
            ReadTrConfResponse.from_json(
    ...
    def test_factory_methods_ok(self, _, response_data, response, json):
        ...
        with self.subTest():
            self.assertEqual(response.get_pcmd(), OSResponseCommands.READ_CFG)
        with self.subTest():
        ...
        if json:
            with self.subTest():
                self.assertEqual(response.get_mtype(), OSMessages.READ_CFG)
            with self.subTest():
    ...
    def test_get_checksum(self, _, data: int, response: ReadTrConfResponse):
        self.assertEqual(response.get_checksum(), data)

    @parameterized.expand([
        [
            'from_dpa',
            OsReadTrConfData(data_ok['result']['configuration']),
            ReadTrConfResponse.from_dpa(data_ok['dpa'])
        ],
        [
            'from_json',
    ...
    def test_get_configuration(self, _, data: OsReadTrConfData, response: ReadTrConfResponse):
        self.assertEqual(response.get_configuration(), data)
    ...
    def test_get_rfpgm(self, _, data: int, response: ReadTrConfResponse):
        self.assertEqual(response.get_rfpgm(), data)
    ...
    def test_get_init_phy(self, _, data: int, response: ReadTrConfResponse):
        self.assertEqual(response.get_init_phy(), data)
