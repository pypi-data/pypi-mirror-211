...
from iqrfpy.peripherals.ledr.responses.set_off import SetOffResponse
from iqrfpy.response_factory import ResponseFactory
from tests.helpers.json import generate_json_response

data_ok: dict = {
    'mtype': LEDRMessages.SET_OFF,
    'msgid': 'setOffTest',
    'nadr': 1,
    'hwpid': 0,
    'rcode': 0,
    'dpa_value': 64,
    'dpa': b'\x01\x00\x06\x80\x00\x00\x00\x40'
}
...
data_ok_1: dict = {
    ...
    'rcode': 0,
    'dpa_value': 35,
    'dpa': b'\x02\x00\x06\x80\x02\x04\x00\x23'
}
...
data_error: dict = {
    ...
    'rcode': 1,
    'dpa_value': 35,
    'dpa': b'\x00\x00\x06\x80\x02\x04\x01\x23'
}
...
class SetOffResponseTestCase(unittest.TestCase):
    ...
    @parameterized.expand([
        ...
        ['from_json', data_ok, SetOffResponse.from_json(generate_json_response(data_ok)), True],
        ['from_json', data_ok_1, SetOffResponse.from_json(generate_json_response(data_ok_1)), True],
        ['from_dpa_error', data_error, SetOffResponse.from_dpa(data_error['dpa']), False],
        ['from_json_error', data_error, SetOffResponse.from_json(generate_json_response(data_error)), True]
    ])
    def test_factory_methods_ok(self, _, response_data, response, json):
        ...
        if json:
            with self.subTest():
                self.assertEqual(response.get_mtype(), LEDRMessages.SET_OFF)
            with self.subTest():
                self.assertEqual(response.get_msgid(), response_data['msgid'])

    def test_from_dpa_invalid(self):
        with self.assertRaises(ValueError):
            ResponseFactory.get_response_from_dpa(b'\x00\x00\x06\x80\x00\x00\x00\x22\x32')
