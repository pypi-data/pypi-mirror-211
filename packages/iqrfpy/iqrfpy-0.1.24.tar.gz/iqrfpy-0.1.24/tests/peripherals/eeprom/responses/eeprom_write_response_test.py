...
class WriteResponseTestCase(unittest.TestCase):
    ...
    @parameterized.expand([
        ['from_dpa', data_ok, WriteResponse.from_dpa(data_ok['dpa']), False],
        ['from_dpa', data_ok_1, WriteResponse.from_dpa(data_ok_1['dpa']), False],
        ['from_json', data_ok, WriteResponse.from_json(generate_json_response(data_ok)), True],
    ...
    def test_factory_methods_ok(self, _, response_data, response, json):
        ...
        with self.subTest():
            self.assertEqual(response.get_pcmd(), EEPROMResponseCommands.WRITE)
        with self.subTest():
        ...
        if json:
            with self.subTest():
                self.assertEqual(response.get_mtype(), EEPROMMessages.WRITE)
            with self.subTest():
                self.assertEqual(response.get_msgid(), response_data['msgid'])

    def test_from_dpa_invalid(self):
        with self.assertRaises(ValueError):
            ResponseFactory.get_response_from_dpa(b'\x00\x00\x03\x81\x00\x00\x00\x22\x32')
