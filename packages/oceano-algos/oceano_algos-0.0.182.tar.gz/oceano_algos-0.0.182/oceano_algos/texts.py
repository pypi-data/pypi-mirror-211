...
# текст сообщения для спам-рассылки

def advertise_text(url):
    return f"""
Всем привет! Появился бот который с помощью искусственного интеллекта помогает искать работу.

Бот мониторит <i>HeadHunter</i> и использует ИИ, чтобы автоматически подбирать и скидывать вам в чат подходящие вакансии.

Также есть крутая фича - <b>фильтр по откликам</b>. Благодаря ей вы сможете всегда быть в числе первых откликнувшихся и HR будут чаще замечать ваше резюме!

<a href="{url}">Присоединяйся!</a>
"""