...
def plot_roc_curve(y_true, y_score, title=''):
    ...
    title2 = merge_titles(title, f'ROC Curve (AUC={auc:.3f})')
    plt.title(title2)
    plt.legend()
    plt.tight_layout()
    plt.show()
...
def plot_confusion_matrix(y_true, y_pred, y_score=None, labels=None, classes=None, title='', as_xpptx=None):
    ...
    post_plot(title=title, legend=False, as_xpptx=as_xpptx, desc=desc)
...
def plot_pie(vals=None, counts=None, title='', sort_by_index=False, figsize=(6,6), colors=None, show=True, with_numbers=True, specific_order=None, cmap=None):
    name = ''
    if vals is not None:
        vals = pd.Series(vals)
...
def plot_multi(df, kind=None, plot_func=None, x=None, y=None, var_name='variable', value_name='value', plot_on=None, group_on=None, color_on=None, cmap=None, label_on=None, style_on=None, size_on=None, figsize=(10,6), alpha=1.0, hdi_probs=(0.1, 0.25, 0.5, 0.8, 0.999), kde_cov=0.25, kde_percentile=None, hist_calc='perc', hist_type='step', hist_bins=10, hist_range=None, color_dict=None, title='', x_axis_type=None, y_axis_type=None, x_axis_fmt=None, y_axis_fmt=None, invert_yaxis=False, legend_loc='best', xlim=None, ylim=None, save_to=None, clear_folder=False, add_date=True, add_counts=True, plot_decorate_func=None, drop_na=True, as_xpptx=None, desc='', reset_colors=True, show=True, **kwargs):
    ...
    if label_on:
        if kind in ['scatter', 'kde', 'hist']:
            label_counts = df.value_counts(label_on).reset_index(name='_count')
        elif kind in ['line'] and group_on:
            label_counts = df.drop_duplicates(group_on).value_counts(label_on).reset_index(name='_count')
    ...
    for df_p, keys, plot_title in xpd.x_iter_groups(df, plot_on):
        ...
        if save_to:
            ...
            if plot_on:
                text = "\n".join([f"{k}={v}" for k,v in keys.iteritems()])
                slide_title = ''
            else:
                slide_title = title
                text = desc
