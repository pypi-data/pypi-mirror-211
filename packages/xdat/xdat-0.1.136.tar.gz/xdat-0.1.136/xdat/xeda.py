...
def x_explore_predictive(df, target_col, feature_cols=None, pipes=None, prs=None):
    df = df.copy()
    should_save = prs is None
    if prs is None:
    ...
    for pipe_name, pipe in pipes:
        ...
        df_eval = xproblem.eval_test(df_test, eval_per='fold_num', metric_list=metric_list)
        df_metrics = df_eval[metric_list].mean().to_frame('CV Mean').reset_index(names='metric')
        xplots.plot_feature_importances(all_folds, show=False)
        plt_fi = prs.capture_image()
        desc = f"Cross validated, {len(df_eval)} folds"
        prs.add_slide_content(title=pipe_name, desc=desc, main_content=plt_fi, sub_content=df_metrics)
