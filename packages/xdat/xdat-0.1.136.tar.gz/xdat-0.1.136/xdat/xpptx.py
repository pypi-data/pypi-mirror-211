...
class Presentation:
    def __init__(self, theme=DEFAULT_THEME, title=None, print_layout=False, fake=False):
        self.theme = theme
    ...
    def add_slide_content(self, title='', desc='', main_content='', sub_content='', sub_title=''):
        if self._no_val(main_content) and self._no_val(sub_content) and self._no_val(sub_title):
            self.add_slide('title_and_body', title=title, text=desc)
    ...
    def add_slide_content_2cols(self, title='', left='', right='', left_title='', right_title='', desc=''):
        if self._has_val(desc):
            self.add_slide('left_with_two_cols', title=title, text=desc, text_2=left, text_3=right, text_4=left_title, text_5=right_title)
    ...
    def save(self, out_path=None):
        if not self.fake:
            if not out_path:
                ...
                out_path = xsettings.OUTPUT_PATH.joinpath(f"{slugify(title, separator=' ')}.pptx")
    ...
    for k, v in kwargs.items():
        ...
        if v is None:
            ...
            for x in range(shape[1]):
                cell = table.cell(0, x)
                cell.text = str(v.columns[x])

            for y in range(shape[0]):
                for x in range(shape[1]):
                    cell = table.cell(y+1, x)
                    cell.text = str(v.iloc[y][x])
            p.text = ' '
