...
from . import xpptx, xplots, xsettings, xmunge, xproblem
...
def x_explore_predictive(df, target_col, feature_cols=None, pipes=None, pipes_limit=None, prs=None):
    df = df.copy()
    should_save = prs is None
    if prs is None:
    ...
    for pipe_name, pipe in pipes:
        ...
        df_metrics = df_eval[metric_list].mean().round(4)
        df_metrics = df_metrics.to_frame('CV Mean').reset_index(names='metric')

        desc = f"Cross validated, {len(df_test.fold_num.unique())} folds"
        xplots.plot_feature_importances(all_folds, show=False)
        ...
        if target_dtype is bool:
            ...
            prs.add_slide_content_2cols(title=pipe_name, desc=df_metrics, left=plt_cm, right=plt_fi, sub_content=desc)

        elif target_dtype is float:
            prs.add_slide_content(title=pipe_name, desc=df_metrics, main_content=plt_fi, sub_content=desc)
