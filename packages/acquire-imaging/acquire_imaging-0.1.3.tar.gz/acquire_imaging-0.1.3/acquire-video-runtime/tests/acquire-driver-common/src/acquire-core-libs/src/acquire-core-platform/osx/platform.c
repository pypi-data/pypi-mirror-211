...
{
    clock->origin = clock_gettime_nsec_np(CLOCK_MONOTONIC_RAW);
}
...
{
    const uint64_t t = clock_gettime_nsec_np(CLOCK_MONOTONIC_RAW);
    if (clock)
        clock->origin = t;
    return t;
}
...
{
    const uint64_t t = clock_gettime_nsec_np(CLOCK_MONOTONIC_RAW);
    const int64_t dt = t - clock->origin;
    return dt;
}
...
{
    void* v;
    const int ret = pthread_join(self->inner_, &v);
    // ignore when the thread is already closed
    if (ret != 0 && ret != ESRCH)
        CHECK_POSIX(ret);
Error:;
}
