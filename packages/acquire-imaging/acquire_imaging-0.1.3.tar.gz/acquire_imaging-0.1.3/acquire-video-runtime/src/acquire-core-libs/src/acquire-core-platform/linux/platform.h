...
extern "C"
{
    ...
    struct thread
    {
        /// thread handle
        pthread_t inner_;
        /// when true, inner_ is a joinable thread handle.
        uint8_t is_live_;
        /// protects coherence between inner_ and is_live_
    ...
    /// @returns -1, 0, or 1 when 'timestamp' is prior, equal, or after the
    /// clock origin.
    ...
    void event_set(struct event* self);
