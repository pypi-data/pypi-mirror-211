...
class Metadata():
    ...
    def __init__(self, conf={}):
        ...
        try:
            ...
            dfs.install()
            dfs = Metadata.Countries()
            dfs.install()
            dfs = Metadata.Parameter()
            dfs.install()
    ...
    class DataField(BasicElement):                    
        ...
        def build(self):
            ...
            if len(self.found) <= 0:
                return
            self.attributes = [f.get('name') for f in self.found]
            self.scopes = {}
            for f in self.found:
        ...
        def install(self):
            ...
            with open(os.path.join(".", "struts", "datafield.json"), mode='r', encoding='utf-8') as infile:
                ...
                for df in dfs:
                    ...
                    to_insert.update(df)
                    ins_obj = self.db.createDocument(to_insert)
                    ins_obj.save()
            ...
            try:
                ...
                version_parameter.value = version
                version_parameter.create()

        def __init__(self, data=None, obj_type=None):
