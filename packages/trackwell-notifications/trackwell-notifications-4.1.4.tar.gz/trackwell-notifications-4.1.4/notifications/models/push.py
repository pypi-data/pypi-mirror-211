# The origianl model code (apart from the Topic model) comes from the fcm-django model and is subject
...
class AbstractFCMDevice(Device):
    ...
    def deactivate_devices_with_error_result(
        ...
        return cls.objects.deactivate_devices_with_error_results(
            [registration_id], [messaging.SendResponse(
                {"name": name}, firebase_exc)]
        )
...
class Topic(models.Model):
    ...
    def send_notification(
        ...
        Args:
            notification (Notification): Notification instance to send
            extra_filter (Optional[models.Q], optional): Extra filtering for user devices to send to. Defaults to None.
        """
        default_language = get_default_language()
        devices = UserDevice.objects.filter(user__in=self.subscribers.all())
        if extra_filter is not None:
            devices = devices.filter(extra_filter)
        devices = devices.annotate(
            language=Coalesce(
                Cast('user__profile__prefs__language',
                     output_field=models.CharField()),
                models.Value(default_language),
            )
        )
        messages = notification.get_push_notification_messages()
        for lang in devices.values_list('language', flat=True):
            message = messages.get(lang, messages.get(default_language))
            if message is not None:
                devices.filter(language=lang).send_message(message)
