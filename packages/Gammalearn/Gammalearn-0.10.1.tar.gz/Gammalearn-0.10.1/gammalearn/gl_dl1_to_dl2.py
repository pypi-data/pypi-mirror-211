...
def build_argparser():
    ...
    parser.add_argument('experiment', 
                        type=Path,
                        help='Path to the experiment directory from which to load the model checkpoint')
    parser.add_argument('checkpoint', help='Name of the checkpoint file to load')
    parser.add_argument('dl1', 
                        type=Path,
                        help='Directory path to the dl1 files')
    parser.add_argument('dl2',
                        type=Path,
                         help='Directory path to write the dl2 files')
...
def main():
    ...
    # Load settings file
    configuration_file = experiment_path.joinpath(experiment_name + '_settings.py')
    # Load experiment
    ...
    for _ in tqdm.tqdm(dl1_file_list):
        ...
        if model is None:
            ...
            # Load checkpoint
            checkpoint_path = experiment_path.joinpath(args.checkpoint)
