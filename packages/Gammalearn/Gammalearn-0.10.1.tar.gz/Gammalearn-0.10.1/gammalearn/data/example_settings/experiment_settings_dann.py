...
from gammalearn.constants import GAMMA_ID, PROTON_ID, ELECTRON_ID, SOURCE, TARGET
...
targets = collections.OrderedDict({
    ...
    'domain_class': {
        'loss': torch.nn.CrossEntropyLoss(),
        'loss_weight': utils.ExponentialW(apply_on_grads=True, gamma=10),
        'metrics': {
            'Accuracy_domain': Accuracy(threshold=0.5),
            'AUC_domain': AUROC(pos_label=0,
                                num_classes=2,
                                compute_on_step=True)
        },
        'mt_balancing': False
