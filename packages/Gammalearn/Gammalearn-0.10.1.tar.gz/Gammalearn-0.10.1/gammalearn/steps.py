...
def get_training_step_mt(**kwargs):
    ...
    def training_step_mt(module, batch):
        ...
        outputs, labels = data['outputs_source'], data['labels_source']

        # Compute loss
        loss, loss_data = module.experiment.LossComputing.compute_loss(outputs, labels)
        loss = module.experiment.loss_balancing(loss)
...
def get_training_step_deepjdot(**kwargs):
    ...
    def training_step_deepjdot(module, batch):
        ...
        # Compute loss
        module.experiment.targets['deepjdot']['loss'].set_train_context()
        loss, loss_data = module.experiment.LossComputing.compute_loss(output, labels, module)
        loss = module.experiment.loss_balancing(loss)
        loss = sum(loss.values())
...
def get_training_step_deepcoral(**kwargs):
    ...
    def training_step_deepcoral(module, batch):
        ...
        #output['deepcoral'] = data['labels_source']['class'].unsqueeze(dim=1).to(torch.float32)
        #labels['deepcoral'] = data['labels_target']['class'].unsqueeze(dim=1).to(torch.float32)
...
def get_training_step_mt_gradient_penalty(**kwargs):
    ...
    def training_step_mt_gradient_penalty(module, batch):
        ...
        # Compute loss
        loss, loss_data = module.experiment.LossComputing.compute_loss(output, labels)
        loss = module.experiment.loss_balancing(loss)
        loss = sum(loss.values())
...
def get_eval_step_mt(**kwargs):
    ...
    def eval_step_mt(module, batch):
        ...
        all_loss, loss_data = module.experiment.LossComputing.compute_loss(outputs, labels)
        loss = module.experiment.loss_balancing(all_loss)
        loss = sum(loss.values())
...
def get_eval_step_deepjdot(**kwargs):
    ...
    def eval_step_deepjdot(module, batch):
        ...
        # Compute loss
        module.experiment.targets['deepjdot']['loss'].set_eval_context()
        loss, loss_data = module.experiment.LossComputing.compute_loss(output, labels, module)
        loss = module.experiment.loss_balancing(loss)
        loss = sum(loss.values())
...
def run_model(module, batch, requires_latent_features=False, requires_pointing=False, forward_params=None, train=True):
    ...
    # Load data
    inputs_source = inputs_target = labels_source = labels_target = outputs_target = None
