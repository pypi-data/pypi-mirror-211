...
from yangsuite.settings.base import DOCKER_RUN
...
class GRPCTelemetryIPAddressHandler:
    def __call__(self) -> str:
        """Get IP address to use in gRPC telemetry.

        If running in a Docker container, returns the IP address of the container.
        Otherwise, returns the local IP address of the current machine.
        ...
        if DOCKER_RUN:
            ...
            # https://github.com/CiscoDevNet/yangsuite/issues/90
...
class YSGrpcTelemetryServer(object):
    ...
    iphandler = GRPCTelemetryIPAddressHandler()
    servers = {}
    stdout_queue = deque()
    out_uri = ''
    out_file = ''
    ...
    @classmethod
    def serve(cls, port="5678", user=None, device=None):
        """Run the server indefinitely."""
        address = cls.iphandler()
    ...
    @classmethod
    def set_output(cls, fileout, uri=None):
