...
def distcrab(download=False, dump=False, ip='192.168.1.200', port=22, username='root', password=None, firmware=None, version=None, branch=None):
    ...
    if download:
        ...
        yield from client.exec_command([
            '/bin/mount -o rw,remount / && /bin/sync && /rbctrl/prepare-update.sh /tmp && /etc/init.d/rbctrl.sh stop && PATH=/usr/local/bin:/usr/bin:/bin:/usr/local/sbin:/usr/sbin:/sbin /var/volatile/update/chrt-sqfs.sh /update/updater /mnt/tmp/update-final.sh'
        ])
    else:
        client = Client(ip=ip, port=port, username=username, password=password)
        yield from client.exec_command([
        ...
        yield from client.putfo(src)
        yield from client.exec_command([
            '/bin/rm -rf /usr/local/crab/ && /bin/mkdir -p /usr/local/crab/ && /bin/sync && /bin/tar -xJf /tmp/firmware.bin -C /usr/local/crab/ && /bin/rm -rf /tmp/firmware.bin && /bin/sync && /usr/local/bin/elite_local_start.sh',
        ])
