...
class HelpFormatter(click.HelpFormatter):
    OPTION_REGEX = r"(--?[\w-]+)"
    WRAP_REGEX = r"(\n\s*|)"
    OPT_REF_REGEX = re.compile(rf"'{WRAP_REGEX}{OPTION_REGEX}'")
    OPT_REF_FIX_REGEX = re.compile(rf'"{WRAP_REGEX}{OPTION_REGEX}"')  # < @TODO join with regular
    ...
    PLACEHOLDER_RR = pt.StringReplacer(
        re.compile(r"<<(\n\s*|)([\w\s.-]+?)>>"),
        lambda m: get_stdout().render(f"<{m.group(1)}{m.group(2)}>", HelpStyles.TEXT_PLACEHOLDER),
    )
    ALTER_RR = pt.StringReplacer(
        re.compile(r"\^([\w .,!/-]+?)\^"),
        lambda m: get_stdout().render(f" {m.group(1)} ", HelpStyles.TEXT_ALTER_MODE),
    )
    WALL_RR = pt.StringReplacer(
        re.compile(r"░{2,}"),
        lambda m: get_stdout().render(m.group(0), HelpStyles.TEXT_WALL),
    )
    ABBREV_RR = pt.StringReplacer(
        re.compile(r"&([A-Z](?=[^A-Z])|[a-z](?=[^a-z]))"),
        lambda m: get_stdout().render(m[1].swapcase(), HelpStyles.TEXT_ABBREV),
    )

    class CTIReplacer(pt.StringReplacer):
    ...
    def format_accent_fixed(self, m: re.Match) -> str:
        return get_stdout().render(f" {m.group(1)}{m.group(2)} ", HelpStyles.TEXT_ACCENT)
    ...
    def write(self, string: str) -> None:
        ...
        string = pt.apply_filters(string, *self.help_filters)
...
class Context(click.Context):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.exec_shell = True
        self.exec_target = None
        self.exec_get_help = False
...
class CliCommand(CliBaseCommand):
    ...
    def format_help(self, ctx: Context, formatter: HelpFormatter) -> None:
        ...
        if self._ext_help_invoker is None:
            return
        stdout = self._ext_help_invoker(ctx)
        ...
        get_stdout().echo(formatter.getvalue().rstrip("\n"))
        formatter.buffer.clear()

        def has_bg_or_reset_param(m: t.Match) -> bool:
            for c in m.group(3).split(";"):
                ...
                try:
                    ...
                    continue
                if i == pt.IntCode.RESET or i in pt.ansi.BG_COLORS or i in pt.ansi.BG_HI_COLORS:
                    return True

        if stdout:
            inject_bg_rr = pt.StringReplacer(
                pt.SGR_SEQ_REGEX,
                lambda m: m.group(0) + (seq_color_bg if has_bg_or_reset_param(m) else ""),
            )
            enclose_bg_rr = pt.StringReplacer(re.compile("(^|\n|$)"), rf"\1{seq_color_bg_line}")
            stdout = pt.apply_filters(stdout, inject_bg_rr, enclose_bg_rr) + str(pt.SeqIndex.RESET)
            get_stdout().echo(stdout)
    ...
    def format_examples(self, ctx: Context, formatter: HelpFormatter, examples: Examples) -> None:
        ...
        with formatter.section(examples.title.capitalize()):
            for example in examples.content:
                example = re.sub(
                    r"(\s)" + formatter.OPTION_REGEX,  # костыль; сам по себе OPTION_REGEX
