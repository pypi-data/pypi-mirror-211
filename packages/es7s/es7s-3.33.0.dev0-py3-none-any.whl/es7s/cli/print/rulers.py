...
from es7s.shared import get_stdout, get_color, FrozenStyle
...
@cli_option(
    "-r",
    "--rulers",
    type=EnumChoice(RulerType),
    default=RulerType.list(),
    show_default=True,
    multiple=True,
    help="Which rulers to draw. Can be specified multiple times: '-r B -r L' "
         "will result in drawing bottom and left rulers.",
)
@cli_option(
    "-g",
    "--grid",
...
class RulersPrinter:
    ...
    def __init__(self, rulers: list[RulerType], no_rulers: bool, grid: int, **kwargs):
        self._def_fmt = FrozenStyle(fg=get_color().get_theme_color(), bg=pt.cv.GRAY_0)
        self._hl_fmt = FrozenStyle(fg=get_color().get_theme_bright_color(), bg=pt.cv.GRAY_0)
    ...
    def _run(self):
        ...
        for y in range(1, self._terminal_height+1):
            ...
            if y == 1:
                if RulerType.TOP in self._rulers:
                    stdout.echo(self._print_hruler(self._terminal_width, RulerType.TOP, line_num_str_left), nl=False)
            elif y == self._terminal_height:
                if RulerType.BOTTOM in self._rulers:
