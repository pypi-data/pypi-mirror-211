...
class ErrorCount:
    ...
    def error_report(self, permissive_mode: bool):
        if not (self.title_error + self.body_error + self.merge_error):
            ...
            if permissive_mode:
                print("WARNING: ignoring command failure because running in permissive mode")
                sys.exit(0)
            else:
                sys.exit(1)
...
class BodyError:
    ...
    :attributes:
        ...
        jama (bool): Indicates if the last line of the commit description should start with a item JAMA.
        duplicate_jama_line (bool): Indicates if there are several lines starting with a item JAMA.
        duplicate_jama_refs (List[str]): List of duplicate JAMA references.
    """
