...
from lupin_grognard.core.tools.utils import is_last_description_line_start_with_jama
...
class CommitValidator(Commit):
    def __init__(
        ...
        self.is_ci_mr_target_branch_main = is_ci_mr_target_branch_main
    ...
    def _validate_body(self) -> bool:
        ...
        # if target branch == main, the last line of the description must be a jama reference
        # if self.is_ci_mr_target_branch_main:
        if True:
            if not self.body:
                body_error.jama_not_referenced = True
            elif not self._is_last_description_line_start_with_jama():
                if self.title not in INITIAL_COMMIT:
                    body_error.jama_not_referenced = True
            else:
                if not self._is_sigle_line_start_with_jama_reference():
                    body_error.duplicate_jama_line = True

                (
                    duplicate_jama_refs,
                    invalid_jama_refs,
                ) = self._validate_jama_items_referenced()
    ...
    def _is_last_description_line_start_with_jama(self) -> bool:
        return is_last_description_line_start_with_jama(body=self.body)

    def _validate_jama_items_referenced(self) -> Tuple[List[str], List[str]]:
    ...
    def _is_sigle_line_start_with_jama_reference(self) -> bool:
        """Checks whether the body of the validation message contains several lines referencing jama"""
        jama_ref_count = 0
