...
#ifndef JSDRV_CDEF_H_
#define JSDRV_CDEF_H_
...
/**
 * @ingroup jsdrv_prv
 * @defgroup jsdrv_prv_cdef Common C utilities
 *
 ...
 */
#define JSDRV_STATIC_ASSERT(COND, MSG) typedef char static_assertion_##MSG[(COND)?1:-1]
 ...
 */
#define JSDRV_ARRAY_SIZE(x) ( sizeof(x) / sizeof((x)[0]) )
 ...
 */
#define JSDRV_STRUCT_INIT(s) (jsdrv_memset(&s, 0, sizeof(s)))
 ...
 */
#define JSDRV_STRUCT_PTR_INIT(s_ptr) (jsdrv_memset(s_ptr, 0, sizeof(*s_ptr)))
 ...
 *
 *     #define RWE JSDRV_RETURN_ON_ERROR
 */
#define JSDRV_RETURN_ON_ERROR(x) do { \
    int32_t rc__ = (x); \
    if (rc__) { \
        return rc__; \
    } \
 ...
 * On error, cause the calling function to return with the error code.
 */
#define JSDRV_RETURN_ON_ERROR_MSG(x, msg) do { \
    int32_t rc__ = (x); \
    if (rc__) {\
        JSDRV_LOGE("[%d:%s] %s", rc__, jsdrv_error_code_name(rc__), msg); \
        return rc__; \
    } \
} while (0)
 ...
 *      Any non-zero value is presumed to be an error.
 */
#define JSDRV_EXIT_ON_ERROR(x) do { \
    int32_t rc__ = (x); \
    if (rc__) { \
        goto exit; \
    } \
 ...
 *      assigned to var.
 */
#define JSDRV_EXIT_WITH_ERROR(var, x) do { \
    int32_t rc__ = (x); \
    if (rc__) { \
        var = rc__; \
        goto exit; \
 ...
 */
#define JSDRV_RESTRICT_TO_RANGE(x_val, x_min, x_max) \
    ( ( (x_val) < (x_min) ) ? (x_min) : \
 ...
 */
#define JSDRV_ROUND_UP_TO_MULTIPLE(x, m) (( ((x) + ( JSDRV_SIGNUM(x) * (m - 1) )) / m) * m)
 ...
 *      for negative values of x.
 *
 * @see JSDRV_ROUND_UP_TO_MULTIPLE() for signed integers.
 */
#define JSDRV_ROUND_UP_TO_MULTIPLE_UNSIGNED(x, m) (( ((x) + (m - 1) ) / m) * m)
 ...
 */
#define JSDRV_CONTAINER_OF(ptr, type, member) \
 ...
 */
#define JSDRV_STATIC_PRINT_SIZEOF(t) \
 ...
 */
#define JSDRV_STATIC_PRINT_INT(i, name) \
