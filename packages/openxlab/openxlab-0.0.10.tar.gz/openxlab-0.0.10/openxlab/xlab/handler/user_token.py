...
AUTH_CLIENT = AuthClient("https://beta.openxlab.org.cn/api/v1/sso-be/api/v1/open/")
...
def calculate_d(sk, nonce, algorithm):
    ...
    try:
        ...
        hmac_obj = hmac.new(hmac_key, nonce.encode('utf-8'),
...
def get_token_via_api(ak=None, sk=None):
    ...
    if user_config is None:
        raise ValueError("Local config must not be empty before get token via api. "
...
def refresh_token(ak=None, sk=None):
    user_config = get_config(ak, sk)
    if user_config is None:
        raise ValueError("Local config must not be empty before refresh token. "
                         "Please use the 'openxlab config' command to set the config")
    local_user_token = get_token_from_local()
    if local_user_token is None:
        raise ValueError("local token must not be empty before refresh token")
    refresh_expiration_datetime = get_datetime_from_formatted_str(local_user_token.refresh_expiration)
    now = get_current_time()
