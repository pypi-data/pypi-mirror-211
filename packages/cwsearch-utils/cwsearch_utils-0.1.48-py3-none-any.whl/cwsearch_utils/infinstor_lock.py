...
def load_from_db(fn):
    ...
    with open(fnu, 'r') as f_in:
        for line in f_in:
            sl = line.split(',')
            name = sl[0].strip()
            dmsg = base64.b64decode(sl[3].strip()).decode('ascii')
            demb = base64.b64decode(sl[4].strip())
            if name in rv:
                rv[name].append((sl[1], sl[2], dmsg, sl[4]))
            else:
                rv[name] = [(sl[1], sl[2], dmsg, sl[4])]
    return rv
