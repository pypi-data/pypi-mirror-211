...
def populate_all_names(bucket, prefix, infinstor_time_spec, resources, max_size_allowed=None, max_time_allowed=None) -> bool:
    ...
    try:
        s3client = boto3.client('s3', infinstor_time_spec=infinstor_time_spec)
        files = get_files(s3client, bucket, prefix, resources)
    except Exception as ex:
        traceback.print_exc()
    ...
    for fn in db_files_to_upload.keys():
        ...
        os.remove(full_fn)
        rv.append( {'vdbindex_bucket': bucket,
                    'vdbindex_prefix': f'{prefix}index/{infinstor_time_spec}',
                    'vdbindex_name': fn})
    infinsnap_s3client = boto3.client('s3')
    for ind in range(len(rv)):
        version_id, psurl = infinsnap_s3client.get_version_id(rv[ind]['vdbindex_bucket'],
                                                    f"{rv[ind]['vdbindex_prefix']}/{rv[ind]['vdbindex_name']}.gz",
                                                    'aggregate.populate_all_names')
        print(f"populate_all_names: bucket={rv[ind]['vdbindex_bucket']}, obj={rv[ind]['vdbindex_prefix']}/{rv[ind]['vdbindex_name']}.gz, psurl={psurl}")
        rv[ind]['data_file'] = psurl
    return rv
