...
def populate_all_names(bucket, prefix, infinstor_time_spec, resources, max_size_allowed=None, max_time_allowed=None) -> bool:
    ...
    time.sleep(10)
    no_infinlice_s3client = boto3.client('s3', infinstor_time_spec=infinsnap())
    for ind in range(len(rv)):
        try:
            _, psurl = no_infinlice_s3client.get_version_id(bucket, f"{rv[ind]['vdbindex_prefix']}/{rv[ind]['vdbindex_name']}.gz", "aggregate.populate_all_names")
            rv[ind]['data_file'] = urllib.parse.quote(psurl)
        except Exception as ex:
            print(f"Caught {ex} while getting presigned_url")
