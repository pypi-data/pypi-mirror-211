...
def populate_all_names(bucket, prefix, infinstor_time_spec, resources, max_size_allowed=None, max_time_allowed=None) -> bool:
    ...
    for fn in db_files_to_upload.keys():
        ...
        os.remove(full_fn)
        version_id, psurl = s3client.get_version_id(bucket, object_name, 'aggregate.populate_all_names')
        print(f"populate_all_names: bucket={bucket}, obj={object_name}, psurl={psurl}", flush=True)
        rv.append({'data_file': psurl,
