    ...
    def run(self, layer):
        ...
        if self._submitter.get_attribute("use_custom_frames").get_bool():
            ...
            for image in images:
                ...
                if not(isect) or (list(isect) != list(frame_seq)):
                    self.add_warning("Not all frames from the image `{}` are contained in the custom sequence.".format( image.get_full_name()))
...
class ValidateUploadDaemon(Validator):
    ...
    def run(self, layer): 
        ...
        msg += "* Install Conductor Core from the Plugins page in the Companion app.\n"
        if utils.is_windows():
...
class ValidateMissingAssets(Validator):
    ...
    def run(self, layer):
        ...
        for gpath in deps.collect(self._submitter, do_glob=False):
            ...
            # we can assunme that all globbable paths, (those containing glob characters) cannot
            # possibly represent missing files because they are literally resolved  by checking what
            # matches on disk. Therfore, we ignore them for thew missing assets test.
...
def run(node):
    if node.get_attribute("disable_validation").get_bool():
        ix.log_info("Validation disabled for {}".format(node.get_name()))
        return
...
def _run_validators(node):
    ...
    for validator in validators:
        msg = "Running Validator: {}".format(validator.__class__.__name__)
        ix.log_info(msg)
        try:
            validator.run("main")
        except BaseException as ex:
            meta_warnings.add(
                "[{}]:\nThis validator failed to run. Don't panic, it's probably due to an unsupported feature and can be ignored. If you are worried, see the stack trace in the script editor.\n{}".format(
                    validator.title(), str(ex)
                )
            )
            ix.log_error(traceback.format_exc())

    errors = list(set.union(*[validator.errors for validator in validators]))
    warnings = list(
        set.union(*[validator.warnings for validator in validators])) + list(
        meta_warnings
    )
