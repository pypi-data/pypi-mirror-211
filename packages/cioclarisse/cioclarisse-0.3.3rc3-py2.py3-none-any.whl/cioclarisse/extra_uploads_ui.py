    ...
    def add_entries(self, entries):
        ...
        for item in self.item_list:
            try:
                ...
                msg = "{} - while resolving {}".format(str(ex), item.get_name())
        ...
        for entry in entries:
            try:
                ...
                msg = "{} - while resolving {}".format(str(ex), entry)
                raise ConductorError(msg)
        ...
        for entry in deduped:
            item = ix.api.GuiTreeItemBasic(root_item, entry.fslash())
...
class ExtraUploadsWindow(ix.api.GuiWindow):
    ...
    def __init__(self, node):
        ...
        self.browse_but = ix.api.GuiPushButton(
            self, (WIDTH - (BTN_WIDTH*2)), current_y, BTN_WIDTH, BTN_HEIGHT, "Browse files"
        )
        self.connect(self.browse_but, "EVT_ID_PUSH_BUTTON_CLICK", self.on_browse_but)

        self.browse_folder_but = ix.api.GuiPushButton(
            self, (WIDTH - BTN_WIDTH), current_y, BTN_WIDTH, BTN_HEIGHT, "Browse folder"
        )
        self.connect(self.browse_folder_but, "EVT_ID_PUSH_BUTTON_CLICK", self.on_browse_folder_but)
    ...
    def on_browse_folder_but(self, sender, eventid):
        """
        User may browse for files.
        """
        app = ix.application
        title = "Select folder to upload..."
        foldername = ix.api.GuiWidget.open_folder(app, "", title)
        self.file_list_wdg.add_entries([foldername])
