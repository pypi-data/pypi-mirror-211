...
#define FIBONACCI_H
  ...
  class FibonacciMesh
  {
    ...
    public:
      ...
      DVector     HPara, VPara, HPerp, VPerp;
      double      MaxAngle, phi_offset, gamma_offset, dOmega, Omega;

      VectorField VVector = VectorField({1,0,0}),
  ...
  void get_properties()
  {
    double SolidAngle   = abs( 2. * PI * ( cos(MaxAngle) - 1. ) ),   //cos(0) =1
