...
def times_print(sim_moments, calc_moments, is_w=True):
    ...
    for j in range(min(len(sim_moments), len(calc_moments))):
        print(
            Fore.CYAN + f"{j + 1:^15d}" + Fore.YELLOW + "|" + f"{calc_moments[j]:^15.5g}" + Fore.CYAN + "|" + Fore.YELLOW + f"{sim_moments[i]:^15.5g}")
...
def probs_print(p_sim, p_ch, size=10):
    ...
    for i in range(size):
        print(
            Fore.CYAN + f"{i:^4d}" + Fore.YELLOW + "|" + Fore.CYAN + f"{p_ch[i]:^15.5g}" + Fore.CYAN + "|" + Fore.YELLOW + f"{p_sim[i]:^15.5g}")
    print(Fore.CYAN + "-" * 36)
