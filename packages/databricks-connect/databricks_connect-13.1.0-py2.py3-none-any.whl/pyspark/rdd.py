...
from pyspark.databricks.deprecated_func_usage_logger import DeprecatedFuncUsageLogger
...
class RDD(Generic[T_co]):
    ...
    def mapPartitionsWithSplit(
        ...
        6
        """
        DeprecatedFuncUsageLogger().log_deprecated(
    ...
    def collect(self: "RDD[T]") -> List[T]:
        ...
        ['x', 'y', 'z']
        """
        if self.ctx._conf.get(self.ctx._jvm.PythonSecurityUtils.USE_FILE_BASED_COLLECT()):
            # This file-based collect path is used for Python DataFrame ACLs.
            # See SC-8571 for design discussion.
            with SCCallSiteSync(self.context):
                import pwd  # Do this lazily since it's available on Linux only

                username = pwd.getpwuid(os.getuid()).pw_name
                filename = self.ctx._jvm.PythonRDD.collectToPythonFile(self._jrdd.rdd(), username)
            try:
                deserializer = self._jrdd_deserializer
                with open(filename, "rb") as f:
                    return list(deserializer.load_stream(f))
            finally:
                os.remove(filename)
        # Default path used in OSS Spark / for non-credential passthrough clusters:
        with SCCallSiteSync(self.context):
    ...
    def collectWithJobGroup(
        ...
        if self.ctx._conf.get(self.ctx._jvm.PythonSecurityUtils.USE_FILE_BASED_COLLECT()):
            ...
            with SCCallSiteSync(self.context):
                ...
                username = pwd.getpwuid(os.getuid()).pw_name
                filename = self.ctx._jvm.PythonRDD.collectToPythonFileWithJobGroup(
                    self._jrdd.rdd(), username, groupId, description, interruptOnCancel
                )
            try:
                serializer = self._jrdd_deserializer
                with open(filename, "rb") as f:
                    return list(serializer.load_stream(f))
            finally:
                os.remove(filename)
        # Default path used in OSS Spark / for non-credential passthrough clusters:
        with SCCallSiteSync(self.context):
    ...
    def saveAsPickleFile(self, path: str, batchSize: int = 10) -> None:
        ...
        reserialized = self._reserialize(ser)
        self.ctx._jvm.PythonRDD.saveAsPickleFile(reserialized._jrdd, path)
    ...
    def saveAsTextFile(self, path: str, compressionCodecClass: Optional[str] = None) -> None:
        ...
        # We must pull this out into two separate if branches because Py4J cannot translate from
        # Python None --> Java null
        if compressionCodecClass:
            self.ctx._jvm.PythonRDD.saveAsTextFileImpl(keyed._jrdd, path, compressionCodecClass)
        else:
            self.ctx._jvm.PythonRDD.saveAsTextFileImpl(keyed._jrdd, path)
...
def _get_workspace_sys_paths() -> List[str]:
    workspace_sys_paths = []
    for path in sys.path:
        if path == "/Workspace" or path.startswith("/Workspace/"):
            workspace_sys_paths.append(path)

    return workspace_sys_paths
...
def _prepare_for_python_RDD(sc: "SparkContext", command: Any) -> Tuple[bytes, Any, Any, Any]:
    ...
    return (
        ...
        broadcast_vars,
        sc.environment,
        sc._python_includes + _get_workspace_sys_paths(),
    )
