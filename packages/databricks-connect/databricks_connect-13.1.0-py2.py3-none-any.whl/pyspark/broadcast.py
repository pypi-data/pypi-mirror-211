...
class Broadcast(Generic[T]):
    ...
    def __init__(
        ...
        if sc is not None:
            ...
            # With process isolation enabled, the user should not be able to read arbitrary files,
            # so we separate the logic to obtain a temporary directory using trusted path.
            if sc._conf.get(
                sc._jvm.PythonSecurityUtils.SPARK_PYTHON_TO_JVM_TRUSTED_DATA_TRANSFER_ENABLED()
            ):
                self.init_with_process_isolation(sc, value, pickle_registry)
                return
            ...
            self._python_broadcast = sc._jvm.PythonRDD.setupBroadcast(sc._jsc, self._path)
    ...
    def init_with_process_isolation(self, sc, value, pickle_registry):
        """
        ...
        # With process isolation enabled, we obtain a directory using trusted path
        temp_dir = session._jsparkSession.getTrustedPath()
        broadcast_out = NamedTemporaryFile(delete=False, dir=temp_dir)
        self._path = broadcast_out.name

        # No encryption, we can just write pickled data directly to the file from python
        self.dump(value, broadcast_out)

        self._jbroadcast = session._jsparkSession.createBroadcastVariableFromTrustedPath(
            broadcast_out.name, self._sc._jsc
        )
        self._python_broadcast = self._jbroadcast.value()
