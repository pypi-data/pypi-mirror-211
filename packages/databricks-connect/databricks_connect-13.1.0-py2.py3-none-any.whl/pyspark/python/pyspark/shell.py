...
# Get SPARK_REMOTE
if not os.getenv("SPARK_REMOTE"):
    import argparse
    parser = argparse.ArgumentParser(
        prog="pyspark"
    )
    parser.add_argument("--remote", default="sc://localhost")
    submit_args = os.getenv("PYTHON_SUBMIT_ARGS", "")
    args = parser.parse_args(submit_args.split())

    if "SPARK_REMOTE" not in os.environ:
        os.environ["SPARK_REMOTE"] = args.remote
