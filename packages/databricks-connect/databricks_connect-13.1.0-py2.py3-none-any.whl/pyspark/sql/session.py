...
class SparkSession(SparkConversionMixin):
    ...
    class Builder:
        ...
        def getOrCreate(self) -> "SparkSession":
            ...
            with self._lock:
                if ("SPARK_REMOTE" in os.environ or "spark.remote" in opts) and not hasattr(
                    self, "_sc"  # Edge
                ):
                ...
                raise RuntimeError(
                    ...
                    "Could not find connection parameters to start a Spark remote session."
                )
                ...
                if session is None or session._sc._jsc is None:
                    ...
                    # BEGIN-EDGE
                    sc = (
                        self._sc  # Set in context.py, see also SC-21535.
    ...
    def _createFromLocal(
        ...
        This would be broken with table acl enabled as user process does not have permission to
        write temp files.
        """
        internal_data, struct = self._wrap_data_schema(data, schema)
        return self._sc.parallelize(internal_data), struct

    def _write_to_trusted_path(self, data, serializer):
        """
        Get a trusted path from the JVM, write the given Python collection to that path, and return
        the path. Can be followed by a call to
        self._jsparkSession.create[RDD,DataFrame]FromTrustedPath to return a Spark data structure
        with the given data.
        """
        # Get a temp dir python can write to
        temp_dir = self._jsparkSession.getTrustedPath()
        # Create the temp file to dump data, this file would be deleted by the createDF call
        temp_file = NamedTemporaryFile(delete=False, dir=temp_dir)

        # Serialize to the trusted path, still reclaim the handler if exception happens in dump
        try:
            serializer.dump_stream(data, temp_file)
        finally:
            temp_file.close()

        return temp_file.name

    def _createFromLocalTrusted(self, data, schema):
        """
        Create a DataFrame from a list or pandas.DataFrame through trusted file path, returns
        the DataFrame and schema. This mimic the workflow we have for self._sc.parallelize but in a
        secure way which would still work when table acl is enabled.
        """
        data, schema = self._wrap_data_schema(data, schema)
        # create the serializer as we do in self._sc.parallelize
        serializer = self._sc.init_batched_serializer(data)
        temp_data_path = self._write_to_trusted_path(data, serializer)

        return (
            self._jsparkSession.createDataFrameFromTrustedPath(temp_data_path, schema.json()),
            schema,
        )

    def _create_rdd_from_local_trusted(self, data, numSlices, serializer):
        """
        Create and return an RDD from a list through a trusted file. This will work when process
        isolation is enabled.
        """
        temp_data_path = self._write_to_trusted_path(data, serializer)
        return self._jsparkSession.createRDDFromTrustedPath(temp_data_path, numSlices)
    ...
    def _create_dataframe(
        ...
        assert self._jvm is not None
        if self._sc._conf.get(  # type: ignore[attr-defined]
