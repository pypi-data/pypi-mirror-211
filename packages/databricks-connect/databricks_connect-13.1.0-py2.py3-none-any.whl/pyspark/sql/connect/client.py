...
class SparkConnectClient(object):
    ...
    def retry_exception(cls, e: grpc.RpcError) -> bool:
        ...
        ----------
        e : grpc.RpcError
            The GRPC error as received from the server.
        ...
        if not isinstance(e, grpc.RpcError):
            return False
        msg = str(e)
        if e.code() in [grpc.StatusCode.INTERNAL, grpc.StatusCode.ABORTED]:
            # There are certain cases where the control plane will return an INTERNAL or ABORTED
            # error whereas the exception is actually retriable. This is a workaround until the
            # control plane will return the correct exceptions.
            msg_pending_state = "is in unexpected state Pending"
            msg_single_user_policy = "RESOURCE_LIMIT_EXCEEDED: Only 1 cluster(s)"
            if any(map(lambda x: x in msg, [msg_single_user_policy, msg_pending_state])):
                return True

        if e.code() == grpc.StatusCode.UNAVAILABLE:
            # This is a terminal error when the client cannot connect to any endpoint and thus
            # cannot be retried.
            msg_connection_failed = "failed to connect to all addresses"
            if msg_connection_failed in msg:
                return False
            else:
                return True
        else:
    ...
    def __init__(
        ...
        if self._builder.userId is not None:
            ...
            # BEGIN-EDGE
            # If no environment variable is set, simply set a default value.
            # In Databricks the value is overriden by the service handler.
        ...
        self._heartbeat = Heartbeat(
            self._channel,
            self._session_id,
            self._analyze_plan_request_with_metadata,
                    ...
                    with attempt:
                        for b in self._stub.ExecutePlan(req, metadata=self._builder.metadata()):
                            ...
                            if b.HasField("streaming_query_command_result"):
                                yield {
                                    "streaming_query_command_result": b.streaming_query_command_result  # noqa: E501
                                }
                            ...
                            if b.HasField("get_resources_command_result"):
                                ...
                                for (
                                    key,
                                    resource,
                                ) in b.get_resources_command_result.resources.items():
                                    name = resource.name
    ...
    def __iter__(self) -> Generator[AttemptManager, None, None]:
        ...
        while True:
            ...
            if retry_state.count() > 0:
                ...
                logger.debug(f"Retrying call after {backoff} ms sleep - {retry_state.exception()}")
...
class Heartbeat:
    """
    The purpose of this class is to avoid the TCP timeout of the
    front-end load balancers of the cloud providers.

    The class sends an AnalyzeRequest for the most simple plan
    so that connection stays alive.
    """
    ...
    def __init__(
        ...
        session_id: str,
        analyze_plan_request: Callable[[], pb2.AnalyzePlanRequest],
        request_metadata: Callable[[], Iterable[Tuple[str, str]]],
        error_handler: Callable[[grpc.RpcError], None],
        retry_policy: Optional[Dict[str, Any]] = None,
    ):
        self._channel_closed = False
        self._session_id = session_id
        self._analyze_plan_request = analyze_plan_request
        self._request_metadata = request_metadata
        self._error_handler = error_handler
        self._retry_policy = retry_policy

        # Ping stub is used to send periodic cheap messages over the channel.
        self._ping_stub = grpc_lib.SparkConnectServiceStub(channel)
        self._ping_lock = threading.Lock()
        with self._ping_lock:
            self._ping_active = 0

        self.ping_count = 0
        self._ping_thread = threading.Thread(target=self._ping_handler)
        self._ping_thread.setDaemon(True)
        self._ping_thread.start()

    def _ping_handler(self) -> None:
        """
        Internal handler function that is executed in a separate thread to avoid
        closing the TCP connection to the server.
        """
        logger.debug("Starting Ping Thread.")
        while not self._channel_closed:
            if self._ping_active > 0:
                self._execute_ping()
            time.sleep(Heartbeat.PING_TIMEOUT)

    def _execute_ping(self) -> None:
        """
        Send a nonce request to the server extracting the schema for a range(0,2,1) query.
        To avoid executing this as a full query, this is sent via the Analyze RPC.

        The request is executed on a separate stub to avoid overlap with the
        main request execution.
        """
        logger.debug("Execute ping request...")
        self.ping_count += 1
        req = self._analyze_plan_request()
        plan = pb2.Plan()
        plan.root.range.start = 0
        plan.root.range.step = 1
        plan.root.range.end = 2
        req.schema.plan.CopyFrom(plan)
        try:
            for attempt in Retrying(
                can_retry=SparkConnectClient.retry_exception, **self._retry_policy
            ):
                with attempt:
                    # Using the ping stub
                    resp = self._ping_stub.AnalyzePlan(req, metadata=self._request_metadata())
                    if resp.session_id != self._session_id:
                        raise SparkConnectException(
                            "Received incorrect session identifier for request:"
                            f"{resp.session_id} != {self._session_id}"
    ...
    def __enter__(self) -> None:
        """
        Called when the caller enters a potentially long-running block and needs to send keep-alive
        messages.
        """
        with self._ping_lock:
            self._ping_active += 1
    ...
    def __exit__(
        ...
        """
        Called when the caller leaves the long-running block.
        """
        with self._ping_lock:
            self._ping_active -= 1
            assert self._ping_active >= 0
