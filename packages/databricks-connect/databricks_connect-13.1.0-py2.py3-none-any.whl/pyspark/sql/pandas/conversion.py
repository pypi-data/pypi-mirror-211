...
class PandasConversionMixin:
        ...
        if jconf.arrowPySparkEnabled():
            ...
            try:
                ...
                if jconf.arrowPySparkFallbackEnabled():
                    msg = (
                        ...
                        "true." % message_with_direct_cause(e)
                    )
                    warn(msg)
                    use_arrow = False
                else:
            ...
            if use_arrow:
                try:
                    ...
                    from pyspark.sql.pandas.utils import warn_inefficient_columns_for_conversion
                    import pyarrow

                    warn_inefficient_columns_for_conversion(self.schema)
    ...
    def _collect_as_arrow(self, split_batches: bool = False) -> List["pa.RecordBatch"]:
        ...
        # BEGIN-EDGE
        should_use_filebase = self._sc._conf.get(
            self._sc._jvm.PythonSecurityUtils.USE_FILE_BASED_COLLECT()
        )

        batch_stream = None
        jsocket_auth_server = None
        if not should_use_filebase:
            # END-EDGE
            ...
            with SCCallSiteSync(self._sc):
                ...
                filename = self._jdf.collectAsArrowToPythonFile(username)
            try:
                serializer = ArrowCollectSerializer()
                with open(filename, "rb") as f:
                    batch_stream = list(serializer.load_stream(f))
            finally:
                os.remove(filename)
            # END-EDGE
        ...
        try:
            if not should_use_filebase:  # Edge
            ...
            if jsocket_auth_server is not None:  # Edge
...
class SparkConversionMixin:
    ...
    def _create_from_pandas_with_arrow(
        ...
        from pyspark.sql.pandas.types import from_arrow_type, to_arrow_schema, to_arrow_type
        from pyspark.sql.pandas.utils import (
            require_minimum_pandas_version,
            require_minimum_pyarrow_version,
            warn_inefficient_columns_for_conversion,
        )
        ...
        if isinstance(schema, (list, tuple)):
            ...
            for name, field in zip(schema, arrow_schema):
                try:
                    arrow_type = from_arrow_type(field.type, prefer_timestamp_ntz)
                except TypeError as e:
                    raise TypeError(
                        "Unable to convert the field {}. If this column is not "
                        "necessary, you may consider dropping it or converting "
                        "to primitive type before the conversion.".format(name)
                    ) from e
                struct.add(name, arrow_type, nullable=field.nullable)
            schema = struct

        if len(pdf) > 1000000:
            warn_inefficient_columns_for_conversion(schema)

        # Determine arrow types to coerce data when creating batches
        if isinstance(schema, StructType):
            arrow_types = to_arrow_schema(schema).types
