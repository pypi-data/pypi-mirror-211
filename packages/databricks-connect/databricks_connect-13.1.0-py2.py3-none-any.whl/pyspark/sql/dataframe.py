...
class DataFrame(PandasMapOpsMixin, PandasConversionMixin):
    ...
    def registerTempTable(self, name: str) -> None:
        ...
        DeprecatedFuncUsageLogger().log_deprecated(
            "def registerTempTable(self, name)", "python/pyspark/sql/dataframe.py"
        )
    ...
    def collect(self) -> List[Row]:
        ...
        if self._sc._conf.get(self._sc._jvm.PythonSecurityUtils.USE_FILE_BASED_COLLECT()):
            ...
            with SCCallSiteSync(self._sc):
                ...
                filename = self._jdf.collectToPythonFile(username)
            try:
                serializer = BatchedSerializer(CPickleSerializer())
                with open(filename, "rb") as f:
                    return list(serializer.load_stream(f))
            finally:
                os.remove(filename)
        # Default path used in OSS Spark / for non-DF-ACL clusters:
        with SCCallSiteSync(self._sc):
    ...
    def toLocalIterator(self, prefetchPartitions: bool = False) -> Iterator[Row]:
        ...
        """
        if self._sc._conf.get(self._sc._jvm.PythonSecurityUtils.USE_FILE_BASED_COLLECT()):
            raise Exception("toLocalIterator() is not supported when using file-based collect")
        with SCCallSiteSync(self._sc):
    ...
    def tail(self, num: int) -> List[Row]:
        ...
        if self._sc._conf.get(self._sc._jvm.PythonSecurityUtils.USE_FILE_BASED_COLLECT()):
            with SCCallSiteSync(self._sc):
                ...
                username = pwd.getpwuid(os.getuid()).pw_name
                filename = self._jdf.tailToPythonFile(num, username)
            try:
                serializer = BatchedSerializer(CPickleSerializer())
                with open(filename, "rb") as f:
                    return list(serializer.load_stream(f))
