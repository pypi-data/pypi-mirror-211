...
def sql_conf(pairs: Dict[str, Any], *, spark: Optional[SparkSession] = None) -> Iterator[None]:
    ...
    old_values = [spark.conf.get(key) for key in keys]
...
def log_advice(message: str) -> None:
    ...
    """
    # BEGIN-EDGE
    # Note that in this function, we issue the advice message to the Databricks notebook
    # rather than raise UserWarning. Because the Databricks Advisor is only supported in DBR,
    # the behavior is different from OSS side.
    # See also SC-86714 and SPARK-37036, which are DBR and OSS implementations respectively.
    sc = SparkContext._active_spark_context  # type: ignore[attr-defined]
    advice_generator = sc._jvm.com.databricks.sql.advice.PandasOnSparkAdviceGenerator
    advice_generator.generateAdvice(message)
    # END-EDGE
