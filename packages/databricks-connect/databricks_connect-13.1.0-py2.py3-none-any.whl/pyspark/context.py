...
from pyspark.util import _normalize_path_for_spark_client_on_windows
...
class SparkContext:
    ...
    # BEGIN-EDGE
    _after_init_hooks: ClassVar[Optional[List[Callable]]] = []
    # END-EDGE
    _lock = RLock()
    ...
    def __init__(
        ...
        try:
            ...
            raise
        # BEGIN-EDGE
        SparkContext._invoke_after_init_hooks()
        # END-EDGE
    ...
    def _do_init(
        ...
        #
        # socket based data transfer won't work when process isolation is enabled, we have
        # different mechanism to protect data from the open source solution.
        self._encryption_enabled = self._jvm.PythonUtils.isEncryptionEnabled(
            self._jsc
        ) and not self._conf.get(
        ...
        self.setLocalProperty("spark.databricks.service.client.type", "python")
    ...
    # BEGIN-EDGE
    @classmethod
    def _add_after_init_hook(
        cls,
        after_init_hook: Callable,
    ) -> None:
        with SparkContext._lock:
            if SparkContext._after_init_hooks is not None:
                SparkContext._after_init_hooks.append(after_init_hook)
                return

        # SparkContext has been initialized since SparkContext._after_init_hooks was None so calling
        # _after_init_hooks directly
        after_init_hook()

    @classmethod
    def _invoke_after_init_hooks(
        cls,
    ) -> None:
        _after_init_hooks = []
        with SparkContext._lock:
            _after_init_hooks = SparkContext._after_init_hooks
            SparkContext._after_init_hooks = None

        # It is possible that more than one SparkContexts are initialized. So
        # check whether _after_init_hooks is None
        if _after_init_hooks is not None:
            for hook in _after_init_hooks:
                hook()
    ...
    def stop(self) -> None:
        ...
        with SparkContext._lock:
            ...
            SparkContext._after_init_hooks = []
    ...
    def init_batched_serializer(
        self, data: Iterable[T], numSlices: Optional[int] = None
    ) -> "BatchedSerializer":
        """
        ...
        batchSize = max(
            1, min(len(data) // numSlices, self._batchSize or 1024)  # type: ignore[arg-type]
        )
    ...
    def parallelize(self, c: Iterable[T], numSlices: Optional[int] = None) -> RDD[T]:
        ...
        serializer = self.init_batched_serializer(c, numSlices)

        if self._conf.get(
            self._jvm.PythonSecurityUtils.SPARK_PYTHON_TO_JVM_TRUSTED_DATA_TRANSFER_ENABLED()
        ):
            # This is a bit of a hack: we call into the SparkSession API from the SparkContext API,
            # since the SparkSession is where the JVM knows the Python username
            from pyspark.sql.session import SparkSession

            # SC-21535: Here we need to do a getOrCreate to make the tests happy. In databricks's
            # env, spark session should always be available here.
            with SparkContext._lock:
                try:
                    SparkSession.Builder._sc = self
                    session = SparkSession.builder.getOrCreate()
                    jrdd = session._create_rdd_from_local_trusted(c, numSlices, serializer)
                finally:
                    del SparkSession.Builder._sc
        else:
    ...
    def textFile(
        ...
        path = _normalize_path_for_spark_client_on_windows(self, name)
        return RDD(self._jsc.textFile(path, minPartitions), self, UTF8Deserializer(use_unicode))
    ...
    def wholeTextFiles(
        ...
        """
        minPartitions = minPartitions or self.defaultMinPartitions
        path = _normalize_path_for_spark_client_on_windows(self, path)
        return RDD(
    ...
    def addFile(self, path: str, recursive: bool = False) -> None:
        ...
        [100, 200, 300, 400]
        """
        self._jsc.addFile(path, recursive)
    ...
    def addPyFile(self, path: str) -> None:
        ...
        try:
            ...
            # Library installation in Python happens in 2 steps:
            # 1. The library file is downloaded onto the machine
            # 2. The path to the downloaded library file is added to the Python process's sys.path
            #
            # The call to addFile() performs step 1, and will fail for clusters with Table ACLs
            # enabled since it calls into a JVM-side method that has not been whitelisted. This
            # prevents non-admin users of Table ACLs clusters from installing arbitrary libraries.
            #
            # If this code path were only called when users attempted to manually add libraries to
            # Pyspark, we would not need to catch Py4JErrors here. However, we also call this code
            # path when we are installing libraries that Databricks users have attached to the
            # cluster. We ignore the Py4JError here because we still need to perform step 2 (below)
            # for these libraries so that the executors can import them. For these libraries we
            # already perform step 1 from the Scala side, so it's fine that the call to addFile()
            # fails.
            pass
        (dirname, filename) = os.path.split(path)  # dirname may be directory or HDFS/S3 prefix
        self.addClusterWideLibraryToPath(filename)
    ...
    def addClusterWideLibraryToPath(self, libname: str) -> None:
        if libname[-4:].lower() in self.PACKAGE_EXTENSIONS:
            path = os.path.join(SparkFiles.getRootDirectory(), libname)
            if path not in sys.path:  # if the path is already added, skip
                isolation_prefix = (
                    self._jvm.org.apache.spark.util.DatabricksUtils.ISOLATED_LIBRARY_PREFIX()
                )
                virtualenv_root_dir = (
                    self._jvm.org.apache.spark.util.DatabricksUtils.PYTHON_VIRTUALENV_ROOTDIR_NAME()
                )
                # set system path for python library (.egg, .zip, and .jar) is necessary
                # library added through addPyFile is considered to be a cluster wide library,
                # therefore, it needs to be added after all the isolated library and the
                # virtualenv site-package (if a virtualenv is created). This is done by
                # traverse the system path in a reverse order, and stop at the first one that
                # is path for the isolated library or directory under the virtualenv dir.
                path_index = 1
                for i, cpath in reversed(list(enumerate(sys.path))):
                    (tmpdir, tmpfile) = os.path.split(cpath)
                    if tmpfile.startswith(isolation_prefix) or (virtualenv_root_dir in tmpdir):
                        path_index = i + 1
                        break
                sys.path.insert(path_index, path)
                # add file to the python includes for worker. Later, in worker.py, it will be
                # inserted it in the correct position of the system path.
                assert self._python_includes is not None
                self._python_includes.append(libname)
        importlib.invalidate_caches()

    # add isolated library to the system path. This is called for libraries installed through
    # dbutils.libraries.install().
    def addIsolatedLibraryPath(self, libname: str) -> None:
        """
        ...
        if libname[-4:].lower() in self.PACKAGE_EXTENSIONS:
            path = os.path.join(SparkFiles.getRootDirectory(), libname)
            if path not in sys.path:  # if the path already exists, skip
                # Add notebook scoped library path at the front of the sys.path, so it has the
                # higher priority
                assert self._python_includes is not None
                self._python_includes.append(libname)
    ...
    def setCheckpointDir(self, dirName: str) -> None:
        ...
        """
        self._jsc.setCheckpointDir(dirName)
    ...
    def getJobGroupId(self) -> Optional[str]:
        """
        Get a job group ID in this thread, or `None` if it is missing.
        """
        job_group_id = self._jsc.sc().getJobGroupId()
        return job_group_id.get() if job_group_id.isDefined() else None
    ...
    def runJob(
        ...
        assert self._jvm is not None
        # Follow the method implemented for rdd.collect() to use the file-based data transfer.
        if self._conf.get(self._jvm.PythonSecurityUtils.USE_FILE_BASED_COLLECT()):
            from pyspark.traceback_utils import SCCallSiteSync

            with SCCallSiteSync(self):
                import pwd  # Do this lazily since it's available on Linux only

                username = pwd.getpwuid(os.getuid()).pw_name
                filename = self._jvm.PythonRDD.runJobToPythonFile(  # type: ignore[attr-defined]
                    self._jsc.sc(), mappedRDD._jrdd, partitions, username
                )
            try:
                deserializer = mappedRDD._jrdd_deserializer  # type: ignore[attr-defined]
                with open(filename, "rb") as f:
                    return list(deserializer.load_stream(f))
            finally:
                os.remove(filename)
