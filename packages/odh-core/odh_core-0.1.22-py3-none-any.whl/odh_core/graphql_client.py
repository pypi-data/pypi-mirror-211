...
class GraphQLClient:
    ...
    async def lock_acquire_async(
        self, lock_name: str, ttl_seconds: int = 30, await_release: bool = False
    ):
        """
        ...
        while True:
            ...
            lock = await self._client.execute_async(query, variable_values=variables)
    ...
    async def lock_refresh_async(self, lock_name: str, ttl_seconds: int = 30):
        """
        Create a lock in the Hasura DB, lock will release after TTL expires
        or when release is called
        """
        ...
        variables = {"id": lock_name}
        lock = await self._client.execute_async(query, variable_values=variables)
        if lock["update_lock_table_by_pk"]["id"] == lock_name:
            return True
        else:
            return False
