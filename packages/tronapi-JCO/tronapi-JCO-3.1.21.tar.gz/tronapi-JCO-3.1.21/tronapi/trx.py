...
from trx_utils.types import is_object, is_string, is_list, is_boolean
...
class Trx(Module):
    ...
    def get_can_withdraw_unfreeze_amount(self, account=None):
        ...
        transaction = self.tron.transaction_builder.get_can_withdraw_unfreeze_amount(
            account
        )
        sign = self.sign(transaction)
        response = self.broadcast(sign)
    ...
    def get_can_delegated_max_size(
        self, owner_address=None, resource="BANDWIDTH", visible=True
    ):
        """
        Get maximum amount of available resource to delegate.

        Args:
            owner_address (int): number of frozen trx
            resource (str): type of resource, must be either "ENERGY" or "BANDWIDTH"
            visible (str): Whether the address is in base58 format
        ...
        if not is_boolean(visible):
            raise InvalidTronError("Invalid visible provided")

        response =  self.tron.manager.request(
            "/wallet/getcandelegatedmaxsize",
            {
                "owner_address": self.tron.address.to_hex(owner_address),
                "type": 1 if resource == "ENERGY" else 0 if resource == "BANDWIDTH" else None,
                "visible": visible,
            },
        )

        return {"max_size": self.tron.fromSun(response["max_size"])}
    ...
    def get_delegated_resource(
        self, from_address=None, to_address=None, visible=True
    ):
        """
        Get maximum amount of available resource to delegate.

        Args:
            from_address (str): owner address
        ...
        if not self.tron.isAddress(from_address):
            raise InvalidTronError("Invalid from_address provided")

        if not self.tron.isAddress(to_address):
            raise InvalidTronError("Invalid to_address provided")
        ...
        response =  self.tron.manager.request(
            ...
            {
                "from_address": self.tron.address.to_hex(from_address),
                "to_address": self.tron.address.to_hex(to_address),
                "visible": visible,
            },
        )
    ...
    def get_bandwidth_prices(self):
        """Get Energy Prices."""

        return self.tron.manager.request("/wallet/getbandwidthprices")
