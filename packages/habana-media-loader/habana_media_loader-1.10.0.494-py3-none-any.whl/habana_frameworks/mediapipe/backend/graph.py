...
class c_hpu_node(MediaCPUNode):
    ...
    def fetch_input_buffers(self):
        ...
        if(self.decoder_op != None):
            ...
            elif isinstance(self.decoder_op, video_decoder):

                if(len(self.decoder_op.input_tensors) > 2):
                    vid_offset = self.decoder_op.input_tensors[1].data_read()
                    rand_crp = self.decoder_op.input_tensors[2].data_read()

                elif(len(self.decoder_op.input_tensors) > 1):
                    vid_offset = self.decoder_op.input_tensors[1].data_read()

                else:
                    raise RuntimeError(
                        "missing frame offset for video decode")
        ...
        for f in self.rpt_buf_funcs:
            f()
        return file_list, rand_crp, crop_after_resize, vid_offset, var_np_buf, ngop_np_buf

    def push_output_buffers(self, outputs):
        pass

    def __call__(self, file_list, rand_crp, is_crp_after_resize, vid_offset, var_np_buf, ngop_np_buf):
        self.run_hpu(file_list, rand_crp, is_crp_after_resize,
                     vid_offset, var_np_buf, ngop_np_buf)
        return []
