...
def install(path):
    ...
    with cd(BASIC_PATH):
        ...
        proc = subprocess.Popen(['php', '-d', 'memory_limit=4G', '/usr/local/bin/composer', 'require', '--prefer-dist',
...
def unit(report, path, report_file):
    ...
    print(f"Extension installed to {result_path}")
    time.sleep(3)
    di_compile()
    print(f"di:compile is successful")
    time.sleep(3)
    ...
    proc = subprocess.Popen(options + [_(result_path, 'Test/Unit')], stdout = subprocess.PIPE, stderr = subprocess.STDOUT)
    print(proc)
