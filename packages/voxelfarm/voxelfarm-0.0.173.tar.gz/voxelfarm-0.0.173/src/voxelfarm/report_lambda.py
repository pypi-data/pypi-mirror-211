...
class voxel_component_property:
    Merge = "voxelfarm_merge"
    Dataframe = "voxelfarm_dataframe"
        ...
        if not df.empty:
            if column:
                if column in df:
                    ...
                    return df.apply(lambda row : self.get_enum_attribute_value(metadata_component, metadata_column, int(row[column])-1), axis = 1)
            else:            
                metadata_column = 'mesh_name'
                return df.apply(lambda row : self.get_enum_attribute_value(metadata_component, metadata_column, int(row.name)-1), axis = 1)
