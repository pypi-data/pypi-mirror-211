...
def response2db(response, table_name, chat=True):
    content = generate_response(response, chat=chat)
    do_db(pd.DataFrame(content), table_name)
    if os.getenv('DEBUG'): logger.success(content)
...
async def chat_completions(body: ChatBody, request: Request, background_tasks: BackgroundTasks):
    ...
    if body.stream:
        async def eval_llm():
            ...
            if os.getenv('DEBUG'): logger.success(content)  # 日志
