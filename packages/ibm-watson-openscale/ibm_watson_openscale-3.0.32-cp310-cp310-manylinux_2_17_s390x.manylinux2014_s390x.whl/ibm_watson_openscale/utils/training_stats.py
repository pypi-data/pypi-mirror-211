...
from ibm_watson_openscale.utils import check_package_exists, get
...
class TrainingStats():
    ...
    def __init__(self, training_data_frame, training_data_info, drop_na=True, **kwargs):
        ...
        self.compute_explain = self.training_data_info.get(
            "enable_explainability")

        if self.compute_explain is None:
            self.compute_explain = kwargs.get("explain", True)

        self.compute_fairness = self.training_data_info.get("enable_fairness")

        if self.compute_fairness is None:
            self.compute_fairness = kwargs.get("fairness", True)
    ...
    def __validate_training_data_info(self):
        ...
        self.prediction = self.training_data_info.get("prediction_column")
        if self.prediction is None:
            self.prediction = self.training_data_info.get("prediction")
        self.probability = self.training_data_info.get("probability_column")
        self.probability = self.training_data_info.get("probability") if \
            not self.probability else self.probability
        self.class_probabilities = self.training_data_info.get("class_probabilities")

        if self.problem_type != "regression" and \
            not self.probability and \
        ...
        if self.compute_fairness:
            ...
            self.fairness_inputs = self.training_data_info.get(
                "fairness_inputs")
            if self.fairness_inputs is None:
                raise Exception(
                    "'fairness_inputs' attribute is missing in the input")
        ...
        if self.problem_type != "regression":
            ...
            if (favourable_class is not None):
                if not (isinstance(favourable_class, (list, tuple))):
                    raise Exception(
                        "'favourable_class' in fairness_inputs must be a list of values")
                ...
                raise Exception(
                    "'favourable_class' attribute is required in fairness_inputs")
            ...
            if (unfavourable_class is not None):
                if not (isinstance(unfavourable_class, (list, tuple))):
                    raise Exception(
                        "'unfavourable_class' in fairness_inputs must be a list of values")
                ...
                raise Exception(
                    "'unfavourable_class' attribute is required in fairness_inputs")
            ...
            if (favourable_class is None):
                ...
                if not (isinstance(favourable_class[0], (list, tuple))):
                    raise Exception(
                        "'favourable class' in parameters must be a list of lists")
            ...
            if (unfavourable_class is None):
                ...
                if not (isinstance(unfavourable_class[0], (list, tuple))):
                    raise Exception(
                        "'unfavourable class' in parameters must be a list of lists")

        # Input validations
        fairness_attributes = self.fairness_inputs.get("fairness_attributes")
        if fairness_attributes is None:
            raise Exception(
                "'fairness_attributes' attribute is missing under fairness_inputs")
        for fea in fairness_attributes:
            if "feature" not in fea:
                raise Exception(
                    "'feature' attributes is missing in 'fairness_attributes' input")
            if "majority" not in fea:
                raise Exception(
                    "'majority' attributes is missing in 'fairness_attributes' input")
            if "minority" not in fea:
                raise Exception(
                    "'minority' attributes is missing in 'fairness_attributes' input")

            self.__validate_feature(fea)

    def __validate_feature(self, feature):
        """
            Validate features for proper majority and majority values
        """
    ...
    def __get_common_configuration(self):
        ...
        common_configuration["prediction"] = self.prediction

        if self.class_probabilities:
            common_configuration["probability"] = self.class_probabilities
        elif self.probability is not None:
            common_configuration["probability"] = [self.probability]
        else:
            common_configuration["probability"] = None
        ...
        try:
            training_data_schema = self.__generate_training_data_schema(
                input_data_schema=input_data_schema)
        except Exception as ex:
            raise Exception(
                "Error generating training_data_schema. Reason:%s" % ex)

        try:
            output_data_schema = self.__generate_output_data_schema(
                input_data_schema=input_data_schema)
        except Exception as ex:
            raise Exception(
                "Error generating output_data_schema. Reason:%s" % ex)

        common_configuration["input_data_schema"] = input_data_schema
        common_configuration["training_data_schema"] = training_data_schema
        common_configuration["output_data_schema"] = output_data_schema

        common_configuration["feature_fields"] = self.feature_columns
        common_configuration["categorical_fields"] = self.categorical_columns
        common_configuration["feature_columns"] = self.feature_columns
        common_configuration["categorical_columns"] = self.categorical_columns

        # Update various monitor flags
        common_configuration["enable_quality"] = self.training_data_info.get("enable_quality", True)
        common_configuration["enable_fairness"] = self.training_data_info.get("enable_fairness", True)
        common_configuration["enable_explainability"] = self.training_data_info.get("enable_explainability", True)
        common_configuration["enable_drift_v2"] = self.training_data_info.get("enable_drift_v2", True)
        common_configuration["enable_drift"] = self.training_data_info.get("enable_drift", False)
        return common_configuration

    # Generate input schema in spark structure
    def __generate_input_data_schema(self):
        schema = dict(self.training_data_frame.dtypes)
        fields = []
        for f_name in schema:
            if f_name == self.label_column:
                continue

            data_type = str(schema[f_name])
            f_type = self.__get_mapped_data_type(
                data_type=data_type,
                column=f_name)
    ...
    def __generate_training_data_schema(self, input_data_schema):
        training_data_schema = copy.deepcopy(input_data_schema)

        schema = dict(self.training_data_frame.dtypes)
        data_type = str(schema[self.label_column])
    ...
    # Generate training schema in spark structure
    def __generate_output_data_schema(self, input_data_schema):
        if not self.prediction and not self.probability:
            return None
        ...
        if schema.get(self.prediction):
            # prediction column found in training data
            data_type = str(schema[self.prediction])
            column = self.prediction

        if not data_type:
            # set prediction column data-type same as label column
            data_type = str(schema[self.label_column])
            column = self.label_column
        ...
        probability = None
        if self.class_probabilities:
            probability = self.class_probabilities
        else:
            add_as_array = True
    ...
    def __get_mapped_data_type(self, data_type, column):
        if "object" in data_type:
            # For very large numbers pandas returns object type, hence checking the value
            if type(self.training_data_frame[column][0]) is int:
                return "long"
            else:
                return "string"
        ...
        for index, protected_attr_value in enumerate(protected_attr_col):
            ...
            if protected_attr_value in frequency_map:
                ...
                if class_value in freq_count_dict:
                    ...
                    freq_count_dict.update(
                        {class_value: counts_for_class_value})
                    # Update the final map of freq
                    frequency_map.update(
                        {protected_attr_value: freq_count_dict})
                else:
    ...
    def __getDistribution(self, dataset, inputs):
        ...
        if is_numeric:
            ...
            for pos in range(1, row_num):
                is_numeric, is_float, is_int = self.__get_data_types(
                    class_labels[-pos])
                if is_numeric or is_float or is_int:
    ...
    def __computeTrainingDataDistribution(self):
        ...
        try:
            ...
            fairness_attributes = self.fairness_inputs.get(
                "fairness_attributes")
            for fea in fairness_attributes:
            ...
            data, distinct_class_label_values = self.__getDistribution(
                self.training_data_frame, fairness_params)

            data_frame, feature_data_types = self.__cleanPayloadData(
                self.training_data_frame, fairness_params)
            ...
            if self.problem_type is not None and self.problem_type == "regression":
                ...
                favourable_unfavourable_class.extend(
                    self.fairness_inputs['favourable_class'])
                favourable_unfavourable_class.extend(
                    self.fairness_inputs['unfavourable_class'])
    ...
    def __cleanPayloadData(self, payload_df, fairness_params):
        ...
        for protected_attribute in fairness_params['fairness_attributes']:
            ...
            for idx in range(row_num - 1):
                ...
                is_numeric, is_float, is_int = self.__get_data_types(
                    protected_attribute_column_value)
                if is_numeric or is_float or is_int:
                    count_numeric += 1
    ...
    def __get_boundaries(self, majority, minority, data_type, bucket_size, feature_min, feature_max, round_value):
        ...
        if data_type is not None:
            ...
            # IF individual bucket range is 0 (should be at least 1) then return empty boundaries
            if range == 0:
                return boundaries
            temp = min
            distribution = {}
            ...
            while temp <= max:
                ...
                if not data_type is None and data_type == "int":
                    ...
                    if (idx < (length_sorted_values)):
                        sorted_value = self.__truncate(
                            sorted_values[idx], round_value)

                        if (sorted_value == end):
                            idx += 1
                            next_start = self.__truncate(
                                temp + range, round_value)
                            boundary = [start, end]
                            boundaries.append(boundary)
                            temp = temp + range
    ...
    def __getDistinctValues(self, dataset, inputs):
        ...
        for protected_attribute in inputs['fairness_attributes']:
            ...
            for i in range(sample_size):
                protected_attribute_column_value = protected_attribute_column[random.randint(
                    0, row_num - 1)]
                is_numeric, is_float, is_int = self.__get_data_types(
                    protected_attribute_column_value)
                if is_numeric or is_float or is_int:
            ...
            protected_attribute_column_sorted = self.__custom_sorted(
            ...
            if is_column_numeric:
                ...
                for pos in range(1, row_num):
                    is_numeric, is_float, is_int = self.__get_data_types(
                        protected_attribute_column_sorted[-pos])
                    if is_numeric or is_float or is_int:
                ...
                for attribute_value in protected_attribute_column_sorted:
                    is_numeric, is_float, is_int = self.__get_data_types(
                        attribute_value)
                    if not (is_numeric or is_float or is_int):
                        distinct_protected_attribute_column.add(
                            attribute_value)

                distinct_values_sync[protected_attribute] = list(
                    distinct_protected_attribute_column)
    ...
    def __custom_sorted(self, l):
        try:
            ...
            def alpha_num_key(key): return [convert(c)
    ...
    def __get_fairness_configuration(self):
        ...
        if "max_records" in self.fairness_inputs:
            parameters_json["max_records"] = self.fairness_inputs["max_records"]
        ...
        if "training_data_distributions" in training_data_statistics:
            ...
            parameters_json["training_data_last_processed_time"] = str(
    ...
    def __get_thresholds(self):
        ...
        all_thresholds = self.fairness_inputs.get("thresholds")
        if all_thresholds is not None:
        ...
        for feature in features:
            ...
            if feature_threshold is None:
                feature_threshold = .8 # set the detault threshold
            specific_value['value'] = feature_threshold * 100
            apply_to = {}
            apply_to['type'] = 'tag'
    ...
    # GET the explainability configuration
    def __get_explainability_configuration(self):
        ...
        try:
            ...
            # Categorical columns index as per feature columns
            ...
            # Compute training stats on discretized data
            ...
            raise Exception(
                "Error while generating explainability configuration.Reason:%s" % ex)
            ...
            for config in exp_config:
                ...
                if isinstance(config_details, list):
                    for x in range(len(config_details)):
                        if isinstance(config_details[x], np.int64):
                    ...
                    for key in config_details:
                        ...
                        if isinstance(key_details, list):
                            for x in range(len(key_details)):
                                if isinstance(key_details[x], np.int64):
                                    key_details[x] = key_details[x].item()
                            config_details[key] = key_details
                        elif isinstance(key_details, np.int64):
                            config_details[key] = int(key_details)
                exp_config[config] = config_details
        except Exception as ex:
            raise Exception(
                "Error while converting numpy int64.Reason:%s" % ex)
        return exp_config
    ...
    def get_training_statistics(self):
        ...
        if self.compute_explain:
            explain_config = self.__get_explainability_configuration()
    ...
    def get_buckets_for_training(self, payload_df, data, feature_attributes, fairness_attributes_list,
                                 feature_data_types):
        sorted_data = {}
        bucket_size = 45
        fairness_params = {"class_label": self.label_column,
                    ...
                    if (val < boundaries[0][0]):
                        if (least_min != boundaries[0][0]):
                            ...
                            if bucket in bucket_dict:
                                ...
                                # Value found and corresponding bucket has been inserted/modified so go to the next value
                        continue
