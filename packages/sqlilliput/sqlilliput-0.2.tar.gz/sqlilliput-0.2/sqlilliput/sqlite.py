...
"""SQLite document-store abstraction layer for Python 3"""
...
class Database:
    def __init__(self, fp: str, isolation_level: typing.Optional[str] = DEFAULT_ISOLATION_LEVEL):
        ...
        # sqlite 3.25.x or higher is required for upserts. If we do not have this, upserts will
        # fall back to catching integrity errors and inserting when the row was not found.
    ...
    def delete(self, table: str, **target):
        ...
        if not target:
            raise DatabaseException(
                "DELETE must have at least one defined target value for locating where to delete from"
            )
    ...
    def update(self, table: str, document: dict, **target):
        ...
        """
        if not target:
            raise DatabaseException("UPDATE must have at one defined target to specify the row to update")
    ...
    def upsert(self, table: str, document: dict, **target):
        ...
        if not target:
            raise DatabaseException("UPSERTs must have at least one defined target value for locating where to upsert")
        k, v = next(iter(target.items()))
        document[k] = v
