...
def print1(x, color=False, noeko=False,n=INFO) :
...
def tt(_loc, n=INFO):
    ...
    return '[' + str(now).zfill(4) + ppp + 'ms' + timestamp +  ']' + dthid + ' '
...
def EKON(*args,n=INFO,pref="", color=False, check=False, withprint=True, llevel=0, pattern="EKOX"):
    ...
    """
    if len(args) == 1 :
        EKOX(args[0], n=n, llevel=llevel+1, pattern="EKON", istuple=len(args)>1)    
    else :
        EKOX(args, n=n, llevel=llevel+1, pattern="EKON", istuple=len(args)>1)            
...
def EKOX(x, n=INFO, pref="", color=False, check=False, withprint=True, llevel=0, pattern="EKOX",
    ...
    if prt :
        ...
        if (ff[4] != None) :
                ...
                try :
                    ...
                    if len(tk) == len(list(x)) :
                        ...
                        def ses(sx, sv) :
                            ...
                            hasquote = ll1 in ll
                            return sx if hasquote else sx + '=' + str(sv) 
            ...
            r = print1(preamb + tt(loc, n=n) + lab(n) + tkk + '.', color=color, noeko=not withprint, n=n)
        else:
            r = print1(preamb + tt(loc, n=n) + lab(n) + str(ff[3]) + '=' + inl + pref + strng + '.', color=color, noeko= not withprint, n=n)
...
def EKOI(x,n=INFO, small=True,dir=tempDir, sz=350, label="", llevel=0):
    """
...
def EKOT(*x,n=INFO,pref="", color=False, llevel=0 ):
    ...
    if (ff[4] != None and n >= el()) :
        loc = str(ff[1]) +  ':' + str(ff[2])
        r += print1( loc +  ': ' + tt(loc) + lab(n) + '=' + pref + ', '.join(map(str,x)) + '.', color=color)
    return r
