...
@click.option(
    "--dump-graph", is_flag=True, default=False, help="Dump the graph to GML markup."
)
@click.option(
    "--build-order",
    is_flag=True,
    default=False,
    help="Show only the build graph.",
)
...
@click.option(
    ...
    multiple=True,
    help=(
        "Show node attribute. Can be specified multiple times. If not specified, all "
...
@click.option(
    ...
    multiple=True,
    help=(
        "Show edge attribute. Can be specified multiple times. If not specified, all "
        "attributes are shown."
    ),
)
@graph_layout_argument
...
def compile(
    ...
    if graph:
        ...
        if not graphviz:
            kwargs = {
                **kwargs,
                # Apply colours to the nodes and edges.
                "node_color_key": lambda node, data: data["type"],
                "edge_color_key": lambda edge, data: data["type"],
                # Other attributes.
...
@click.option(
    ...
    multiple=True,
    help=(
        "Show node attribute. Can be specified multiple times. If not specified, all attributes "
        "are shown."
    ),
)
@click.option(
    ...
    multiple=True,
    help=(
        "Show edge attribute. Can be specified multiple times. If not specified, all attributes "
        "are shown."
    ),
)
@graph_layout_argument
