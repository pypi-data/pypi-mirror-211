...
def _search_syntax(
    ...
    matches = sorted(
        matches,
        key=lambda adapter: list(directives.keys()).index(adapter.full_name)
    )
    ...
    if not parameter:
        ...
        for adapter in matches:
            ...
            for attr in dir(adapter.documenter.item_type):
                ...
                if isinstance(reduce(getattr, [attr], adapter.documenter.item_type), parameterproperty):
    ...
    for adapter, parameters in matches:
        ...
        if parameters:
            for parameter in parameters:
                doc = inspect.getdoc(reduce(getattr, [parameter], adapter.documenter.item_type))
                if doc is None:
                    doc = parameter
