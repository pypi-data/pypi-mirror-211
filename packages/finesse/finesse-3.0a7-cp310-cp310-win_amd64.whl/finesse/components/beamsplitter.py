...
class Beamsplitter(Surface):
    ...
    def __init__(
        self, name, R=None, T=None, L=None, phi=0, alpha=0, Rc=np.inf, xbeta=0, ybeta=0
    ):
        ...
        self.dofs.z = DOFDefinition(f"{self.name}.dofs.z", self.phi, self.mech.z, 1)
        ...
        self.dofs.F_z = DOFDefinition(
            f"{self.name}.dofs.F_z", None, self.mech.F_z, None
        )
        self.dofs.F_yaw = DOFDefinition(
            f"{self.name}.dofs.F_yaw", None, self.mech.F_yaw, None
        )
        self.dofs.F_pitch = DOFDefinition(
            f"{self.name}.dofs.F_pitch", None, self.mech.F_pitch, None
        )
    ...
    def __symbolise_ABCD(self, from_node, to_node, direction):
        ...
        if self.interaction_type(from_node, to_node) == InteractionType.REFLECTION:
            ...
            if from_node.port is self.p3 or from_node.port is self.p4:
                M_sym = abcd.beamsplitter_refl(
                    -Rc, alpha2, nr1=nri, nr2=nro, direction=direction
                )
            else:
                M_sym = abcd.beamsplitter_refl(
                    Rc, alpha1, nr1=nri, nr2=nro, direction=direction
                )
        else:  # transmission
            if from_node.port is self.p3 or from_node.port is self.p4:
                M_sym = abcd.beamsplitter_trans(
                    -Rc, alpha2, nr1=nri, nr2=nro, direction=direction
                )
            else:
                M_sym = abcd.beamsplitter_trans(
