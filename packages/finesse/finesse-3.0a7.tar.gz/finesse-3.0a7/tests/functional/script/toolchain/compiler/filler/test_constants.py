...
def spec(spec, set_spec_constructs, fake_element_adapter_factory, fake_element_cls):
    ...
    set_spec_constructs("constants", {"A": 1, "B": 2, "C": 3, "D": 4})
...
@pytest.mark.parametrize(
    ...
    (
        pytest.param(
            "fake_element myelement A B C D",
            dedent_multiline(
                """
                graph [
                    directed 1
            ...
            id="element with 4 constants",
        ),
    ),
)
def test_constants(assert_graphs_match, script, graph_definition):
