...
def test_math_detector_shape_init(model):
    model.add(MathDetector("md_Aout", Constant(model.fd_Aout), dtype=np.complex128, dtype_shape=(len(model.modes()),)))
    sol = model.run()
...
def test_math_detector_shape_set_expression(model):
    ...
    model.md_Aout.set_expression(model.md_Aout.expression, dtype=np.complex128, dtype_shape=(len(model.modes()),))
    sol = model.run()
    assert sol["md_Aout"].shape == (len(sol.x1), len(model.modes()))
    assert np.all(np.isclose(sol["md_Aout"], sol["fd_Aout"]))
