...
cdef cy_expr* cy_expr_new() nogil except NULL:
    cdef cy_expr* ce_p = <cy_expr*> malloc(sizeof(cy_expr))
    if not ce_p:
        with gil:
            raise MemoryError()
    ce_p.expr = NULL
...
cdef int cy_expr_init(cy_expr* ex, object operation) except -1:
    ...
    ex.variables = <te_variable*> malloc(Nparams * sizeof(te_variable))
    if not ex.variables:
        raise MemoryError()
...
cdef void cy_expr_free(cy_expr* ex):
    if ex != NULL:
        if ex.expr != NULL:
            te_free(ex.expr)
...
cdef double cy_expr_eval(cy_expr* ex) nogil:
