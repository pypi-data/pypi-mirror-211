...
cdef class KnmConnectorWorkspace(ConnectorWorkspace):
    ...
    def __init__(self, object owner, object sim, *args, **kwargs):
        ...
        assert(isinstance(sim, CarrierSignalMatrixSimulation))
    ...
    cdef update_map_data(KnmConnectorWorkspace self):
        cdef:
            KnmInfo *info
    ...
    cdef void flag_changing_knm_workspaces(
        KnmConnectorWorkspace self,
        set changing_mismatch_edges
    ):
    ...
    cdef void update_changing_knm_workspaces(KnmConnectorWorkspace self):
        cdef:
            NodeBeamParam *q_from
            NodeBeamParam *q_to
            KnmInfo *info
            ...
            if info.use_map:
                ...
                if map_ws.new_map_data or not map_ws.q_from.is_fixed or not map_ws.q_to.is_fixed:
                    ...
                    if w/map_ws.dx < 10 or w/map_ws.dy < 10:
                        warn(
                            f"Spot size vs map resolution is low, increase map resolution or spot size\n" +
