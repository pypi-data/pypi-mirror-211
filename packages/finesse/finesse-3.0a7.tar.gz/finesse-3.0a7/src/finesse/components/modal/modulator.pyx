...
cdef void set_coupling_orders(ndarray orders, int set_order, double f, mtx, modulator_coupling_order** rtn, Py_ssize_t* N, dict coupling_orders):
    ...
    for i, ((f1, f2), _order) in enumerate(items):
        ...
        rtn[0][i].order_index = int(np.argwhere(orders == _order))
...
# def modulator_fill(object modulator, ModulatorWorkspace ws, MatrixSystemSolver mtx, connections, dict values, dict cporders, unicode
