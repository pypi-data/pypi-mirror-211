...
from finesse.cymath.complex cimport csqrt, creal, cimag, COMPLEX_0
...
cdef class CavityWorkspace(ElementWorkspace):
        ...
        for tree_idx in range(sim.trace_forest.size()):
            ...
            if tree.dependency == owner and tree.is_source:
                if self.tree is not None:
                    raise RuntimeError(
                        "Bug encountered! Found multiple trees in the simulation "
    ...
    cdef void __update_geometric(self):
        cdef:
            bint qx_ok, qy_ok
    ...
    cdef bint update_eigenmode(self, cavity_plane plane):
        cdef:
    ...
    cdef void update_stability(self, cavity_plane plane):
        cdef:
            double[:, ::1] abcd = self.abcd_x if plane == cavity_plane.X else self.abcd_y
            double A = abcd[0][0]
            double D = abcd[1][1]
    ...
    cdef void update_rt_gouy(self, cavity_plane plane):
        cdef:
            double[:, ::1] abcd = self.abcd_x if plane == cavity_plane.X else self.abcd_y
    ...
    cdef void update_Df(self, cavity_plane plane):
        cdef:
    ...
    cdef void update_S(self, cavity_plane plane):
        cdef:
            double gouy = self.rt_gouy_x if plane == cavity_plane.X else self.rt_gouy_y
