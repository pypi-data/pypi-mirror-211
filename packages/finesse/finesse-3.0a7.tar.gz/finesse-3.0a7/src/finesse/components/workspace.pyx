...
cdef class FillFuncWrapper:
    ...
    >>> cdef void c_fill(ConnectorWorkspace ptr_ws):
...
cdef class GouyFuncWrapper:
    ...
    >>> cdef int c_set_gouy(ConnectorWorkspace ptr_ws):
    >>>    cdef MirrorWorkspace ws = <MirrorWorkspace>ptr_ws
    >>>    ...
    >>>
...
cdef class ConnectorWorkspace(ElementWorkspace):
    """
    ...
    def set_gouy_function(self, callback):
        """ This sets the callback function that will be used by the model to
        compute the gouy phase terms for an element.
    ...
    # TODO ddb shouldn't this raise a not implemented error?
        pass
