...
cdef class FrequencyContainer:
    """Contains an array of frequency objects and their associated
    faster access C struct information.
    """
    ...
    def __init__(self, frequencies, FrequencyContainer carrier_cnt=None):
        ...
        if self.frequency_info != NULL:
            raise MemoryError()
        self.frequency_info = <frequency_info_t*> malloc(self.size * sizeof(frequency_info_t))
...
def generate_frequency_list(model):
    ...
    This method relies on using :class:`.Symbol`. Using
...
class Fsig(ModelElement):
    """This element represents the signal frequency (``fsig``) used in a model. It is
    a unique element, which means only one can be added to any given model. This is done
    automatically with the name ``fsig``. It has a single parameter ``f`` for the frequency
    of the signal.
...
cdef class Frequency:
    ...
    def __init__(
        ...
        self.__lambdified = eval(f"lambda: {symbol}", sim.model.elements)
        ...
        if audio:
            ...
            self.__carrier_obj = sim.carrier.optical_frequencies.frequencies[audio_carrier_index]
        else:
            self.__order = 0
            self.__carrier = 0
    ...
    def audio_carrier(self):
        """Frequency object for the carrier frequency of this sideband, if it
        is a sideband.
