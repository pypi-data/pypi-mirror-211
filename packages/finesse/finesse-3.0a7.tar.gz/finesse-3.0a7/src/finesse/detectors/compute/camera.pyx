...
from finesse.cymath.math cimport cos, sin, ceil, floor
from finesse.cymath.math cimport float_eq, nmin, nmax
...
cdef class CameraWorkspace(MaskedDetectorWorkspace):
    ...
    def __init__(self, owner, sim, values=None):
        ...
        self.node_id = self.sim.carrier.node_id(owner.node)
...
cdef complex_t field_beam_pixel(ComplexCameraWorkspace cws, double x, double y) nogil:
    cdef:
        ...
        if float_eq(freq.f, cws.v.f):
            ...
            for k in range(Nfields):
                ...
                at += (
                    unm
                    * crotate2(
                        cws.sim.carrier.get_out_fast(node_id, freq_idx, field_idx),
                        cph,
                        sph
                    )
                    * INV_ROOT2
...
cdef double ccd_beam_pixel(CameraWorkspace cws, double x, double y) nogil:
    cdef:
        ...
        for freq_idx_inner in range(Nfreqs):
            ...
            for k in range(Nfields):
                ...
                at1 += (
                    unm
                    * crotate2(
                        cws.sim.carrier.get_out_fast(node_id, freq_idx_outer, field_idx),
                        cph,
                        sph
                    )
                    * INV_ROOT2
                ...
                at2 += (
                    unm
                    * crotate2(
                        cws.sim.carrier.get_out_fast(node_id, freq_idx_inner, field_idx),
                        cph,
                        sph
                    )
                    * INV_ROOT2
