...
from finesse.cymath.complex cimport conj, creal, cimag, cexp, carg, cabs
from finesse.cymath.complex cimport crotate, cnorm, COMPLEX_0
from finesse.cymath.math cimport sqrt, sin, cos, float_eq
from finesse.cymath.homs cimport in_mask, field_index
...
from finesse.simulations.basematrix cimport CarrierSignalMatrixSimulation, MatrixSystemSolver
...
cdef inline void do_zcsrgecmv(
        csr_matrix *M, MatrixSystemSolver mtx,
    ...
    cdef int i = mtx.field_fast(node_id, f1_index, 0)
    cdef int j = mtx.field_fast(node_id, f2_index, 0)
    # Get optical field vector pointers
    cdef complex *E1 = &mtx.out_view[i]
    cdef complex *E2 = &mtx.out_view[j]
...
cdef inline void do_zcsrgecmv2(
        csr_matrix *M,
        int N,
        MatrixSystemSolver mtx1,
        int node1_id,
        int f1_index,
        MatrixSystemSolver mtx2,
        int node2_id,
        int f2_index,
    ...
    cdef complex z
    # get indicies pointing to start of particular frequency
    cdef int i = mtx1.field_fast(node1_id, f1_index, 0)
    cdef int j = mtx2.field_fast(node2_id, f2_index, 0)
    # Get optical field vector pointers
    cdef complex *E1 = &mtx1.out_view[i]
    cdef complex *E2 = &mtx2.out_view[j]
    # Compute vector matrix vector to compute
    # segmentation effects
    zcsrgecmv(M, E1, N, E2, N, &z)
    if conjugate_result:
...
cdef c_pd0_DC_output(DetectorWorkspace dws):
    ...
    for i in range(ws.rhs_index, ws.rhs_index + ws.size):
        tmp += cnorm(ws.sim.carrier.out_view[i])
...
cdef c_pd0_DC_output_segmented(DetectorWorkspace dws):
    ...
    cdef:
        ...
        const complex_t* E
        frequency_info_t *f1
        Py_ssize_t Nf = ws.sim.carrier.optical_frequencies.size
        Py_ssize_t N = ws.sim.model_settings.num_HOMs

    for i in range(Nf):
        f1 = &ws.sim.carrier.optical_frequencies.frequency_info[i]
        rhs_idx = ws.sim.carrier.field_fast(ws.dc_node_id, f1.index, 0)
        E = &ws.sim.carrier.out_view[rhs_idx]
...
cdef c_pd0_DC_output_masked(DetectorWorkspace dws):
    ...
    for i in range(ws.sim.carrier.optical_frequencies.size):
        freq_idx = ws.rhs_index + i
        for j in range(ws.num_unmasked_HOMs):
            k = freq_idx + ws.unmasked_mode_indices[j]
            tmp += cnorm(ws.sim.carrier.out_view[k])
...
cdef class PD1Workspace(MaskedDetectorWorkspace):
    ...
    cpdef update_beats(self):
        ...
        cdef:
            ...
            Py_ssize_t Nf = self.sim.carrier.optical_frequencies.size

        self.update_parameter_values() # update values we might need
        prev_num_beats = self.num_pre_set_beats

        for i in range(Nf):
            f1 = &self.sim.carrier.optical_frequencies.frequency_info[i]
            for j in range(Nf):
                f2 = &self.sim.carrier.optical_frequencies.frequency_info[j]
                if f1.f - f2.f == self.cvalues.f:
                    beats.append((
                        self.sim.carrier.field_fast(self.dc_node_id, f1.index, 0),
                        self.sim.carrier.field_fast(self.dc_node_id, f2.index, 0),
                    ))
        ...
        if self.num_pre_set_beats > 0:
            if prev_num_beats > 0:
                self.pre_set_beats[0] = <Py_ssize_t*>realloc(
                    self.pre_set_beats[0],
                    self.num_pre_set_beats
                )
                if not self.pre_set_beats[0]:
                    raise MemoryError()
                self.pre_set_beats[1] = <Py_ssize_t*>realloc(
                ...
                self.pre_set_beats[0] = <Py_ssize_t*>calloc(
                    self.num_pre_set_beats,
                    sizeof(Py_ssize_t)
                )
                if not self.pre_set_beats[0]:
                    raise MemoryError()
                self.pre_set_beats[1] = <Py_ssize_t*>calloc(
                    self.num_pre_set_beats,
                    sizeof(Py_ssize_t)
                )
                if not self.pre_set_beats[1]:
            ...
            for n in range(self.num_pre_set_beats):
                self.pre_set_beats[0][n] = beats[n][0]
                self.pre_set_beats[1][n] = beats[n][1]
...
cdef object c_pd1_AC_output(DetectorWorkspace dws):
    ...
    if ws.is_segmented:
        for i in range(Nf):
            f1 = &ws.sim.carrier.optical_frequencies.frequency_info[i]
            # get indicies pointing to start of particular frequency
            i = ws.sim.carrier.field_fast(ws.dc_node_id, f1.index, 0)
            # Get optical carrier field vector pointers
            Ec = &ws.sim.carrier.out_view[i]
            ...
            j = ws.sim.signal.field_fast(ws.ac_node_id, f1.audio_upper_index, 0)
            Es = &ws.sim.signal.out_view[j]
            zcsrgecmv(&ws.K.M, Ec, N, Es, N, &z)
            ws.Aij += z
            # lower sideband needs unconjugated carrier, as we compute
            # the conjugated lower sideband propagation
            j = ws.sim.signal.field_fast(ws.ac_node_id, f1.audio_lower_index, 0)
            Es = &ws.sim.signal.out_view[j]
            zcsrgevmv(&ws.K.M, Ec, N, Es, N, &z)
            ws.Aij += z
    else:
        for i in range(Nf):
            f1 = &ws.sim.carrier.optical_frequencies.frequency_info[i]
            for k in range(ws.num_unmasked_HOMs):
                ...
                tmp = ws.sim.carrier.get_out_fast(ws.dc_node_id, f1.index, m)
                ws.Aij += conj(tmp) * ws.sim.signal.get_out_fast(ws.ac_node_id, f1.audio_upper_index, m)
                # lower sideband conjugate is modelled
                ws.Aij += tmp * ws.sim.signal.get_out_fast(ws.ac_node_id, f1.audio_lower_index, m)
    ...
    if ws.num_pre_set_beats > 0:
        if ws.is_segmented:
            for n in range(ws.num_pre_set_beats):
                ...
                i = ws.pre_set_beats[0][n]
                j = ws.pre_set_beats[1][n]
                E1 = &ws.sim.carrier.out_view[i]
                E2 = &ws.sim.carrier.out_view[j]
                # Compute vector matrix vector to compute
                # segmentation effects
            ...
            for n in range(ws.num_pre_set_beats):
                ...
                for k in range(ws.num_unmasked_HOMs):
                    m = ws.unmasked_mode_indices[k]
                    ws.Aij += ws.sim.carrier.out_view[i+m] * conj(ws.sim.carrier.out_view[j+m])
    else:
        # Do dumb thing and check everything if we have/can not optimise
        for i in range(ws.sim.carrier.optical_frequencies.size):
            f1 = &ws.sim.carrier.optical_frequencies.frequency_info[i]
            for j in range(ws.sim.carrier.optical_frequencies.size):
                f2 = &ws.sim.carrier.optical_frequencies.frequency_info[j]
                if float_eq(f1.f - f2.f, ws.cvalues.f):
                    if ws.is_segmented:
                        # get indicies pointing to start of particular frequency
                        i = ws.sim.carrier.field_fast(ws.dc_node_id, f1.index, 0)
                        j = ws.sim.carrier.field_fast(ws.dc_node_id, f2.index, 0)
                        # Get optical field vector pointers
                        E1 = &ws.sim.carrier.out_view[i]
                        E2 = &ws.sim.carrier.out_view[j]
                        # Compute vector matrix vector to compute
                        ...
                        for k in range(ws.num_unmasked_HOMs):
                            m = ws.unmasked_mode_indices[k]
                            ws.Aij += (ws.sim.carrier.get_out_fast(ws.dc_node_id, f1.index, m) *
                                conj(ws.sim.carrier.get_out_fast(ws.dc_node_id, f2.index, m)))
...
cdef object c_pd2_DC_output(DetectorWorkspace dws):
    ...
    for i in range(Nf):
        ...
        for j in range(Nf):
            ...
            if f1.f - f2.f == f_plus:
                if ws.is_segmented:
                    do_zcsrgecmv(&ws.K.M, ws.sim.carrier, ws.dc_node_id, f2.index, f1.index, N, &ws.z1)
                else:
                    for k in range(ws.num_unmasked_HOMs):
                        m = ws.unmasked_mode_indices[k]
                        ws.z1 += (ws.sim.carrier.get_out_fast(ws.dc_node_id, f1.index, m) *
                            conj(ws.sim.carrier.get_out_fast(ws.dc_node_id, f2.index, m)))
            ...
            if f1.f - f2.f == f_minus:
                if ws.is_segmented:
                    ...
                    for k in range(ws.num_unmasked_HOMs):
                        m = ws.unmasked_mode_indices[k]
                        ws.z2 += (ws.sim.carrier.get_out_fast(ws.dc_node_id, f1.index, m) *
                            conj(ws.sim.carrier.get_out_fast(ws.dc_node_id, f2.index, m)))
        ...
        for j in range(Nf):
            ...
            if float_eq(df, -ws.cvalues.f1):
                ...
                if ws.is_segmented:
                    ...
                    do_zcsrgevmv2(
                        &ws.K.M, N,
                        ws.sim.signal, ws.ac_node_id, f2.audio_lower_index,
                        ws.sim.carrier, ws.dc_node_id, f1.index,
                        False, &ws.z2
                    )
                    # Compute E(carrier)^H @ K @ E(upper)
                    do_zcsrgecmv2(
                        &ws.K.M, N,
                        ws.sim.signal, ws.ac_node_id, f1.audio_upper_index,
                        ws.sim.carrier, ws.dc_node_id, f2.index,
                        False, &ws.z2
                    )
                else:
                    for k in range(ws.num_unmasked_HOMs):
                        m = ws.unmasked_mode_indices[k]
                        car1 = ws.sim.carrier.get_out_fast(ws.dc_node_id, f1.index, m)
                        ws.z2 += car1 * ws.sim.signal.get_out_fast(ws.ac_node_id, f2.audio_lower_index, m)

                        car2 = ws.sim.carrier.get_out_fast(ws.dc_node_id, f2.index, m)
                        ws.z2 += ws.sim.signal.get_out_fast(ws.ac_node_id, f1.audio_upper_index, m) * conj(car2)
            ...
            if float_eq(df, ws.cvalues.f1):
                ...
                if ws.is_segmented:
                    ...
                    for k in range(ws.num_unmasked_HOMs):
                        m = ws.unmasked_mode_indices[k]
                        car1 = ws.sim.carrier.get_out_fast(ws.dc_node_id, f1.index, m)
                        ws.z1 += car1 * ws.sim.signal.get_out_fast(ws.ac_node_id, f2.audio_lower_index, m)

                        car2 = ws.sim.carrier.get_out_fast(ws.dc_node_id, f2.index, m)
                        ws.z1 += ws.sim.signal.get_out_fast(ws.ac_node_id, f1.audio_upper_index, m) * conj(car2)
...
# TODO ddb this all needs changing over to use the CarrierSignalMatrixSimulation
...
# cdef _create_mix_table(Py_ssize_t num_demod, double[:, ::1] fmix_view):
#     cdef:
#         int num_mixes = 1
...
# cdef bint in_index_array(long long[::] indices, Py_ssize_t idx) nogil:
#     cdef:
#         Py_ssize_t i
