...
cdef class OutputInformation:
    ...
    def dtype_size(self):
        ...
        """
        return int(np.prod(self.dtype_shape))
...
cdef class OutputFuncWrapper:
    ...
    >>> cdef void c_output(DetectorWorkspace ptr_ws):
...
cdef class DetectorWorkspace(ElementWorkspace):
    """A base class that all detector workspaces should inherit from. Provides a
    generic set of data needed to compute values and output them with metadata
    needed for storing the outputs.
...
cdef class MaskedDetectorWorkspace(DetectorWorkspace):
    ...
    indices, e.g::
        ...
        for k in ws.unmasked_indices_arr:
            ...
            a_0k = ws.sim.carrier.get_out_fast(ws.dc_node_id, 0, k)
    ...
    """
    def __init__(self, owner, CarrierSignalMatrixSimulation sim, values=None, *, oinfo=None, **kwargs):
    ...
    cdef bint hom_in_modes(self, Py_ssize_t hom_idx):
