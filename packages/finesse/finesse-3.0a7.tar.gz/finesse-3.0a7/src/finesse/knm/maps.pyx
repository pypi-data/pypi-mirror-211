...
cdef class Map:
    ...
    def update(self, model=None, weight_spot_size:float=None):
        ...
        if self.has_opd_function:
            ...
            if self.do_remove_curvatures or self.do_remove_astigmatism:
                a, b = self.remove_curvatures(weight_spot_size, "average" if self.do_remove_curvatures else "astigmatic", model=model)
    ...
    def get_thin_lens_f(self, spot_size : float):
        r"""
...
cdef init_knm_map_workspace(
    ...
    if Nm <= 0:
        raise RuntimeError(f"Nm <= 0")
    ws.Nm = Nm

    if ws.Un != NULL: raise MemoryError()
    ws.Un = <complex_t*> malloc(Nm * ws.Nx * sizeof(complex_t))
    if not ws.Un:
        raise MemoryError()

    if ws.Um != NULL: raise MemoryError()
    ws.Um = <complex_t*> malloc(Nm * ws.Ny * sizeof(complex_t))
    if not ws.Um:
        raise MemoryError()

    if ws.Un_ != NULL: raise MemoryError()
    ws.Un_ = <complex_t*> malloc(Nm * ws.Nx * sizeof(complex_t))
    if not ws.Un_:
        raise MemoryError()

    if ws.Um_ != NULL: raise MemoryError()
    ws.Um_ = <complex_t*> malloc(Nm * ws.Ny * sizeof(complex_t))
    if not ws.Um_:
        raise MemoryError()

    if ws.Unn_ != NULL: raise MemoryError()
    ws.Unn_ = <complex_t*> malloc(Nm * Nm * ws.Nx * sizeof(complex_t))
    if not ws.Unn_:
        raise MemoryError()

    if ws.Umm_ != NULL: raise MemoryError()
    ws.Umm_ = <complex_t*> malloc(Nm * Nm * ws.Ny * sizeof(complex_t))
    if not ws.Umm_:
        raise MemoryError()

    if ws.tmp != NULL: raise MemoryError()
    ws.tmp = <complex_t*> malloc(Nm * Nm * ws.Ny * sizeof(complex_t))
    if not ws.tmp:
        raise MemoryError()

    if ws.K != NULL: raise MemoryError()
    ws.K = <complex_t*> malloc(Nm * Nm * Nm * Nm * sizeof(complex_t))
    if not ws.K:
        raise MemoryError()

    if ws.uiws != NULL: raise MemoryError()
    ws.uiws = <unm_workspace*> malloc(sizeof(unm_workspace))
    if not ws.uiws:
        raise MemoryError()

    if ws.uows != NULL: raise MemoryError()
    ws.uows = <unm_workspace*> malloc(sizeof(unm_workspace))
    if not ws.uows:
        raise MemoryError()

    if ws.unm_i_factor_ws != NULL: raise MemoryError()
    ws.unm_i_factor_ws = <unm_factor_store*> malloc(sizeof(unm_factor_store))
    if not ws.unm_i_factor_ws:
        raise MemoryError()

    if ws.unm_o_factor_ws != NULL: raise MemoryError()
    ws.unm_o_factor_ws = <unm_factor_store*> malloc(sizeof(unm_factor_store))
