...
from finesse.cymath.complex cimport carg, conj, creal, cimag, csqrt # Standard complex.h functions
...
cdef class KnmMatrix:
    ...
    def __init__(self, const int[:, ::1] modes, comp_name="", kdir=""):
        ...
        self.modes = np.asarray(self.modes_view)
    ...
    def plot(self, mode="amplitude", log=False, cmap=None, show=True, filename=None):
        ...
        if mode == "amplitude":
            ...
            raise ValueError(f"Invalid mode={mode} argument. Must be one of "
                             "'amplitude', 'phase', 'real' or 'imag'.")

        if log:
            if mode != "amplitude":
                raise ValueError(
                    f"Logarithmic scaling only supported for amplitude mode."
                )

            norm = colors.LogNorm()
            # Make sure any zero couplings get displayed as black rather than white
            cmap_handle = copy.copy(plt.cm.get_cmap(cmap))
