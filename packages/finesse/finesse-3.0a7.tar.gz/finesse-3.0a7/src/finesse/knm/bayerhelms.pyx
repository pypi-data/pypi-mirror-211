...
from finesse.cymath.complex cimport carg, conj, creal, cimag, csqrt, COMPLEX_0 # Standard complex.h functions
from finesse.cymath.complex cimport ceq, inverse_unsafe, crotate, crotate2, cpow_re, cnorm
from finesse.cymath.math cimport exp, sqrt, cos, sin, ceil, floor # Standard math.h functions
...
cdef void knm_bh_ws_free(knm_bh_workspace* kws) nogil:
...
cdef void knm_bh_ws_reset_mm_cpow_caches(knm_bh_workspace* kws) nogil:
    cdef Py_ssize_t i
...
cdef void knm_bh_ws_reset_alignment_cpow_caches(knm_bh_workspace* kws) nogil:
    cdef Py_ssize_t i
...
cdef inline bint knm_bh_ws_is_changing(const knm_bh_workspace* kws) nogil:
    return kws.is_mm_changing or kws.is_alignment_changing

cdef void knm_bh_ws_recompute(knm_bh_workspace* kws, complex_t q1, complex_t q2, double beta) nogil:
    knm_bh_ws_recompute_mismatch(kws, q1, q2)
    knm_bh_ws_recompute_misalignment(kws, beta)

cdef void knm_bh_ws_recompute_mismatch(knm_bh_workspace* kws, complex_t q1, complex_t q2) nogil:
    cdef:
        double zr
        complex_t K
...
cdef void knm_bh_ws_recompute_misalignment(knm_bh_workspace* kws, double beta) nogil:
    cdef:
...
cdef void fast_compute_knm_matrix_bh(
    ...
    cdef:
        ...
        int n1, m1, n2, m2
        complex_t coeff, coeff_rg, coeff_y_refl
...
cdef void compute_knm_matrix_bh(
    ...
    if parallel:
        # See CarrierSignalMatrixSimulation.initialise_sim_config_data
...
cdef complex_t S_g(const knm_bh_workspace* kws, int m1, int m2) nogil:
    cdef:
        int s1, s2
        complex_t z, z1, z2
        complex_t nom1, nom2
...
cdef complex_t S_u(const knm_bh_workspace* kws, int m1, int m2) nogil:
    cdef:
        int s3, s4
        complex_t z, z1, z2
        complex_t nom1, nom2
