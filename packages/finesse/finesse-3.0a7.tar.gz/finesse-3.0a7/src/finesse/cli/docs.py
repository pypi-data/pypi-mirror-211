...
@click.option(
    ...
    show_default=True,
    help=(
        "Don't append `*` to search queries."
...
@click.pass_context
def syntax(ctx, query, elements, commands, analyses, keyword_arguments, exact, suggestions, verbose):
    ...
    for q in query:
        syntax(
            q, None, verbose, elements, commands, analyses, keyword_arguments, exact, suggestions
        )
