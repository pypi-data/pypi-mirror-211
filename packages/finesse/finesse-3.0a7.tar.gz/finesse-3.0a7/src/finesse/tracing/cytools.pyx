    ...
    while t is not None:
        if is_x_plane:
            abcd_multiply(
                ...
                m2_refl_transform=t.is_left_surf_refl,
            )
        else:
            abcd_multiply(
...
cpdef np.ndarray[object, ndim=2] compute_symbolic_abcd(TraceTree last_branch, unicode direction, bint simplify, tuple symbols_to_keep=None):
    cdef:
...
def propagate_beam_symbolic(
    ...
    with simplification():
        if symbols_to_keep is not None:
            symbols_to_keep = tuple(s if isinstance(s, str) else s.full_name for s in symbols_to_keep)
