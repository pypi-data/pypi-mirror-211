...
from finesse.exceptions import BeamTraceException, TotalReflectionError
...
cdef class TraceForest:
    ...
    def plant(self, list trace_order):
        ...
        cdef set internal_cav_nodes = self.forest.node_coverage.copy()
    ...
    cdef int _add_gauss_tree(self, object gauss) except -1:
        ...
        if (
            ...
            back_new_tree = TraceTree.from_node(
                ...
                gauss,
                self.symmetric,
                exclude=self.forest.node_coverage - set([gauss_node]),
            )
        ...
        for node in unreachable_nodes:
            ...
            try:
                ...
                try:
                    ...
                    ttr.set_left_abcd_x_memory(comp.ABCD(
                        ttb.node, ttr.node, direction="x", copy=False, retboth=True,
                    ))
                    ttr.set_left_abcd_y_memory(comp.ABCD(
    ...
    cpdef set find_untraversed_nodes(self):
        ...
        cdef:
            set nodes_traversed = self.forest.node_coverage
    ...
    cpdef tuple get_mirror_reflection_couplings(
        ...
        cdef set internal_cav_nodes = set()
    ...
    def get_nodes_with_changing_q(self):
        ...
        if len(affected_cavities) == 0:
            ...
            if len(output_trees) > 0:
                ...
                q_changing_nodes = set.union(*(t.get_all_nodes() for t in output_trees))
        else:
            # otherwise get all the nodes in each of the cavities being affected
            q_changing_nodes = set.union(*(tree.get_all_nodes() for tree in chain.from_iterable(model.trace_forest.trees_of_dependency(c) for c in affected_cavities)))
        ...
        if model.trace_forest.symmetric and len(q_changing_nodes) > 0:
            ...
            q_changing_nodes = set.union(q_changing_nodes, {n.opposite for n in q_changing_nodes})
...
cdef bint dep_match_pred(TraceTree tree, object dependency):
    return tree.dependency is dependency

cdef bint internal_cav_pred(TraceTree tree):
    return tree.is_source and tree.dep_type == DependencyType.CAVITY

cdef bint external_cav_pred(TraceTree tree):
    return not tree.is_source and tree.dep_type == DependencyType.CAVITY

cdef bint gauss_pred(TraceTree tree):
