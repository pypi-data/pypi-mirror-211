...
cdef class TraceTree:
    ...
    def __init__(self, object node, object dependency):
        ...
        self.c_left_abcd_x = NULL
        self.c_left_abcd_y = NULL
        self.c_right_abcd_x = NULL
        self.c_right_abcd_y = NULL
    ...
    def from_cavity(cls, cavity):
        ...
        parent.set_left_abcd_x_memory(
            parent.node.component.ABCD(
                parent.node, root.node, direction='x', copy=False, retboth=True,
            )
        )
        parent.set_left_abcd_y_memory(
            parent.node.component.ABCD(
                parent.node, root.node, direction='y', copy=False, retboth=True,
            )
        )
    ...
    cpdef TraceTree add_left(self, TraceTree sub_tree):
        ...
        try:
            self.set_left_abcd_x_memory(comp.ABCD(
                parent_node, sub_tree.node, direction='x', copy=False, retboth=True
            ))
            self.set_left_abcd_y_memory(comp.ABCD(
    ...
    cpdef TraceTree add_right(self, TraceTree sub_tree):
        ...
        try:
            self.set_right_abcd_x_memory(comp.ABCD(
                parent_node, sub_tree.node, direction='x', copy=False, retboth=True
            ))
            self.set_right_abcd_y_memory(comp.ABCD(
                parent_node, sub_tree.node, direction='y', copy=False, retboth=True
            ))
        except TotalReflectionError:
            raise
    ...
    cdef void set_left_abcd_x_memory(self, tuple Ms):
        self.sym_left_abcd_x, self.left_abcd_x = Ms
        self.c_left_abcd_x = &self.left_abcd_x[0][0]

    cdef void set_left_abcd_y_memory(self, tuple Ms):
        self.sym_left_abcd_y, self.left_abcd_y = Ms
        self.c_left_abcd_y = &self.left_abcd_y[0][0]

    cdef void set_right_abcd_x_memory(self, tuple Ms):
        self.sym_right_abcd_x, self.right_abcd_x = Ms
        self.c_right_abcd_x = &self.right_abcd_x[0][0]

    cdef void set_right_abcd_y_memory(self, tuple Ms):
        self.sym_right_abcd_y, self.right_abcd_y = Ms
        self.c_right_abcd_y = &self.right_abcd_y[0][0]
    ...
    cdef _get_all_nodes(self, set nodes):
        nodes.add(self.node)
    ...
    cpdef bint contains(self, object o):
    ...
    cdef bint _contains_tree(self, TraceTree tree):
        if self is tree:
            return True
    ...
    cdef bint _contains_node(self, object node):
        return self.find_tree_at_node(node) is not None

    cdef bint _contains_space(self, object space):
    ...
    cdef bint _contains_comp(self, object comp):
    ...
    cdef void c_compute_rt_abcd(self, double* abcdx, double* abcdy):
        cdef Py_ssize_t i
        ...
        while t is not None:
            if abcdx != NULL:
                c_abcd_multiply(abcdx, t.c_left_abcd_x, out=abcdx, m2_refl_transform=t.is_left_surf_refl)
            if abcdy != NULL:
