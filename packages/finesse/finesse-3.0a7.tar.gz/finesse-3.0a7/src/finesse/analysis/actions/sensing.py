...
class OptimiseRFReadoutPhaseDC(Action):
    ...
    def __init__(self, *args, d_dof=1e-9, name="optimise_demod_phases_dc"):
        super().__init__(name)
        self.args = args
        self.dofs = elements_to_name(args[::2])
        self.readouts = elements_to_name(args[1::2])
        self.d_dof = d_dof
...
class SensingMatrixSolution(BaseSolution):
    ...
    def display(
        ...
        tablefmt : str, optional
            ...
            use `finesse.utilities.Table`. Defaults to 'pandas' if available.
...
class CheckLinearity(Action):
    ...
    *locks : list, optional
    ...
    def _do(self, state):
        ...
        lock_dof_names = [deref(lck.feedback).component.name for lck in locks]
