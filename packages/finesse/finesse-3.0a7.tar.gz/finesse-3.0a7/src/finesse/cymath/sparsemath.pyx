...
cdef class CSRMatrix:
    ...
    def __init__(self, np.ndarray[complex, ndim=2] A):
        ...
        self.M.nnz = len(rows)
        self.M.row_ptr = <unsigned int*> malloc(sizeof(unsigned int) * (self.M.rows+1))
        if not self.M.row_ptr:
            raise MemoryError()
        self.M.col_index = <unsigned int*> malloc(sizeof(unsigned int) * self.M.nnz)
        if not self.M.col_index:
            raise MemoryError()
        self.M.values = <complex*> malloc(sizeof(complex) * self.M.nnz)
...
cdef inline int zcsrgemv(csr_matrix *M, complex *x, Py_ssize_t nx, complex *y, Py_ssize_t ny) nogil:
...
cdef inline int zcsrgecmv(csr_matrix *M, complex *x, Py_ssize_t nx, complex *y, Py_ssize_t ny, complex *z) nogil:
    """Complex (z) valued CSR (csr) general (ge) hermitiain conjugate vector (c) matrix (m) vector (v) product.
...
cdef inline int zcsrgevmv(csr_matrix *M, complex *x, Py_ssize_t nx, complex *y, Py_ssize_t ny, complex *z) nogil:
    """Complex (z) valued CSR (csr) general (ge) transpose vector (v) matrix (m) vector (v) product.
