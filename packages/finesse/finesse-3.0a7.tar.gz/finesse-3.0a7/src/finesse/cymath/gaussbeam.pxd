...
cdef inline double bp_beamsize(const beam_param* bp) nogil:
    return beam_size(bp.q, bp.nr, bp.wavelength)

cdef inline double bp_waistsize(const beam_param* bp) nogil:
    return waist_size(bp.q, bp.nr, bp.wavelength)

cdef inline double bp_waistpos(const beam_param* bp) nogil:
    return creal(bp.q)

cdef inline double bp_rayleigh(const beam_param* bp) nogil:
    return cimag(bp.q)

cdef inline double bp_gouy(const beam_param* bp) nogil:
    return gouy(bp.q)

cdef inline double bp_divergence(const beam_param* bp) nogil:
    return divergence(bp.q, bp.nr, bp.wavelength)

cdef inline double bp_radius_curvature(const beam_param* bp) nogil:
    return radius_curvature(bp.q)

cdef inline double bp_defocus(const beam_param* bp) nogil:
    return defocus(bp.q)


cdef inline double bp_overlap(const beam_param* bp_x, const beam_param* bp_y) nogil:
    return overlap(bp_x.q, bp_y.q)


### Corresponding functions acting on standard complex_t data ###

cpdef inline double beam_size(complex_t q, double nr, double lambda0) nogil:
    return cabs(q) * sqrt(lambda0 / (nr * PI * cimag(q)))

cpdef inline double waist_size(complex_t q, double nr, double lambda0) nogil:
    return sqrt(lambda0 * cimag(q) / (PI * nr))

cpdef inline double gouy(complex_t q) nogil:
    return atan2(creal(q), cimag(q))

cpdef inline double divergence(complex_t q, double nr, double lambda0) nogil:
    return lambda0 / (waist_size(q, nr, lambda0) * PI)

cpdef inline double radius_curvature(complex_t q) nogil:
    cdef:
        double z = creal(q)
        double zr = cimag(q)
        double zr_on_z = zr / z
...
cpdef inline double defocus(complex_t q) nogil:
    cdef:
        double z = creal(q)
        double zr = cimag(q)
...
cpdef inline double overlap(complex_t qx, complex_t qy) nogil:
