...
ELSE:
    ...
    # --------------------------------------------------------------------
    cdef inline double cabs(complex_t z) nogil:
        return _cabs((<_Dcomplex*>&z)[0])

    cdef inline double carg(complex_t z) nogil:
        return _carg((<_Dcomplex*>&z)[0])

    cdef inline complex_t cexp(complex_t z) nogil:
        cdef _Dcomplex _z = _cexp((<_Dcomplex*>&z)[0])
        return (<complex_t*>&_z)[0]

    cdef inline complex_t csqrt(complex_t z) nogil:
        cdef _Dcomplex _z = _csqrt((<_Dcomplex*>&z)[0])
        return (<complex_t*>&_z)[0]

    cdef inline complex_t conj(complex_t z) nogil:
        cdef _Dcomplex _z = _conj((<_Dcomplex*>&z)[0])
        return (<complex_t*>&_z)[0]

    cdef inline complex_t clog(complex_t z) nogil:
        cdef _Dcomplex _z = _clog((<_Dcomplex*>&z)[0])
        return (<complex_t*>&_z)[0]

    cdef inline double creal(complex_t z) nogil:
        return z.real

    cdef inline double cimag(complex_t z) nogil:
        return z.imag
    # --------------------------------------------------------------------
