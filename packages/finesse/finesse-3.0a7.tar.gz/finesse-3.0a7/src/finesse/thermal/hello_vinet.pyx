...
from scipy.special import jv, j0, eval_genlaguerre, eval_laguerre
import numpy as np
cimport numpy as np
...
def eval_p_n_s_numerical(result):
    ...
    I_r : ndarray
        Irradiance from fit
    """
    r, chi, p_n_s, eta_n_s, Jn_k_ns_r_a, material = result
...
def substrate_thermal_expansion_depth(data, z, h):
    ...
    omega_0 *= p_n_s * jv(0, eta_n_s)/(eta_n_s**3) * sinh_gamma_d1 # Eq 3.113

    omega_1 = -12*material.alpha * material.E * chi *a/(np.pi*material.kappa*h**3)
    omega_1 *= (p_n_s * jv(0, eta_n_s)/(eta_n_s**3) * (gamma_n_s * cosh_gamma - sinh_gamma)/d2_n_s).sum(1) # Eq 3.113
    # Eq 3.117
...
def surface_deformation_substrate_heating(data, h):
    ...
    eta_n_s_quart = eta_n_s_cubed * eta_n_s
