...
from finesse.cymath.complex cimport complex_t, inverse_unsafe, ceq, cpow_re, cexp, conj, creal, cimag, crotate2, csqrt, COMPLEX_0
from finesse.cymath.gaussbeam cimport waist_size, bp_waistsize, beam_param, beam_size, gouy
from finesse.cymath.math cimport factorial, sqrt_factorial, hermite
...
cdef void unm_ws_recache_from_bp(
    ...
    int* n=NULL,
    int* m=NULL,
) nogil: # Initialise uws from beam_param constructs
...
cdef class HGModeWorkspace:
    ...
    cpdef u_n(self, x, complex_t[::1] out=None):
        ...
        print(self.uws.xpre_factor)
        print(hermite(self.uws.n, self.uws.root2_on_wx * xs[0]))
        print(cexp(self.uws.xgauss_exponent * xs[0] * xs[0]))
...
cdef class HGModes:
    ...
    def __init__(self, q, modes, bint zero_tem00_gouy=False, bint reverse_gouy=False, bint flip_odd_x_modes=False):
        ...
        self.unique_n, a = np.unique(np.array(modes)[:,0], return_inverse=True)
        self.unique_m, b = np.unique(np.array(modes)[:,1], return_inverse=True)
    ...
    cdef void set_q(self, beam_param* qx, beam_param* qy) nogil:
    ...
    cdef complex_t fast_Unm(self, int n, int m, double x, double y) nogil:
        return u_nm__fast(&self.unm_ws, &self.unm_factor_ws, n, m, x, y)

    cdef complex_t fast_Un(self, int n, double x) nogil:
        return u_n__fast(&self.unm_ws, &self.unm_factor_ws, n, x)

    cdef complex_t fast_Um(self, int m, double y) nogil:
