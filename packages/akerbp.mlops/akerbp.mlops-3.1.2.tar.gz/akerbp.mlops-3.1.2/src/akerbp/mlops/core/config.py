...
@dataclass
class CdfClientSecrets:
    """
    dataclass for keeping track of CDF client secrets
...
def read_cdf_client_secrets() -> Dict[str, str]:
    secrets = asdict(
        CdfClientSecrets(
    ...
    return {k.replace("_", "-"): v for k, v in secrets.items()}


client_secrets = read_cdf_client_secrets()
...
def read_project_settings(
    ...
    for setting in settings:
        ...
        if valid_metadata:
            ...
            logger.error("Invalid metadata specification")
