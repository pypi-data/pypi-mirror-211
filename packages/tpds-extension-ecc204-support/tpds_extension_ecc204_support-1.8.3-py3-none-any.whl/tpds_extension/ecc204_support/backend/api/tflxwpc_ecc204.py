...
class ECC204TFLXWPCPackage(GenerateProvisioningPackage):
    ...
    def __init__(self, config_string) -> None:
        try:
            super().__init__(config_string, "ECC204_TFLXWPC")
            self.process_xml()
            self.process_enc()
...
def ecc204_tflxwpc_generate_fw_resources(fw_resource, base_folder):
    """
    ...
    with open(os.path.join(base_folder, "ecc204_tflxwpc.h"), "w") as f:
...
def ecc204_tflxwpc_proto_prov_handle(config_str):
    """
    ...
    base_folder = os.path.join(TPSettings().get_base_folder(), "wpc_proto_provision")
    ...
    try:
        ...
        for address in [0x33, 0x38, user_device_address]:
            try:
                ecc204_proto_provision = ECC204Provision(
        ...
        if is_device_connected:
            msg_box_info = "<font color=#0000ff>You are about to proto provision a blank ECC204 device. Changes cannot be reverted once provisioning is done. Do you want to continue?"
            blank_device_prov = tp_userinput.TPMessageBox(
                title="ECC204 Blank device Provisioning", info=msg_box_info
            )
            if not ecc204_proto_provision.element.is_config_zone_locked():
                ...
                if blank_device_prov.user_select != "Cancel":
                    ...
                    # Set Counter
                    counter_bytes = ecc204_proto_provision.int_to_binary_linear(
                    ...
                    # write Config slot
                    ecc204_proto_provision.element.load_tflx_test_config(
                        bytes(configBytes)
                    )
                else:
                    display_msg = "ECC204 Proto provisioning aborted."
            else:
            ...
            if device_prov.user_select != "Cancel":
                ...
                # read public key
                puc_pubkey = ecc204_proto_provision.element.read_device_public_key(
                    0)
                device_sn = ecc204_proto_provision.element.get_device_serial_number()

                key_file = os.path.join(
                    base_folder, f"wpc_puc_ecc204_{device_sn.hex().upper()}.key"
                )
                cert_file = os.path.join(
                    base_folder, f"wpc_puc_ecc204_{device_sn.hex().upper()}.crt"
                )
                ...
                fw_resource = ecc204_proto_provision.provision_wpc_slots(
                    root_cert, mfg_cert, puc_cert
                )
                ecc204_tflxwpc_generate_fw_resources(fw_resource, base_folder)
                ...
                for slot in slot_info:
                    ...
                    if slot.get("key_load_config") not in ["cert", "no_load"] and slot.get("data"):
                        ...
                        ecc204_proto_provision.perform_slot_write(slot_id, slot_data)
            ...
            display_msg = "Unable to connect to ECC204 device, Please try with different device address"
    except BaseException as e:
