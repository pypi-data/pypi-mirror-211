...
class ECC204TFLXAuthPackage(GenerateProvisioningPackage):
    ...
    def __init__(self, config_string) -> None:
        try:
            super().__init__(config_string, "ECC204_TFLXAUTH")
            self.process_xml()
            self.process_cert()
            self.process_enc()
            self.process_csr()
...
def ecc204_tflxauth_proto_prov_handle(config_str):
    data = config_str
    if isinstance(config_str, str):
        data = json.loads(config_str)
    log(f"Data for Proto Provisioning: {data}")

    log("Provisioning ECC204-TFLXAUTH")
    response_msg = "Error"
    display_msg = (
    ...
    try:
        ...
        if is_device_connected:
            msg_box_info = (
                "<font color=#0000ff>You are about to proto provision a blank ECC204 device."
            ...
            if not ecc204_proto_provision.element.is_config_zone_locked():
                ...
                if blank_device_prov.user_select != "Cancel":
                    ...
                    # CSZ2
                    # Set Counter Value
                    counterBytes = ecc204_proto_provision.int_to_binary_linear(
            ...
            display_msg = "Unable to connect to ECC204 device, Check device address and connections before retrying"
    except BaseException as e:
