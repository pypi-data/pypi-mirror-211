...
class TensorRTRunner(Runner):
    ...
    ------
        MissingLibraryException
            This exception will be raised if the tensorrt and pycuda library
            is not installed.
    ...
    def __init__(
        ...
        super(TensorRTRunner, self).__init__(engine_path)
    ...
    def run_single_instance(self, image):
        ...
        the ImageBatcher class. Memory copying to and from the GPU device
    ...
    def resize_pad(self, image, pad_color=(0, 0, 0)):
        """
        This function is a subroutine to implement padding and resizing.
        This will resize the image to fit fully within the input
    ...
    def preprocess_input(self, image_path):
        """
        This method uses the image preprocessor which loads an image from disk
        ...
        * EfficientDet: Resizes and pads the image to fit the input size.
        ...
        if self.preprocessor == "EfficientDet":
            ...
            print(
                "Preprocessing method {} not supported".format(
                    self.preprocessor))
            exit(1)
        if self.format == "NCHW":
    ...
    def postprocessing(self, outputs, img_scale, width, height):
        """
        This method processing the prediction outputs to parse the
        ...
        for n in range(int(nums[0])):
            ...
            if img_scale:
                scale /= img_scale
            if scores[0][n] < self.score_threshold:
                continue
            ...
            label = int(classes[0][n]) + self.label_offset
    ...
    def input_spec(self):
        """
        This method gets the specs for the input tensor
        of the network. Useful to prepare memory allocations.
        Unit-test for this method is defined under:
            file: test/deepview/validator/runners/test_tensorrt.py
            function: test_input_spec
    ...
    def output_spec(self):
        """
        This method gets the specs for the output tensors of the network.
