...
def create_mask_image(height, width, gt_instance):
        ...
        gt_instance: dict
            ...
            .. code-block:: python

                {
                    ...
                    "height": image height,
                    "width": image width,
                    "boxes": contains the polygons in each list,
    ...
    ------
        ValueError
            This method will raise an exception if gt_instance is not a
    ...
    if not (isinstance(gt_instance, dict)):
        ...
        if (width <= 0 or height <= 0):
            ...
            for c, polygon in zip(gt_labels, gt_boxes):
...
def create_binary_mask(mask):
    """
    This function creates a binary numpy array of 1's and 0's
    encapsulating every object (regardless of class) in the image as a 1
    and background as 0.
    Unit-test for this function is defined under:
        file: test/deepview/validator/metrics/test_segmentationutils.py
...
def create_mask_class(mask, cls):
    """
    This function separates a mask with more than one classes into
...
def create_mask_classes(new_mask, cls, current_mask=None):
    """
    This function appends a current mask with another
    mask of different class i.e converting a binary
    mask (new mask) into a mask with its class and
    then appending the original mask to include
    ...
    ------
        ValueError
            This function will raise an exception if the input
            masks is not of type np.ndarray or if the input
            cls is not of type Integer.
    """
...
def create_mask_background(mask):
    """
    This function creates a binary mask for the background class with ones
    ...
    ------
        ValueError
            This function will raise an exception if the input
            masks is not of type np.ndarray.
    """
...
def classify_mask(height, gt_class_mask, dt_class_mask, dt_mask):
    """
    This function classifies if the pixel is either a true positive,
    ...
    ------
        ValueError
            This function will raise an exception if the input
            masks is not of type np.ndarray or if the input
            height is not of type Integer or has invalid dimension.
    """
