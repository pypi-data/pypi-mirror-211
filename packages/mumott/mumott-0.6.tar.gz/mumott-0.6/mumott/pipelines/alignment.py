...
from ..data_handling import DataContainer, Geometry
from .reconstruction import run_sirt
...
def shift_center_of_reconstruction(geometry: Geometry,
    ...
    :class:`geometry <mumott.data_handling.Geometry>` based on a three-dimensional
    shift vector. Use this to reposition the reconstruction within the volume.

    Parameters
    ----------
    geometry
        A :class:`geometry <mumott.data_handling.Geometry>` instance.
    ...
    >>> from mumott.data_handling.geometry import Geometry, GeometryTuple
...
def run_cross_correlation_alignment(data_container: DataContainer,
                                    ...
                                    reconstruction_pipeline: Callable = run_sirt,
                                    reconstruction_pipeline_kwargs: Dict[str, any] = None,
                                    use_gpu: bool = False,
                                    use_absorbances: bool = True,
                                    maxiter: int = 20,
                                    upsampling: int = 1,
                                    shift_tolerance: float = 1.,
                                    shift_cutoff: float = 5,
                                    relative_sample_size: float = 0.3,
                                    relaxation_weight: float = 0.01,
                                    center_of_mass_shift_weight: float = 0.01,
                                    align_j: bool = True,
    ...
    data_container
        ...
        factors in :class:`data_container.geometry <mumott.data_handling.Geometry>` will be
    ...
    optimizer_kwargs = reconstruction_pipeline_kwargs.get('optimizer_kwargs', dict())
    optimizer_kwargs['no_tqdm'] = True
    reconstruction_pipeline_kwargs['optimizer_kwargs'] = optimizer_kwargs
    ...
    number_of_samples = int(np.ceil(len(data_container.geometry) * relative_sample_size))
    ...
    for i in tqdm.tqdm(range(maxiter), file=sys.stdout):
        ...
        sampled_subset = rng.choice(len(projections), number_of_samples, replace=False)
