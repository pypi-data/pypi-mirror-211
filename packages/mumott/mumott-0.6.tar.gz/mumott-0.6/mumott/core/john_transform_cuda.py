...
from numba import config, cuda, int32, float32, float64
...
def john_transform_cuda(field: NDArray, projections: NDArray, unit_vector_p: NDArray,
    ...
    # Start by transferring field/projections to GPU. (TODO: Handle better)
    ...
    def bilinear_interpolation(field: NDArray,
        ...
        if not (0 <= r1 < dimensions[1] and 0 <= r2 < dimensions[2]):
            return
        # At edges, use nearest-neighbor interpolation.
        if (1 <= int32(r1) + 1 < dimensions[1]):
            r1_weight = 1
        else:
            r1_weight = 0

        if (1 <= int32(r2) + 1 < dimensions[2]):
            r2_weight = 1
        else:
            r2_weight = 0
        ...
        if (direction_0 == 0):
            y_weight = (r1 - floor(r1)) * r1_weight
            z_weight = (r2 - floor(r2)) * r2_weight
            x = int32(r0)
            ...
            for i in range(accumulator.size):
                ...
                accumulator[i] += field[x, y + r1_weight, z][i] * t.z
                accumulator[i] += field[x, y, z + r2_weight][i] * t.w
                accumulator[i] += field[x, y + r1_weight, z + r2_weight][i] * t.y

        elif (direction_0 == 1):
            x_weight = (r1 - floor(r1)) * r1_weight
            z_weight = (r2 - floor(r2)) * r2_weight
            x = int32(r1) + r1_edge_weight
            y = int32(r0)
            z = int32(r2) + r2_edge_weight
            t = cuda_x4_type((1 - x_weight) * (1 - z_weight),
                             x_weight * z_weight,
                             x_weight * (1 - z_weight),
                             (1 - x_weight) * z_weight)
            for i in range(accumulator.size):
                accumulator[i] += field[x, y, z][i] * t.x
                accumulator[i] += field[x + r1_weight, y, z][i] * t.z
                accumulator[i] += field[x, y, z + r2_weight][i] * t.w
                accumulator[i] += field[x + r1_weight, y, z + r2_weight][i] * t.y

        elif (direction_0 == 2):
            x_weight = (r1 - floor(r1)) * r1_weight
            y_weight = (r2 - floor(r2)) * r2_weight
            x = int32(r1) + r1_edge_weight
            y = int32(r2) + r2_edge_weight
            z = int32(r0)
            t = cuda_x4_type((1 - x_weight) * (1 - y_weight),
                             x_weight * y_weight,
                             y_weight * (1 - x_weight),
                             (1 - y_weight) * x_weight)
            for i in range(accumulator.size):
                accumulator[i] += field[x, y, z][i] * t.x
                accumulator[i] += field[x, y + r2_weight, z][i] * t.z
                accumulator[i] += field[x + r1_weight, y, z][i] * t.w
                accumulator[i] += field[x + r1_weight, y + r2_weight, z][i] * t.y
    ...
    def john_transform_inner(field: NDArray, projection: NDArray):
        ...
        for k in range(start_k, end_k):
            ...
            position_0 = cuda_float(start_slice)
            position_1 = step_size_1 * (cuda_float(start_slice) - 0.5 * dimensions[0]) + \
                centering_step_1 + 0.5 * dimensions[1]
            position_2 = step_size_2 * (cuda_float(start_slice) - 0.5 * dimensions[0]) + \
                centering_step_2 + 0.5 * dimensions[2]
    ...
    john_transform_inner[bpg, tpb](device_field,
                                   device_projections)
...
def john_transform_adjoint_cuda(field: NDArray, projections: NDArray, unit_vector_p: NDArray,
    ...
    # Projection and frame vectors. s for positioning the frame.
    ...
    # Translate volume steps to normalized frame steps. Can add support for non-square voxels.
    ...
    def bilinear_interpolation_projection(projection: NDArray,
        ...
        if not (-1 <= r1 < max_j and -1 <= r2 < max_k):
            return
        # At edges, use nearest-neighbor interpolation.
        if (0 <= int32(r1) + 1 < max_j):
            r1_weight = 1
        else:
            r1_weight = 0

        if (0 <= int32(r2) + 1 < max_k):
            r2_weight = 1
        else:
            r2_weight = 0
        ...
        for i in range(channels):
            ...
            accumulator[i] += projection[x, y + r1_weight, z][i] * t.z
            accumulator[i] += projection[x, y, z + r2_weight][i] * t.w
            accumulator[i] += projection[x, y + r1_weight, z + r2_weight][i] * t.y
    ...
    def john_transform_adjoint_inner(field: NDArray, projection: NDArray, start_index: int, stop_index: int):
        ...
        fx = x - 0.5 * field.shape[0]
        fy = y - 0.5 * field.shape[1]
        fz = z - 0.5 * field.shape[2]
        ...
        for a in range(start_index, stop_index):
            ...
            fj = norm_offset_j_c[a] + fx * norm_j_c[a][0] + fy * norm_j_c[a][1] + fz * norm_j_c[a][2]
            fk = norm_offset_k_c[a] + fx * norm_k_c[a][0] + fy * norm_k_c[a][1] + fz * norm_k_c[a][2]
    ...
    john_transform_adjoint_inner[bpg, tpb](device_field,
                                           device_projections,
                                           0, projections.shape[0])
