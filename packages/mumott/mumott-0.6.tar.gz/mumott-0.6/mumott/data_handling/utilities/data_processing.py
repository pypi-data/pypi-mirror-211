...
def get_transmittivities(diode: NDArray,
    ...
    the flux is constant over each frame. This corresponds to the slightly modified computation
    ...
    normalize_per_frame
        If ``True``, the diode will be normalized frame-wise.
        This is the appropriate choice for absorption tomography.
        For :term:`SAXS`, it is preferable to normalize the diode across the entire stack
        of diode measurements in order to account for possible variations in flux.
        Default value is ``False``.
    normalization_percentile
        The percentile of values in either the stack of diode measurements or each frame
        (depending on :attr:`normalize_per_frame`) to use for normalization.
    ...
    if normalize_per_frame:
        for i, t in enumerate(transmittivity):
            # Normalize each frame's maximum value to 1, cutting off values outside the range.
        ...
        # Normalize the whole stack's maximum value to 1, cutting off values outside the range.
...
def get_absorbances(diode: NDArray, **kwargs) -> Dict[str, Any]:
    r""" Calculates the absorbance based on the transmittivity of the diode in the stack.
