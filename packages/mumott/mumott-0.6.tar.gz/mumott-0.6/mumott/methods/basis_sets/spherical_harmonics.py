...
class SphericalHarmonics(BasisSet):
    ...
    def __init__(self,
        ...
        self._get_coefficient_indices()
        self._get_projection_matrix()

    def _get_coefficient_indices(self) -> None:
        """
        ...
        changes.
        """
        mm = np.zeros((self._ell_max + 1) * (self._ell_max // 2 + 1), dtype=int)
        ll = np.zeros((self._ell_max + 1) * (self._ell_max // 2 + 1), dtype=int)
        count = 0
        for h in range(0, self._ell_max + 1, 2):
    ...
    def _get_projection_matrix(self) -> None:
    ...
    def ell_max(self, val: int) -> NDArray[int]:
        if (val % 2 != 0) or (val < 0) or (val != round(val)):
            raise ValueError('ell_max must be an even, non-negative integer,'
                             f' but a value of {self._ell_max} was given!')
        self._ell_max = val
        self._get_coefficient_indices()
        self._get_projection_matrix()
