...
class HuberLoss(LossFunction):
    ...
    functional : Functional
        The :ref:`functional instance <functionals>` from which the
    ...
    preconditioner : np.ndarray
        ...
        :attr:`functional.coefficients` or it must be possible to broadcast by multiplication.
    ...
    def __init__(self,
        ...
        super().__init__(functional, use_weights, preconditioner, residual_norm_multiplier)
        self._delta = delta

    def _get_residual_norm_internal(self, get_gradient: bool = False) -> Dict:
        """ Gets the residual norm, and if needed,
        the gradient, using the attached :attr:`functional`.
        ...
        """
        functional_output = self._functional.get_residuals(
            get_gradient=False, get_weights=self._use_weights)
        residuals = functional_output['residuals']
        ...
        if self.use_weights:
            ...
            residual_norm += 0.5 * np.reciprocal(self._delta) * np.einsum(
                ...
                residuals[where].ravel(),
                functional_output['weights'][where].ravel(),
                optimize='greedy')
            residual_norm += np.dot(abs(residuals[~where]) - 0.5 * self._delta,
                                    functional_output['weights'][~where])
        else:
        ...
        if get_gradient:
            ...
            if self.use_weights:
                residuals *= functional_output['weights']
            gradient = self._functional.get_gradient_from_residual_gradient(residuals)
        else:
            gradient = None
