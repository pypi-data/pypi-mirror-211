...
class LossFunction(ABC):
    ...
    preconditioner
        ...
        :attr:`functional.coefficients <mumott.methods.functionals.base_functional.Functional.coefficients>`
    ...
    def get_loss(self,
        ...
        ----------
        coefficients
            An ``NDArray`` of values of the same shape as :attr:`functional.coefficients
            <mumott.methods.functionals.base_functional.Functional.coefficients>`. Default value
            is ``None``, which leaves it to :meth:`get_residual_norm`
            to handle the choice of coefficients, which in general defaults to using the coefficients
            of the attached :attr:`functional`.
        get_gradient
            If ``True``, returns a ``'gradient'`` of the same shape as :attr:`functional.coefficients
            <mumott.methods.functionals.base_functional.Functional.coefficients>`.
        ...
        regularization = self.get_regularization_norm(self._functional.coefficients, get_gradient)
        ...
        if get_gradient:
            ...
            if self.preconditioner is not None:
                if self._functional.coefficients.shape[:-1] != self.preconditioner.shape[:-1]:
                    raise ValueError('The first three dimensions of the preconditioner must'
                                     ' have the same size as the coefficients of the functional,'
                                     ' and the last index must be the same or 1, but the'
                                     ' functional coefficients have shape'
                                     f' {self._functional.coefficients.shape},'
    ...
    def get_residual_norm(self,
        ...
        :attr:`functional <mumott.methods.functionals.base_functional.Functional>`.
        If :attr:`coefficients` is given, :attr:`functional.coefficients
        <mumott.methods.functionals.base_functional.Functional.coefficients>` will be updated
        with these new values, otherwise, the residual norm and possibly the gradient
        ...
        if coefficients is not None:
            self._functional.coefficients = coefficients
    ...
    def get_regularization_norm(self,
        ...
        If no coefficients are provided, the ones from the attached :attr:`functional` are used.

        Parameters
        ----------
        coefficients
            An ``np.ndarray`` of values of the same shape as :attr:`functional.coefficients
            <mumott.methods.functionals.base_functional.Functional.coefficients>`.
        get_gradient
        ...
        if coefficients is None:
            coefficients = self._functional.coefficients
