...
class SquaredLoss(LossFunction):
    ...
    gradient from a given :ref:`functional <functionals>`.
    ...
    of the :attr:`functional` to compute.
    ...
    def _get_residual_norm_internal(self, get_gradient: bool = False) -> Dict:
        ...
        if self.use_weights:
            ...
            residual_norm = 0.5 * np.einsum(
                'ijkh, ijkh, ijkh -> ...', residuals, residuals, functional_output['weights'])
        else:
            residual_norm = 0.5 * np.einsum(
        ...
        return dict(residual_norm=residual_norm, gradient=functional_output['gradient'])
