...
def search(q, prefix='all', start=0, max_results=100, sort_by='relevance', sort_order='descending'):
    ...
    for entry in parser['entries']:
        result = {
            'arxiv_id': re.search(regx_id, entry['id']).group(),
            'title': entry['title'],
...
def google(q):
    ...
    # results = requests.get(url, params=params, headers=headers).content
    ...
    for item in parser['items']:
        result = {
            'arxiv_id': item['pagemap']['metatags'][0]['citation_arxiv_id'],
            'title': item['pagemap']['metatags'][0]['citation_title'],
            'published': item['pagemap']['metatags'][0]['citation_date'],
            'authors': item['pagemap']['metatags'][0]['citation_author'],
            'abstract': item['pagemap']['metatags'][0]['citation_abstract'],
        }
        results.append(result)
...
if __name__ == '__main__':
    ...
    # arxiv_id = "0201082v1"
    # arxiv_id = "0608197" # incorrect_id_format
    # arxiv_id = "quant-ph/0608197" # incorrect_id_format

    # print(fetch(arxiv_id))
    # print(fetch(arxiv_id))

    # results = search('styleGAN3')
    # print(results[0])

    # result = fetch('2106.12423')
    # print(result)

    # results = fetchall(['2106.12423', '2303.08774'])
    # print(results)
