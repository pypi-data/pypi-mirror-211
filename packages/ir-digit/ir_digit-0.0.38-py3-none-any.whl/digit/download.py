...
class Download():
    ...
    def _generate_config(self,data_obj):
        ...
        #     "description": ""
        # }
        sort_json = ['data_id','user_id','url','name','author','storage_type','data_type','language','task_type','tags','description','picture','md_instruction','usage_number','permission_level','status','create_time','update_time']
        temp = {i:data_obj.get(i) for i in sort_json}
        with open(os.path.join(self.cache_path,data_obj.data_id),'w', encoding="utf-8" ) as f:
            json.dump(temp, f,indent=4)
        print(f"资源信息已保存至config.json：{os.path.join(self.cache_path,data_obj.data_id,'config.json')}")
        return True
    ...
    def _update_cache(self, data_id):
        ...
        url = data_obj.url if data_obj.url.endswith(".git") else data_obj.url + ".git" # git仓库
        print("正在下载/更新 Git远程仓库")
        if not  os.path.exists(os.path.join(self.cache_path, data_id)):
            # 获取资源
            path = os.path.mkdir(os.path.join(self.cache_path,data_id)) # 创建缓存目录，以data_id命名
