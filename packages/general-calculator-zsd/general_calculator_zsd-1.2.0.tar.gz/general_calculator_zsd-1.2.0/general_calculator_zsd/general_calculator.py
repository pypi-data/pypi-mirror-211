...
def insert_json_sql(json_data, insert_db, insert_tbl, db_config):
    ...
    # insert_conn = pymysql.connect(host=db_config[insert_db]['host'], port=db_config[insert_db]['port'],
    #                               user=db_config[insert_db]['user'], password=db_config[insert_db]['password'],
    #                               db=db_config[insert_db]['database'])
    # insert_cursor = insert_conn.cursor()
    # 写入数据库
    # keys = ", ".join(json_data.keys())
    # values = ", ".join(['%s'] * len(json_data.keys()))
    # insert_sql = '''INSERT INTO {table}({keys}) VALUES ({values}) ON DUPLICATE KEY UPDATE'''.format(table=insert_tbl,
    #                                                                                                 keys=keys, values=values)
    # update = ','.join([" {key} = %s".format(key=key) for key in json_data.keys()])
    # insert_sql += update
    keys = ', '.join(json_data.keys())
    ...
    #                  today, arr_a[3], arr_a[9], arr_a[23], float(arr_a[11]), float(arr_a[13]), float(arr_a[15]),
    #                  float(arr_a[17]), today, arr_a[3], arr_a[9], arr_a[23], float(arr_a[11]), float(arr_a[13]),
    #                  float(arr_a[15]), float(arr_a[17])
...
def create_task_id(task_owner, db_config, platform):
    ...
    insert_json_sql(task_dict, db, 'log_task_info', db_config)  # 默认都会根据主键对数据进行更新
    return task_dict
