...
class Xml:
    ...
    def dumps(self, obj):
        return self.serialize_to_str(convert(obj))
    ...
    def serialize_to_str(self, obj):
        ...
        elif type(obj) in (tuple,list,dict,set,frozenset):
    ...
    def serialize_dict(self, obj):
        ...
        for key, value in obj.items():
            result += ' ' * self.indent + f'<{self.serialize_to_str(convert(key))}>\n'
            self.indent += 4
            result += ' ' * self.indent + f'{self.serialize_to_str(convert(value))}' + '\n'
            self.indent -= 4
    ...
    def serialize_collection(self, obj):
        ...
        for i in obj:
            result += ' ' * self.indent + f'{self.serialize_to_str(convert(i))}\n'
    ...
    def loads(self, s):
        self.pos = 0
        return deconvert(self.deserialize_from_str(s))
