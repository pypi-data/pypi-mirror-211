...
def rio2patches(img_r, cfg):
    ...
    stride_height, stride_width = cfg.stride if "stride" in cfg else (512, 512)
...
class Inference:
    def __init__(self, cfg_string, weight_file, 
                 GS_ACCESS_KEY_ID=None, GS_SECRET_ACCESS_KEY=None):
    ...
    def segmentation(self, tif_path):
        ...
        for patch in patches:
            ...
            mask_patch = result[0].seg_logits.data.detach().cpu().numpy()
            mask_patches.append(mask_patch)

        full_mask = merge_seg_patches(
            np.array(mask_patches), location_of_patches
        )
    ...
    def detection(self, tif_path):
        ...
        for patch in patches:
            ...
            bboxes.append(list(result.pred_instances.bboxes.detach().cpu().numpy()))
            scores.append(list(result.pred_instances.scores.detach().cpu().numpy()))
            labels.append(list(result.pred_instances.labels.detach().cpu().numpy()))
