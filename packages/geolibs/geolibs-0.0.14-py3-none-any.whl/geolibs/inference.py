...
class Inference:
    ...
    def detection(self, tif_path):
        r, img = read_raster(tif_path)
        patches, location_of_patches = rio2patches(img, self.cfg)
        img_shape = patches.shape

        model_inputs = {
            "inputs": torch.from_numpy(patches), 
            "data_samples": [
        ...
        for res in result:
            bboxes.append(list(res.pred_instances.bboxes.detach().cpu().numpy()))
            scores.append(list(res.pred_instances.scores.detach().cpu().numpy()))
            labels.append(list(res.pred_instances.labels.detach().cpu().numpy()))
