...
async def _generic_error(error_type: str, number_of_retries: int, max_num_retries: int, max_retry_wait_time: int) -> None:
    upload_attempt = max_num_retries - number_of_retries + 1
    wait_time = get_wait_time(upload_attempt, max_retry_wait_time)
    log.warning(
        f"Failed to send request. {error_type}"
        f"Attempt {upload_attempt}/{max_num_retries}, retrying in {int(wait_time)} seconds."
    )
