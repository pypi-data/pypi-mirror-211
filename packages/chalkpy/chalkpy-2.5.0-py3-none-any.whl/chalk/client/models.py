...
class DatasetRevisionResponse(BaseModel):
    ...
    revision_id: uuid.UUID  # Currently, the revision ID is the job ID that created the revision
    creator_id: str
    outputs: List[str]
...
class BranchDeployResponse(BaseModel):
    ...
    def _repr_markdown_(self) -> str:
        if self.new_branch_created:
            prefix = "Created new "
        else:
            prefix = "Deployed "
        return f"{prefix} branch `{self.branch_name}` with source from deployment `{self.source_deployment_id}`."
