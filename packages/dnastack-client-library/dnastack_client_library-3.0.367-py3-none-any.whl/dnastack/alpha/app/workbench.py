...
### end batch additions
	...
	def stream_task_log_by_task(self, run: Union[ExtendedRun, str], task_name: str, logtype: LogType, max_bytes: Optional[int]) -> Iterable[bytes]:
		if type(run) is str:
	...
	def stream_log(self, log: Log, logtype: LogType, max_bytes: Optional[int]) -> Iterable[bytes]:
		if logtype==LogType.STDOUT:
