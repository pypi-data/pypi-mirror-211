...
class Workbench:
	_CUSTOM_WORKFLOW_SOURCE_NAME = "CUSTOM"
	...
	def get_workflow(self, workflow_name: str) -> Workflow:
		for workflow in self._get_workflow_client().list_workflows(source=Workbench._CUSTOM_WORKFLOW_SOURCE_NAME):
...
### end batch additions
	...
	def stream_task_log_by_task(self, run: Union[ExtendedRun, str], task_name: str, logtype: LogType, max_bytes: Optional[int] = None) -> Iterable[bytes]:
		if type(run) is str:
			run = self.describe_run(run, True)
		for task_log in run.task_logs:
			if task_log.name == task_name:
				if logtype==LogType.STDOUT:
					return self._get_ewes_client().stream_run_logs(task_log.stdout, max_bytes)
				else:
					return self._get_ewes_client().stream_run_logs(task_log.stderr, max_bytes)
	def stream_run_log(self, run: Union[ExtendedRun, str], logtype: LogType, max_bytes: Optional[int] = None) -> Iterable[bytes]:
		if type(run) is str:
			run = self.describe_run(run, True)
		return self.stream_log(run.run_log, logtype, max_bytes)

	def stream_log(self, log: Log, logtype: LogType, max_bytes: Optional[int] = None) -> Iterable[bytes]:
		if logtype==LogType.STDOUT:
			return self._get_ewes_client().stream_run_logs(log.stdout, max_bytes)
		else:
			return self._get_ewes_client().stream_run_logs(log.stderr, max_bytes)
	...
	def get_failed_task_logs(self, run: Union[ExtendedRun, str]) -> List[Log]:
		...
		for task_log in run.task_logs:
			...
			if runstatus == RunStatus.has_failed():
