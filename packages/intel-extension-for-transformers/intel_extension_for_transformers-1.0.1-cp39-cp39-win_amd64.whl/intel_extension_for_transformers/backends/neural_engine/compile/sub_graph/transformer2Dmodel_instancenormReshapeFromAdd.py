...
"""The Transformer2Dmodel_InstancenormReshapeFromAdd Pattern."""
...
@pattern_registry(pattern_type='Transformer2Dmodel_InstancenormReshapeFromAdd')
class Transformer2Dmodel_InstancenormReshapeFromAdd(Pattern):
    """The Transformer2Dmodel_InstancenormReshapeFromAdd pattern.

    Fuse the original sub-graph into the custom acceleration 'Transformer2Dmodel_InstancenormReshapeFromAdd' graph.
    ...
    def __call__(self, model):
        ...
        pattern_mapping_config = {
            'Transformer2Dmodel_InstancenormReshapeFromAdd': [
                {
                    'patterns': {
                        'in': [[(0, 'Add'), (1, 'Reshape'), (2, 'InstanceNormalization'), (4, 'Reshape')],
                               [(0, 'Add'), (3, 'Shape'), (4, 'Reshape')]],
                        'out': [[(0, 'Add'), (1, 'Reshape'), (2, 'InstanceNormalization'), (3, 'Reshape')]]
                    },
                    'search_mode': 'op_type',
                    'node_names': {
                        0: 0,
        ...
        def _set_attr(batch_size, out_channel, height, weight, node_names, model):
            attr = OrderedDict()
            attr['dst_shape'] = str(batch_size) + ',' + str(out_channel) + ',' + str(height) + ',' + str(
                weight)
            attr['dims'] = '0, 2, 3'
        ...
        for i in range(len(pattern_mapping_config['Transformer2Dmodel_InstancenormReshapeFromAdd'])):
            pattern_dict = pattern_mapping_config['Transformer2Dmodel_InstancenormReshapeFromAdd'][i]
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "Transformer2Dmodel_InstancenormReshapeFromAdd", pattern_dict, model)

            print('i = ', i, 'Transformer2Dmodel_InstancenormReshapeFromAdd = ', new_node_names)
            if len(new_node_names) != 0:
                for j in range(len(new_node_names)):
                    continue
                    # first_ret_old_node = ret_old_nodes[j][0]
                    # assert first_ret_old_node.op_type == 'Div'
                    # div_node_idx = model.get_node_id(new_node_names[j][0])
                    # model.nodes[div_node_idx].attr = first_ret_old_node.attr
