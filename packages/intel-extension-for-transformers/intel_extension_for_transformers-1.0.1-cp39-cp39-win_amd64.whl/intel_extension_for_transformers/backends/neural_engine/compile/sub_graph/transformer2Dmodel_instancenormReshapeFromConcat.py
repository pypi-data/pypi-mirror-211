...
"""The Transformer2Dmodel_InstancenormReshapeFromConcat Pattern."""
...
@pattern_registry(pattern_type='Transformer2Dmodel_InstancenormReshapeFromConcat')
class Transformer2Dmodel_InstancenormReshapeFromConcat(Pattern):
    """The Transformer2Dmodel_InstancenormReshapeFromConcat pattern.

    Fuse the original sub-graph into the custom acceleration 'Transformer2Dmodel_InstancenormReshapeFromConcat' graph.
    ...
    def __call__(self, model):
        ...
        pattern_mapping_config = {
            'Transformer2Dmodel_InstancenormReshapeFromConcat': [
                {
                    'patterns': {
                        'in': [[(0, 'Concat'), (1, 'Reshape'), (2, 'InstanceNormalization'),(4, 'Reshape')],
                        [(0, 'Concat'), (3, 'Shape'), (4, 'Reshape')]],
                        'out': [[(0, 'Concat'), (1, 'Reshape'),(2, 'InstanceNormalization'), (3, 'Reshape')]]
                    },
                    'search_mode': 'op_type',
                    'node_names': {
                        0: 0,
        ...
        for i in range(len(pattern_mapping_config['Transformer2Dmodel_InstancenormReshapeFromConcat'])):
            pattern_dict = pattern_mapping_config['Transformer2Dmodel_InstancenormReshapeFromConcat'][i]
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "Transformer2Dmodel_InstancenormReshapeFromConcat", pattern_dict, model)

            print('i = ', i, 'Transformer2Dmodel_InstancenormReshapeFromConcat = ', new_node_names)
