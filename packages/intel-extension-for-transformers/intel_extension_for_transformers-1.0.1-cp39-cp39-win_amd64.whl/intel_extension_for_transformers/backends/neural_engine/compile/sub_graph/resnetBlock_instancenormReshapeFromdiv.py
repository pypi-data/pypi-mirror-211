...
"""The ResnetBlock_InstanceNormReshapeFromDiv Pattern."""
...
@pattern_registry(pattern_type='ResnetBlock_InstanceNormReshapeFromDiv')
class ResnetBlock_InstanceNormReshapeFromDiv(Pattern):
    """The ResnetBlock_InstanceNormReshapeFromDiv pattern.

    Fuse the original sub-graph into the custom acceleration 'ResnetBlock_InstanceNormReshapeFromDiv' graph.
    ...
    def __call__(self, model):
        ...
        pattern_mapping_config = {
            'ResnetBlock_InstanceNormReshapeFromDiv': [
                {
                    'patterns': {
                        'in': [[(0, 'Div'), (1, 'Reshape'), (2, 'InstanceNormalization'),(4, 'Reshape')],
                        [(0, 'Div'), (3, 'Shape'), (4, 'Reshape')]],
                        'out': [[(0, 'Div'), (1, 'Reshape'),(2, 'InstanceNormalization'), (3, 'Reshape')]]
                    },
                    'search_mode': 'op_type',
                    'node_names': {
                        0: 0,
        ...
        for i in range(len(pattern_mapping_config['ResnetBlock_InstanceNormReshapeFromDiv'])):
            pattern_dict = pattern_mapping_config['ResnetBlock_InstanceNormReshapeFromDiv'][i]
            model, new_node_names, ret_old_nodes = util.pattern_mapping(
                "ResnetBlock_InstanceNormReshapeFromDiv", pattern_dict, model)

            print('i = ', i, 'ResnetBlock_InstanceNormReshapeFromDiv = ', new_node_names)
