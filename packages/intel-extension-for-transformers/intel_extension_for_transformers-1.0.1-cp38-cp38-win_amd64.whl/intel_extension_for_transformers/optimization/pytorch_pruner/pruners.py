...
def get_pruner(config, modules):
    ...
    if name not in PRUNERS.keys():  # pragma: no cover
...
class BasePruner:
    ...
    def __init__(self, config, modules):
        ...
        if self.total_prune_cnt == 0:  # pragma: no cover
    ...
    def _init(self):  # pragma: no cover
    ...
    def on_step_begin(self, local_step):
        ...
        if self.handled_global_step == self.global_step:  # pragma: no cover
            return
        self.update_masks(local_step)
        self.handled_global_step = self.global_step

    def update_masks(self, local_step):  # pragma: no cover
    ...
    def on_step_end(self):  # pragma: no cover
        """Implement at the end of each step."""
        pass

    def on_before_optimizer_step(self):  # pragma: no cover
    ...
    def on_train_begin(self):  # pragma: no cover
        """Implement at the beginning of training phase."""
        pass

    def on_train_end(self):  # pragma: no cover
        """Implement at the end of training phase."""
        pass
 
    def on_before_eval(self):  # pragma: no cover
        """Implement at the beginning of evaluation phase."""
        pass

    def on_after_eval(self):  # pragma: no cover
...
class BasicPruner(BasePruner):
    ...
    def set_global_step(self, global_step):  # pragma: no cover
...
class PatternLockPruner(BasePruner):  # pragma: no cover
    """Pruning Pruner.
...
class ProgressivePruner(BasicPruner):
    ...
    def check_progressive_validity(self):  # pragma: no cover
    ...
    def update_masks_progressive(self, local_step):
        ...
        if self.global_step == self.start_step:
            if self.config['lock_init_sparsity']:  # pragma: no cover
                self.masks = self.pattern.get_pattern_lock_masks(self.modules)
                self.init_sparsity_ratio = self.pattern.get_sparsity_ratio(self.masks)
                self.current_sparsity_ratio = self.init_sparsity_ratio
    ...
    def on_step_begin(self, local_step):
        ...
        if not self.use_progressive:  # pragma: no cover
    ...
    def print_progressive_sparsity(self):  # pragma: no cover
