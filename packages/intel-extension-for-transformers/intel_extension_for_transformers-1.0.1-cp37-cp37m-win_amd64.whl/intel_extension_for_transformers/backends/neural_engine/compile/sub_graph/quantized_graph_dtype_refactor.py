...
class QuantizedGraphDtypeCheck(Pattern):
    ...
    def __call__(self, model):
        ...
        def _remove_redundant_quant_node(model, graph_dtype):
            remove_node_names = []
            for node in model.nodes:
                if node.op_type == 'Quantize':
                    pre_node = None
                    try:
                        ...
                        pre_node = None
                    if pre_node and pre_node.attr.get('output_dtype', None) == graph_dtype:
                        ...
                        if len(dst_ops) == 1 and dst_ops[0].op_type == "Quantize":
                            ...
                            if len(dst_ops) == 2 and 'Output' in [o.op_type for o in dst_ops]:
                                t_name = pre_node.output_tensors[0].name
                                pre_node.output_tensors = copy.deepcopy(node.output_tensors)
                                pre_node.output_tensors[0].dest_op.append(model.nodes[-1].name)
                                for t in model.nodes[-1].input_tensors:
                                    if t.name == t_name:
                                        t.name = node.output_tensors[0].name
                                        break
                                remove_node_names.append(node.name)
        ...
        bf16_op = [
            ...
            'Concat', 'Softmax', 'LayerNorm', 'LogSoftmax', 'Convolution', 'Gather'
        ]
        ...
        for ret in match_ret:
            ...
            if dst_ops_type == ['Sin', 'Cos'] or dst_ops_type == ['CosSin', 'CosSin']:
                for n in dst_ops:
                    model = _insert_quant_node(n, model, graph_dtype)

        if graph_dtype in ['bf16']:
            for node in model.nodes:
                if EXECUTOR_TYPE.get(node.op_type, node.op_type) in ['Quantize']:
                    dst_ops = _get_dst_ops(node, model)
                    for op in dst_ops:
                        if op.attr.get('output_dtype', 'fp32') != graph_dtype and \
                            EXECUTOR_TYPE.get(op.op_type, op.op_type) in checker[graph_dtype]:
                            op.attr['output_dtype'] = graph_dtype
                            _check_dst_op(op, model, checker, graph_dtype)
            _scan_nodes_graph_dtype(model, checker, graph_dtype)
            _remove_redundant_quant_node(model, graph_dtype)
