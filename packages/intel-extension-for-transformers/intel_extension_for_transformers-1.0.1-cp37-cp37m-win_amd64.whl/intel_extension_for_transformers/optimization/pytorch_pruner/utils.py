...
from ..config import PrunerV2, WeightPruningConfig
...
def process_weight_config(global_config, local_configs, default_config):
    ...
    if len(local_configs) == 0:  # pragma: no cover
...
def process_config(config):
    ...
    if isinstance(config, str):
        ...
        except FileNotFoundError as f:  # pragma: no cover
...
def parse_to_prune(config, model):
    ...
    """
    modules = {}
    if config["op_names"] == None or config["op_names"] == []:  # pragma: no cover
        config["op_names"] = [".*"]
