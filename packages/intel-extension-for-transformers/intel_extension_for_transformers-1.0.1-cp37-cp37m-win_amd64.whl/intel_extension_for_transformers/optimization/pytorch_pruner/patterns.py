...
def get_pattern(config, modules):
    ...
    assert False, f"currently only support {PATTERNS.keys()}"  # pragma: no cover
...
class BasePattern:
    ...
    def reduce_tensor(self, data, dim):
        ...
        elif name == "sum":  # pragma: no cover
            return torch.sum(data, dim=dim)
        elif name == "max":  # pragma: no cover
    ...
    def get_masks_local(self, scores, target_sparsity_ratio, pre_masks):
        ...
        """
        masks = {}
        if isinstance(self, PatternNxM) and not isinstance(self.block_size, dict):  # pragma: no cover
    ...
    def get_sparsity_ratio(self, pre_masks, return_dict=False):
        ...
        if return_dict:  # pragma: no cover
    ...
    def adjust_ratio(self, masks: dict, layer_name: str, key_new_sparsity: SparsityInfo,
        ...
        if adjust_sparsity_ratio > max_sparsity_ratio:  # pragma: no cover
            need_adjust = True
            adjust_sparsity_ratio = max_sparsity_ratio
            adjust_zero_cnt = int(adjust_total_cnt * max_sparsity_ratio)

        if adjust_sparsity_ratio < min_sparsity_ratio:  # pragma: no cover
...
class PatternNxM(BasePattern):
    ...
    def __init__(self, config, modules):
        ...
        if self.N == "channel":  # pragma: no cover
    ...
    def get_masks_global(self, scores, cur_target_sparsity_ratio, pre_masks,
        ...
        while True:
            ...
            for key in not_exceed_layers:
                ...
                if need_adjust:  # pragma: no cover
                    # uptade status
            ...
            if not_exceed_layers == new_not_exceed_layers or len(new_not_exceed_layers) == 0:  # pragma: no cover
                break
    ...
    def update_progressive_masks_linear(self, pre_masks, cur_masks, 
           progressive_step, progressive_configs):  # pragma: no cover
...
class PatternNInM(BasePattern):
    ...
    def reshape_orig_to_pattern(self, data, key):  # pragma: no cover
    ...
    def reshape_reduced_to_orig(self, data, key, orig_shape):  # pragma: no cover
