...
class SequentialUpdate(AbstractUpdate):
    mode: Optional[UpdateMode]


class UpdatableSequentialDocumentProcessor(AbstractUpdatableModel, SequentialDocumentProcessor):
    def __init__(self, processors: Iterable[Tuple[str, AbstractDocumentProcessor]]):
        ...
        for model_name, model in processors:
        ...
        if len(update_types) == 1:
            self._update_type = next(iter(update_types.values()))[0]
