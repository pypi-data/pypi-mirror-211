...
def convert_bucketizer(scope, operator, container):
    ...
    container.add_initializer(reshape_data, onnx_proto.TensorProto.INT64, [3],
                              [-1, input_shape[1], 1])
    outputs = []
    for split in splits[1:]:
        less_output = scope.get_unique_variable_name('less_output_tensor')
        initializer_name = 'initializer_' + str(split)
        container.add_initializer(initializer_name, onnx_proto.TensorProto.FLOAT, [1],
                                  [split])
        container.add_node('Less', [operator.inputs[0].full_name, initializer_name], less_output,
                           name=scope.get_unique_operator_name('Less'),
                           op_version=7)
        casted_output = scope.get_unique_variable_name('cast_output_tensor')
        container.add_node('Cast', less_output, casted_output,
                           name=scope.get_unique_operator_name('Cast'),
                           op_version=6,
                           to=1)
        redim_output = scope.get_unique_variable_name('reshape_output_tensor')
        container.add_node('Reshape', [casted_output, reshape_data], redim_output,
