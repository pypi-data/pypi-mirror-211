...
def convert_sparkml_naive_bayes(scope, operator, container):
    ...
    if model_type == 'bernoulli':
        ...
        container.add_initializer(theta_tensor, onnx_proto.TensorProto.FLOAT,
                                  list(transposed_theta.shape), transposed_theta.flatten().tolist())
        container.add_node('MatMul', [operator.input_full_names[0], theta_tensor], probability1_output,
                           op_domain='ai.onnx',
                           name=scope.get_unique_operator_name('MatMul'),
                           op_version=1)
        prior_tensor = scope.get_unique_variable_name('raw_predictions_tensor')
        container.add_initializer(prior_tensor, onnx_proto.TensorProto.FLOAT, [len(op.pi)], op.pi)
        container.add_node('Add', [probability1_output, prior_tensor], raw_predictions_tensor,
                           op_domain='ai.onnx',
                           name=scope.get_unique_operator_name('Add'),
                           op_version=7)
    argmax_tensor = scope.get_unique_variable_name('argmax_tensor')
    container.add_node('ArgMax', raw_predictions_tensor, argmax_tensor,
                       op_domain='ai.onnx',
                       name=scope.get_unique_operator_name('ArgMax'),
                       op_version=1,
                       axis=1)
    container.add_node('Cast', argmax_tensor, operator.output_full_names[0],
                       op_domain='ai.onnx',
    ...
    container.add_node('ReduceMax', raw_predictions_tensor, max_prediction_tensor,
                       op_domain='ai.onnx',
    ...
    container.add_node('Sub', [raw_predictions_tensor, max_prediction_tensor], raw_minus_max_tensor,
                       op_domain='ai.onnx',
    ...
    container.add_node('Exp', raw_minus_max_tensor, exp_tensor,
                       op_domain='ai.onnx',
    ...
    container.add_node('ReduceSum', exp_tensor, sum_prediction_tensor,
                       op_domain='ai.onnx',
    ...
    container.add_node('Div', [exp_tensor, sum_prediction_tensor], operator.output_full_names[1],
                       op_domain='ai.onnx',
...
register_shape_calculator('pyspark.ml.classification.NaiveBayesModel', calculate_sparkml_naive_bayes_output_shapes)
