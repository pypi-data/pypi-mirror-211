...
from tecton.framework.base_tecton_object import TectonObjectInfo  # noqa
...
from tecton_proto.args import repo_metadata_pb2  # noqa
...
# NOTE: some imports above use the broader noqa since pycln + ruff handle the
# config differently. These imports are apparently needed for SDK docs.
# TODO(brian): remove the import once sdk docs in docusaurus is merged

# Disable cleanup since import is needed for SDK docs generation
    ...
    def run(
        ...
        **mock_inputs_kwargs: Union[pandas.DataFrame, pyspark_dataframe.DataFrame],
        ...
        if any(ds._data_source_type == DataSourceType.PUSH_NO_BATCH for ds in self.sources):
            msg = f"The `run()` method is currently unsupported for Feature Views that are backed by a PushSource without a batch_config"
            raise TectonValidationError(msg)
...
def batch_feature_view(
    *,
    ...
    :param aggregation_interval: How frequently the feature values are updated (for example, `"1h"` or `"6h"`). Only applicable when using aggregations.
...
class OnDemandFeatureView(FeatureView):
    ...
    def __init__(
        ...
        *,
        ...
        schema: List[types.Field],
        feature_view_function: Callable,
        supported_online_compute: Optional[List[configs.OnlineComputeConfig]] = None,
    ):
        from tecton.cli import common as cli_common
        ...
        on_demand_args = feature_view__args_pb2.OnDemandArgs(
            ...
            supported_online_compute_config=[c._to_proto() for c in supported_online_compute]
            if supported_online_compute
            else [],
        )
    ...
    def test_run(
        ...
        """
        run_api.validate_ondemand_mock_inputs_keys(mock_inputs, self._args.pipeline)
    ...
    *,
    ...
    def decorator(feature_view_function):
        return OnDemandFeatureView(
            ...
            schema=schema,
            feature_view_function=feature_view_function,
            supported_online_compute=supported_online_compute,
        )
