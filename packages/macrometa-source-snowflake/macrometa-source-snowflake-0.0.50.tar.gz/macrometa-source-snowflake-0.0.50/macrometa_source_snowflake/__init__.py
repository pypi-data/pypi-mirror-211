...
def do_discover(snowflake_conn, config):
    db = config['dbname']
    escaped_db = common.escape(db)
    db_exists_query = f'SHOW DATABASES LIKE {escaped_db}'
    db_exists = snowflake_conn.query(
            db_exists_query, max_records=1)
    if not db_exists:
        raise Exception (f'Database: {escaped_db} does not exist.')
