 ...
 * A binary polynomial model, which can be regarded as an extended model of the binary quadratic model, can handle Ising and
 * HUBO models.
 * @subsection ss_bpm_Ising Ising model
 * An energy of an "extended" Ising model \f$E_{\mathrm{Ising}}\f$ is represented by
 * \f[
 * E_{\mathrm{Ising}} = \sum_{i} h_i s_i + \sum_{i \neq j} J_{ij} s_i s_j +  \sum_{i \neq j \neq k} J_{ijk} s_i s_j s_k +
 * \ldots \f] Here \f$s_i \in \{+1, -1\}\f$ denotes the spin at the site \f$i\f$, \f$ h_i \f$ denotes the external magnetic
 * field at the site \f$ i \f$, and \f$J_{ijk\ldots}\f$ represents the interaction between the sites. Note that \f$ i \neq j
 * \neq k \f$ means \f$ i \neq j \f$, \f$ j \neq k \f$, and \f$ i \neq k \f$. This library assumes that the interaction is
 * not symmetric. For example, \f$J_{ij} \neq J_{ji}\f$ for \f$  i\neq j\f$, \f$J_{ijk} \neq J_{jik}\f$ for \f$ i \neq j \neq
 * k \f$, and so on.
 *
 * @subsection ss_bpm_hubo HUBO model
 * An energy of an "extended" QUBO model \f$ E_{\mathrm{HUBO}}\f$, here we call polynomial unconstrained binary optimization
 * (HUBO), is represented by \f[ E_{\mathrm{HUBO}} = \sum_{i \neq j} Q_{ij} x_i x_j +  \sum_{i \neq j \neq k} Q_{ijk} x_i x_j
 * x_k + \ldots \f] Here \f$ x_i \in \{0, 1\} \f$ denotes the spin at the site \f$ i \f$ and \f$Q_{ijk\ldots}\f$ represents
 * the interaction between the sites. Note that \f$ i \neq j \neq k \f$ means \f$ i \neq j \f$, \f$ j \neq k \f$, and \f$ i
 * \neq k \f$. This library assumes that the interaction is not symmetric. For example, \f$Q_{ij} \neq Q_{ji}\f$ for \f$
 * i\neq j\f$, \f$Q_{ijk} \neq Q_{jik}\f$ for \f$ i \neq j \neq k \f$, and so on.
 *
...
namespace cimod {
  ...
  //! @brief Type alias for the indices of the polynomial interactions (namely, the list of keys of the polynomial
  //! interactions as std::unordered_map) as std::vector<std::vector>>.
  //! @tparam IndexType
  template<typename IndexType>
  using PolynomialKeyList = std::vector<std::vector<IndexType>>;

  //! @brief Type alias for the values of the polynomial interactions (namely, the list of values of the polynomial
  //! interactions as std::unordered_map) as std::vector.
  //! @tparam FloatType
...
#pragma omp parallel for
    ...
    //! @brief Get the specific value of the interaction according to the key representing the indices of the polynomial
    //! interactions.
...
#pragma omp parallel for
    ...
    //! @brief Normalizes the values of the interactions of the BinaryPolynomialModel such that they fall in the provided
    //! range(s).
    //! @param range
    //! @param ignored_interactions
    //! @param ignored_offset
...
#pragma omp parallel for
  ...
  protected:
    ...
    //! @brief The list of the indices of the polynomial interactions (namely, the list of keys of the polynomial
    //! interactions as std::unordered_map) as std::vector<std::vector>>.
    PolynomialKeyList<IndexType> poly_key_list_;

    //! @brief The list of the values of the polynomial interactions (namely, the list of values of the polynomial
    //! interactions as std::unordered_map) as std::vector.
