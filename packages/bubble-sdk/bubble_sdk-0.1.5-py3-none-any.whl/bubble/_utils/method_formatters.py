...
from bubble._utils.encoding import (
    hexstr_if_str,
    to_hex,
)
...
if TYPE_CHECKING:
    from bubble.bub import AsyncBub  # noqa: F401
    from bubble.bub import Bub  # noqa: F401
    from bubble.module import Module  # noqa: F401
...
bub_wallets_formatter = type_aware_apply_formatters_to_dict(BUB_WALLETS_FORMATTER)

PYTHONIC_REQUEST_FORMATTERS: Dict[RPCEndpoint, Callable[..., Any]] = {
    # Bub
    RPC.bub_feeHistory: compose(
        apply_formatter_at_index(to_hex_if_integer, 0),
        apply_formatter_at_index(to_hex_if_integer, 1),
    ),
    RPC.bub_getBalance: apply_formatter_at_index(to_hex_if_integer, 1),
    RPC.bub_getBlockByNumber: apply_formatter_at_index(to_hex_if_integer, 0),
    RPC.bub_getBlockTransactionCountByNumber: apply_formatter_at_index(
        to_hex_if_integer,
        0,
    ),
    RPC.bub_getCode: apply_formatter_at_index(to_hex_if_integer, 1),
    RPC.bub_getStorageAt: apply_formatter_at_index(to_hex_if_integer, 2),
    RPC.bub_getTransactionByBlockNumberAndIndex: compose(
        apply_formatter_at_index(to_hex_if_integer, 0),
        apply_formatter_at_index(to_hex_if_integer, 1),
    ),
    RPC.bub_getTransactionCount: apply_formatter_at_index(to_hex_if_integer, 1),
    RPC.bub_getRawTransactionByBlockNumberAndIndex: compose(
        apply_formatter_at_index(to_hex_if_integer, 0),
        apply_formatter_at_index(to_hex_if_integer, 1),
    ),
    RPC.bub_getRawTransactionByBlockHashAndIndex: apply_formatter_at_index(
        to_hex_if_integer, 1
    ),
    RPC.bub_newFilter: apply_formatter_at_index(filter_params_formatter, 0),
    RPC.bub_getLogs: apply_formatter_at_index(filter_params_formatter, 0),
    RPC.bub_call: apply_one_of_formatters(
        (
            (is_length(2), call_without_override),
            (is_length(3), call_with_override),
        )
    ),
    RPC.bub_estimateGas: apply_one_of_formatters(
        (
            (is_length(1), estimate_gas_without_block_id),
            (is_length(2), estimate_gas_with_block_id),
        )
    ),
    RPC.bub_sendTransaction: apply_formatter_at_index(transaction_param_formatter, 0),
    RPC.bub_signTransaction: apply_formatter_at_index(transaction_param_formatter, 0),
    RPC.bub_getProof: apply_formatter_at_index(to_hex_if_integer, 2),
    # personal
...
PYTHONIC_RESULT_FORMATTERS: Dict[RPCEndpoint, Callable[..., Any]] = {
    # Bub
    RPC.bub_accounts: apply_list_to_array_formatter(to_checksum_address),
    RPC.bub_blockNumber: to_integer_if_hex,
    RPC.bub_chainId: to_integer_if_hex,
    RPC.bub_call: HexBytes,
    RPC.bub_estimateGas: to_integer_if_hex,
    RPC.bub_feeHistory: fee_history_formatter,
    RPC.bub_maxPriorityFeePerGas: to_integer_if_hex,
    RPC.bub_gasPrice: to_integer_if_hex,
    RPC.bub_getBalance: to_integer_if_hex,
    RPC.bub_getBlockByHash: apply_formatter_if(is_not_null, block_formatter),
    RPC.bub_getBlockByNumber: apply_formatter_if(is_not_null, block_formatter),
    RPC.bub_getBlockTransactionCountByHash: to_integer_if_hex,
    RPC.bub_getBlockTransactionCountByNumber: to_integer_if_hex,
    RPC.bub_getCode: HexBytes,
    RPC.bub_getFilterChanges: filter_result_formatter,
    RPC.bub_getFilterLogs: filter_result_formatter,
    RPC.bub_getLogs: filter_result_formatter,
    RPC.bub_getProof: apply_formatter_if(is_not_null, proof_formatter),
    RPC.bub_getRawTransactionByBlockHashAndIndex: HexBytes,
    RPC.bub_getRawTransactionByBlockNumberAndIndex: HexBytes,
    RPC.bub_getRawTransactionByHash: HexBytes,
    RPC.bub_getStorageAt: HexBytes,
    RPC.bub_getTransactionByBlockHashAndIndex: apply_formatter_if(
        is_not_null,
        transaction_result_formatter,
    ),
    RPC.bub_getTransactionByBlockNumberAndIndex: apply_formatter_if(
        is_not_null,
        transaction_result_formatter,
    ),
    RPC.bub_getTransactionByHash: apply_formatter_if(
        is_not_null, transaction_result_formatter
    ),
    RPC.bub_getTransactionCount: to_integer_if_hex,
    RPC.bub_getTransactionReceipt: apply_formatter_if(
        is_not_null,
        receipt_formatter,
    ),
    RPC.bub_protocolVersionndingTransactionFilter: compose(
        apply_formatter_if(is_0x_prefixed, to_integer_if_hex),
        apply_formatter_if(is_integer, str),
    ),
    RPC.bub_sendRawTransaction: to_hexbytes(32),
    RPC.bub_sendTransaction: to_hexbytes(32),
    RPC.bub_sign: HexBytes,
    RPC.bub_signTransaction: apply_formatter_if(is_not_null, signed_tx_formatter),
    RPC.bub_signTypedData: HexBytes,
    RPC.bub_syncing: apply_formatter_if(is_not_false, syncing_formatter),
    # personal
    RPC.personal_importRawKey: to_checksum_address,
    RPC.personal_listAccounts: apply_list_to_array_formatter(to_checksum_address),
    RPC.personal_listWallets: apply_list_to_array_formatter(bub_wallets_formatter),
...
METHOD_NORMALIZERS: Dict[RPCEndpoint, Callable[..., Any]] = {
    RPC.bub_getLogs: apply_formatter_at_index(FILTER_PARAM_NORMALIZERS, 0),
    RPC.bub_newFilter: apply_formatter_at_index(FILTER_PARAM_NORMALIZERS, 0),
}
...
def raise_contract_logic_error_on_revert(response: RPCResponse) -> RPCResponse:
    ...
    # Bub Revert without error message case:
...
NULL_RESULT_FORMATTERS: Dict[RPCEndpoint, Callable[..., Any]] = {
    RPC.bub_getBlockByHash: raise_block_not_found,
    RPC.bub_getBlockByNumber: raise_block_not_found,
    RPC.bub_getBlockTransactionCountByHash: raise_block_not_found,
    RPC.bub_getBlockTransactionCountByNumber: raise_block_not_found,
    RPC.bub_getTransactionByHash: raise_transaction_not_found,
    RPC.bub_getTransactionByBlockHashAndIndex: raise_transaction_not_found_with_index,
    RPC.bub_getTransactionByBlockNumberAndIndex: raise_transaction_not_found_with_index,
    RPC.bub_getTransactionReceipt: raise_transaction_not_found,
...
def filter_wrapper(
    ...
    if method == RPC.bub_newBlockFilter:
        if module.is_async:
            return AsyncBlockFilter(filter_id, bub_module=cast("AsyncBub", module))
        else:
            return BlockFilter(filter_id, bub_module=cast("Bub", module))
    elif method == RPC.bub_newPendingTransactionFilter:
        if module.is_async:
            return AsyncTransactionFilter(
                filter_id, bub_module=cast("AsyncBub", module)
            )
        else:
            return TransactionFilter(filter_id, bub_module=cast("Bub", module))
    elif method == RPC.bub_newFilter:
        if module.is_async:
            return AsyncLogFilter(filter_id, bub_module=cast("AsyncBub", module))
        else:
            return LogFilter(filter_id, bub_module=cast("Bub", module))
    else:
        raise NotImplementedError(
            "Filter wrapper needs to be used with either "
            f"{RPC.bub_newBlockFilter}, {RPC.bub_newPendingTransactionFilter}"
            f" or {RPC.bub_newFilter}"
        )


FILTER_RESULT_FORMATTERS: Dict[RPCEndpoint, Callable[..., Any]] = {
    RPC.bub_newPendingTransactionFilter: filter_wrapper,
