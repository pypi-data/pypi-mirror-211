...
class Torii(object):
    ...
    def __init__(self, properties_file=None, login=None, timeout=30,
        ...
        :param login:            autologin
                                 (True=force the loging, False=no loging, None=try to reuse the existing session or log in)
    ...
    def update_session(self, session_id = None):
        ...
        if not session_id and 'session_id' in self.properties:
            session_id = self.properties['session_id']

        if session_id:
            try:
                self.session = self.sessions.check(session_id)
            except Exception as e:
                self._logger.error(e.splitlines()[0])
    ...
    def login(self, username=None, password=None, prompt_password=False, retry_prompt=None, timeout=30):
        # clear the session
        self.session = None
        ...
        try:
            self.session = self.sessions.login(login, secret, timeout=timeout)
        except Exception as e:
        ...
        elif retry_prompt and prompt_password:
            # retry to log with prompting
            self.login(username=username, retry_prompt=retry_prompt, prompt_password=prompt_password, timeout=timeout)
