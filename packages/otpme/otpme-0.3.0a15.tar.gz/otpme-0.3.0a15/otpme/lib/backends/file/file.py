...
def write(object_id, object_config, index_journal=None,
    ...
    full_data_update=False, cluster=False, no_index_writes=False,
    ...
    if not uuid:
        ...
        if cluster and not config.use_api:
...
def index_search(realm=None, site=None, attribute=None, value=None, values=None,
    ...
    if return_acls or return_raw_acls:
        # Join with ACL table (this needs from_self()!).
        acl_q = sorted_query.from_self().join(IndexObjectACL,
                        IndexObject.id==IndexObjectACL.ioid)
...
def index_add(object_id, object_paths=None, object_config=None, uuid=None,
    ...
    if full_index_update and index_object and object_index:
        ...
        for x in list(object_attributes):
            del_attribute = True
            for i in object_index:
                n = i[0]
                v = i[1]
                if x.name == n and x.value == v:
                    del_attribute = False
                    break
            if not del_attribute:
                continue
            session.delete(x)
        ...
        for x in object_index:
            ...
            n = x[0]
            v = x[1]
            # Check if we have to add the attribute.
            add_attribute = True
            for i in object_attributes:
                # No need to add the attribute if it already exists in the DB.
                if i.name == n and i.value == v:
                    add_attribute = False
