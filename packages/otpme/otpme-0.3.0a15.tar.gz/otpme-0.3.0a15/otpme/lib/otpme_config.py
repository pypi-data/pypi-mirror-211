...
class OTPmeConfig(object):
    def __init__(self, tool_name="otpme", use_syslog=False,
        ...
        self.dictionary_dir = os.path.join(self.otpme_lib_dir, "dicts")
        ...
        self.register_config_var("node_vote_file", str, None)
    ...
    def load(self, quiet=False):
        ...
        self.node_vote_file = os.path.join(self.spool_dir, "node_vote")
    ...
    def touch_node_vote_file(self, force=False):
        from otpme.lib import filetools
        if not force:
            if not self.daemon_mode:
                return
        filetools.touch(self.node_vote_file)
