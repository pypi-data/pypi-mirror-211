...
def cluster_sync_object(object_uuid, object_id, action, object_config=None,
    ...
    multiprocessing.cluster_event.set()
    if wait_for_write:
        print("waiting for cluster data written: %s %s %s" % (config.daemon_name, action, object_id))
        object_event_name = "/%s" % cluster_journal_entry.timestamp
        object_event = multiprocessing.Event(object_event_name)
        object_event.wait()
        object_event.clear()
        object_event.unlink()
        print("finished cluster data written: %s %s %s" % (config.daemon_name, action, object_id))
...
class ClusterJournalEntry(object):
    def __init__(self, timestamp, object_uuid=None, action=None,
        ...
        self.finish_file = os.path.join(self.entry_dir, ".finished")
...
#            msg = "Cluster data entry not finished."
#            raise OTPmeException(msg)
#        journal_file = self.get_journal_file()
#        file_content = filetools.read_file(path=journal_file)
#        object_data = json.loads(file_content)
...
class ClusterDaemon(OTPmeDaemon):
    ...
    def do_master_node_election(self, first_election):
        """ Do master node election. """
        node_fails = {}
        try:
            node_vote = multiprocessing.node_votes[self.host_name]
        except:
            try:
                node_vote = os.path.getmtime(config.node_vote_file)
    ...
    def calc_quorum(self):
        ...
        for node_name in multiprocessing.init_sync_done:
            ...
            if not clusterd_conn.get_node_sync_status(self.host_name):
                continue

            current_votes += node.cluster_votes
    ...
    def switch_master_node(self, current_master_node, new_master_node):
        ...
        if new_master_node == self.host_name:
            print("MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM", new_master_node)
    ...
    def do_node_check(self, node_name):
        ...
        try:
            if not self.node_conn.get_node_sync_status(self.host_name):
    ...
    def _start_cluster_communication(self):
        ...
        while True:
            ...
            try:
                new_master_node = self.do_master_node_election(first_election)
    ...
    def _start_cluster_connection(self, node_name):
        ...
        while True:
            ...
            # - send events to blocking write requests (even for requests running
            #   while the second goes down).
            # - make sure cluster journal entries are deleted.
            if config.two_node_setup:
                if len(multiprocessing.member_nodes) == 0:
                    try:
                        ...
                        self.logger.critical(msg)
                        #print(msg)
                # As soon as the second node gets online we need to keep write requests.
                if len(multiprocessing.online_nodes) == 0:
                    for journal_entry_dir in self.get_cluster_journal():
                        ...
                        try:
                            ...
                            config.logger.critical(msg)
                            continue
                        # Make sure we delete cluster entries if second node is offline.
            ...
            try:
                self.handle_cluster_journal(node_name)
            except Exception as e:
                start_over = True
    ...
    def handle_cluster_journal(self, node_name):
        ...
        for journal_entry_dir in self.get_cluster_journal():
            ...
            if object_checksum in node_checksums:
                ...
                processed_journal_entries.append(entry_timestamp)
    ...
    def _run(self, **kwargs):
        ...
        # Replay transactions.
        try:
            backend.replay_transactions()
        except Exception as e:
            msg = "Failed to replay transactions: %s" % e
            self.logger.critical(msg)
