...
class OTPmeJoinP1(OTPmeServer1):
    ...
    def join_realm(self, site, host_type, host_name,
        ...
        try:
            ...
            message = (_("Error enabling %s: %s") % (host_type, e))
            status = False
            self.logger.debug(message)
            config.raise_exception()
            return self.build_response(status, message)
    ...
    def _process(self, command, command_args):
        ...
        callback = JobCallback(uuid=job_uuid)
        ...
        if command == "join":
            ...
            if host and host.joined and not force:
                host_desc = "%s%s" % (host.type[0].upper(), host.type[1:])
                message = (_("WARNING: %s has already joined this realm. "
            ...
            if host and jotp:
                ...
                if not host.jotp_enabled:
                    message = "JOTP disabled for host: %s" % host
                    status = False
                    self.logger.debug(message)
                    return self.build_response(status, message)
                # Make sure we compare JOTP as string.
                if host.jotp != str(jotp):
                    message = (_("Permission denied: Wrong JOTP"))
                    status = False
                    self.logger.debug(message)
                    return self.build_response(status, message)
            ...
            if not host_type in self.supported_host_types:
                message = (_("Unknown host type: %s") % host_type)
                self.logger.debug(message)
                status = False
                return self.build_response(status, message)
            ...
            if host_type == "host":
                if self.request_site.name != config.site:
                    message = (_("Cannot join host of other site."))
                    status = False
                    self.logger.warning(message)
                    return self.build_response(status, message)
                ...
                if host:
                    if host_unit != host.unit:
                        if replace_existing_host:
                            ...
                            message = (_("%s already exists in unit: %s")
                                        % (host.type, host.unit))
                            self.logger.debug(message)
                            status = False
                            return self.build_response(status, message)
            ...
            if finish:
                ...
                if not jotp:
                    message = (_("Need JOTP to finalize join process."))
                    status = False
                    self.logger.debug(message)
                    return self.build_response(status, message)
                ...
                if not host:
                    status = False
                    message = (_("Unknown %s: %s") % (host_type, host_fqdn))
                    self.logger.debug("Join phase 2 error: %s" % message)
        ...
        if command == "leave":
            ...
            if not host:
                status = False
                message = (_("Unknown host: %s") % host_fqdn)
                self.logger.debug("Leaving host error: %s" % message)
                # Prevent hostname testing for not authenticated users.
                if not self.authenticated:
                    message = (_("Access denied."))
                return self.build_response(status, message)
            ...
            if host.site_uuid != config.site_uuid:
                message = (_("Cannot leave %s from other site: %s")
                            % (host.type, host.name))
                self.logger.warning(message)
                status = False
                return self.build_response(status, message)
            ...
            if self.client_cn != host.fqdn:
                message = (_("Access denied: Client certificate does not match "
            ...
            if lotp:
                ...
                if not host.lotp_enabled:
                    message = "LOTP disabled for host: %s" % host
                    status = False
                    self.logger.debug(message)
                    return self.build_response(status, message)
                # Make sure we compare LOTP as string.
                if host.lotp != str(lotp):
                    message = (_("Access denied: Wrong LOTP"))
                    status = False
                    self.logger.debug(message)
                    return self.build_response(status, message)
            ...
            if not lotp and not self.authenticated:
                message = (_("Access denied: Not authenticated"))
                status = False
                self.logger.debug(message)
                return self.build_response(status, message)
            ...
            try:
                ...
                message = (_("Error leaving realm: %s") % e)
                status = False
                config.raise_exception()
                return self.build_response(status, message)
            ...
            try:
                ...
                message = (_("Error disabling %s: %s") % (host.type, e))
                status = False
                self.logger.debug(message)
                config.raise_exception()
                return self.build_response(status, message)
