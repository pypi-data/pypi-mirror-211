...
class CommandHandler(object):
        ...
        if self.command == "tool":
            if subcommand == "master_failover":
                ...
                try:
                    ...
                    random_node = False
                return self.handle_master_failover(random_node=random_node)

            if subcommand == "cluster_status":
    ...
    def delete_object(self, object_id):
        ...
        self.init()
        mgmt_client = self.get_mgmt_client()
        mgmt_cmd = "delete_object %s" % object_id
        status, \
        reply = mgmt_client.send(command=mgmt_cmd, client_type="CLIENT")
        if status == False:
            raise OTPmeException(reply)
            ...
            for y_node in sorted(node_status):
                ...
                for x_object in x_object_checksums:
                    ...
                    try:
                        ...
                        if x_object in already_missed_objects:
                            continue
                        already_missed_objects.append(x_object)
                    ...
                    if y_checksum == x_checksum:
                        continue
                    if x_object in already_diffed_objects:
                        continue
                    already_diffed_objects.append(x_object)
                ...
                for y_object in y_object_checksums:
                    ...
                    try:
                        ...
                        if y_object in already_missed_objects:
                            continue
                        already_missed_objects.append(y_object)
                    ...
                    if y_checksum == x_checksum:
                        continue
                    if y_object in already_diffed_objects:
                        continue
                    already_diffed_objects.append(y_object)
                    msg = ("Object %s differs on node %s. (%s)"
                ...
                for x_session in x_session_checksums:
                    ...
                    try:
                        ...
                        if x_session in already_missed_objects:
                            continue
                        already_missed_objects.append(x_session)
                    ...
                    if y_checksum == x_checksum:
                        continue
                    if x_session in already_diffed_objects:
                        continue
                    already_diffed_objects.append(x_session)
                ...
                for y_session in y_session_checksums:
                    ...
                    try:
                        ...
                        if y_session in already_missed_objects:
                            continue
                        already_missed_objects.append(y_session)
                    ...
                    if y_checksum == x_checksum:
                        continue
                    if y_session in already_diffed_objects:
                        continue
                    already_diffed_objects.append(y_session)
                    msg = "Session %s differs on node %s." % (y_session, x_node)
                    msg = colored(msg, 'yellow')
                    diff_objects.append(msg)

        cluster_status = sorted(cluster_status)
    ...
    def handle_master_failover(self, random_node=False):
        """ Handle auth command. """
        import random
        register_module("otpme.lib.classes.realm")
        ...
        try:
            ...
            if random_node:
                try:
                    cluster_nodes = clusterd_conn.get_cluster_nodes()
                except Exception as e:
                    msg = ("Failed to get cluster quorum: %s: %s"
                            % (this_node.name, e))
                    self.logger.warning(msg)
                    return msg
                cluster_nodes.remove(this_node.name)

                while True:
                    if len(cluster_nodes) == 0:
                        msg = "Master failover failed."
                        print(msg)
                        return False
                    new_master_node = random.choice(cluster_nodes)
                    cluster_nodes.remove(new_master_node)
                    try:
